...
How the organization actually _builds_ the knowledge graph that powers codebase
...
> **Two passes, never conflated.** A _deterministic_ pass extracts everything a
> parser can prove (manifests, imports, AST, API specs, config) as high-confidence
> nodes + structural edges with zero model calls. An _enrichment_ pass has agents
...
no separate ingestion. The org's _own_ graph builds itself by doing work.
...
correction). Determinism computes the _candidate_ edge; a hat confirms; the kernel
...
The prior parts are the _shape_. This is the _engineering reality_ — what actually
...
_categories_ — markdown-AST, fuzzy-match, Hindsight-recall, the existing Cockroach
executor — are fixed.)
...
async function runDocConstruction(job: DocConstructionJob, deps: ConstructionDeps) {
  ...
  for (const u of units) {
    ...
    if (await deps.existsByKey(u.unitId))
      await deps.cockroach.upsertUnit(u); // idempotent
    ...
    await deps.appendEvent(graphNodeAdded(u));
    for (const e of edges) await deps.appendEvent(edgeEvent(e));
  }
  if (job.canonicalize) await deps.git.writeHandbook(canonicalize(units)); // markdown-as-row, PR-reviewable
}
```

Deterministic stages are pure TS over the AST; the only awaited _model_ calls are
...
readout), so the graph's _trustworthiness_ is visible, not assumed.
...
- **Impact analysis** — a _query_, not stored: "what breaks if I change this
...
codebase-intelligence process runs; _inferring_ architecture is an architecture-hat
...
- **One graph:** code, docs, work, decisions, and memory share the same node/edge
  ...
  intelligences (codebase / docs / org-memory) are three _lenses_ on one graph.
...
The kernel guarantees the graph is _built from facts, validated, versioned, and
trust-tiered_; agents supply the semantic judgment, bounded by the facts and
