...
diverging — and the North Star names it as the _gate on adding more commands_,
...
not part of the vocabulary, so inventing them would _create_ the divergence the
...
strings _literally_ (not by importing the application package) to preserve the
