...
1. **Documents every gap** between what the org system actually does today (the
   ...
   work types, work flowing in and out of the system, a _standing_ QA department
...
makes the _work itself_ a living, observable, self-moving system — not a single
linear pipeline.
...
| `packages/metrics/`                                                        | Quantitative **code** metrics + a 3-agent qualitative **review board**. Real, but about _source text_, not _work-group health_.                                                                                          |
...
1. `WorkItemState` (domain) — the _work-item_ lifecycle (intake → done). Unused by the runtime.
2. `RunLifecyclePhase` (`observe.ts`) — the _execution_ lifecycle of one run.
3. `PipelineStage` (`pipeline.ts`) — the _7 quality gates_. What the runtime actually drives.
...
- **The P5 pipeline becomes one workflow** (the `task`/feature workflow) among
  several, instead of _the_ pipeline. `runOrgCycle` dispatches by the item's type
  to its workflow. This **unifies the three models**: `WorkItem` is the spine,
  `RunLifecyclePhase` is how a single execution run advances _within_ an
...
**`observeForHat(hat, orgState)`** is the new organizational read — _distinct from_
...
- **Authority scope** is derived from `hat.level` + `departmentId` + the work
  ...
  org-wide rollups. _Each higher scope sees a higher-level rollup_ — this is "the
  observe is different for each hat."
- **Prioritization rolls up**: an IC orders its own queue; a Lead orders items
  _within_ its batch; a Director orders _batches_ across teams; exec orders across
...
type WorkBatchMetrics = {
  ...
  completionPct: number; // completion %
  blocked: number;
  inQa: number;
  openDefects: number;
  defectsOpenedInTestSetup: number; // # defects QA opened during test-case setup
...
defects, and observes regressions, _evolving the product_.
...
- **Execution is a port** (`TestExecutor`) so the _real_ runner can be computer-use,
...
- A **failed feature** = an `active` test case that has _never_ passed on the
...
- Regressions and failed features **roll into `WorkBatchMetrics`** (C2) and each
  opens (or links to) a **Defect** (C4). QA thereby _continuously evolves the
  product_: every BRD becomes scenarios, scenarios become runs, runs surface
...
the item out of the normal loop into an escalation decision** — the loop is _broken
deliberately_ instead of spinning.
...
(EM/TPM/Director) chooses within them — _exactly_ the ANTI_STALL blocker table,
...
- **"Architect changes approach"** = the escalation chooses `bring_in_architect`,
  ...
  _replaced_ by a new approach instead of repeating the failing one.
...
functional and proved in kind.** Explicitly _deferred_ (named so they are not
