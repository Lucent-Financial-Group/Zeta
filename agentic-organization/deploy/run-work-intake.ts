...
async function main(): Promise<void> {
  ...
  const after = await pool.query<{ status: string }>(
    `SELECT status FROM agentic_org_initiatives WHERE initiative_id = $1`,
    [initId],
  );
    ...
    JSON.stringify(
      {
        a1WorkIntake: {
          seededInitiative: {
            ...
            claimed: after.rows[0]?.status === InitiativeStatus.Active,
          },
          workOsTicks,
          drovenCycleOnClaim:
            workOsTicks[0]?.status.startsWith("work-os:") === true && workOsTicks[0]?.status !== "work-os:idle",
