...
for (const m of [
  ...
  {
    serviceKey: "services/billing",
    serviceName: "Billing",
    dependsOn: ["services/auth"],
    exposes: ["POST /charges"],
    persistsTo: ["postgres:billing"],
    testedBy: ["test/billing.spec"],
  },
...
const inf = inferEdge(
  {
    ...
    toNodeId: auth,
    agent: "arch-enricher",
    rationale: "billing is the payments boundary over auth",
  },
  deps,
);
...
const changeEdge = {
  ...inferEdge(
    {
      fromNodeId: billing,
      kind: GraphEdgeKind.ChangedBy,
      toNodeId: id("cs"),
      agent: "work-loop",
      rationale: "release",
      changeSetId: "cs-release-42",
    },
    deps,
  ).edge,
...
await store.upsertEdge({
  ...changeEdge,
  edgeId: graphEdgeId(ORG, billing, GraphEdgeKind.ChangedBy, "cs-node"),
  toNodeId: "cs-node",
  changeSetId: "cs-release-42",
});
// a wrong edge, then retracted (kept with correction)
const wrong = inferEdge(
  {
    fromNodeId: billing,
    kind: GraphEdgeKind.DependsOn,
    toNodeId: graphNodeId(ORG, GraphNodeKind.Service, "services/legacy"),
    agent: "x",
    rationale: "guess",
  },
  deps,
...
const verifiedAboutEdge = await store
  .outEdges(ORG, billing)
  .then((es) => es.find((e) => e.kind === GraphEdgeKind.About));
  ...
  JSON.stringify(
    {
      trackGProof: {
        impactOfAuth: {
          ...
          webIsTransitiveDependent: impact.depthByNode[graphNodeId(ORG, GraphNodeKind.Service, "services/web")] === 2,
        },
