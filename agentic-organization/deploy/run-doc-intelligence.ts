...
async function ingestActive(
  over: Partial<DocUnit> & { markdown: string; title: string; type: DocType; scopeId: string },
): Promise<DocUnit[]> {
  const raw = {
    sourceId: id("src"),
    externalRef: id("wiki"),
    ...
    boundStageIds: over.boundStageIds ?? [],
  };
  const { units, events: evs } = ingestDocument(raw, {
    organizationId: ORG,
    now: () => NOW,
    createId: id,
    provenanceChangeSetId: id("cs"),
  });
  for (const e of evs) await events.append(e);
  const active = units.map((u) => ({
    ...u,
    status: over.status ?? DocLifecycleState.Active,
    freshnessAt: over.freshnessAt ?? u.freshnessAt,
  }));
...
await ingestActive({
  ...
  markdown: "# Release Handbook\nrun the billing service checks before shipping",
...
await ingestActive({
  ...
  scopeId: "eng",
  title: "Ancient guide",
  markdown: "# Ancient guide\nold stuff",
  status: DocLifecycleState.Active,
  freshnessAt: new Date(NOW - 200 * DAY).toISOString(),
});

// 8-stage retrieval for an eng release query that mentions the billing service
const corpus = [...(await store.listByOrgStatus(ORG, DocLifecycleState.Active))];
...
const maint = runDocMaintenanceCycle(corpus, {
  organizationId: ORG,
  now: NOW,
  createId: id,
  stalenessFloorMs: 30 * DAY,
  archiveFloorMs: 180 * DAY,
});
