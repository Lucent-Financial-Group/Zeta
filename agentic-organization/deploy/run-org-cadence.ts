...
async function main(): Promise<void> {
  ...
  const memState: MemoryState = {
    ...
    confidence: 0.2,
    weight: 0.1,
    freshnessAt: new Date(NOW - 120 * 86_400_000).toISOString(),
    reinforcementCount: 1,
    outcome: { successCount: 0, failureCount: 5, inconclusiveCount: 0, workItemsObserved: [] },
  ...
  const cs: ChangeSet = {
    ...
    title: "Cadence-driven change",
    targetRef: "feat/cadence",
    phase: ChangeSetPhase.InReview,
    pipelineId: buildInternalOnlyPipeline(ORG).pipelineId,
    currentStageIndex: 0,
  ...
  const reputationObservations = [
    reputationObservation(
      ...
      { kind: "binary" as const, success: true },
      "evidence:agent-observe-act:quality-pass",
    ),
    reputationObservation(
      "agent-observe-act",
      ReputationOutcomeClass.Collaboration,
      { kind: "binary" as const, success: true },
      "evidence:agent-observe-act:collaboration-pass",
    ),
    reputationObservation(
      "agent-explorer",
      ReputationOutcomeClass.Quality,
      { kind: "binary" as const, success: true },
      "evidence:agent-explorer:quality-pass",
    ),
    reputationObservation(
      "agent-regressed",
      ReputationOutcomeClass.Quality,
      { kind: "binary" as const, success: false },
      "evidence:agent-regressed:quality-fail",
    ),
    reputationObservation(
      "agent-regressed",
      ReputationOutcomeClass.Quality,
      { kind: "binary" as const, success: false },
      "evidence:agent-regressed:quality-fail-2",
    ),
  ];
  for (const [index, observation] of reputationObservations.entries()) {
    await orgEventStore.append(
      createReputationOutcomeOrgEvent({
  ...
  if (seededReputationEvents.length < reputationObservations.length) {
    throw new Error(
      `expected ${reputationObservations.length} durable reputation observations, found ${seededReputationEvents.length}`,
    );
  }
  ...
  const workMarketProof = runWorkMarketProof({
    implementationEvidenceRef: createContentAddressedEvidenceRef("test-result", {
      proof: "work-market",
      status: "implemented",
    }),
  ...
  const simulationReport = runOrgPolicySimulation({
    ...
    stream: [
      {
        eventId: "sim-intake-1",
        kind: "work_intake",
        occurredAt: new Date(NOW - 900_000).toISOString(),
        workItemId: OBSERVE_ACT_WORK_ITEM_ID,
        priority: 100,
      },
      {
        eventId: "sim-complete-1",
        kind: "work_completed",
        occurredAt: new Date(NOW - 480_000).toISOString(),
        workItemId: OBSERVE_ACT_WORK_ITEM_ID,
        leadTimeMs: 420_000,
      },
      {
        eventId: "sim-review-1",
        kind: "review_lag",
        occurredAt: new Date(NOW - 420_000).toISOString(),
        workItemId: OBSERVE_ACT_WORK_ITEM_ID,
        lagMs: 180_000,
      },
      {
        eventId: "sim-stale-1",
        kind: "stale_claim",
        occurredAt: new Date(NOW - 360_000).toISOString(),
        workItemId: OBSERVE_ACT_WORK_ITEM_ID,
      },
    ],
    baseline: {
  ...
  const observeActScheduleBlock = {
    ...
    metadata: {
      updatedAt: NOW_ISO,
      version: 1,
      correlationId: "corr-schedule-kind",
      causationId: "cause-schedule-kind",
      traceId: "trace-schedule-kind",
    },
  };
  await pool.query(
    `
  ...
  await cadence.done;
  const observedShadowEvents = (await orgEventStore.listByOrganization(ORG, 100_000)).filter(
    (event) => event.kind === OrgEventKind.ObserveActTick && event.subjectId === OBSERVE_ACT_WORK_ITEM_ID,
  );
  ...
  if (observedPromotionWindow.shadowTickCount < 100 && observedPromotionWindow.shadowSoakHours < 24) {
    throw new Error(
      `expected measured observe-act shadow window to satisfy tick or soak gate, got ${JSON.stringify(observedPromotionWindow)}`,
    );
  }
  ...
  if (
    !primaryPromotionTicks.some(
      (tick) => tick.lane === "work-os" && tick.status === "work-os:observe-act-primary-suppressed",
    )
  ) {
  ...
  const recent = events.filter(
    (e) =>
      e.kind.startsWith("work_item") ||
      e.kind.startsWith("memory_") ||
      e.kind.startsWith("change_set") ||
      e.kind.startsWith("review_") ||
      e.kind.startsWith("stage_"),
  );
    ...
    JSON.stringify(
      {
        orgCadence: {
          ...
          observeActShadow: {
            ...
            promptFlowEvidenceRows: observeActEvents.filter((e) =>
              e.evidenceRefs.some((ref) => ref.startsWith("observe-act:prompt_flow:")),
            ).length,
            shadowSelectedRows: laneTicks.filter(
              (tick) =>
                tick.lane === "observe-act-work-item" && tick.status === "observe-act-shadow:command:shadow_selected",
            ).length,
          ...
          observeActPrimaryPromotion: {
            ...
            legacySuppressed: primaryPromotionTicks.some(
              (tick) => tick.lane === "work-os" && tick.status === "work-os:observe-act-primary-suppressed",
            ),
          ...
          seededMemory: {
            memoryId: memId,
            phaseAfter: memAfter?.state.phase,
            weightAfter: memAfter?.state.weight,
            surfaces: memAfter?.state.phase !== MemoryPhase.Archived,
          },
          ...
          reputation: {
            ...
            selection:
              reputationSelection.outcome === "selected"
...
function runWorkMarketProof(input: { implementationEvidenceRef: string; reviewEvidenceRef: string }) {
  const first = claimNextWorkShard(
    workMarketQueue("queue-kind-work-market"),
    claimInput("agent-backend-1", "claim-api", "fence-api"),
  );
  ...
  if (
    first.runtimeLease.fencingToken !== first.claim.fencingToken ||
    second.runtimeLease.fencingToken !== second.claim.fencingToken
  ) {
    throw new Error("expected runtime leases to carry claim fencing tokens");
  }
  const duplicateRuntimeLease = claimNextWorkShard(
    second.queue,
    claimInput("agent-backend-3", "claim-duplicate-runtime", "fence-duplicate-runtime", {
      runtimeLeaseId: first.runtimeLease.leaseId,
    }),
  );
  if (
    duplicateRuntimeLease.outcome !== WorkMarketClaimOutcome.Rejected ||
    duplicateRuntimeLease.reason !== "duplicate_runtime_lease_id"
  ) {
  ...
  if (afterFirstComplete.outcome !== WorkMarketCompleteOutcome.Completed)
    throw new Error(`expected first completion, got ${afterFirstComplete.reason}`);
  ...
  if (afterSecondComplete.outcome !== WorkMarketCompleteOutcome.Completed)
    throw new Error(`expected second completion, got ${afterSecondComplete.reason}`);
  ...
  const firstReview = peerReview(
    afterSecondComplete.queue,
    first.claim.shardId,
    first.claim.ownerAgentId,
    input.reviewEvidenceRef,
  );
  const secondReview = peerReview(
    firstReview.queue,
    second.claim.shardId,
    second.claim.ownerAgentId,
    input.reviewEvidenceRef,
  );
  ...
  if (merged.outcome !== WorkMarketMergeOutcome.Merged)
    throw new Error(`expected reviewed shard merge, got ${merged.reason}`);

  const staleClaim = claimNextWorkShard(
    workMarketQueue("queue-kind-stale"),
    claimInput("agent-backend-1", "claim-stale", "fence-stale", {
      now: new Date(NOW - 60_000).toISOString(),
  ...
  const reclaimed = claimNextWorkShard(
    reaped.queue,
    claimInput("agent-backend-2", "claim-reclaimed", "fence-reclaimed"),
  );
  ...
  if (oldToken.outcome !== WorkMarketCompleteOutcome.Rejected)
    throw new Error("expected old fencing token completion rejection");

  const staleRuntimeLease = claimNextWorkShard(
    workMarketQueue("queue-kind-runtime-stale"),
    claimInput("agent-backend-1", "claim-runtime-stale", "fence-runtime-stale", {
      heartbeatDeadlineAt: new Date(NOW + 60_000).toISOString(),
      leaseExpiresAt: new Date(NOW + 600_000).toISOString(),
    }),
  );
  if (staleRuntimeLease.outcome !== WorkMarketClaimOutcome.Claimed)
    throw new Error("expected stale runtime lease setup");
  const runtimeReaped = reapStaleWorkClaims(staleRuntimeLease.queue, {
    now: new Date(NOW + 120_000).toISOString(),
    reason: "runtime_lease_stale",
  });
  if (runtimeReaped.reapedClaims.length !== 1) throw new Error("expected runtime-stale claim reaped");
  if (runtimeReaped.queue.runtimeLeases?.[0]?.state !== RuntimeLeaseState.Expired)
    throw new Error("expected runtime lease expired during reaping");

  const mismatchedRuntimeLease = claimNextWorkShard(
    workMarketQueue("queue-kind-runtime-mismatch"),
    claimInput("agent-backend-1", "claim-runtime-mismatch", "fence-runtime-mismatch", {
      leaseExpiresAt: new Date(NOW + 600_000).toISOString(),
    }),
  );
  if (mismatchedRuntimeLease.outcome !== WorkMarketClaimOutcome.Claimed)
    throw new Error("expected mismatched runtime lease setup");
  const mismatchedRuntimeQueue = {
    ...mismatchedRuntimeLease.queue,
    runtimeLeases: mismatchedRuntimeLease.queue.runtimeLeases?.map((lease) => ({
      ...lease,
      workspaceRef: "worktree:wrong-runtime",
    })),
  };
  const mismatchReaped = reapStaleWorkClaims(mismatchedRuntimeQueue, {
    now: new Date(NOW + 120_000).toISOString(),
    reason: "runtime_lease_authority_mismatch",
  });
...
function peerReview(queue: HatWorkQueue, shardId: string, producerAgentId: string, evidenceRef: string) {
  ...
  if (review.outcome !== WorkMarketQuorumOutcome.Accepted)
    throw new Error(`expected peer review accepted, got ${review.reason}`);
  return review;
}
...
function claimInput(
  ...
  return {
    ...
    heartbeatDeadlineAt:
      override.heartbeatDeadlineAt ?? override.leaseExpiresAt ?? new Date(NOW + 300_000).toISOString(),
