...
function fakeExecutor(): CockroachGenericSqlExecutor {
  ...
  const exec = async (s: { sql: string; parameters: readonly unknown[] }) => {
    ...
    if (s.sql.includes("INSERT INTO")) {
      rows.set(p[0] as string, { organization_id: p[0], config: p[1], updated_at: p[2], version: p[3] });
      return { rows: [] };
    }
    if (s.sql.includes("WHERE organization_id = $1")) {
...
test("tenant config store round-trips the whole config (autonomy + workflow + bindings)", async () => {
  ...
  const c = {
    ...
    autonomy: { level: AutonomyLevel.Manual, humanGatedStageIds: ["external-code-review"] },
  };
...
test("tenant config store round-trips context-pack curation layer policy", async () => {
  ...
  equal(
    got?.layers?.[0]?.policy.contextPack?.synthesisRequirement?.requirements?.[0]?.appliesTo?.phases?.[0],
    "blocked",
  );
});
