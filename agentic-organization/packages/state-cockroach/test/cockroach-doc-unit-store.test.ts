...
function fakeExecutor(): CockroachGenericSqlExecutor {
  ...
  const toRow = (p: readonly unknown[]): Record<string, unknown> => ({
    ...
    freshness_at: p[11],
    bound_hat_ids: p[12],
    bound_stage_ids: p[13],
    supersedes_id: p[14],
    provenance_change_set_id: p[15],
    created_at: p[16],
    updated_at: p[17],
    version: p[18],
  });
  const exec = async (s: { sql: string; parameters: readonly unknown[] }) => {
    ...
    if (sql.includes("INSERT INTO")) {
      const r = toRow(p);
      rows.set(r["doc_unit_id"] as string, r);
      return { rows: [] };
    }
    if (sql.includes("WHERE doc_unit_id = $1")) {
    ...
    if (sql.includes("scope_kind = $2 AND scope_id = $3")) {
      return {
        rows: [...rows.values()].filter(
          (r) => r["organization_id"] === p[0] && r["scope_kind"] === p[1] && r["scope_id"] === p[2],
        ),
      };
    }
    ...
    if (sql.includes("content_hash = $2")) {
      return {
        rows: [...rows.values()].filter((r) => r["organization_id"] === p[0] && r["content_hash"] === p[1]).slice(0, 1),
      };
    }
    return { rows: [] };
  };
  return {
    execute: exec,
    executeTransaction: async (op: (e: { execute: typeof exec }) => unknown) => op({ execute: exec }),
...
function unit(over: Partial<DocUnit> = {}): DocUnit {
  return {
    ...
    title: "Onboarding",
    summary: "Welcome",
    contentRef: "wiki:onboarding#Onboarding",
    contentHash: "a".repeat(64),
    status: DocLifecycleState.Draft,
    freshnessAt: "2026-05-30T00:00:00Z",
    boundHatIds: ["documentation_reviewer"],
    boundStageIds: ["brd"],
    provenanceChangeSetId: "cs-42",
...
test("doc unit store filters by scope, status, and content hash (the retrieval pre-filter substrate)", async () => {
  const store = createCockroachDocUnitStore({ executor: fakeExecutor() });
  await store.upsert(
    unit({ docUnitId: "du-1", scopeId: "eng", status: DocLifecycleState.Active, contentHash: "b".repeat(64) }),
  );
  await store.upsert(
    unit({ docUnitId: "du-2", scopeId: "sales", status: DocLifecycleState.Draft, contentHash: "c".repeat(64) }),
  );
