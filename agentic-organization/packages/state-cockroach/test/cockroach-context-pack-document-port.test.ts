...
import { DocLifecycleState, DocScopeKind, DocType, type DocUnit } from "../../domain/src/index.ts";
import {
...
import { createCockroachContextPackDocumentPort } from "../src/cockroach-context-pack-document-port.ts";
...
test("Cockroach context-pack document port applies consult outcome utility from a generic reader", async () => {
  ...
  ok(
    result.retrieval.hits.some(
      (hit) => hit.unit.docUnitId === "historically-risky" && hit.reasons.includes("kpi:-1.00"),
    ),
  );
...
function fakeExecutor(): CockroachGenericSqlExecutor {
  ...
  const execute = async (statement: { sql: string; parameters: readonly unknown[] }) => {
    ...
    if (sql.includes("scope_kind = $2 AND scope_id = $3")) {
      return {
        rows: [...docUnitRows.values()].filter(
          (row) =>
        ...
        rows: [...docUnitRows.values()].filter(
          (row) =>
            ...
            (boundHatIds.some((hatId) => (JSON.parse(row["bound_hat_ids"] as string) as string[]).includes(hatId)) ||
              boundStageIds.some((stageId) =>
                (JSON.parse(row["bound_stage_ids"] as string) as string[]).includes(stageId),
              )),
        ),
      };
    }
