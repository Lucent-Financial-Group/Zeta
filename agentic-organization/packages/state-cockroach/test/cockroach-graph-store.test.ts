...
function fakeExecutor(): CockroachGenericSqlExecutor {
  ...
  const exec = async (s: { sql: string; parameters: readonly unknown[] }) => {
    ...
    const sql = s.sql;
    if (sql.includes(`INTO agentic_org_graph_nodes`)) {
      nodes.set(p[0] as string, {
        node_id: p[0],
        organization_id: p[1],
        kind: p[2],
        source_key: p[3],
    ...
    if (sql.includes(`INTO agentic_org_graph_edges`)) {
      edges.set(p[0] as string, {
        edge_id: p[0],
        organization_id: p[1],
        from_node_id: p[2],
        to_node_id: p[3],
    ...
    if (sql.includes("WHERE node_id = $1")) {
      const r = nodes.get(p[0] as string);
      return { rows: r ? [r] : [] };
    }
    if (sql.includes("from_node_id = $2")) {
      return {
        rows: [...edges.values()].filter(
          (e) =>
            e["organization_id"] === p[0] &&
            e["from_node_id"] === p[1] &&
            e["confidence"] !== GraphConfidence.Retracted,
        ),
      };
    }
    if (sql.includes("to_node_id = $2")) {
      return {
        rows: [...edges.values()].filter(
          (e) =>
            e["organization_id"] === p[0] && e["to_node_id"] === p[1] && e["confidence"] !== GraphConfidence.Retracted,
        ),
      };
    }
    if (sql.includes("change_set_id = $1")) {
      return { rows: [...edges.values()].filter((e) => e["change_set_id"] === p[0]) };
    }
    return { rows: [] };
  };
  return {
