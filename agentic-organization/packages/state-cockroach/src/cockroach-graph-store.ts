...
function rowToNode(r: NodeRow): GraphNode {
  return {
    ...
    label: r.label,
    confidence: r.confidence as GraphConfidence,
    provenance: parseJson<GraphProvenance>(r.provenance),
    attributes: parseJson<Record<string, string>>(r.attributes),
...
function rowToEdge(r: EdgeRow): GraphEdge {
  return {
    ...
    confidence: r.confidence as GraphConfidence,
    provenance: parseJson<GraphProvenance>(r.provenance),
    ...(r.change_set_id !== null ? { changeSetId: r.change_set_id } : {}),
    ...(r.retraction_reason !== null ? { retractionReason: r.retraction_reason } : {}),
    createdAt: toIso(r.created_at),
    updatedAt: toIso(r.updated_at),
    version: toNum(r.version),
  };
...
export function createCockroachGraphStore(input: CreateCockroachGraphStoreInput): GraphStore {
  ...
  const edgeCols =
    "edge_id, organization_id, from_node_id, to_node_id, kind, confidence, provenance, change_set_id, retraction_reason, created_at, updated_at, version";
  ...
  return {
    ...
    async getNode(nodeId: string): Promise<GraphNode | null> {
      const r = await input.executor.execute<NodeRow>({
        name: "get_graph_node",
        sql: `SELECT node_id, organization_id, kind, source_key, label, confidence, provenance, attributes, created_at, updated_at, version FROM ${N} WHERE node_id = $1`,
        parameters: [nodeId],
      });
      return r.rows[0] ? rowToNode(r.rows[0]) : null;
    },
    async outEdges(organizationId: string, fromNodeId: string): Promise<readonly GraphEdge[]> {
      const r = await input.executor.execute<EdgeRow>({
        name: "graph_out_edges",
        sql: `SELECT ${edgeCols} FROM ${E} WHERE organization_id = $1 AND from_node_id = $2 AND ${activeEdge}`,
        parameters: [organizationId, fromNodeId],
      });
    ...
    async inEdges(organizationId: string, toNodeId: string): Promise<readonly GraphEdge[]> {
      const r = await input.executor.execute<EdgeRow>({
        name: "graph_in_edges",
        sql: `SELECT ${edgeCols} FROM ${E} WHERE organization_id = $1 AND to_node_id = $2 AND ${activeEdge}`,
        parameters: [organizationId, toNodeId],
      });
      return r.rows.map(rowToEdge);
    },
    async edgesByChangeSet(changeSetId: string): Promise<readonly GraphEdge[]> {
      const r = await input.executor.execute<EdgeRow>({
        name: "graph_edges_by_change_set",
        sql: `SELECT ${edgeCols} FROM ${E} WHERE change_set_id = $1`,
