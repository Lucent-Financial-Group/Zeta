...
test("a well-formed row validates", () => {
  const result = validateRow(
    row({ id: "42", status: "ready", title: "t", project_id: "7", depends_on: ["8", "9"] }),
    taskSchema,
  );
...
test("a stray 'table' key in values is flagged as unknown column (no bypass)", () => {
  const result = validateRow(
    { table: "task", values: { id: "42", status: "ready", title: "t", table: "task" } },
    taskSchema,
  );
  equal(result.outcome, "invalid");
  if (result.outcome !== "invalid") return;
...
test("missing required column is reported", () => {
  ...
  if (result.outcome !== "invalid") return;
  equal(
    result.violations.some((v) => v.column === "title" && v.reason === "required_missing"),
    true,
  );
});
...
test("unknown column is reported (schema is the contract)", () => {
  ...
  if (result.outcome !== "invalid") return;
  equal(
    result.violations.some((v) => v.column === "nonsense" && v.reason === "unknown_column"),
    true,
  );
});
...
test("neighbors resolves edge ids against a store", () => {
  ...
  const found = neighbors(
    row({ id: "42", status: "ready", title: "t", depends_on: ["8", "9"] }),
    taskSchema,
    "depends_on",
    store,
  );
