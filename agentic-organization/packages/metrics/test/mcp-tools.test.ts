...
import { METRICS_TOOL_DESCRIPTORS, MetricsToolName, dispatchMetricsTool } from "../src/mcp-tools.ts";
...
test("dispatch analyze_source returns a metrics report", () => {
  const result = dispatchMetricsTool(MetricsToolName.AnalyzeSource, {
    filePath: "x.ts",
...
test("dispatch run_review_board routes through the board", () => {
  ...
  const result = dispatchMetricsTool(MetricsToolName.RunReviewBoard, {
    findings: [finding],
    votes: [mk("a"), mk("b"), mk("c")],
  });
  equal(result.outcome, "ok");
