...
export async function runWorkOsCycle(deps: WorkOsCycleDeps): Promise<WorkOsCycleReport> {
  ...
  const intakeDeps: IntakeDeps = {
    ...
    nowIso: nextIso,
    existsByExternalRef: () => false,
    appendEvent: emit,
  };

  // ── 1. External customer defect flows IN → triaged into the backlog ──
  const norm = normalizeIntake({
    source: "customer_portal",
    externalId: "TICKET-501",
    kind: "defect",
    title: "checkout returns 500 on coupon",
  ...
  const emanager =
    deps.hats.find((h) => h.level === HatLevel.Manager && h.id === "engineering_manager") ??
    deps.hats.find((h) => h.level === HatLevel.Manager)!;
  const batch: WorkBatch = {
    batchId: deps.createId("batch"),
    organizationId: deps.organizationId,
    scopeKind: "initiative",
    scopeId: deps.initiativeId,
    ownerHatId: emanager.id,
  ...
  for (const [from, to] of [
    [WorkBatchState.Created, WorkBatchState.Scoped],
    [WorkBatchState.Scoped, WorkBatchState.CapacityPlanned],
    [WorkBatchState.CapacityPlanned, WorkBatchState.Scheduled],
    [WorkBatchState.Scheduled, WorkBatchState.Active],
  ] as const) {
    await emit({
      id: deps.createId("evt"),
      ...
      occurredAt: nextIso(),
      organizationId: deps.organizationId,
      actorHatId: emanager.id,
      subjectId: batch.batchId,
      fromState: from,
      toState: to,
      decision: `batch ${from} → ${to}`,
      supervisorChain: [],
      evidenceRefs: [],
      correlationId: batch.batchId,
      causationId: batch.batchId,
  ...
  await simpleTransition(
    ...
    "backend_implementer",
    "fix implemented; into review/QA",
  );
  ...
  const cases = deriveTestCasesFromBrd(
    {
      ...
      projectId: deps.projectId,
      initiativeId: deps.initiativeId,
      suiteId: "suite-coupon",
      authoredByHatId: "qa_verifier",
      acceptanceCriteria: ["coupon applies discount", "checkout succeeds with coupon"],
    },
  ...
  const priorRuns: TestRun[] = [
    {
      testRunId: deps.createId("run"),
      testCaseId: cases[1]!.testCaseId,
      suiteId: "suite-coupon",
      organizationId: deps.organizationId,
      ...
      evidence: [],
      startedAt: iso(deps.baseTimeMs - 86_400_000),
      finishedAt: iso(deps.baseTimeMs - 86_400_000),
    },
  ];
  const failPlan = new Map([
    [cases[0]!.testCaseId, TestRunOutcome.Failed],
    [cases[1]!.testCaseId, TestRunOutcome.Failed],
  ]);
  ...
  for (let round = 0; round < CHURN_TARGET; round += 1) {
    const qa = await runQaCycle({
      ...
      cases,
      priorRuns: accumulatedRuns,
      executor: createDeterministicExecutor(failPlan),
      qaHatId: "qa_verifier",
      qaAgentId: "agent-qa",
      createId: deps.createId,
      nowIso: nextIso,
    ...
    await simpleTransition(
      ...
      WorkItemState.InProgress,
      "qa_verifier",
      `QA round ${round + 1} failed → rework (bounce-back)`,
    );
    await simpleTransition(
      item.workItemId,
      WorkItemState.InProgress,
  ...
  if (churn) {
    await emit({
      ...
      decision: `churn: ${bounceBackCount(item.workItemId, allEvents)} QA bounce-backs — escalating`,
      supervisorChain: [],
      evidenceRefs: [],
    ...
    const escCtx = {
      ...
      nowIso: nextIso,
      supervisorChain: ["ceo", emanager.id],
      correlationId: item.workItemId,
      causationId: item.workItemId,
      traceId: item.workItemId,
    };
    ...
    const addAgents = decideEscalation(
      {
    ...
    if (addAgents.outcome === "escalated") {
      ...
      if (addAgents.change.kind === "expand_supply") {
        for (const hatId of addAgents.change.ownerHatIds) {
          const votes: HatSupplyVote[] = ["engineering_director", "cfo", "cost_controller"].map((v) => ({
            voterHatId: v,
            approve: true,
            proposedTarget: 2,
          }));
          const supply = decideHatSupply(
            { hatId, hatName: byId.get(hatId)?.name ?? hatId, requiredCount: 2, currentCount: 1, votes },
            { ...escCtx, supervisorChain: ["ceo", "cfo"] },
          );
    ...
    const bringArchitect = decideEscalation(
      {
        trigger: EscalationTrigger.RepeatedQaBounceBack,
        workItemId: item.workItemId,
        ownerHatIds: owners,
        deciderHat: emanager,
        chooser: () => ({ index: 1, reason: "the approach is failing QA repeatedly; bring in an architect" }),
      },
      escCtx,
    );
    if (bringArchitect.outcome === "escalated" && bringArchitect.change.kind === "new_approach") {
      ...
      await emit({
        ...
        organizationId: deps.organizationId,
        actorHatId: emanager.id,
        subjectId: bringArchitect.change.architectHatId,
        decision: `architect assigned to re-approach ${item.workItemId}`,
        supervisorChain: ["ceo", emanager.id],
        evidenceRefs: [],
        correlationId: item.workItemId,
        causationId: item.workItemId,
      ...
      await emit({
        ...
        occurredAt: nextIso(),
        organizationId: deps.organizationId,
        actorHatId: "architect",
        subjectId: item.workItemId,
        fromState: bringArchitect.change.reopenGate,
        toState: "reopened",
        decision: `architecture gate reopened — new approach for ${item.workItemId}`,
        supervisorChain: ["ceo", "engineering_director", "architect"],
        evidenceRefs: [],
        correlationId: item.workItemId,
        causationId: item.workItemId,
        traceId: item.workItemId,
  ...
  const greenQa = await runQaCycle({
    ...
    cases,
    priorRuns: accumulatedRuns,
    executor: createDeterministicExecutor(new Map()), // empty plan → all Passed
    qaHatId: "qa_verifier",
    qaAgentId: "agent-qa",
    createId: deps.createId,
    nowIso: nextIso,
  ...
  testSummary = {
    runs: greenQa.summary.runs,
    failures: greenQa.summary.failures,
    regressionsOpen: greenQa.summary.regressionsOpen,
    defectsOpenedInTestSetup: testSummary.defectsOpenedInTestSetup,
  };
  await simpleTransition(
    item.workItemId,
    WorkItemState.Review,
    WorkItemState.Done,
    "release_manager",
    "QA green after architect re-approach → released to main",
  );
