...
export function createJiraHttpClient(input: CreateJiraHttpClientInput): JiraClient {
  ...
  return {
    async transition(issueKey, statusName): Promise<void> {
      ...
      await fail(
        await doFetch(`${api}/issue/${issueKey}/transitions`, {
          method: "POST",
          headers,
    ...
    async comment(issueKey, body): Promise<void> {
      await fail(
        await doFetch(`${api}/issue/${issueKey}/comment`, {
          method: "POST",
          headers,
          body: JSON.stringify({
            body: {
...
export function createJiraCardPort(deps: {
  client: JiraClient;
  statusMap: JiraStatusMap;
  issueKeyFor: (cs: ChangeSet) => string;
  nowMs: () => number;
}): ChangeControlPort {
  return {
    ...
    async project(cs: ChangeSet): Promise<ProjectionRef> {
      ...
      await deps.client.comment(
        key,
        `Org ChangeSet ${cs.changeSetId} entered review (${cs.artifacts.length} artifact(s)).`,
      );
      return {
        system: ExternalSystem.Jira,
        externalId: key,
        url: `${key}`,
        lastSyncedState: deps.statusMap.reviewStatus,
    ...
    async pull(ref: ProjectionRef): Promise<ExternalReviewState> {
      ...
      const decision =
        status === deps.statusMap.approvedStatus
          ? ExternalDecision.Approved
          : status === deps.statusMap.changesStatus
