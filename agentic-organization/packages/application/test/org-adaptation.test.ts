...
test("C2: onboarding is planned as WORK the org runs through its normal loop (ingest/graph/handbooks/autonomy)", () => {
  ...
  ok(
    titles.some((t) => t.includes("autonomy dial")),
    "the autonomy confirmation is itself a work item",
  );
...
test("C6: self-healing emits corrective work ONLY for non-zero drift signals (no busy-work)", () => {
  const none = planSelfHealing(
    { staleDocCount: 0, failingGateCount: 0, lowConfidenceEdgeCount: 0, docConflictCount: 0 },
    deps,
  );
  equal(none.work.length, 0, "a healthy org plans no healing work");

  const drifting = planSelfHealing(
    { staleDocCount: 3, failingGateCount: 1, lowConfidenceEdgeCount: 5, docConflictCount: 2 },
    deps,
  );
...
test("C6: a partial drift produces only the matching corrections", () => {
  const partial = planSelfHealing(
    { staleDocCount: 7, failingGateCount: 0, lowConfidenceEdgeCount: 0, docConflictCount: 0 },
    deps,
  );
