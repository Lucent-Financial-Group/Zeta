...
function harness(
  plan: Map<string, TestRunOutcome>,
  cases: ReturnType<typeof deriveTestCasesFromBrd>,
  priorRuns: readonly TestRun[] = [],
) {
  ...
  const deps: QaCycleDeps = {
    ...
    openDefect: async (input) => {
      defects.push(input);
      return { defectId: `def-${defects.length}` };
    },
  };
  ...
  const cases = deriveTestCasesFromBrd(
    {
      ...
      acceptanceCriteria: ["login works", "logout works", "session expires"],
    },
...
test("a standing QA cycle runs every case, records runs, and opens a defect on failure", async () => {
  ...
  const plan = new Map([
    ["tc-1", TestRunOutcome.Passed],
    ["tc-2", TestRunOutcome.Failed],
  ]);
  ...
  const prior: TestRun[] = [
    {
      ...
      finishedAt: "2023-01-01T00:00:00.000Z", // before the cycle clock
    },
  ];
