...
test("structural facts are EXTRACTED (a parser proved them); every edge carries provenance", () => {
  ...
  ok(
    r.nodes.every((n) => n.confidence === GraphConfidence.Extracted),
    "manifest-parsed nodes are extracted",
  );
  ...
  ok(
    kinds.has(GraphEdgeKind.DependsOn) &&
      kinds.has(GraphEdgeKind.Exposes) &&
      kinds.has(GraphEdgeKind.PersistsTo) &&
      kinds.has(GraphEdgeKind.TestedBy),
  );
...
test("ownership is INFERRED even though parsed (a structural fact differs from an ownership signal)", () => {
  ...
  equal(
    ...
    "owned_by is a candidate needing confirmation, not an extracted fact",
  );
});
