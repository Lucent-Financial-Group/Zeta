...
test("hat stage: a satisfiable gate advances the cursor (ReviewStageAdvanced)", () => {
  ...
  const r = runReviewStage(changeSet({ currentStageIndex: 0 }), pl, deps({ blockingFindings: () => 0 }));
...
test("quorum stage: the board's agreement IS the gate (≥ threshold → approve)", () => {
  ...
  const agreed = runReviewStage(changeSet({ currentStageIndex: 0 }), pl, deps({ quorumApprovals: () => 3 }));
...
test("external stage: an external approval flows IN as a gate satisfaction (not a bypass)", () => {
  ...
  const approved = runReviewStage(changeSet({ currentStageIndex: 0 }), pl, deps({ externalDecision: () => ExternalDecision.Approved }));
...
test("a blocking stage may reject — terminal", () => {
  ...
  const r = runReviewStage(changeSet(), pl, deps({ blockingFindings: () => 0, hatChooser: (legal) => ({ index: legal.indexOf(StageOutcome.Reject), reason: "unsafe" }) }));
