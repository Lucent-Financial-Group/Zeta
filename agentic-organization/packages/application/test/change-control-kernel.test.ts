...
test("openChangeSet: simulation evidence permits config policy changes to enter review", () => {
  ...
  const r = openChangeSet(
    configChangeSet({ phase: ChangeSetPhase.Drafted, currentStageIndex: 5 }),
    deps({
...
test("openChangeSet: simulation evidence must be verified and bound to the change set", () => {
  ...
  const r = openChangeSet(
    configChangeSet({ phase: ChangeSetPhase.Drafted }),
    deps({
...
test("openChangeSet: code artifacts targeting policy surfaces also require simulation evidence", () => {
  const r = openChangeSet(
    changeSet({
      ...
      targetRef: "org-policy/rmo",
      artifacts: [
        { kind: ChangeArtifactKind.CodeDiff, path: "config/rmo-policy.ts", diff: "+rate = 0.2", language: "ts" },
      ],
    }),
    deps(),
  );
...
test("openChangeSet: plural and underscored policy/config paths require simulation evidence", () => {
  const promptFlowChange = openChangeSet(
    changeSet({
      phase: ChangeSetPhase.Drafted,
      targetRef: "feat/prompt-flow-loader",
      artifacts: [
        { kind: ChangeArtifactKind.CodeDiff, path: "packages/prompt-flows/director.ts", diff: "+flow", language: "ts" },
      ],
    }),
    deps(),
  );
  const tenantConfigChange = openChangeSet(
    changeSet({
      phase: ChangeSetPhase.Drafted,
...
test("applyChangeSet: emergency waiver can release an approved config policy change", () => {
  ...
  const r = applyChangeSet(
    configChangeSet({ phase: ChangeSetPhase.Approved }),
    deps({
...
test("hat stage: a satisfiable gate advances the cursor (ReviewStageAdvanced)", () => {
  ...
  const r = runReviewStage(
    changeSet({ currentStageIndex: 0 }),
    pl,
    deps({
      blockingFindings: () => 0,
...
test("quorum stage: the board's agreement IS the gate (≥ threshold → approve)", () => {
  const pl = pipeline([
    stage({
      id: "security",
      authority: { kind: "quorum", hatIds: ["a", "b", "c"], threshold: 3 },
      gate: ReviewGateKind.QuorumAgreed,
    }),
    stage({ id: "final" }),
  ]);
  const agreed = runReviewStage(
    changeSet({ currentStageIndex: 0 }),
    pl,
    deps({
      quorumApprovals: () => 3,
...
test("human stage PAUSES the change set (HITL) — does not advance until resumed", () => {
  ...
  const resumed = resumeHumanStage(
    paused.changeSet,
    pl,
...
test("external stage: an external approval flows IN as a gate satisfaction (not a bypass)", () => {
  const pl = pipeline([
    stage({
      ...
      authority: { kind: "external", system: "github" },
  ...
  const waiting = runReviewStage(
    changeSet({ currentStageIndex: 0 }),
    pl,
    deps({ externalDecision: () => ExternalDecision.Pending }),
  );
...
test("changes_requested → resubmit bumps revision + re-enters review at stage 0", () => {
  const r = resubmitChangeSet(
    changeSet({ phase: ChangeSetPhase.ChangesRequested, revision: 1, currentStageIndex: 2 }),
    deps(),
  );
  equal(r.changeSet.phase, ChangeSetPhase.InReview);
