...
function doc(id: string, scopeId: string, title: string, summary: string): DocUnit {
  return {
    ...
    scopeKind: DocScopeKind.Service,
    scopeId,
    title,
    summary,
    contentRef: "r",
...
const corpus = [
  doc("d-billing", "billing", "Billing runbook", "restart the billing service"),
  doc("d-other", "sales", "Sales doc", "unrelated"),
];

test("C5 joins graph structure + doc prose into one answer about a service", async () => {
  const intel = await summarizeService("billing", "Billing", graph, corpus, [billingEntity], {
    organizationId: "org-lfg",
    docScope: { kind: DocScopeKind.Service, id: "billing" },
  });
  ...
  ok(
    !intel.describingDocs.some((d) => d.docUnitId === "d-other"),
    "a doc from another scope does not describe this service",
  );
});
