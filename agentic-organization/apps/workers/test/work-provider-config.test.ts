...
import {
  resolveChangeControlExternalPort,
  resolveWorkProviderFromEnv,
  WorkProviderConfigError,
} from "../src/work-provider-config.ts";
...
test("GEN3: WORK_PROVIDER=github → live ChangeControlPort, mode external:github", () => {
  const r = resolveChangeControlExternalPort(
    { WORK_PROVIDER: "github", GITHUB_TOKEN: "t", GITHUB_OWNER: "o", GITHUB_REPO: "r" },
    { nowMs: NOW, fetchImpl: noFetch },
  );
  ok(r.port !== null);
...
test("GEN3: WORK_PROVIDER=gitlab → live ChangeControlPort, mode external:gitlab", () => {
  const r = resolveChangeControlExternalPort(
    { WORK_PROVIDER: "gitlab", GITLAB_TOKEN: "t", GITLAB_PROJECT_ID: "g/p" },
    { nowMs: NOW, fetchImpl: noFetch },
  );
  ok(r.port !== null);
...
test("GEN3: WORK_PROVIDER=jira → a work-item provider leaves change-control internal-only", () => {
  const r = resolveChangeControlExternalPort(
    { WORK_PROVIDER: "jira", JIRA_TOKEN: "t", JIRA_BASE_URL: "https://x/rest/api/3", JIRA_PROJECT_KEY: "ENG" },
    { nowMs: NOW, fetchImpl: noFetch },
  );
...
test("GEN3: WORK_PROVIDER=linear resolves a work-item provider", () => {
  const provider = resolveWorkProviderFromEnv(
    { WORK_PROVIDER: "linear", LINEAR_TOKEN: "t", LINEAR_TEAM_ID: "team" },
    { nowMs: NOW, fetchImpl: noFetch },
  );
  ok(provider !== null);
...
test("GEN3: a SELECTED provider missing a required field throws (fail fast, never silent)", () => {
  ...
  try {
    resolveChangeControlExternalPort(
      { WORK_PROVIDER: "github", GITHUB_TOKEN: "t" },
      { nowMs: NOW, fetchImpl: noFetch },
    );
  } catch (e) {
    threw = true;
    ok(e instanceof WorkProviderConfigError);
    ok(String((e as Error).message).includes("GITHUB_OWNER"));
  }
  ok(threw);
});
...
test("GEN3: an unknown WORK_PROVIDER value throws", () => {
  let threw = false;
  try {
    resolveWorkProviderFromEnv({ WORK_PROVIDER: "bitbucket" }, { nowMs: NOW });
  } catch (e) {
    threw = true;
    ok(String((e as Error).message).includes("unknown WORK_PROVIDER"));
  }
  ok(threw);
});
