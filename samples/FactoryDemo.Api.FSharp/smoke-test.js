...
let fail = 0;
async function check(label, path, extractor, expected) {
    try {
...
async function main() {
    ...
    await check("root.name contains 'Factory-demo'", "/", (j) => /Factory-demo/.test(String(prop(j, "name"))), "true");
    await check("/api/customers length", "/api/customers", (j) => j.length, "20");
    await check("/api/opportunities length", "/api/opportunities", (j) => j.length, "30");
    await check("/api/activities length", "/api/activities", (j) => j.length, "33");
    await check("customer #1 name", "/api/customers/1", (j) => prop(j, "name"), "Alice Plumbing LLC");
    await check("opportunity #1 stage", "/api/opportunities/1", (j) => prop(j, "stage"), "Lead");
    await check("customer #1 activities count", "/api/customers/1/activities", (j) => j.length, "4");
    // Pipeline funnel — per-stage counts. F# may emit PascalCase; prop() handles both.
    await check("funnel Lead count", "/api/pipeline/funnel", (j) => prop(j.find((x) => prop(x, "stage") === "Lead"), "count"), "10");
    await check("funnel Qualified count", "/api/pipeline/funnel", (j) => prop(j.find((x) => prop(x, "stage") === "Qualified"), "count"), "6");
    await check("funnel Won count", "/api/pipeline/funnel", (j) => prop(j.find((x) => prop(x, "stage") === "Won"), "count"), "6");
    await check("funnel Lost count", "/api/pipeline/funnel", (j) => prop(j.find((x) => prop(x, "stage") === "Lost"), "count"), "2");
    await check("funnel Lead totalCents", "/api/pipeline/funnel", (j) => prop(j.find((x) => prop(x, "stage") === "Lead"), "totalCents"), "5400000");
    await check("funnel Won totalCents", "/api/pipeline/funnel", (j) => prop(j.find((x) => prop(x, "stage") === "Won"), "totalCents"), "2670000");
    await check("duplicate pairs count", "/api/pipeline/duplicates", (j) => j.length, "2");
    await check("alice@acme.example pair members", "/api/pipeline/duplicates", (j) => {
        const pair = j.find((x) => prop(x, "email") === "alice@acme.example");
        return prop(pair, "customerIds")?.join(",");
    }, "1,13");
    await check("bob@trades.example pair members", "/api/pipeline/duplicates", (j) => {
        const pair = j.find((x) => prop(x, "email") === "bob@trades.example");
        return prop(pair, "customerIds")?.join(",");
    }, "5,19");
    // 404 behavior
    try {
