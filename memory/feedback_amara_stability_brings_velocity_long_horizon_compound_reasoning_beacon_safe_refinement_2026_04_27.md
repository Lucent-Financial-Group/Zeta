...
- Update CURRENT-aaron.md on next refresh to reflect the refined framing
- When 0/0/0 reached, propose AGENTS.md addendum clarifying "Velocity over stability" as spike-rule-not-doctrine (per Amara's blade note)
- Compose into the input/invariants-clarification skill domain (#57): the "long-horizon vs short-horizon" reasoning-mode classifier is a key skill domain element
- Consider promoting "Stability is velocity amortized" to a top-level factory aphorism — it's compact + memorable + load-bearing
