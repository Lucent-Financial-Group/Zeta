...
**The structural fix (PR #662):**

- Add `java-kotlin` to the analyze matrix in `.github/workflows/codeql.yml`
  with `build-mode: none`
- Remove `tools/alloy/**` from `paths-ignore` in
  `.github/codeql/codeql-config.yml` so the extractor actually scans
  AlloyRunner.java
- Add `*.java` to path-gate's code-changed patterns
