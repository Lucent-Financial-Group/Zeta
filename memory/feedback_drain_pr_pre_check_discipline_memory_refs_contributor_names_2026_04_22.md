...
2. **Contributor-name prose in filenames or body** — `docs/AGENT-BEST-PRACTICES.md` L284-L290: names appear only in `memory/<role>/<persona>/<name>/` and optionally `docs/BACKLOG.md`. Research-doc filenames like `oss-contributor-handling-lessons-from-aaron-2026-04-21.md` carry the name through the filename reference into any doc that cites the file — two levels of violation. Rename the files during drain; update any cross-refs.
