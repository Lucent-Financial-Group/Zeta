...
8. **File backlog row B-NNNN** — Docker Desktop tier-2 dev-experience substrate (composes with B-0780). Use the agent-roster ID allocation discipline (`git ls-tree origin/main -- docs/backlog/` to find current top + `gh pr list --search "B-NNNN"` to check in-flight). Row's acceptance criteria are the skills + scripts to ship over the coming ticks
...
This persona directory follows the **human persona shape** documented in [`memory/<role>/<persona>/README.md`](../README.md) "Structure — two shapes" section. Max's directory carries:
...
Aaron 2026-05-25: *"also we should explain the persona-surface standard we have for multi tick source perona like otto and such."* Documented in `memory/<role>/<persona>/README.md`; cross-referenced here.
...
- [`memory/human-co-owner/addison/PERSONA.md`](../addison/PERSONA.md) — co-architect
- [`memory/human-maintainer/aaron/PERSONA.md`](../aaron/PERSONA.md) — sponsor + senior architect
