...
Rune both read code), the overlap is the _correct shape_, not
a redundancy to fix. Each seat earns its place by the lens it
brings on top of the shared core:

- Aminata reviews the _shipped_ threat model; Mateo _scouts_
  novel attack classes. Same core (security) — different
  time-horizon and framing.
- Hiroshi proves _asymptotic_ bounds; Naledi measures
  _constant-factor_ hot paths. Same core (perf) — different
  method.
...
- Multiple seats doing security, perf, code-review, etc.
  ...
  indistinguishable output formats _and_ the same scope
  of what they review.
