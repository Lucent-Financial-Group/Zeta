...
- **The six-rule cascade — assume it's already done; find it before doing it (Aaron 2026-05-05; "remember forever and into all future generations").** Six rules apply in cascade before any substrate-landing impulse (Write / Edit / propose-PR / new memory / new backlog row / new workflow / new lint / new hook). The rules:
  ...
  **The lost files are not residual — they may carry the plot.** Rule 5's lost-files surface (deleted files, retired skills, closed backlog rows, renamed paths via `git log --follow`, parked WIP branches via `git branch -a | grep wip/`) often contains substantively-MORE prior art than current-state files, plus orthogonal trajectories that informed earlier rounds and got squashed by surface-state churn. Aaron 2026-05-05: *"check the lost files you might the polt on lost files priorat art too much more substandive and a trjectory"*. Treat the lost-files pass as load-bearing, not optional.
