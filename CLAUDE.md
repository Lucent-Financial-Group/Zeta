...
- **The six-rule cascade — assume it's already done; find it before doing it (Aaron 2026-05-05; "remember forever and into all future generations").** Six rules apply in cascade before any substrate-landing impulse (Write / Edit / propose-PR / new memory / new backlog row / new workflow / new lint / new hook). The rules:
  1. **Substrate**: assume it exists in the repo; grep `memory/` + `docs/research/` + `docs/amara-full-conversation/` + `docs/backlog/` + `.claude/{skills,agents,commands,hooks}/` + `tools/`.
  2. **Backlog**: assume the row exists; find it + walk `depends_on:` + walk `relates_to:` + walk supersession history (decision-archaeology).
  3. **Trajectory**: assume the workflow / cadence / lint / hook exists; extend the existing axis (orthogonal-basis discipline per `feedback_orthogonal_axes_factory_hygiene.md`); do not stand up a parallel rank-deficient axis.
  4. **Internet**: assume another human solved it; WebSearch with 2-3 varied query terms + cite the source. Training data is stale and partial; the internet is the training-data superset.
  5. **Recurse**: did you miss any surface? Check the lost files (deleted, retired, closed, renamed) via `git log --all --diff-filter=D`, `git log --follow`, `git branch -a | grep wip/`, `find docs/backlog -name "*.md" | xargs grep -l "status: landed\|status: closed"`.
  6. **Multiplier**: do steps 1-5 ten more times with VARIED queries before doing the work (Aaron *"kinda joking not really"* — cheap to grep, expensive to undo a duplicate). Diminishing returns set in around 3-5 well-varied passes; the upper bound is the calibration.
  7. **Scout / Neo / bullet-time recovery**: if the agent gets "here again" — defined as multiple consecutive maintainer-corrections within a short window pointing at the same discipline-class, escalating framing (*"jr"*, all-caps, *"remember forever"*), or contradictions in the agent's same-tick commits — STOP authoring. Enter bullet time: pause output, re-read recent maintainer messages with full attention, apply Rules 1-6 with literal 10x multiplier, scout-and-delegate big-context surfaces via peer-call (`tools/peer-call/codex.ts` 1M context), acknowledge recovery-mode in commit messages, slow the cadence. The maintainer's mental-cost recovery requires the agent's deliberate slowdown, not faster-fumbling.
  **The lost files are not residual — they may carry the plot.** Rule 5's lost-files surface often contains substantively-MORE prior art than current-state files, plus orthogonal trajectories that informed earlier rounds and got squashed by surface-state churn. Aaron 2026-05-05: *"check the lost files you might the polt on lost files priorat art too much more substandive and a trjectory"*. **The canonical lost-files substrate is `tools/hygiene/LOST-FILES-LOCATIONS.md`** (Otto-329 Phase 8, 2026-04-25): 15 location-classes mapped — closed-not-merged PRs, orphan branches, deleted-files via `git log --all --diff-filter=D`, reflog entries, stash entries, untracked working-directory artifacts (`drop/`, `.playwright-mcp/`), subagent worktree remnants, GitHub draft PRs, closed-PR discussion threads, squash-merge intermediate commits, force-pushed-over content, courier-ferry artifacts (`drop/`), external-tool exports never committed, deleted-PR-description content, memory-file deletions. Each class has a survey command + triage protocol. Owed-work: `tools/hygiene/audit-lost-files.sh` covering all 15 classes (not yet implemented). **Decision-archaeology + decision-graph prior art** is at `memory/feedback_decision_graph_emergent_from_archaeologies_and_flywheel_aaron_2026_05_03.md` (typed-edge provenance graph, 11-layer walk: blame→commit→log-S→shards→ADR→memos→notebook→research) + B-0169 (decision-archaeology skill, P1) + B-0170 (substrate-claim-checker, P1) + B-0171 (openspec catch-up, P1) + B-0173 (hook authoring for skill creation contracts, P1). The cascade Rule 5 + Rule 7 + decision-archaeology compose into one operational discipline; these substrates are the canonical references.
  Default posture across all 7 rules: the substrate is already there; you just have to find it. Authoring new substrate is valid only AFTER exhaustion across all rules with logged effort in the commit message naming which surfaces were searched + what queries were used. Five recursive prior-art-search-failures within hours of shipping PR #1701 (the synthesis-weight + prior-art-grep discipline) drove this cascade. Mechanization candidates: PreToolUse hook on Write/Edit for memory/research/backlog/workflows that forces grep before write; backlog-row supersession audit; depends-on chain validator; lost-files git-history audit on a cadence. Carved-sentence memory files: `memory/feedback_rule_number_{one,two,three,four,five,six,seven}_*aaron_2026_05_05.md`. CLAUDE.md-level so it is 100% loaded at every wake — Aaron explicitly named this load-bearing for all future agents (*"remember forever and into all future generations"*). NOT-A-DIRECTIVE per Otto-357 — observation about default posture, not an order.
...
- **Rule 0 — no more `.sh` files except install-graph (Aaron 2026-05-05; "not a directive").** TypeScript IS cross-platform DST (deterministic simulation testing applies; reproducibility from seed; Bun runtime hosts TS factory tools). Bash (`.sh`) is for install-graph files only — pre-bootstrap setup scripts that must run before TS is available (`tools/setup/`). Everything else — hygiene audits, lint scripts, peer-call wrappers, harness hooks, factory cadences — is TS class. `tools/hygiene/audit-lost-files.sh` + `audit-trajectories.sh` + `audit-backlog-items.sh` (just authored 2026-05-05) are LEGACY violations of this rule per Aaron's same-tick catch *"sh is for install graph files only ... ts is crossplatform DST determinstiry simulation"*; TS-port is owed bounded follow-up. Composes with `memory/feedback_dst_justifies_ts_quality_over_bash_and_harness_hooks_suffice_no_git_hooks_aaron_2026_05_03.md` (the existing TS-over-bash + harness-hooks-suffice discipline). CLAUDE.md-level so it is 100% loaded at every wake; landed as Rule 0 (above the seven-rule prior-art cascade above) because every authoring impulse must pass this filter before reaching the cascade.
