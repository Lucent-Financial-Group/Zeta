...
function runCommand(command: string, args: readonly string[], allowFailure = false): CommandOutput {
...
function repoRootFromGit(): string {
  return runCommand("git", ["rev-parse", "--show-toplevel"]).stdout.trim();
}

export function parseNodeNames(raw: string): readonly string[] {
  return raw
    .split(/\s+/)
    .map((node) => node.trim())
...
export function isSafeNodeName(node: string): boolean {
  return SAFE_NODE_NAME.test(node);
}

function assertSafeNodeName(node: string): void {
  if (!isSafeNodeName(node)) {
    throw new Error(`unsafe node name: ${node}`);
  }
}

function allClusterNodes(): readonly string[] {
  const output = runCommand("kubectl", ["get", "nodes", "-o", "jsonpath={.items[*].metadata.name}"]).stdout;
  return parseNodeNames(output);
}

export function nfdLabelLines(labels: Readonly<Record<string, unknown>>): readonly string[] {
...
function nodeLabels(node: string): Readonly<Record<string, unknown>> {
  ...
  const parsed = JSON.parse(output.stdout) as {
    readonly metadata?: { readonly labels?: Readonly<Record<string, unknown>> };
  };
  return parsed.metadata?.labels ?? {};
}

function firstLinesMatching(lines: readonly string[], pattern: RegExp): readonly string[] {
  return lines.filter((line) => pattern.test(line)).slice(0, 10);
}

export function renderSummary(node: string, labels: readonly string[], capturedAt: string): string {
  ...
  const output: string[] = [`# ${node}`, "", `Captured: ${capturedAt}`, ""];
...
function supportsKubectlDebug(): boolean {
  return runCommand("kubectl", ["debug", "--help"], true).status === 0;
}

function captureTopologyXml(node: string, outDir: string, useKubectlDebug: boolean): void {
  let xml: string;
  if (useKubectlDebug) {
    const debug = runCommand(
      "kubectl",
      [
        "debug",
        `node/${node}`,
...
function renderTopologySvg(outDir: string): void {
  if (runCommand("lstopo", ["--version"], true).status !== 0) return;
  ...
  try {
    ...
    for (const node of nodes) {
      ...
      writeFileSync(
        ...
        renderSummary(node, labels, new Date().toISOString().replace(/\.\d{3}Z$/, "Z")),
      );
    ...
    process.stderr.write(`cluster-inventory: ${(err as Error).message}\n`);
    return 2;
  }
}
