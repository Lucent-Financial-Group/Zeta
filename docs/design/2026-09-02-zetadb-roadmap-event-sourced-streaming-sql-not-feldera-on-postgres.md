...
**Agent fairness protocol (Aaron 2026-09-10, named not invented):** if A references B, B sees the edge; B may reply; the reply cannot overwrite the cite. Two graphs (content DAG vs EntityId references) share the algebra and must not be fused into a knowledge-graph filesystem. Reverse index is `IncrementalJoin`; notify is a Reaqtor standing query; both accounts held. Research-grade: [`docs/research/2026-09-10-agent-fairness-protocol-inbound-references-are-visible-reply-cannot-overwrite.md`](../research/2026-09-10-agent-fairness-protocol-inbound-references-are-visible-reply-cannot-overwrite.md) (`081M26HWSZ6087G0R00373BN0Q`).

**Reverse indexes (Aaron 2026-09-11):** materialized views via `IncrementalJoin`, stored on a host `GroupCommitDiskDeltaLog`, not as jsonl on `main`. `CitedByIndex` (entities ⋈ cites) and `SearchIndex` (query ⋈ postings) are in `src/Core/ReverseIndex.fs` (`081M29ESZCQ087G0R001SM29DQ`). Ingest (`ReverseIndexIngest`, `081M2K1PF0K087G0R001JAF8V4`) tokenizes a host tree onto that log under the TS corpus policy (allowlist, excluded trees, 512 KiB cap). Query `landauer` hits ingested files. Cite ingest (`ReverseIndexCite`, `081M2NH3N4W087G0R0023MXFEB`) extracts a closed relation vocabulary onto the same WAL; `FairnessInbox` is subscriber-local notify (drain does not filter the shared fold; reply cannot overwrite). Schema deltas, evolution-window readers, and key-grant windows ride `ZetaDbFact` on that host directory. `OverlapRotator` (`081M2VDSD40087G0R00058B2ND`) is the one phase-bounded gate: schema expand-into and key Previous are two callers. Expiry needs no event. Not live catalog overlap. CI does not walk the Zeta tree. The TS git-rev builder is a local cache and must not flush to main (#16919 measured 59.7 MB). Cite-before-entity still lands (retroactive join = fairness). Not FUSE. Not a second git repo. Not live catalog overlap. Not Apple.
