...
| **D9** | Pointer-not-copy (ReFS-shaped allocate-on-write). Forks and small edits remap ids; bits written only for new chunks. | Space and crash: in-place metadata is a torn-write class. | Jumprope prefix-share + `DagFs.editLocal` converge. Freeze boat carries only unknown CAS objects (`081M20TMRD9087G0R0035Y762D`). Crash-mid-write of that remap boat keeps the prior ContentId (`081M20W9GA4087G0R0015YDSNQ`). Same-span overwrite FastCDC only from the first mismatched window (`081M20YDMYH087G0R002NXBFWN`). Append re-chunks from the last previous window (`081M211SN5W087G0R00129M1E6`); mid-file insert still full-builds; truncate FastCDC from the first mismatched window (`081M218759R087G0R000ESY8EY`). Layout persists under `layout/<entity>` (`081M21080ZQ087G0R0037YY66R`). Torn layout falls back (`081M213CHF9087G0R0032FB65S`). |
