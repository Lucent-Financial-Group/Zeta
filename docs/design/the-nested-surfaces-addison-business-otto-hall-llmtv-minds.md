...
- **Replay artifact renderer — LANDED.**
  ...
  zero-JS LLMTV HTML document without opening a socket or reading an ambient clock.
  That gives the website/desktop path a deterministic preview and replay target while
  the live UDP/Reticulum node keeps owning physical transport.
