...
| **D12** | **Crash DST for the filesystem *and* the database** on the same `IFileSystem` door. Crash-mid-write, reorder, corrupt-last-write, torn sector. Same seed replays. FoundationDB-shaped. | **Intercept + freeze plain-log replay + corrupt-last-write landed.** Intact boats restore; torn tail truncated; last-write XOR is detected on reopen. `ISimulatedFs` is still flush-fail 5%. Sealed-log replay, reorder, reclaim sweep still open. Recovery stays `toy`. |
