...
| **D12** | **Crash DST for the filesystem *and* the database** on the same `IFileSystem` door. Crash-mid-write, reorder, corrupt-last-write, torn sector. Same seed replays. FoundationDB-shaped. | **Intercept + freeze plain- and sealed-log replay + corrupt-last-write + reorder door landed.** Intact boats restore; torn tail truncated; last-write XOR detected on reopen; two matching Disposes commit second-first. Freeze still finishes object puts before the log boat. `ISimulatedFs` is still flush-fail 5%. Reclaim crash-mid-sweep intercept landed (`ArmCrashOnDelete`): extra garbage, not a missing live object. Native `IBlockIo` still open. Recovery stays `toy`. |
