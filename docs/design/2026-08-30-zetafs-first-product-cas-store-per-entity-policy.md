...
| Adapter | Role | Status |
|---|---|---|
| Linux FUSE | first-class Linux mount | not built; Fake VFS lookup/readdir landed (`ZetaFsPosixVfs`, ordinal). `..` path-contextual is a merge gate. Default collator = ordinal (C7). |
| FUSE-T (macOS, ungated) | first-class Mac mount without paid entitlement | not built; Fake VFS lookup/readdir landed (`ZetaFsPosixVfs`, Ascii refuse). Same gates; store stays ordinal. No kernel FUSE-T binary, so Apple Developer Program is not the next blocker. |
