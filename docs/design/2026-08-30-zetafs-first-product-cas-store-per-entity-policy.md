...
- **Changes:** POSIX is a view. WebDAV untouched. NFS not in this PR (C10). Linux collator = ordinal. FUSE-T collator = `CaseFold.Ascii` + refuse collisions (C7). Both mounts over the same ordinal store. **Slice landed (`081M1XXQ99J087G0R002XKCSRC`):** `src/Core/ZetaFsCollator.fs` — `Ordinal` / `Ascii` fold; `refuseCreate` reports `ConfusableWithExisting`; existing pairs enumerate and do not merge. **Slice landed (`081M1Y0722X087G0R002BRXZRJ`):** `ZetaFsInode` + `ZetaFsPosixNode` — `st_ino` side table; node cache stashes arrival parent; `lookup("..")` is path-contextual; Handle has no ArrivalParent. **Slice landed (`081M1Y1J92F087G0R002WKAWY3`):** `bindSymlink` / `readSymlink` — Symlink hub + UTF-8 target satellite; reopen keeps the bytes; target is not resolved. **Slice landed (`081M1Y32J02087G0R003V91R9G`):** POSIX rename dest rules — Eisdir / Enotdir / Enotempty do not tombstone; dest absent moves the hub; same parent+name is a no-op. No kernel FUSE, no FUSE-T binary, no Apple Developer Program enrollment.
