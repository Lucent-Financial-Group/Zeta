...
| DST gap | `docs/FOUNDATIONDB-DST.md`, `src/Core/ChaosEnv.fs` `ISimulatedFs`, `src/Core/FileSystem.fs` | **PR1 door landed:** `ZetaFsDeltaLog` / `ZetaFsStore.init` use `FileSystem.Current`; `IBlockIo` is a sketched polyfill adapter (`FileSystemBlockIo`). `InMemoryFileSystem` latency is virtual milliseconds, not `Thread.Sleep`. **Crash-mid-write intercept landed** (`ArmCrashMidWrite`). Plain and sealed freeze-log replay of intact boats landed. Still missing: native `IBlockIo`, reclaim sweep. Crash recovery claims stay `toy`. |
...
- **Changes:** This **promotes** recovery from `toy`. **Slice landed (081M1HR580V087G0R000NMY47N):** `ArmCrashMidWrite` + freeze torn-tail + GroupCommit in-memory torn-tail. **Slice landed (081M1HVVN9P087G0R003Z7E7B3):** plain freeze-log replay of intact boats; torn tail dropped. **Slice landed (081M1HY9K92087G0R002GN49HS):** `ArmCorruptLastWrite` — write acks, last bytes XOR 0xA5, reopen truncates. **Slice landed (081M1HZPTX7087G0R0013PARBX):** `ArmReorderNextTwo` — first matching Dispose is held; second commits first. Freeze still puts objects before the log boat. Still not the promotion. **Slice landed (081M1J2JYJ7087G0R000JSCD3K):** `ArmCrashOnDelete` — partial apply leaves extra garbage, not a missing live object. Still open: native `IBlockIo`. Still not the promotion. Seed list includes pwrite-during-freeze (ContentId equals snapshot G, never a mix). Until this corpus is green, READMEs keep the peel.
