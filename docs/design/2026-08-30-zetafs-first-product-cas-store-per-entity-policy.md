...
| **D12** | **Crash DST for the filesystem *and* the database** on the same `IFileSystem` door. Crash-mid-write, reorder, corrupt-last-write, torn sector. Same seed replays. FoundationDB-shaped. | **Intercept + freeze plain-log replay landed.** Intact intent+commit pairs restore on create; torn tail / intent-without-commit truncated. `ISimulatedFs` is still flush-fail 5%. Sealed-log replay, reorder, corrupt-last-write, reclaim sweep still open. Recovery stays `toy`. |
