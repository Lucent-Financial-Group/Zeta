...
> **First-class factory surface** (Aaron 2026-04-30, B-0109):
> *"looking at github status should be first class for us we
> live on git and github for now until we get a 2nd host in the
> future."* The factory's hot path runs through GitHub today;
> any GitHub degradation IS a factory degradation, with this
> page as the visibility layer.

This page answers three questions in under 30 seconds:

1. **What does the factory depend on?** (watched list — § Watched dependencies)
2. **Are any of those dependencies currently flagged or degraded?** (current state — § Live status sources)
...
|---|---|---|---|
| GitHub (LFG repo) | canonical development; PRs; auto-merge; required checks; merge target | <https://www.githubstatus.com/> + <https://www.githubstatus.com/api/v2/summary.json> | Pull Requests, Actions, API Requests, Webhooks, Codespaces (if used) |
...
| Dependency | Used for | Status source | Factory-relevant components |
|---|---|---|---|
| Anthropic API (Claude Code harness) | this autonomous-loop session; all Otto-attributable substrate authoring | <https://status.anthropic.com/> | API, Claude Code |
| OpenAI API (ChatGPT) | Aaron's ferry channel with Amara; Codex review | <https://status.openai.com/> | API, ChatGPT |
| Google Gemini | additional substrate ferry channel | <https://status.cloud.google.com/> | Gemini API |

### Language runtimes + package registries

| Dependency | Used for | Status source | Factory-relevant components |
|---|---|---|---|
| .NET SDK (mise-pinned `10.0.203`) | F# build; tests; analyzers | <https://github.com/dotnet/core/issues> (no formal status page) | NuGet feed |
...
| Python 3.14 (mise-pinned) | helper scripts; `uv tool install` flow | (none) | (uv registry: <https://pypi.org/>) |
| Java 26 (mise-pinned) | TLA+ + Alloy verifier jars | (none) | (none — local jar) |
| Lean (elan-managed, outside mise) | formal-verification proofs | <https://github.com/leanprover/lean4/issues> | Mathlib |
| Bun registry | npm package fetch under bun resolution | <https://status.npmjs.org/> | Registry |
| Homebrew | system-level macOS packages (mise itself + curl bootstrap) | <https://status.brew.sh/> | (formula fetch) |
| apt (Debian/Ubuntu) | system-level Linux packages | (no canonical status; per-mirror) | (none) |
| mise | language-runtime version manager | <https://github.com/jdx/mise/issues> | (none) |
...
Programmatic poll snippet (no auth required for GitHub status):
...
Aaron 2026-04-30 surfaced this concern citing Trunk.io's [merge-queue-builds-on-wrong-commit post](https://trunk.io/blog/what-happens-if-a-merge-queue-builds-on-the-wrong-commit). Verified at write-time:

- We do **not** use GitHub's merge-queue feature (`merge_queue: None` in branches/main/protection). The specific Trunk.io bug class doesn't apply directly.
- We **do** use auto-merge with squash + `allow_update_branch: true` (auto-rebases stale branches before merging).
...
- Otto sessions may stall on tool-call timeouts
- Aaron's ferry channels (Claude.ai, ChatGPT, Gemini) become unreliable
- Distinguish API-down from prompt-injection-defense (the latter is normal protective behavior)

### Package-registry availability

When npm / NuGet / pypi degrade:
- `tools/setup/install.sh` may fail with cryptic timeouts
- CI jobs that hit the registries (build, test, dotnet restore) may report failure when the underlying issue is registry-side

## Discovery hooks

This page is referenced from:

- `CLAUDE.md` — pending pointer line under "Read these, in this order" (will land in follow-up; this PR creates the surface, the pointer follows in a subsequent PR to keep the diffs reviewable)
- `AGENTS.md` — same plan
...
1. **CLAUDE.md + AGENTS.md pointer lines** — discoverability landing without rebuilding the surface
2. **Cron-driven scraper** writing `docs/dependency-status/current.json` from the GitHub status API (the most operationally-load-bearing source); other sources added incrementally
3. **Incident log** at `docs/dependency-status/incident-log.jsonl` for retrospective correlation between session-time anomalies and external-incident windows
4. **Tick-shard cross-reference** — when shard rows mention CI behaviour, allow a soft pointer to the incident-log row covering that timeframe
5. **Mitigation rules** — the conditional "when GitHub Pull Requests is degraded, do not arm auto-merge" rules belong in distinct backlog rows once the visibility surface is operational
