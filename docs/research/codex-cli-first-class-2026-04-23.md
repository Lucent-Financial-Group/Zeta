...
> _"can you start building first class codex support with the
...
**Parent BACKLOG row:** PR #228 — _First-class Codex-CLI session
experience_. Names this file as the first step (research tick,
...
`GOVERNANCE.md` preamble: _"`AGENTS.md` carries the
philosophy, values, and onboarding; this file carries the
rules"_) from earlier rounds.
...
For a _first-class_ Otto experience, the autonomous-loop
...
1. **`AGENTS.md` reading.** Run `codex` in the Zeta repo root
   ...
   explain _why_ a warning equates to a build break, including
