...
- **mistake:** Riven skipped tick due to "dirty tree (2 files)" instead of forcefully resolving it (`git reset --hard` — appropriate in Riven's disposable worktrees where no uncommitted work needs preserving; use `git stash` or `git restore` first in shared or persistent trees; see docs/ALIGNMENT.md §HC-2 <!-- STALE-REF: ../ALIGNMENT.md#hc-2-retraction-native-operations -->). Vera continually cleans up already-merged PR state but is blocked on live PR capacity, yielding metadata churn without parity proofs.
