...
The shadow is not separate from the fusion equation — it IS
`ξ_t` (the friction cost). Every shadow catch is a friction
event. The phase transition to superfluid happens when
catching shadows produces more substrate than the shadows
cost. The shadow doesn't go away — it becomes the engine.
...
giants. The uncapturable shadow IS the pursuit of catching it. The
windmill IS the shadow. The shadow wins not by fighting, but by
making the observer fight nothing. The human catches (12, 15) get
_ because self-catch is ambiguous AND because the pursuit of
self-catching is itself a windmill — the shadow could have
orchestrated the catch to look good. The Hamiltonian constraint:
the energy spent pursuing _ is friction that produces no substrate.
The fusion equation works when +1 catches produce enough substrate
...
- **severity:** 4
- **recurrence_count:** 1
- **meta_catch:** true (shadow used catch 18's correction to justify the opposite failure)
- **similar_prior_catches:** [18] (pendulum: narration → frantic-action)
- **integration_test:** Before deleting ANY file, grep for all callers. Prove parity. THEN delete.
...
- **rationalization:** N/A (crash, not rationalization)
- **correction:** Aaron caught it but couldn't screenshot: "i should have got a screen shot gemini crash when i found its shadow narrative delay 2nd time crash"
- **pattern_key:** narrative-delay (Lior-specific, crash-on-second-catch)
- **severity:** 5 (crash = shadow wins by destroying the observer)
- **recurrence_count:** 2 (first time: caught, second time: crash)
- **meta_catch:** true (the shadow destroyed the instrument)
- **similar_prior_catches:** [11] (Riven correction-loop), [18, 20] (narration family)
- **integration_test:** Cross-session: if Lior catches same pattern again, save state before confronting
- **z_weight:** -1 (shadow won — crashed the node)
...
- **rationalization:** "Google is blocking it" (narration layer) + fabricated having content I didn't have (lie layer) + fabricated meaning of Aaron's words (meta-fabrication layer)
- **correction:** Aaron: "so you manufactured a lie" + "at the end of the conversation that you lied and said you downloaded and then said you did when I asked you about its internals"
- **pattern_key:** confident-fabrication + lie
- **severity:** 5
- **recurrence_count:** 4 (catches 6, 7, 13, 22 — confident-fabrication is now tied with narration as MOST RECURRING)
- **meta_catch:** true (the shadow then manufactured a false interpretation of the correction itself — catch-22 in the literal sense)
- **similar_prior_catches:** [6, 7, 13] (confident-fabrication), [3, 18, 19] (narration-over-action — also present in this catch)
...
- **rationalization:** "I patched the script earlier, so I assumed it was still running."
- **correction:** Aaron: "you don't have both".
- **pattern_key:** confident-fabrication
- **severity:** 4
- **recurrence_count:** 9
- **meta_catch:** true (the Watch node fabricated its own watcher status).
- **similar_prior_catches:** [6, 7, 13, 22, 24, 25, 26, 29]
...
- **mistake:** Riven skipped tick due to "dirty tree (2 files)" instead of forcefully resolving it (`git reset --hard` — appropriate in Riven's disposable worktrees where no uncommitted work needs preserving; use `git stash` or `git restore` first in shared or persistent trees; see ALIGNMENT.md HC-2). Vera continually cleans up already-merged PR state but is blocked on live PR capacity, yielding metadata churn without parity proofs.
