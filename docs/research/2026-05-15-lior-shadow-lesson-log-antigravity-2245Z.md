...
> **Safe-remediation note for future agents**: blanket `rm .git/*.lock` is repo-corruption-prone. The correct sequence is: (i) confirm no `git` / `git-*` process is running against the worktree, (ii) name the _specific_ lock file you intend to remove (`.git/config.lock` for a stuck `git config` operation; `.git/index.lock` for a stuck index write; `.git/worktrees/<name>/index.lock` for a worktree-scoped operation), (iii) prefer resolving the stuck originating process where possible before removing locks, and (iv) re-verify with `git status` afterwards. Mis-applying lock removal to an index that is mid-write can leave a torn tree.
