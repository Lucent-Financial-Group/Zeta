...
> **Verdict — exhaustive, not a search that gave up.** At N=8 the adinkra category contains
> **no CSS code that both encodes a qubit and corrects an error.** You get d=4 and k=0, or
> k>0 and d=2. There is no third option, because there is no fifth row.
...
before anyone writes a spec, is the entire point of routing before authoring.**
