...
per BACKLOG row _"Imperfect-enforcement hygiene audit"_ +
durable policy
...
audit), which cross-checks _claims about_ verification but
...
- `memory/feedback_missing_hygiene_class_gap_finder.md` —
  sibling tier-3 audit for _missing_ classes; this audit
  is the _imperfect-enforcement_ cousin.
