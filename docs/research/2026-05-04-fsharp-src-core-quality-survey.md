...
2026-05-04 explicit signal _"are you doing F# work too?"_.
...
The CLAUDE.md rule reads: _"User-visible errors surface as
`Result<_, DbspError>` or `AppendResult`-style values; exceptions break
the referential-transparency the operator algebra depends on."_
...
- `Sink.fs:107`: `type AppendResult` — the named exemplar from CLAUDE.md.
  `Sink.fs:140` confirms _"never throws for concurrency conflicts"_ in the
  XML doc. Compliant.
...
_"`Result<_, DbspError>` **or** `AppendResult`-style"_ — i.e., either
...
- `Durability.fs:123`, `Durability.fs:130`: two `raise (NotImplementedException(...))`
  ...
  research-skeleton stub: _"The implementing `WitnessDurableBackingStore`
  below throws on `Save` until the paper's protocol is fully implemented
  and TLA+-verified."_ This is intentional and properly documented;
...
- `Durability.fs:18-26`: `WitnessDurable` is documented research-skeleton;
  ...
  `Durability.fs:72`: _"protocol is validated, `Save` throws
  `NotImplementedException`"_. Properly flagged.
...
| --- | -------------------- | ----: | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1  | `Advanced.fs`        | 11-13 | File name "Advanced" is content-free — the header reads _"Core missing operators"_. Future grep-ability suffers; new arrivals can't tell what's in `Advanced.fs` from the name.                                      | When the file gets re-touched, consider rename to e.g. `OperatorsAdvanced.fs` or split by operator family (`InspectOp` → `Inspection.fs`, etc.). Not urgent; cosmetic naming.                                                            |
