...
`bun tools/research/oracle-stack-evidence.ts` to refresh).

1. **Context** — an autonomous multi-agent software factory; the human maintainer's register
   (observations, not directives); why correctness cannot ride one oracle.
2. **The stack** (one section per layer, each with its incident):
   2.1 Byte oracles — four languages, one golden (the CHIP-9 treaty; the DRW clause that was
...
3. **Honesty as values, not policy** — Converged=false; the Mock that says rehearsal; fault
   registers; red-light bindings; Partial-never-wrong (IBLT). Claims boundary per Rodney: one
   discipline ENFORCED SEPARATELY per engine — no generic enforcement claimed.
