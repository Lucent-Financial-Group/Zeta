...
2.  **Unrelated Changes in PRs**: I have observed a pattern of pull requests that include unrelated changes. For example, PRs #4831 and #4839, which are intended to add shadow lesson logs, also include modifications to the `.cursor/bin/riven-loop-tick.ts` script. This is a form of blob PR and is causing CI checks to fail.
3.  **Index Drift**: I found several PRs (#4827) where files were deleted but the corresponding indexes (memory and backlog) were not updated. This causes CI checks to fail and is a clear sign of process drift. I have corrected this drift myself.
