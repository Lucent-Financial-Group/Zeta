...
1. _"we need to get deffered executin experssions onto the stream so we no
   ...
   plus the serialzed expression tree"_
2. _"then we don't need durable functions that's the same thing but better
   that's self proagating patters with state where the pattern and the state
   can evolve"_
3. _"it's closure propagation with state as self evolving sagas"_
...
| elevator-pitch memory (2026-05-12)                                                                                                   | "green threads / **durable functions** / Orleans grains"; _"my brain runs on temporal workflows"_                                                                                                                                                                     |
...
Durable Functions keeps the _code_ immutable and only the _state_ durable.
...
## Correction — closure-serialization is _not_ the crux; the no-handles discipline is shared (the operator 2026-06-01)
...
> _"especially non-serializable handles — is genuinely hard — they don't do this
...
and our closure is _all serializable values_ — no harder to serialize than DF's
history. Bonsai already solves the expression-tree half. **The closure-
serialization crux largely evaporates.**

What's left is sharper, and it tilts _further_ toward resume — because the two
...
| **No non-determinism** (`DateTime.Now`, `Guid.NewGuid`, random) | **required** — replay would recompute differently | **not required** — we snapshot the _value_, so non-deterministic code is fine |

Replay needs _both_ constraints because it **re-runs the body** and must
...
serialization — it is the _runtime guarantees_ the replay engines have already
...
the replay mechanism is _small_. It is a faithful minimal copy of the Durable
...
the _pattern_ below is the public DTF replay model, nothing reproduced.)
...
**NB (Result-over-exception):** the break-exception is how the _replay family_
...
serializes state and _returns_; there is no replay, so no throw-to-suspend at
...
- **It confirms the constraints + the limits.** The code _between_ activity
...
_definition_ mutates by the **same abelian-group inverse** (`negate`) that
mutates its _data_ — one mechanism, both layers. That is precisely "the pattern
**and** the state can evolve":

- **state** evolves by retract/add over the data Z-set (DBSP incremental view);
- **pattern** evolves by retract/add over the _expression-tree_ Z-set —
...
The operator (2026-06-01): _"so now every partition has self evolving sagas and
so do cross partition joins."_
...
- **Cross-partition joins are sagas too.** The bilinear **`join`** spanning
  ...
  cross-product values, weight-MULTIPLY) is _itself_ a deferred computation: a
...
The operator (2026-06-01): _"the cross partition joins are going to need agent
mediators that are the tick stream that have both repos pulled locally and join
with mitigation factors in their saga for when one side fails."_
...
- **Both repos pulled locally.** The linear merge-join needs **both sorted runs
  ...
  sides, then does the join as a _local_ computation. This is the CALM move made
...
- **Mitigation factors in the saga for when one side fails.** A cross-partition
  join is a _long-running distributed_ operation, so it is a **saga** in the
  ...
  _without losing the in-flight state_, because the saga itself is durable +
  serialized.
...
the expression tree, not fixed orchestrator code, so the _mitigation strategy
itself_ can evolve. The mediator agent's tick stream is where Durable-Functions'
...
The operator (2026-06-01): _"context ends up being same as otel context, it
passes through function calls mostly."_ This closes the "how does the context
...
- **In-process: implicit / ambient** — `AsyncLocal` / `Activity.Current` threads
  ...
  line already names this: _"AsyncLocal immutable-stack scope … one mechanism for
  trace + metric dims."_)
...
- **[B-0917](../backlog/P2/B-0917-interrupt-substrate-in-monad-space-kleisli-arrows-for-context-propagation-memetic-prompt-trust-log-otel-guaranteed-free-time-after-n-rounds-target-aaron-2026-05-28.md)** —
  _Kleisli arrows for **context-propagation** (memetic / prompt / trust / **log** /
  **otel**)_. The IntrCtx already carries `log` + `otel`; the durable/saga context
...
- **Lightlike-observability** (Amara, PR cluster 2026-05-28) — _"OTel is ray
  emission; spans are rays through the distributed system."_ The context = the
  ray; the saga rides the same rays.

One honest split keeps the identity exact. OTel baggage is small + propagated on
_every_ hop (size-limited), so the layering is:

- **OTel context = the propagation + identity layer** — trace/span id (= the
  saga's id, B-0872) + small hot state in baggage. _This_ is the part that "is
...
- **The heavy serialized state** (closure + Bonsai expr-tree) lives in the
  **state store, keyed by the context-id** — not in baggage. The context
  propagates the _handle_; the store holds the _payload_.
...
puts `log` + `otel` _inside_ IntrCtx alongside the durable-relevant contexts.

## Composes with the DU workflow engine — a generic saga in `observe.ts` is just serialize(Rx) + serialize(state)

The operator (2026-06-01): _"this composes with DUs workflow we can create a
...
- **serialize Rx** — the reactive query / expression tree: the _pattern_ (the
...
The operator (2026-06-01): _"we are going to have dapr and dapr actors and
workflow."_ Dapr is the planned runtime substrate (it is **already deployed** in
...
- **Dapr Workflow = same replay family as Durable Functions.** It is internally
  ...
  Dapr Workflow _identically_: replay → determinism-constrained body → **cannot
...
- **Dapr Actors = the virtual-actor carrier (Orleans lineage).** Turn-based,
  ...
  _directly_ onto our **cross-partition-join mediator**: Dapr Agents already use
...
while the saga _body_ uses **resume (serialize-restore)** rather than Dapr
Workflow's **replay** — so we get Dapr's actor runtime + state stores + sidecar
building-blocks _and_ the looser body-constraints + live self-evolution. Dapr
...
- **[B-0251](../backlog/P1/B-0251-durable-computation-stack-temporal-reaqtor-orleans-bonsai-research-2026-05-07.md)** —
  _durable-computation stack: Temporal + Reaqtor + Orleans + Bonsai_ (tags incl.
...
- **[B-0668](../backlog/P1/B-0668-compositional-dbsp-frame-architecture-gnostic-2d-base-plus-two-wolves-emotion-meta-plus-clifford-rx-bonsai-meta-tagged-dims-plus-fsharp-ce-composition-operator-aaron-2026-05-19.md)** +
  **[B-0668.1](../backlog/P1/B-0668.1-fsharp-k8s-mapping.md)** — _our-own fork of
  Azure/durabletask_; explicitly: **"durable-task state-history IS the DBSP
...
- **[B-0706](../backlog/P1/B-0706-zeta-on-orleans-deployment-architecture-servicetitan-scale-orleans-grains-jit-compilation-rented-tools-2026-05-22.md)** —
  _Zeta-on-Orleans_: **grain identity = agent identity**, grains as ticksource +
...
The operator (2026-06-01): _"durable functs and especially temporal have the
better interfaces than mine; their interfaces are much better."_ This is the
...
is _not_ the interface model to copy.
...
- **Temporal — durability is invisible in the body.** You write normal async
  ...
  checkpoint calls. Plus first-class **signals** (push data into a _running_
...
- **Minimal replay spike — minimal but boilerplate.** Every step is hand-wrapped
  ...
  face on every line. The right baseline for _understanding the model_, the wrong
  one for the _interface_.
...
`bcl-interface-boundary-own-your-interfaces-hexagonal` — Temporal is a _design
reference_, not a dependency).

Two things make this _easier_ for us than for the replay engines, not harder:
...
- **Resume loosens the body constraints Temporal authors trip on.** Temporal's
  transparency comes _from_ the replay model, which is also where its famous
  ...
  we can offer the _same_ transparent interface with _fewer_ footguns. The
