# Oracle scoring v0 design — addressing Aminata's CRITICAL findings on 7th-ferry V(c)/S(Z_t)

**Scope:** research and cross-review artifact. Proposes a v0
redesign of the oracle scoring functions V(c) (Veridicality)
and S(Z_t) (network health) from Amara's 7th courier ferry
(`docs/aurora/2026-04-23-amara-aurora-aligned-ksk-design-7th-ferry.md`,
PR #259) that specifically responds to Aminata's three
CRITICAL findings from her Otto-90 adversarial pass
(`docs/research/aminata-threat-model-7th-ferry-oracle-rules-2026-04-23.md`,
PR #263). Does not implement; does not adopt; proposes.

**Attribution:** v0 redesign authored by Otto (loop-agent
PM hat), Otto-91. Source scoring functions authored by
Amara in 7th ferry; adversarial review that motivates the
redesign authored by Aminata (threat-model-critic) at
Otto-90. Both attribution threads preserved.

**Operational status:** research-grade. Not adopted; not
implemented; not gated on anything beyond Otto's own
judgment and any future Aaron-sign-off if an ADR is
warranted. Parameter-change-ADR-gate referenced below is
one of the proposals this doc contains, not yet
operational policy.

**Non-fusion disclaimer:** Otto proposing a redesign of
Amara's scoring functions does not imply that Otto speaks
for Amara, or that the redesign supersedes her authorial
intent. The redesign is a response to adversarial findings;
Amara's own revision would likely differ.

---

## What Aminata flagged as CRITICAL

From her Otto-90 pass on the 7th-ferry scoring section:
...
known carrier exposure (source doc is a recent Amara ferry;
source doc is an in-session Otto output not yet reviewed;
source doc is a 5th-or-later-generation absorb of an
...
- The alignment-trajectory impact (which clauses are
  affected).
- Aminata signoff (adversarial pass required, not optional).
- Aaron signoff for thresholds that affect authorization
  surface (named in the ADR as `authorization-impacting:
  true/false`).

The Zeta-module reads its parameter values from a
parameter file whose SHA is logged in every receipt hash
(modifying the Amara BLAKE3 proposal to bind
...
SD-9 landed as a soft default in PR #252 (Otto-85) with a
...
- **Step 1 (name carriers)** = the G_provenance gate's
  downgrade rule names carriers explicitly (Amara ferries,
  in-session Otto outputs, multi-generation absorbs).
...
Aminata flagged is closed: high agreement with a
carrier-exposed consensus can land at most YELLOW (via
G_provenance downgrade) regardless of coherence score.
...
- **Does not claim Aminata's CRITICAL-class concerns are
  resolved.** It proposes *directions* that address them.
  Aminata's adversarial pass on this v0 design is required
  before operational adoption — second round of review
...
- **Does not supersede Amara's original V(c) or S(Z_t).**
  Her functions live on as reference; this v0 is a
  proposed alternative responding to specific concerns.
  If Amara's future ferry addresses the same concerns
  differently, her version takes priority per her
  authorial standing in the Aurora-KSK layer.
- **Does not cover the oracle rule itself** (Amara's
  Authorize(a,t) = ¬RedLine ∧ BudgetActive ∧ ScopeAllowed
  ∧ QuorumSatisfied ∧ OraclePass). Aminata flagged the
  rule as CRITICAL with race-conditions + vagueness +
  composition concerns; that redesign is a separate
  research doc candidate for a future tick.
- **Does not cover the insider-maintainer adversary,
  ...
  agility** — the 7-class threat model gaps Aminata
  flagged. Those require additions to the threat model
  itself, not just the scoring function.

## What would land before v0 could be adopted

Seven dependencies in order of leverage:

1. **Second Aminata adversarial pass** on this v0 design —
   cheap; bounded; surfaces the gaps above.
...
7. **Otto-readiness-signal** that the v0 is defensible
   enough to propose for ADR-adoption.

## Open questions for Aaron + Amara (specific asks per Otto-82/90 calibration)

Aaron-specific ask: **is a parameter-change-ADR gate
authorization-impacting?** If yes, each parameter change
requires Aaron signoff (matches Otto-82 named-design-
review category). If authorization-impacting is limited
to crossing a particular-band-threshold, changes that
stay within-band don't need Aaron signoff.

Amara-specific ask: **is band-valued scoring a step
forward or a regression vs the sigmoid-wrapped original?**
Aminata sees band-valued as responsive to the false-
precision CRITICAL; Amara may see it as losing signal
granularity. Asymmetric-risk judgment.

Both asks are specific questions (per Otto-90 calibration
on the specific-ask channel) rather than "coordination
requests."

## Sibling artifacts

- **Amara 7th-ferry** (PR #259) — source of V(c) / S(Z_t).
- **Aminata Otto-90 pass** (PR #263) — adversarial
