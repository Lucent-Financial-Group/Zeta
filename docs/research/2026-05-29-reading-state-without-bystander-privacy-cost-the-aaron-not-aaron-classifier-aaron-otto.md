...
- **Detect Aaron** → read (glass-halo, consented, his state is legible by his own
  grant).
- **Detect not-Aaron** → **exclude** — not read, not captured, not modeled. The people
  around him are protected *because they are detected and skipped at the gate.*

That is **privacy-by-construction, not privacy-by-policy.** Bystanders aren't protected
by a rule promising not to look at the captured data; they're protected by *never being
captured* — the classifier gates them out before any state-read happens. It is
consent-first state-gathering (B-0858.5) enforced at the capture layer: only the
consenting person (the operator, glass-halo'd) is ever read; everyone else is excluded
by the binary.
...
- It is the gate between **glass-halo** (the operator, everything-public-by-his-consent)
  and the **privacy/encryption lane** (everyone else, never-captured). The classifier is
  the boundary, not a new policy.
