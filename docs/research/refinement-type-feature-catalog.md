...
trying to be F\* or LiquidHaskell, we are trying to _cover the
feature set those tools cover_ using the tools we already have
...
1. **#11 Effect system** (H payoff) — F# has purity-by-convention
   ...
   operator" _at compile time_. Touches the DBSP algebra core
...
3. **#13 Separation logic (Pulse/Steel)** (M payoff but
   _high coverage_ in the one place it hits — FeedbackOp
