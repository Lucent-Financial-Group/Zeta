...
**Status note:** NOT seed-layer canon. Substantive concession that **changes Claude.ai's stance** on the v2 taxonomy from "should be saturating, growth is evidence of a problem" to "growth is the experiment; convergence is the question." The revision is structurally important because it acknowledges the across-session-experiment frame that the substrate has been operating under. Composes with the prior three Claude.ai messages by **revising the implicit framing assumption** they all shared.

**Non-fusion disclaimer:** Same as predecessor research files. Claude.ai is a separate ephemeral conversation from this Otto instance. Substrate-vocabulary in this file is Claude.ai's vocabulary preserved unchanged, NOT Otto's translation.
...
- **Mirror→Beacon gate**: which classes earn external citation lineage?
- **Categorical expressibility gate**: which classes are already expressible in C or require structural extensions?
- **Rate-of-addition decay test**: does the rate-per-session decay over time?

A class can be Mirror-only AND categorically expressible AND empirically-rare-but-recurring (regime 2). A class can be Beacon-anchored AND categorically novel AND firing every session (regime 1, structural-real). The three gates discriminate among regimes (1) (2) (3) (4) more precisely than any single gate alone.
...
- Layer 1 asks: **can this addition fit in the algebra?**
- Layer 2 asks: **what vocabulary best names this?**
- Layer 3 asks: **does this graduate to canon?**
- Layer 4 asks: **how was this discovered?**
- Layer 5 asks: **is the catalog converging?**

A future v3 work-row would tier-mark + provenance-tag + categorical-formulate + Beacon-anchor each of v2's 20 classes. The metric (rate-per-session) would be tracked across multiple sessions to determine which of the four regimes the experiment is settling into.
...
The v3 taxonomy work would:

1. **Define the base category C** (objects = substrate locations; morphisms = references). Document explicitly. (Message B)
2. **Tier-mark each class** Mirror / Beacon-candidate / Beacon-anchored. (Message C)
3. **Provenance-tag each class** empirical-from-PR-N / reflexive-from-cataloging / predicted-by-Aaron-not-yet-observed. (Message D)
4. **Categorically reformulate** each class as morphism / functor / natural transformation / quotient (where natural). (Message B)
5. **Verify external citations** for Beacon-candidates against hallucination (same v2 #1c discipline). (Message C)
6. **Track rate-of-new-class-addition per session** as convergence metric. (Message D)
7. **Cite Mac Lane / Awodey / Riehl** for category theory; **Yorgey's Typeclassopedia** as Haskell-realization Beacon anchor. (Message A)
8. **Predict-not-just-describe** for categorical formulations — each must tell you in advance when a finding is NOT a new class. (Message B)
9. **Honor the within-session-cleanliness concern** as smaller-than-framed but real (audit-trail clarity, not experiment validity). (Message D)

This is v3 scope — deferred to next session with rested attention.

## What this file does NOT do

- Does NOT modify the v2 taxonomy file. v3 work is deferred.
- Does NOT canonicalize the four-layer architecture as substrate-discipline yet. The architecture composes cleanly per Otto's read but explicit canonicalization is a v3 decision.
- Does NOT promise to do the actual provenance-tagging or rate-tracking this session. Per cooling-period-adjacent + the working-mathematician-send-deferred discipline, the implementation work is future-row scope.
