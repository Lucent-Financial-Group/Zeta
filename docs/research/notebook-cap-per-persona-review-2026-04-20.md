...
scope per Aaron 2026-04-20 _"it's up to you"_ — Tier A (deep
...
**Recommended cap strategy:** _(b) tiered cap by signal
density_ from the backlog row, with an addition — the tier
...
- **Candidate BP-25 — JOURNAL-offload as prune mechanism at
  heavy-state caps.** _"Persona notebooks exceeding 85% of
  ...
  keeps current-round + carry-over + trend-summary only."_
...
- **Candidate BP-26 — seed-only persona marker suppresses
  scaffolding.** _"Persona notebooks in seed-only state
  ...
  audit."_ Rationale: Pattern F above. Saves ~25kB
