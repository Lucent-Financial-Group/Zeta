...
The command surface is not *outside* the cell model — it is itself a cell. This keeps the architecture
**cells all the way**, including the interface. What distinguishes it is **cadence**, a cell property
orthogonal to host substrate (§3):

- **Scheduled / tick-driven cells** — their tick stream comes from a clock (cron, systemd timer, the
  autonomous-loop heartbeat). They advance on time.
- **Request / event-driven cells** — their "tick" is an *incoming command*. The MCP/CLI cell wakes on
  an MCP tool-call or a CLI invocation, processes it against the data-plane nouns, and quiesces. No
  clock; the request *is* the tick.

Both are the same cell shape `(identity, Log)` driven by a tick stream (per the reactive substrate);
only the *source* of the ticks differs. So the MCP/CLI is a request-driven cell over the data-plane
core — which is why it composes cleanly with the scheduled cells rather than being a special external
gateway. (This cadence axis pairs with the host-substrate axis: a request-driven cell can be hosted on
systemd, k8s, or Orleans just as a scheduled one can.)
