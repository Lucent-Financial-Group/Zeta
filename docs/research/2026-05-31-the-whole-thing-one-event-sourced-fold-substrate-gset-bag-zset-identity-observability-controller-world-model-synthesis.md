...
- **Code:** `tools/observe/observe.ts` (world model + controller) · `tools/observe/grammar-16.ts` (4×4 grammar) · `tools/accelerator/local-llm.ts` (composer) · `tools/agent-bus/` (G-Set bus) · `tools/workflow-engine/` (action algebra) · `src/Core` (Z-set algebra)
