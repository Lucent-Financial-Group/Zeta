...
2. **Purpose-Aware Design Decisions:** Design debates often stall because one side argues from origins and constraints (past) while the other argues from desired outcomes (future). Making both boundaries explicit turns these debates into a clear architectural question: _Does this component still belong inside the diamond defined by where we came from and where we’re trying to go?_
