...
**Key recommendation:** Use the measured RFFH as a typed **reference-frame evidence blackboard** and benchmark foundation. It carries object-plus-pose hypotheses through unary factor stores and `Cl3`, while parent/lateral topology gates admission outside inference. It still does **not** implement multi-variable heterarchical factor inference, learn object models, reproduce a cortical column, support every neocortical capability, ground English generally, or derive meaning from the Four Color Theorem.
...
The unresolved historical term does not block the current engineering vocabulary. Zeta provides a generic **composable Bayesian factor graph** library, but current RFFH uses only two one-variable unary-factor stars plus a compensated blackboard fold. **Heterarchy** describes the mixed parent-child and lateral admission topology, not the current factor topology. GMDH remains useful prior art for structure search and model selection, but it is explicitly not promoted as the user’s remembered name. These layers remain separate in code and evidence.
...
| Multiple-module voting | Implemented in Monty’s benchmark | Logarithmic opinion pool combines independently identified likelihood contributions after pose alignment | Duplicate provenance changes evidence as if it were independent |
...
For a pose `g=(R,t)` acting on point `x`, use `g·x = R x R̃ + t`. Composition must be declared as `(R₂,t₂)∘(R₁,t₁) = (R₂R₁, R₂t₁R̃₂+t₂)`. The current blackboard carries distributions over position and object identity; the Clifford action transports means, while covariance transport is a separate `QΣQᵀ` calculation. This is a real geometry–probability composition, not an identification of probabilities with multivectors.
...
The implemented finite **Reference-Frame Factor Heterarchy (RFFH)** is not a “complete artificial cortex.” Each module contributes typed object-plus-pose evidence. Messages carry the sender’s frame, generator convention, logical sequence, full Gaussian uncertainty, evidence identity, and an explicit unresolved state. Pose alignment occurs before insertion; two unary factor stores retain evidence; a deterministic compensated fold combines log-likelihood and information contributions without treating duplicated provenance as independent. Parent/lateral links gate admission but are not factor-graph edges. A separate graph-color scheduler batches nonadjacent updates only when the declared conflict graph and certificate permit it.

The measured release now covers typed message compatibility; pose-action laws; full-covariance uncertainty preservation; compensated order-invariant finite blackboard fusion; retention of contradictory hypotheses; duplicate-evidence refusal; explicit parent-child and lateral admission wiring; signed handedness; independently calculated frame alignment and coordinate naturality; an executable unary-star architecture census; a real two-variable equality-factor positive control; and conditional scheduling that keeps `K4`, `K5`, and `K3,3` planarity/colorability roles separate. Twenty-three focused F# tests, fifteen TypeScript coloring scenarios, an eighteen-group F#/TypeScript dispatcher, and two axiom-audited Lean modules pass. Multi-variable heterarchical inference, learning, active sensing, language grounding, abstract concepts, theory of mind, and universal neocortical capability remain future measurements.

The practical capability ceiling is therefore **L1** in the linked RFFH contract: this is a verified blackboard-fusion and coordinate-transport substrate, not yet heterarchical graphical inference and not a system that learns reusable models or selects actions. The source-grounded StepGame/CLUTRR comparison has been preregistered separately and has no result. “All neocortical capabilities” remains an unmeasured design aspiration in Thousand Brains publications and an unsupported Zeta claim.
