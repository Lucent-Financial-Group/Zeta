...
| 38 | Claude-surface cadenced audit | Every 5-10 rounds (same cadence as skill-tune-up and agent-QOL) | `.claude/agents/claude-code-guide` (extension of existing remit; no new persona) | factory | Audit Anthropic's Claude surfaces — Claude the model, Claude Code CLI, Claude Desktop app, Claude Agent SDK, Claude API — for new features, cut features, behavioural changes. Update `docs/CLAUDE-SURFACES.md` living inventory with adoption statuses (adopted / watched / untested / rejected). When audit surfaces drift (factory assumption no longer matches platform), either adopt (ADR if Tier-3), retire the workaround the new feature obsoletes, or record explicit rejection in `docs/WONT-DO.md`. Triggering incident: 2026-04-20 AutoMemory miss where Anthropic's Q1-2026 feature was being described as factory-native until Aaron corrected. | `docs/CLAUDE-SURFACES.md` audit log row per cycle; ADRs under `docs/DECISIONS/` for Tier-3 adoptions; `docs/research/meta-wins-log.md` entry when a pre-existing factory assumption is found to have been wrong | `feedback_claude_surface_cadence_research.md` + `reference_automemory_anthropic_feature.md` + `reference_autodream_feature.md` |
