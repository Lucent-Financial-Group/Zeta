...
signatures `M : Zeta → Zeta` and `A : Ω → Ω`. The earlier [PR #3626](https://github.com/Lucent-Financial-Group/Zeta/pull/3626) addressed the _terminology_
...
- Replaces with a structured "resolution paths" table:
  - (a) Lawvere-Tierney-style lifting `Ã : Zeta → Zeta` (complicated because `A` is _not_ a
    closure operator)
