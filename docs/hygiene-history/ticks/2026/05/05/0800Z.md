...
- **PR #1587** (B-0193 bootstrap-razor row) -- Aaron 2026-05-05 _"specs and open spec the source of truth ... recreate everything in 23 hours"_ + scope correction _"lets not delete the code here, we can test that in a new repo"_.
...
1. `CronList` returned no jobs -> re-armed via `CronCreate` (`* * * * *`, `<<autonomous-loop>>` sentinel, durable=true intent-doc). Per the CLAUDE.md _"Tick must never stop -- every-tick-verify because the cron mechanism is unreliable"_ rule (line 661+), harness output confirmed session-only despite durable flag.
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Crash recovery checklist (cron-first, git-state-second, PR-state-third)                                                                                                                                               | This shard + the _"Tick must never stop -- every-tick-verify because the cron mechanism is unreliable"_ CLAUDE.md bullet |
