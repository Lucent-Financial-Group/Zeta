...
The pattern: `git reset --soft HEAD~1` + `git checkout -b new-branch`
+ `git commit` sometimes commits to the previously-checked-out branch
rather than the new branch. Suspect cause: working-tree-state inheritance
+ index state carrying over. Mitigation: verify `git branch --show-current`
between checkout and commit in scripted sequences.
