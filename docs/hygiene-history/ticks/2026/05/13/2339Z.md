...
Copilot's first finding was a _consistency_ call (matches existing
sibling tools). Second was a _semantic_ call (the field name
implied a wrong promise). Third was a _minimalism_ call (dead code).
Three different concern classes in one review pass — the same
"better fix the second time" pattern as PR #3043's Codex round-2
re-review. Round-1 ships fast; round-2 ships _correctly_.
