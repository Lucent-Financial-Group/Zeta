...
The empirical fact that a fresh-cold-boot session found `CronList` empty is itself a useful anchor. Per [`.claude/rules/tick-must-never-stop.md`](../../../../../../.claude/rules/tick-must-never-stop.md): _"durable:true doesn't persist, auto-expire is ~3 days not 7. Every session MUST CronList at start; re-arm if missing."_ This is operational confirmation that the discipline is load-bearing — not a hypothetical concern. The catch-43 12-hour loss-of-tick happens precisely because the session-start CronList check gets skipped.
