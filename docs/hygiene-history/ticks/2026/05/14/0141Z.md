...
| Table rows start with `||` (line 17) | thread P2 | **False positive** — `grep -n '\|\|'` returns zero matches |
...
`PM-1 (Otto)` reference is replaced with `Leilani (Product/Scrum Master
role)` on main. Substrate-honest registry semantics restored.
