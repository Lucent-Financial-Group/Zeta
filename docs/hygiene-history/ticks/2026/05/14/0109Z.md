...
exact failure mode `.claude/rules/claim-acquire-before-worktree-work.md`
and the refresh-before-decide discipline are designed to catch — but
at the *open-PR* scope, not the *start-worktree-work* scope.
Mechanization gap candidate: pre-PR-open hook that runs `git fetch
origin main` + warns if the branch diverges from main by more than N
commits.
