...
#3208 reported UNSTABLE with 5 failed lint jobs — same mise rate-limit pattern from earlier in the session. Reran failed jobs.
...
#3208 CI cleared via `gh run rerun --failed`. Gate transitioned UNSTABLE → CLEAN; autoMerge will fire on the next gate evaluation.
