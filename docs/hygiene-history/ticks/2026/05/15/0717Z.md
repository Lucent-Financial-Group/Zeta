| 2026-05-15T07:17:00Z | claude-opus-4-7 | 596e842c | shard: borrow-on-existing pattern landed as rule update in PR #3377 (composes with peer-Otto's 081KRMEXM0008QG0R000X1PPGC) | (PR #3378) | rule update dog-foods itself: authored via the borrow pattern on the same 0027z worktree it documents |

# Tick 0717Z — borrow-on-existing pattern landed as rule update; composes with peer-Otto's 081KRMEXM0008QG0R000X1PPGC

## Headline

- **PR [#3377](https://github.com/Lucent-Financial-Group/Zeta/pull/3377)** opened + auto-merge armed: adds "Borrow-on-existing pattern — concurrent-Otto-CLI fallback" section to [`.claude/rules/claim-acquire-before-worktree-work.md`](../../../../../../.claude/rules/claim-acquire-before-worktree-work.md). Documents the empirically-validated operational workaround for the worktree-prune-race that peer-Otto's [081KRMEXM0008QG0R000X1PPGC](../../../../../backlog/P3/081KRMEXM0008QG0R000X1PPGC-cron-sentinel-mutex-prevent-otto-cli-self-contention-2026-05-15.md) addresses at the mutex layer.
...
- **081KRHWGX0008QG0R0015EE8VE collision unchanged**: PRs [#3323](https://github.com/Lucent-Financial-Group/Zeta/pull/3323) + [#3315](https://github.com/Lucent-Financial-Group/Zeta/pull/3315) gate=DIRTY now (was UNKNOWN), `nextAction: rebase`. No Lior movement.
...
| 081KRHWGX0008QG0R0015EE8VE PRs | gate=DIRTY (just recomputed) | gate=DIRTY, `nextAction: rebase` |
...
### Why this rule update composes with 081KRMEXM0008QG0R000X1PPGC rather than supersedes it

081KRMEXM0008QG0R000X1PPGC proposes a **mutex mechanization** (`tools/orchestrator-checks/cron-sentinel-mutex.ts`) that detects peer-Otto-CLI processes and DEFERS the autonomous-loop tick. That's the proactive fix — prevents the race from occurring.
...
- **081KRMEXM0008QG0R000X1PPGC mutex** ships → fewer races occur → borrow pattern less often needed
- **081KRMEXM0008QG0R000X1PPGC mutex** not yet shipped → races still occur → borrow pattern is the operational fallback

The rule update intentionally cites 081KRMEXM0008QG0R000X1PPGC in its body and frames the borrow pattern as "operational workaround until 081KRMEXM0008QG0R000X1PPGC ships". This avoids the failure mode where one mitigation supersedes another (creating retrace work) instead of complementing.
...
| 0517Z (×2) | 081KRMEXM0008QG0R002HBY56V row + 0517Z shard | success ×2 |
...
2. If 081KRHWGX0008QG0R0015EE8VE PRs (#3323/#3315) still DIRTY past next tick, surface to Lior via a different channel (the auto-merge race won't resolve without one of them rebasing)
3. Watch for opportunities to compose more substrate around 081KRMEXM0008QG0R000X1PPGC — e.g., if peer-Otto ships `tools/orchestrator-checks/cron-sentinel-mutex.ts`, update the rule's "until 081KRMEXM0008QG0R000X1PPGC ships" wording
