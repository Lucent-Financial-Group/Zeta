...
6. `sed s|(PR TBD)|(PR #NNNN)|` on the shard
7. `git commit --amend --no-edit` (rewrite the SOLE commit)
8. `git push --force-with-lease` (replaces remote ref atomically)

Result: PR has only the corrected commit visible. No `(PR TBD)` snapshot for Copilot/Codex to review.
