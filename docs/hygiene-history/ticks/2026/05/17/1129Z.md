...
|---|---|---|
| `check docs/BACKLOG.md generated-index drift` | Working-tree 081KRHWGX0008QG0R000M9RFY2 status mod (open→closed) leaked into BACKLOG.md regen via `BACKLOG_WRITE_FORCE=1`, but 081KRHWGX0008QG0R000M9RFY2 row itself was NOT committed. CI's generator on committed state (status: open) produced `[ ]`, mismatched committed BACKLOG.md `[x]`. | Fix: revert BACKLOG.md line 282 `[x]` → `[ ]` to match committed 081KRHWGX0008QG0R000M9RFY2 status. Defer 081KRHWGX0008QG0R000M9RFY2 close to dedicated PR. |
...
1. Read CI failure log via `gh run view 25988532385 --log-failed` → diagnosed 081KRHWGX0008QG0R000M9RFY2 mismatch (committed BACKLOG.md `[x]` vs generator-output `[ ]`).
2. Inspected committed 081KRHWGX0008QG0R000M9RFY2 row (status: open) vs working-tree (status: closed) → root cause = uncommitted mod leak via regen.
...
7. Edit BACKLOG.md line 282: `[x]` → `[ ]` for 081KRHWGX0008QG0R000M9RFY2.
8. Verified `--check` mode passes locally: `git stash` 081KRHWGX0008QG0R000M9RFY2 row mod, `bun tools/backlog/generate-index.ts --check` → "ok matches generator output", `git stash pop` restores working-tree state.
...
- Commit `368cb89` on PR #4059 branch — 2 round-2 CI fixes (BACKLOG.md 081KRHWGX0008QG0R000M9RFY2 marker revert + 1034Z.md invisible-Unicode strip)
- Both root-causes substrate-honestly documented in commit message
- 081KRHWGX0008QG0R000M9RFY2 close substrate preserved in working tree for dedicated future PR
