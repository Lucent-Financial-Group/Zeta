# PR #6304 -- docs(B-0634): agent-native key-custody design + hardware-to-buy list (FPGA open-bitstream + HSM/TPM)
...
|---|---|
| Number | 6304 |
| Title | docs(B-0634): agent-native key-custody design + hardware-to-buy list (FPGA open-bitstream + HSM/TPM) |
...
## Agent-native key custody (B-0634 design detail) + hardware-to-buy list
...
2. **FROST threshold across society key-guards** (B-0634's N-of-M at the *encryption* scope) — durability (survive M−N guard loss) + sovereignty (no single party, incl. Aaron, uses it alone).
...
Linked from B-0634 + inventory README. Docs-only.
