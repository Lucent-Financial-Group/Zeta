# PR #1811 -- fix(budget): add jq JSON validation before every --argjson site (081KQNJ500008QG0R000T4QGD7)

> Git-native PR-review archive. Generated by
> `tools/archive/archive-pr-reviews.ts`. Schema version: 1.
>
> Wall-clock generation timestamp deliberately omitted from the
> rendered file so deterministic re-runs against unchanged
> upstream substrate produce byte-identical content (manifest
> `fetched_at` carries the audit timestamp instead).

## Metadata

| Field | Value |
|---|---|
| Number | 1811 |
| Title | fix(budget): add jq JSON validation before every --argjson site (081KQNJ500008QG0R000T4QGD7) |
| Author | `AceHack` (human) |
| State | MERGED |
| Created at | 2026-05-07T06:06:49Z |
| Merged at | 2026-05-07T06:17:50Z |
| Merge commit SHA | `d9909afa82a60ace5b04905113d5ede661db2d70` |
| Branch | `fix/budget-snapshot-jq-argjson-validation` |
| Base branch | `main` |
| URL | https://github.com/Lucent-Financial-Group/Zeta/pull/1811 |
| Changed files | 3 |
| Additions / deletions | +16 / -6 |

## Description

## Summary
- Adds `ensure_json()` helper to `tools/budget/snapshot-burn.sh` that validates JSON with `jq empty` before each `--argjson` site
- Catches the case where `gh api` returns partial stdout + non-zero exit, producing invalid JSON that the `|| echo "{}"` fallback appends to rather than replaces
- Applied to all 4 API call sites: copilot billing, workflow runs, per-run timing, and PR stats
- Verified with `--dry-run` — script produces valid JSON output

## Test plan
- [x] `bash tools/budget/snapshot-burn.sh --dry-run` produces valid JSON
- [ ] Next Sunday cron fire (2026-05-10) confirms CI fix
- [ ] Closes 081KQNJ500008QG0R000T4QGD7

🤖 Generated with [Claude Code](https://claude.com/claude-code)

## Outcome

| Field | Value |
|---|---|
| Merged | true |
| Re-reviewed post-fix | true |
| Total threads | 3 |
| Resolved threads | 3 |
| Unresolved threads | 0 |
| Total review comments | 6 |
| Total fix commits (touching thread paths) | 4 |

## Review threads

### Thread 1 -- resolved [outdated, collapsed]

- **Thread ID**: `PRRT_kwDOSF9kNM6ANLsY`
- **Path**: `tools/budget/snapshot-burn.sh`
- **Replies**: 1

**Initial comment:**

- **`chatgpt-codex-connector[bot]` (bot)** at 2026-05-07T06:11:25Z on `tools/budget/snapshot-burn.sh`:68 (association: NONE)

  <pre>
  **&lt;sub&gt;&lt;sub&gt;![P1 Badge](https://img.shields.io/badge/P1-orange?style=flat)&lt;/sub&gt;&lt;/sub&gt;  Reject JSON streams in ensure_json**

  `ensure_json` currently validates with `jq empty`, which succeeds for a stream of multiple JSON values (for example, a successful API payload followed by the `|| echo "{}"` fallback when `gh api` still exits non-zero). In that case this helper returns the multi-document text unchanged, and the later `jq --argjson ...` sites fail because `--argjson` requires exactly one JSON value. This means the new guard can still miss the failure mode it is meant to prevent, so the workflow can continue to hit the same `invalid JSON text passed to --argjson` break under partial-failure API responses.

  Useful? React with 👍 / 👎.
  </pre>

**Replies:**

  - **`AceHack` (human)** at 2026-05-07T06:12:45Z on `tools/budget/snapshot-burn.sh`:68 (association: MEMBER)

    <pre>
    Fixed in 360ae5a — changed jq empty to jq -se 'length == 1' which rejects multi-value JSON streams. Good catch.
    </pre>

### Thread 2 -- resolved [outdated, collapsed]

- **Thread ID**: `PRRT_kwDOSF9kNM6ANN8F`
- **Path**: `tools/budget/snapshot-burn.sh`
- **Replies**: 1

**Initial comment:**

- **`Copilot` (bot)** at 2026-05-07T06:13:42Z on `tools/budget/snapshot-burn.sh`:68 (association: NONE)

  <pre>
  ensure_json uses `echo` for both validation input and output. `echo` has unspecified escape/option handling in shell scripts (and can be affected by bash `xpg_echo`), which can mutate JSON strings before `jq` sees them. Prefer `printf '%s'` (and `printf '%s
  '` for output) to preserve the JSON bytes exactly across environments.

  </pre>

**Replies:**

  - **`AceHack` (human)** at 2026-05-07T06:15:12Z on `tools/budget/snapshot-burn.sh`:68 (association: MEMBER)

    <pre>
    Fixed in 22bef28 — replaced echo with printf '%s' for validation input and printf '%s\n' for output.
    </pre>

### Thread 3 -- resolved [collapsed]

- **Thread ID**: `PRRT_kwDOSF9kNM6ANN8n`
- **Path**: `docs/backlog/P1/081KQNJ500008QG0R000T4QGD7-budget-snapshot-cadence-jq-argjson-failure-2026-05-03.md`
- **Replies**: 1

**Initial comment:**

- **`Copilot` (bot)** at 2026-05-07T06:13:42Z on `docs/backlog/P1/081KQNJ500008QG0R000T4QGD7-budget-snapshot-cadence-jq-argjson-failure-2026-05-03.md`:6 (association: NONE)

  <pre>
  `closed_by` is still a placeholder ("PR #TBD"). Since the row is being marked closed, this should be updated to the actual closing PR number (or other closure evidence), consistent with other closed backlog rows.
  </pre>

**Replies:**

  - **`AceHack` (human)** at 2026-05-07T06:15:16Z on `docs/backlog/P1/081KQNJ500008QG0R000T4QGD7-budget-snapshot-cadence-jq-argjson-failure-2026-05-03.md`:6 (association: MEMBER)

    <pre>
    Already fixed in aa01039 — closed_by updated to PR #1811.
    </pre>

## Fix commits (touching thread paths)

### `4dcd465251f49acabd61e2627e03e81ee175836a` -- 2026-05-07T06:06:40Z -- `AceHack`

**Touched files (intersect with thread paths):**

- `docs/backlog/P1/081KQNJ500008QG0R000T4QGD7-budget-snapshot-cadence-jq-argjson-failure-2026-05-03.md`
- `tools/budget/snapshot-burn.sh`

**Message:**

```
fix(budget): add jq JSON validation before every --argjson site (081KQNJ500008QG0R000T4QGD7)

When CI's GITHUB_TOKEN lacks read:org, gh api can return
partial stdout before failing, producing invalid JSON that
breaks jq --argjson. The ensure_json() helper validates with
jq empty and falls back to a safe default if parsing fails.

Applied to: copilot_raw, runs_json, timing (t), and pr_raw.

Closes 081KQNJ500008QG0R000T4QGD7.

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>
```

### `fd87a177ecb308a62346f028b8f2174203ca7be5` -- 2026-05-07T06:09:11Z -- `AceHack`

**Touched files (intersect with thread paths):**

- `docs/backlog/P1/081KQNJ500008QG0R000T4QGD7-budget-snapshot-cadence-jq-argjson-failure-2026-05-03.md`

**Message:**

```
chore: update 081KQNJ500008QG0R000T4QGD7 closed_by to PR #1811

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>
```

### `360ae5a237570fe892735d3003a8b78347d87b65` -- 2026-05-07T06:12:34Z -- `AceHack`

**Touched files (intersect with thread paths):**

- `tools/budget/snapshot-burn.sh`

**Message:**

```
fix: reject JSON streams in ensure_json (Codex P1 review)

jq empty accepts multi-value JSON streams, which means
partial API responses concatenated with the || fallback
({partial}{}...) would pass validation but break --argjson.

Use jq -se 'length == 1' instead — slurps the stream into
an array and asserts exactly one JSON value.

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>
```

### `22bef28f78767bb91bf5ac4177e6b4a428edad75` -- 2026-05-07T06:15:06Z -- `AceHack`

**Touched files (intersect with thread paths):**

- `tools/budget/snapshot-burn.sh`

**Message:**

```
fix: use printf instead of echo in ensure_json (Codex review)

echo has unspecified escape/option handling across shells.
printf '%s' preserves JSON bytes exactly.

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>
```
