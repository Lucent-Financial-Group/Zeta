# PR #6170 -- feat(B-0944 slice 4): Rust tri-boolean digital qubit (oracle #4, first Rust crate, build+clippy clean + 9/9)
...
|---|---|
| Number | 6170 |
| Title | feat(B-0944 slice 4): Rust tri-boolean digital qubit (oracle #4, first Rust crate, build+clippy clean + 9/9) |
...
Fourth + final **summonable-BFT oracle** for the digital qubit — Rust parity of the tri-boolean primitive (B-0944), mirroring the TS/F#/C# oracles.

This is the repo's **first Rust crate** (Rust was greenfield — no prior `.rs`, no Cargo, no Rust CI; B-0679 ZetaId-Rust still open). Layout per the B-0679 spec convention: `src/Core.Rust.TriBoolean/` with `Cargo.toml` + `src/lib.rs`.
...
There is **no Rust CI in this repo yet**, so this crate is verified locally but not gated in CI. Adding a rust toolchain + cargo test/clippy to CI is a cross-cutting infra follow-up (like B-0947) for your decision — not added autonomously here.
