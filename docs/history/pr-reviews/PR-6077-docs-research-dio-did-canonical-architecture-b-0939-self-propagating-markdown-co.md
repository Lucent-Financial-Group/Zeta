# PR #6077 -- docs(research): DIO/DID canonical architecture + B-0939 self-propagating-Markdown compiler-rule
...
|---|---|
| Number | 6077 |
| Title | docs(research): DIO/DID canonical architecture + B-0939 self-propagating-Markdown compiler-rule |
...
- **Self-propagating-Markdown-or-it-doesn't-compile** + **bootstrap-traveler** → **B-0939**.

## What's CONFIRMED (independently re-derived → triangulation)
- Origin = the git-monster (6 agents too fast to PR) → the accelerator.
- Everything-in-the-stream (schema→ontologies→DUs→workflows→state; RX not PSQL) → B-0867 + move-next.
...
- 128-bit **Zeta-ID** trie-into-Git = the merge primitive (accelerator's placeholder ULID → canonical Zeta-ID, B-0893).
...
**B-0939** owns the self-propagating-Markdown compiler-rule + bootstrap-traveler template (per-partition-root resolution via RX-join-over-CRDTs).
