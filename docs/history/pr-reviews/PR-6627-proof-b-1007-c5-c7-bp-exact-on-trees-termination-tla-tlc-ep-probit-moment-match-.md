# PR #6627 -- proof(B-1007 C5,C7): BP exact-on-trees + termination (TLA+/TLC) · EP probit moment-match over generated cavities (FsCheck)
...
|---|---|
| Number | 6627 |
| Title | proof(B-1007 C5,C7): BP exact-on-trees + termination (TLA+/TLC) · EP probit moment-match over generated cavities (FsCheck) |
...
Closes the two **P1** items in Soraya's B-1007 cadence, each on the tool the row routed it to (BP-16, anti-hammer):
...
Per **formal-proof-first**: this closes acceptance **half-(a)** for C5/C7. The **hex/4×4 seed-lineage edge (half b)** is still owed before either is *canonical* — consensus/registry tier ≠ proof tier. TLC's TLA+ leg runs in CI where the toolchain is provisioned (unlike the Z3 legs, still self-skipping pending **B-1009**).

With this, **B-1007 P0 + the two TLA+/FsCheck P1 proofs** are landed. Remaining P1 = C4 (marginal fold) + C12/C13/C14 (registry codec/Tick/Z-set laws), all FsCheck.
