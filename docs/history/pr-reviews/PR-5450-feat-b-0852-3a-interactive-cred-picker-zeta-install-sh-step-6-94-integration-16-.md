...
```
feat(B-0852.3a): interactive cred-picker + zeta-install.sh Step 6.94 …
...
```
fix(B-0852.3a CI): 7 Copilot+CodeQL findings — P0 passphrase leak via…
...
```
fix(B-0852.3a CodeQL P0 re-fire): build DRY RUN display from known-sa…
...
```
fix(B-0852.3a CI Copilot): activate mise + BUN_INSTALL in picker bash…
