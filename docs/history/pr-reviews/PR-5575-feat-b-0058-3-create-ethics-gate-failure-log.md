...
This commit creates the initial 'honesty dashboard' for logging candidates that fail the ethics and safety gate, as per backlog item B-0058.3.
```
...
```
docs(B-0058.3): address Copilot review threads + regen BACKLOG.md

Two Copilot review threads on docs/hygiene-history/ethics-gate-failures.md:

- P2 (line 3): Replace bare B-0058 reference with explicit B-0058.3
  pointer (and parent B-0058 link) so the slice is directly traceable
...
Also regenerate docs/BACKLOG.md (BACKLOG_WRITE_FORCE=1) so B-0058.3 row
