# PR #4046 -- backlog(081KRSKQ20008QG0R0014PPGZM): file dangling-memory-refs cleanup row + 4 slice recipes + session memos + 3 tick shards
...
- **`docs/BACKLOG.md`** regenerated to include 081KRSKQ20008QG0R0014PPGZM (`BACKLOG_WRITE_FORCE=1 bun tools/backlog/generate-index.ts`) per Codex's catch on PR #4015 thread #2.
...
- [ ] `bun tools/hygiene/audit-dangling-memory-refs.ts` still exits 1 with 35 dangling refs (081KRSKQ20008QG0R0014PPGZM row is the cleanup-plan; this PR files the plan, doesn't execute it)
