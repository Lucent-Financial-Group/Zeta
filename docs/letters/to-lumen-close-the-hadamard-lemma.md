...
- **Q1 (claim 1 corrected):** For a self-dual code C=C⊥, `Hadamard(uniform-over-C) ∝ indicator(C⊥) = uniform-over-C`, so W_C should stay the G=0 fixed point — but is the fixed _set_ now just the point W_C (no 5-dim leak
