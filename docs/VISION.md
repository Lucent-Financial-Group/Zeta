...
| TypeSchema from store `DynamicValue` (then generators consume it) | **DESIGNED** | `src/Core.CSharp/TypeSchema.cs` + `SchemaSourceGenerator` consume AdditionalFiles / `*.zetaschema.json` today — a bootstrap IR, not the store. `gen/README.md` is the plan; not wired to `ZetaFs`/`DagFs`. A guessed schema from a `SoftValue` is a **different constructor** that keeps the SoftValue (`snap` does not change). `DynamicValue` is also a tiny CFG; context attaches via holes (second DV/SoftValue), not a rewrite. `081M125DNKK087G0R00292E3ET` |
