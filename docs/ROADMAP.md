...
the small-write bench against host FS + `GroupCommitDiskDeltaLog`. Both legs
now run (`081M1ZA8S7C087G0R002P9NX40`); numbers still `toy`.
...
[`docs/design/2026-08-30-zetafs-first-product-cas-store-per-entity-policy.md`](design/2026-08-30-zetafs-first-product-cas-store-per-entity-policy.md).
...
- Blocked + counting Bloom (`BloomFilter.fs`) ✅ — 4-bit counters, `CounterSaturated` at 15. **Bloom vs anti-bloom** (two grow-only filters, three-valued membership) has a toy type plus unit measurements (ZD10); counting stays the join-probe path. Not a filesystem feature. Does not reopen WONT-DO deletable Bloom.
- HyperLogLog ✅
- Count-Min Sketch ✅
- KLL quantile ✅
- HyperMinHash ✅
...
- **ZetaDB product** — map landed 2026-09-02 (event log as WAL, tables as views, Flink/Reaqtor feel, Feldera as DBSP competitor not Postgres-on-DBSP). SQL is a package targeting Core, not inside Core. ZD2 freeze-through-ferry (D4) landed (`FreezeLog`). ZD4 first peel: both-legs host-FS storm runs (numbers still `toy`; `081M1ZA8S7C087G0R002P9NX40`). Next: PR12 crash DST for FS *and* DB, ZD1 standing-query bench, named ZD4 bench run. D9–D12: pointer-not-copy, one cache authority, CoW amplification bound, crash DST. **Bloom vs anti-bloom** (two G-set filters, `{present, absent, unknown}`) is ZD10: toy type plus unit measurements; counting stays shipped. [`docs/design/2026-09-02-zetadb-roadmap-event-sourced-streaming-sql-not-feldera-on-postgres.md`](design/2026-09-02-zetadb-roadmap-event-sourced-streaming-sql-not-feldera-on-postgres.md) `081M1HGD1QA087G0R001GRHPFW` · `081M1HK4AQE087G0R002RRJXWE` · `081M1T9SMM9087G0R002FS29S4`
