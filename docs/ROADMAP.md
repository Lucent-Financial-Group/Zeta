...
- **ZetaFS dual-fold remaining** — parent edge on `ZetaFsDeltaLog` (truncate reversible), factory path off `git`/`LibGit2Sharp` (`081M108RYNT087G0R001JSRNZE`). Own BLAKE3 (`Blake3Spec`) is the store hasher; the NuGet package is a test oracle (official vectors + differential). Core's `defaultHasher` stays XxHash128 (hexagonal).
