...
| **B7** | **OpenBao comes up sealed at wave −60 and nothing unseals it.** A sealed pod is NotReady by design; the repo's own doc says an unhealthy wave blocks all higher waves. Metal applies the whole catalogue (no `excludeGlob` outside the dev entrypoint). The only written procedure is marked superseded/history. | `openbao/Application.yaml:29-35, 59, 103-113`; `dev-cluster/SYNC-WAVES.md:191-195`; `openbao/TOPOLOGY.md:25-30` | **PAYLOAD LANDED (#17003)** — adapters on `main`. **IMAGE RECIPE OPEN (PR #17143, 081M23BTKZX087G0R0034VN466)** — `zeta-bao-unseal` matrix entry; extraContainers wait for anonymous GHCR 200 (an unpullable sidecar makes Otto's included OpenBao NotReady). Missing share cache waits, does not crash-loop. |
| **B8** | **Twelve Secrets are named by `automated: true` Applications and nothing on metal creates any of them.** No `Secret`/`SealedSecret`/`ExternalSecret`/`ClusterSecretStore` exists anywhere under `full-ai-cluster/k8s/`, against 22 references. Several exist only as a `kubectl create secret` in a code comment. | `k3s-server.nix:200-254` mints none; per-app refs listed in the secrets review | **OPEN — measured, and the count was stale.** 2026-09-08: **9** distinct Secrets across **10** sites, **0** producer files (not 12/22). 3 have no producer on ANY path (`kubevirt-operator-certs`, `temporal-default-store`, `temporal-visibility-store`); the other 6 have a dev-path producer, so metal simply does not run one. `external-secrets` and `sealed-secrets` are DEPLOYED and declare nothing. Now checked by `secret-reference-audit.test.ts` (ratchet: a new unproduced reference fails). |
...
| id | item | state |
|---|---|---|
| **R1** | **extraContainer Shamir sidecar** (MENO item 7) — `valuesObject` only, same commit as the sidecar, Shamir kind path only, **do not fork the chart**. Adapters: `src/Core.TypeScript/cluster/openbao-unseal-sidecar.ts` (`#17003`). Image recipe: `full-ai-cluster/openbao-unseal/Dockerfile` + `build-platform-images.yml` matrix `zeta-bao-unseal`. **extraContainers still open** until the image is anonymously pullable and digest-pinned. Missing share cache must wait, not crash-loop. | Riven |
