...
| id | item | state |
|---|---|---|
| **R1** | **extraContainer Shamir sidecar** (MENO item 7) — `valuesObject` only, same commit as the sidecar, Shamir kind path only, **do not fork the chart**. The payload is already written: `src/Core.TypeScript/cluster/vault-unsealer.ts` says of itself *"this file is the payload that commit will call"*. It has **no CLI entrypoint** and injected `ShareFetcher`/`UnsealHttp` ports, so the commit must wire real adapters. **This is what unblocks B7.** | Riven |
