...
| 2 | **P0** (Z3/Lean primary open · FsCheck leg ✅ #8743) | NFT **binding / collision-resistance** (specificity): `H_AB ≠ H_AC ⇒ Commit≠Commit`. **Z3 (QF_BV) domain-separation proof + Lean domain-sep theorem stay OPEN (Kenji's).** Commit surface = `CostarZSet.toDynamicValue rating (link a b)` → canonical CBOR → content hash; `link` canonicalizes A≤B (unordered-pair invariant). | **Z3 (QF_BV)** (open) | ✅ **FsCheck leg DONE (#8743)** — `NftCommitBinding.Tests.fs` (6 tests, verified 6/6 on main): binding (distinct `(link,rating)` ⇒ distinct commit), rating-binding (no silent re-rating), pair-binding, unordered-pair invariant (`commit(a,b)=commit(b,a)`, a feature), determinism, specificity fact. Evidence over the REAL encoder; encoding is injective by CBOR construction (length-prefixed strings + distinct key tags), hash CR is the named premise — the *formal* injectivity proof is the Z3/Lean primary. · Lean domain-sep | Kenji |
