...
|---|---|---|---|---|---|
| 1 | **P0** — operational leg ✅ #8794 + **MEASURE-THEORETIC (Mathlib) leg ✅ #8808** · single-body premise open | NFT **forgery-resistance**: `H_∞(H_AB \| E) ≥ k` (the entropy floor = G3b/Bell lifted to a pair). The lift is now proven at BOTH tiers; the **single-body premise** (the physical/Bell source actually meets a floor) stays the math team's. | ✅ **TWO Lean legs, verified on main:** (1) **operational** `src/Core.Lean4/Lean4/EntropyFloorLift.lean` (#8794, `[propext, Quot.sound]`) — guessing-space form, `floor_lifts` ka,kb → ka+kb. (2) **measure-theoretic** `src/Core.Lean4/Lean4/EntropyMeasureTheoretic.lean` (#8808, lean-proof gate green; `#print axioms` = `[propext, Classical.choice, Quot.sound]`, no `sorryAx`) — real-ℝ min-entropy `H_∞(D) = −logb 2 (max_x p x)`; `Hmin_product : H_∞(A×B) = H_∞(A)+H_∞(B)` as an **exact equality** (stronger than the handoff `≥`), via `pmax_product` + `Real.logb_mul`. Scope map: `docs/research/2026-06-20-measure-theoretic-entropy-tier-mathlib-scope.md`. | Z3 finite instance + `ρ_owe` empirical floor | Tariq + Adaeze |
| 2 | **Z3 PRIMARY ✅ DONE (#8748)** + FsCheck leg ✅ #8743 · Lean domain-sep optional/open | NFT **binding / collision-resistance** (specificity): `H_AB ≠ H_AC ⇒ Commit≠Commit`. Commit surface = `CostarZSet.toDynamicValue rating (link a b)` → canonical CBOR → content hash; `link` canonicalizes A≤B (unordered-pair invariant). | ✅ **Z3 (QF_BV) DONE (#8748)** — `NftCommitDomainSep.Z3.Tests.fs` (verified 2/2 on main, z3 4.16.0, not skipped): distinct canonical `(A,B,rating)` ⇒ distinct length-prefixed encoding `lenA‖A‖lenB‖B‖rating` is **UNSAT** (no collision) at bound CELLS=3/CELL_W=8/LEN_W=8; **SAT negative control** (content-only, no length prefix → boundary-ambiguity collision `(x,yz)` vs `(xy,z)`) proves non-vacuity + that the length prefix is the load-bearing defense. Hash CR is the named premise (structure proven; crypto premise named, as Merkle does). Lean domain-sep cross-check optional. | ✅ **FsCheck leg DONE (#8743)** — `NftCommitBinding.Tests.fs` (6 tests, 6/6 on main): binding, rating-binding (no silent re-rating), pair-binding, unordered-pair invariant (`commit(a,b)=commit(b,a)`, a feature), determinism, specificity. Evidence over the REAL encoder. | Kenji |
| 3 | **P0** (Lean STRUCTURAL LEG ✅ #8799 · measure-theoretic primary open · FsCheck leg ✅ #8715) | **anti-mirror `ρ_owe`** soundness (CMI own-entropy as primary; data-processing inequality). **The measure-theoretic Shannon DPI (`I(A;U\|C) ≥ I(A;f(U)\|C)`) over real distributions stays OPEN (math team's; needs Mathlib log-sum/Jensen).** | ✅ **Lean structural leg DONE (#8799)** — `src/Core.Lean4/Lean4/DecorrelationDpi.lean` (gated by `lake build`, verified on main; `#print axioms dpi_operational` = `[propext, Quot.sound]`, no `sorryAx`): the operational/combinatorial heart of DPI — the conditioning side `U` modelled as a partition (cells of masses), post-processing = cell-merges; `support_mergeIter_le` (coarsening can't increase distinguishing support — post-processing can't manufacture shared cause), `mass_mergeIter` (mass conserved, never invented/lost), `postprocess_coarsens` (equational `f∘g` kernel form), `dpi_operational` (combined) + non-vacuity both endpoints. No Mathlib. The convexity step (real entropy `−∑p log p`) is the named measure-theoretic primary. | ✅ **FsCheck leg DONE (#8715)** — `DecorrelationDpiCrossCheck.Tests.fs` (5 net-new cases, verified 5/5 on main): DPI dual (refining U cannot *raise* `ρ_owe`), two with-context soundness endpoints (`A` determined by `(U,C)`→0; `A ⫫ U \| C`→1), determinism/uniform-stake. Net-new over the existing 11 `DecorrelationEstimator.Tests.fs`. **Honest non-finding (no-weaken held):** a post-process-**A** DPI direction was probed and is genuinely **non-monotone** for the plug-in estimator (expected — plug-in MI isn't monotone under arbitrary target coarsening), so it was NOT asserted. `ρ_owe` is evidence, not the lemma. · Z3 estimator lemmas + CHSH cross-check still open | Tariq + Adaeze |
| 4 | **✅ FULLY DONE** — inclusion + no-forge (#8713) + 4-lang proof byte-lock (#8719 F# · #8720 Rust · #8721 TS · #8723 C#) | **Inclusion + no-third-party-forge leg DONE** — `ZSetMerkle.proofFor`/`verify` (+ hash-parameterized variants) + `tests/Tests.FSharp/Formal/MerkleInclusion.Laws.Tests.fs` (9 tests, verified 9/9 on main): audit-path proof bound to one `(LeafKeyBytes, LeafWeight)` entry, verifiable by a third party holding ONLY proof+root (never the tree). Completeness; no-forge ×4 (non-member has no proof, tampered weight fails, corrupted path fails, fails vs a different set's root); determinism; single-leaf; **retraction-native** (a `+1/−1` net-zero entry has no proof — DBSP correction / idempotency #6). The **hiding / root tamper-evidence** half ("equal roots ⟹ equal leaves") was already done in `Merkle.Laws.Tests.fs` (structural + Z3). Honest scope: structure proven; collision-resistance is the named crypto premise (XxHash128 → BLAKE3 for Byzantine). **4-LANG PROOF BYTE-LOCK ✅ DONE:** generic raw-leaf `MerkleTree.Proof`/`verifyProof` ported across all four oracles against one frozen `golden-vectors-merkle-proofs.json` (22 vectors, hex-in-JSON) — F# (#8719), Rust (#8720), TS (#8721), C# (#8723); each verified on main reproducing every vector byte-for-byte (zero divergence). | ✅ existing merkle golden vectors + ✅ 4-lang `golden-vectors-merkle-proofs.json` + ✅ `MerkleInclusion` FsCheck | ✅ Z3 root-determines-leaves (`Merkle.Laws.Tests.fs`) | Adaeze |
| 5 | **P1** (Lean/Z3 metering lemma open · FsCheck leg ✅ #8745) | **NTP noninterference**: `H_AB` depends only on the captured-at-mint **soft** clock (`clock ± uncertainty`), never on a post-mint "now"; render-clock quarantined to display. **Lean/Z3 metering lemma stays OPEN (Tariq's).** | **Lean/Z3** (metering lemma — open) | ✅ **FsCheck leg DONE (#8745)** — `NtpNoninterference.Tests.fs` (4 tests, verified 4/4 on main): over the real `MintPanel` renderer, varying the render clock changes ONLY the `<div class="clock">` line (cards+grounding byte-identical, `stripClock p1 = stripClock p2`); `renderCard` takes no clock (pure fn of the link); the card carries no phase/UTC/±uncertainty markers; a "later now" cannot re-rate/re-identify a minted link (entropy-quarantine #13). The metering lemma (`±uncertainty` interval itself bound, no retro-narrowing) is the formal primary. | Tariq |
...
| 10 | **IStarRing CLIFFORD LEG ✅ DONE (#8707)** · Face 3 prereqs MET, behavioral leg IN PROGRESS (gen-gen trajectory) · Lean theorem open | **Clifford leg DONE** — `Cl3.algebra : IStarRing<Mv>` added to `src/Core/Cl3.fs` (built ON the existing `Cl3.Mv`, no dup) + `tests/Tests.FSharp/Formal/CliffordStarRing.Laws.Tests.fs` (17 tests, verified 17/17 on main). **Cl(3,0)** comparison-free `IStarRing`; involution = **reversion** (the anti-automorphism `~(xy)=(~y)(~x)` the `*`-ring law requires; grade-involution would FAIL the order — locked by a negative-control fact). Laws: additive abelian group, multiplicative monoid (geometric product), left+right distributivity, star laws (`Conj∘Conj=id`, additive, anti-homomorphism, `Conj One=One`); basis sanity (`eᵢ²=+1`, anticommute, bivectors²=−1) + **even-subalgebra ≅ ℍ agreement with `CayleyDickson` test-locked**. **Face 3 UPDATE (2026-06-20): the two blockers are now MET** — freeze-`zeta-ir-v1` landed (#8725; legacy `*.ir.json` single-source-derived from the v1 envelope #8733) and the **multi-language generator** landed (#8735 Phase B: `tests/cross-verification/_harness/codegen-from-ir.ts` emits total-interpreter oracles in TS/F#/C#/Rust that fold the IR over inputs; #8736 Phase C: zset-merkle codegen). The **behavioral fixed-point leg is actively in progress on the gen-gen trajectory** — `zeta-ir-v1-gen` cross-verify asserts N-way agreement + value-preservation ("the precondition a gen(gen)=gen fixed-point needs"). Adjacent: the 4-language Merkle inclusion-proof byte-lock (#8719–#8723) complements Phase C's zset-merkle codegen. **STILL OPEN:** the formal **Lean Futamura `mix(mix,mix)=cogen`** theorem (Homoiconicity/Kestrel backstop; Faces 1+2 proven in `AdinkraCode.fs`) — stays the math team's. Full: `memory/project_kestrel_homoiconicity_…` | Lean (homoiconicity/Futamura — theorem open) + ✅ `IStarRing` algebra | ✅ gen-gen Phase B/C behavioral byte-lock + ✅ `CliffordStarRing`/`AdinkraCode` FsCheck | gen-gen trajectory (behavioral) · math team (Lean theorem) |
