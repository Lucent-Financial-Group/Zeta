...
|---|---|---|---|---|---|
| 1 | **P0** (Lean STRUCTURAL LEG ✅ #8794 · measure-theoretic primary open) | NFT **forgery-resistance**: `H_∞(H_AB \| E) ≥ k` (the entropy floor = G3b/Bell lifted to a pair). **The measure-theoretic `H_∞` over real distributions + the single-body premise stay OPEN (math team's).** | ✅ **Lean structural leg DONE (#8794)** — `src/Core.Lean4/Lean4/EntropyFloorLift.lean` (gated by `lake build`, verified on main; `#print axioms` = `[propext, Quot.sound]`, no `sorryAx`): min-entropy in operational guessing-space form (`H_∞ ≥ k ⟺ 2^k ≤ g`); `floor_lifts` proves independent single-body floors `ka,kb` compose to a PAIR floor `ka+kb` (the Bell/G3b single-body floor lifted to a pair, and it ADDS); `pair_floor_ge_left` (never weaker than either body); `zero_entropy_only_trivial_floor` (non-vacuity). No Mathlib. The lift is proven over the guessing-space abstraction; measure-theoretic `H_∞` is the primary. | Z3 finite instance + `ρ_owe` empirical floor | Tariq + Adaeze |
