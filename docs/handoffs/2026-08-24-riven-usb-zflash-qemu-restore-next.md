...
Mise-trust is **on `main`**. Live restore QEMU **got far enough to prove
that**, then died on the **next** bug: 6.95-picker blocks on a TTY prompt
inside QEMU until the 1800s phase-1 timeout.

Do **not** re-litigate `MISE_TRUSTED_CONFIG_PATHS`. The next software slice
is a non-interactive picker path for QEMU.
...
### P0 — non-interactive 6.95-picker under QEMU

`zeta-creds-picker.ts` uses `readline.question` and has **no**
`--defer-all` / stdin-closed path. HC-8 forbids default-**bake**. Empty
answer is already **defer**. QEMU has no TTY, so `question()` hangs.

Do:

1. Falsifying test: picker with no TTY / `--defer-all` returns without
   hanging and still forwards `--uefi-keyfile` into persist.
2. Smallest fix: `--defer-all` (QEMU install.sh passes it when
   `/zeta-qemu-creds-passphrase` was consumed) **or** treat non-TTY stdin
   as defer-all. Do **not** auto-bake.
3. Persist must still write `/mnt/boot/zeta-creds.enc` bound to the
   keyfile so phase-2 restore can decrypt the envelope (baked cred set
   may be empty). Confirm that contract with a unit test before QEMU.
4. Re-dispatch `build-ai-cluster-iso.yml` on `main` **only when the
...
- wrote-prefix **or** already-present
...
Because restore now sits before wifi/write/picker, a restore red skips
those steps (`if: github.event_name == 'workflow_dispatch'` is not
`always()`). Add `if: always() && github.event_name == 'workflow_dispatch'`
(or `continue-on-error: true` on restore only). Keep them off
`gate (required)`.
...
- Feature branch `riven/mise-trust-default-and-restore-order` was merged
  and deleted locally after this handoff branched from `origin/main`.
