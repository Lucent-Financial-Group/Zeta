...
The selected candidate carrier for the score-free preflight is `durable-room-evidence`, not ARC or a quantum simulator. Its existing canonical receipt encoding, content-addressed root, signed `+1`/`−1` uncertainty atoms, unresolved pre-assertion retractions, canonical replay, and Adinkra-compatible transport make it a useful substrate. The first F#/Python preflight is now implemented: a self-contained F# emitter and an independently authored Python verifier produce byte-identical canonical receipts for the pinned three-entry catalogue. The thirteen targeted controls cover anchor, metric, witness, catalogue, current-candidate preload, declared prior-memory, retraction order, CHSH sidecar, canonical receipt, control-manifest, observation-barrier, and ordered-candidate-evidence-digest faults. Its reported state-distance values are `0`, `150023`, and `150023`; it intentionally has no winner/result-selection field. No measured finite optimum, Tsirelson conclusion, mutual-empowerment score, agent ranking, or ARC result exists yet.

### 3.9 ARC and game-playing evidence roadmap

The next game-playing path is intentionally staged, with each layer producing a receipt before the next layer can interpret it. First, source-owned ARC/CHIP-8/CHIP-9 rooms should retain persisted episode identity, room fingerprint, action trace, caller-owned tick envelope, outcome, and content-addressed signature. They may carry declared online memory—including prior-level and cross-game memory—but must retain origin, availability, uncertainty, retraction, and current-level observation-barrier information.

Second, an ARC offline carry-forward preflight should classify state as current-level live state, prior-level memory, cross-game memory, static program, candidate transfer, or runner metadata. A held-out reset-versus-carry comparison may be designed only after those classifications, deterministic replay, memory-removal, early-read/preload, transition-policy, and identity controls pass. A finite comparison remains distinct from general transfer or an official benchmark result.

Third, the existing hosted-wrapper and REST seams can support a no-agent, non-scoring official public-game conformance probe only with the required authorization. It must pin official game/toolkit/harness/scorecard/replay identity, reject undeclared tools or network paths, and detect task/interface/context drift. It must not submit, train, access private tasks, or claim a score.

Only after those layers are independently reviewed may an official rule-constrained ARC-AGI-3 evaluation receipt be proposed. That receipt must declare model, context, tool, data, compute, harness, scorecard, replay, public/private task authorization, and competition/submission status. A source-owned room clear, a CHIP-8 trajectory, a finite durable-room catalogue observation, and a synthetic transfer control remain non-interchangeable with that official result.
