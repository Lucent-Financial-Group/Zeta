---
pr_number: 4073
title: "backlog(081KQGDBJ0008QG0R000A4EZS5): close as substrate-drift \u2014 all 6 acceptance criteria met"
author: "AceHack"
state: "MERGED"
...
1. **Six non-install `.sh` files ported then deleted.** Phase 1-2: `snapshot-github-settings.sh`, `check-github-settings-drift.sh`, `check-tick-history-shard-schema.sh` (last via #1986). Phase 3-4: `tools/profile.sh` (#1962 under 081KQGDBJ0008QG0R0022EW5ZE), `tools/peer-call/amara.sh` + `tools/peer-call/ani.sh` (subsequent ports). Self-test `find tools -name '*.sh' …` now returns only install-graph files under `tools/setup/`.
...
- `git diff docs/BACKLOG.md` → single-line flip `[ ]` → `[x]` on 081KQGDBJ0008QG0R000A4EZS5
...
- [x] Verify backlog index regenerates without diff churn beyond the 081KQGDBJ0008QG0R000A4EZS5 status flip
...
| File | Description |
| ---- | ----------- |
| docs/backlog/P1/081KQGDBJ0008QG0R000A4EZS5-typescript-standardization-non-install-scripts-aaron-2026-05-01.md | Flip `status` to `closed`, bump `last_updated`, and add a Resolution section documenting the substrate-drift close with evidence. |
| docs/BACKLOG.md | Update the index entry for 081KQGDBJ0008QG0R000A4EZS5 to show it as closed. |
