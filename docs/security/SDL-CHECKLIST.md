...
| 4 | Perform threat modeling | yes | ✅ | `docs/security/THREAT-MODEL.md` STRIDE × components matrix + supply-chain expansion (round 30) + **hardware root of trust (round 38, modelled from hardware measurements; 7 findings HRT-1..HRT-7, 4 work items)**; teaching variant `THREAT-MODEL-SPACE-OPERA.md` (31 adversaries) |
| 5 | Establish design requirements | yes | ✅ | `docs/ROADMAP.md` + `AGENTS.md` greenfield policy + GOVERNANCE.md §23-§27 |
| 6 | Define cryptography standards | yes | 🔜 | `docs/security/CRYPTO.md` (CRC32C vs SHA-256 rationale) — P1. **Round-30 decision:** no crypto surface today, so deferred; revisit when HMAC / signed-checkpoint lands. **Round-38 trigger fired:** key custody is arriving in hardware (YubiHSM 2, measured). The standards this now needs are *consumption* rules, not primitive choices — pin the attestation **key** not the issuer **name**, demand proof-of-possession, take expiry from cluster phase and never from a clockless device (`THREAT-MODEL.md` §Hardware root of trust). Still 🔜: none of it is written as a standard yet |
...
| 11 | Penetration testing | **defer** | N/A | post-v1. **Round-38 partial evidence, not a status change:** the YubiHSM 2 parsing surfaces were exercised on a throwaway device (2026-08-19) — the first adversarial *measurement* in this repo rather than a documentation review. It covered one device and left the deepest surface (the SSH-template parser, S4) unmetered, so it is a probe, not a pen test. Naming it here so the distinction survives |
