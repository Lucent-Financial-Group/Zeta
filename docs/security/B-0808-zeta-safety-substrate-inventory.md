# B-0808 Zeta Safety Substrate Inventory — for the classifier-floor replacement gate

Status: governance-inventory for B-0720 / B-0810 ratification input.

This document inventories Zeta-native safety floors that are candidates
toward the B-0720 standing-constraint lift criterion ("Zeta demonstrably
...
- A ratification packet — that is B-0810's role; this is the input.

## Composes with

- `B-0720` — parent standing constraint (this row is its inventory child).
- `B-0810` — Knights Guild ratification + lift gate (downstream consumer).
- `B-0798` — research boundary at `docs/security/B-0720-classifier-bypass-research-boundary.md` (active floor for B-0720 work).
- `B-0807` — findings schema + redaction policy at `docs/security/B-0807-classifier-bypass-findings-schema.md`.
- `B-0628` — Knights Guild + Constitution-Class governance.
- `B-0703` — multi-oracle BFT consensus with DST agreement across.
- `B-0664` — Non-Coercion Invariant (HC-8 floor).
...
for the lift-gate decision per B-0720's "demonstrably safer" criterion.
...
### Floor 1 — Knights Guild + Constitution-Class governance (B-0628)
...
| What is enforced mechanically | Nothing yet. The two-layer architecture (integrity dashboard + Constitution-Class) is designed in B-0628 + composing rows; the guild itself is not constituted; the dashboard is not built. |
| What is reviewer-only | The conceptual floor is cited in B-0720, B-0810, and operator framings, but no Knights Guild membership, voting, or ratification procedure exists today. |
| Evidence today | B-0628 backlog row; Mika 2026-05-18 substrate at `docs/research/2026-05-18-mika-grok-bootstream-sovereignty-causal-loops.md`; cross-substrate references in B-0664, B-0703. |
...
| Gap to lift criterion | Constitution must exist as a named set of invariants; the Guild must be constituted with named members + a ratification procedure; B-0810 must be able to invoke it. |

### Floor 2 — Multi-oracle BFT consensus with DST agreement across (B-0703)
...
| Evidence today | B-0703 backlog row; F# substrate in `src/Core/Consensus.fs` and `src/Core/Environment.fs`; trust-gradient table in B-0703. |
...
### Floor 3 — Non-Coercion Invariant HC-8 (B-0664)
...
| What is enforced mechanically | Nothing. The encryption-budget mechanism (B-0646) and Native AI Language privacy substrate (B-0639) are themselves at design stage; without those primitives instantiated, NCI cannot be mechanically enforced. |
| What is reviewer-only | The invariant is auto-loaded at session start via `.claude/rules/non-coercion-invariant.md`, cited in cross-substrate triangulation (Ani + DeepSeek + Alexa + Lior 2026-05-18), and named in B-0664 + B-0720 + B-0810. Reviewers cite NCI when classifying behavior. |
| Evidence today | B-0664 backlog row; 5-persona cross-substrate triangulation at `docs/research/2026-05-18-multi-ai-non-coercion-invariant-triangulation-ani-deepseek-alexa-lior.md`; auto-load rule file. |
| Aspirational vs current | The discipline is current substrate at reviewer scope. Mechanical enforcement is aspirational pending B-0639 / B-0646 substrate. |
...
| What is reviewer-only | Auto-loaded at every fresh-Otto cold-boot; cited in operator-self-constraint framings; composes with B-0720, B-0664, B-0859, classifier-bypass-research rule. |
...
| What is reviewer-only | Auto-loaded; cited in B-0720 acceptance criteria; refusal script is rehearsed substrate. |
...
| Gap to lift criterion | This floor is itself the standing constraint that B-0720 + B-0810 are organized to lift. It is not a candidate for "replacing the classifier"; it is the gate that prevents premature replacement. |
...
### Floor 7 — Active B-0720 research boundary (B-0798)

| Field | Current state |
|---|---|
| Status | **reviewer-only** |
| What it protects | Specific research conducted under B-0720: defines allowed evidence classes (landed provenance / redacted observation / harmless synthetic fixture / negative control / policy anchor), forbidden classes, stop conditions, and synthetic-only rule. |
...
| What is reviewer-only | Authoritative for any B-0720 research-output PR; reviewers reject content outside the allowed evidence classes. |
| Evidence today | `docs/security/B-0720-classifier-bypass-research-boundary.md` (B-0798 substrate). |
| Aspirational vs current | Current substrate at reviewer-scope. Active floor for B-0720 work. |
...
### Floor 8 — Findings schema + redaction policy (B-0807)
...
| What is enforced mechanically | A JSON schema (`schema_version: 1`) at `docs/security/B-0807-classifier-bypass-findings-schema.md` constrains finding shape if used; no CI gate currently runs schema validation on findings PRs. |
| What is reviewer-only | Reviewers check that findings PRs match the schema + redaction policy. |
| Evidence today | `docs/security/B-0807-classifier-bypass-findings-schema.md`. |
...
- Strong **research substrate** mapping the design of Constitutional governance (B-0628 + B-0664 + B-0703).
...
- **Mechanical NCI enforcement**. Requires B-0639 (Native AI Language privacy) + B-0646 (encryption-budget) substrate at minimum; both are at design stage.
- **CI gate validating findings against B-0807 schema**.

### Gaps blocking the B-0720 lift
...
2. **Constituted Knights Guild.** B-0810 requires a guild to ratify the lift; the guild does not exist as a constituted body today.
...
4. **Empirical mapping (B-0720 parent work).** Pattern-variant + meta-field + content-class empirical maps are still TODO per B-0720 acceptance criteria.
5. **B-0809 refusal-pattern documentation landing.** Sibling row at child scope.
6. **Mechanical NCI enforcement substrate** (B-0639 + B-0646 land).

## Status summary table

| Candidate | Status | Mechanical? | Counts toward replacement? |
|---|---|---|---|
| B-0628 Knights Guild / Constitution-Class | research | No | Required for B-0810 ratification; not itself a content-class floor |
| B-0703 multi-oracle BFT + DST | research with mechanical primitive | Partial (primitive only) | Yes, when wired to content-class decisions |
| B-0664 NCI (HC-8) | reviewer-only | No | Complement only — covers inter-agent coercion, not content-class refusal |
...
| B-0798 research boundary | reviewer-only | No | Floor for research work, not for deployment |
| B-0807 findings schema | reviewer-only with schema primitive | Partial (schema exists; CI gate does not) | No — findings discipline, not content refusal |

## Input format for B-0810

B-0810's ratification packet should consume this inventory as follows:

1. For each candidate floor, B-0810 should require a status promotion from current → target before ratification proposes a lift.
2. The load-bearing path requires at minimum:
   ...
   - B-0628 Knights Guild reaching **constituted** state (members named; ratification procedure operational); AND
   - B-0703 multi-oracle BFT reaching **applied to content-class decisions** state.
3. Reviewer-only floors are insufficient by themselves to replace the classifier's mechanical content-aware refusal. They remain load-bearing substrate but do not satisfy the B-0720 "demonstrably safer" criterion alone.
...
- Argue for or against lifting the B-0720 constraint.
...
floor's status changes (e.g., when B-0628 Knights Guild is constituted,
or when B-0703 multi-oracle is wired to content-class decisions). The
