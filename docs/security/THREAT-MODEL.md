...
| Verifier jar (Alloy) | `AlloyTools/org.alloytools.alloy@v6.2.0/org.alloytools.alloy.dist.jar` | **sha256 pin** in `tools/setup/manifests/from-url` — no longer TOFU (081M23AST90087G0R00150MK76) | digest verified before the atomic `.part` rename, AND re-verified against a file already present, so a re-uploaded or substituted asset fails closed instead of being adopted; `lint-verifier-jar-provenance.ts` re-checks that pin against the docs | Manual on bump — the digest is the pin, the URL is only a name | Playbook B |
| Verifier jar (TLA+) | `src/Core.TLA/tla2tools.jar` — **committed to git, not downloaded** | byte-pinned by the diff | sha256 + TLC banner asserted on every gated run via `registry/tlc-models.json`; derived identity checked by `lint-verifier-jar-provenance.ts` | Deliberately blocked from becoming a fetch: `v1.8.0` is a ROLLING prerelease whose asset is re-uploaded in place (three builds on one tag: 2026-05-18, 2026-08-11, 2026-09-09), so the running build is not obtainable upstream. `docs/INSTALLED.md` | Playbook B |
