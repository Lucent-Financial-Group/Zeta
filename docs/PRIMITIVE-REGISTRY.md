...
- **Codec / IO** (extends the Codec line above) — ⬜ `BitReader`/`BitWriter` (sub-byte variable-width int codec — foundation for bit-packing/FOR/delta compression) · ⬜ `RangeSet` (`"1-5,8,10-17"` parse/contains/render, auto-merged ranges — sparse-int-set primitive)
