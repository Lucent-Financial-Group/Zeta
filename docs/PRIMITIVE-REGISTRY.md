...
- **Algebra ladder** — ✅ G-Set (4/4, **Tier 1/2** — per-lang golden-vector replay, no N-way byte-diff harness yet; C# #6363 merged) · 🚧 Bag / multiset (**3/4** — TS #6364 + Rust #6367 + F# `src/Core/Bag.fs`; C# remaining) · 🚧 Z-set (F# only) · ✅ IndexedZSet (F#) · ✅ CRDTs — G-Counter / PN-Counter / OR-Set / Delta-CRDT (F# `Crdt.fs` / `DeltaCrdt.fs`)
...
| ------------------------------------------------------------------ | --- | --- | --- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Bag / multiset** — non-negative-weight multiset                  | ✅  | ✅  | ❌  | ✅   | The **middle rung** (G-Set ⊂ Bag ⊂ Z-set; ℕ / per-key sum). **3/4** — TS reference + `bag/golden-vectors.json` (#6364), Rust (#6367), F# `src/Core/Bag.fs` (per-lang golden-vector replay, Tier 1/2); **C# remaining**. F# uses structural `compare` (ordinal for strings — fixes the G-Set `Comparer.Default` culture gap, B-0969). Combiner is SUM (NOT idempotent — `union(a,a)` doubles counts), the step toward Z-set's signed ℤ. |
