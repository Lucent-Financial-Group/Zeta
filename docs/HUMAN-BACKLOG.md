...
| HB-004 | 2026-04-23 | decision / branch-protection | Decide whether `submit-nuget` should be a required check in branch protection. Context: nearly every recent PR (#155-#170 inclusive) has `submit-nuget: FAILURE` blocking merge. Investigation (auto-loop-67): the job is inside GitHub's **automatic** `Automatic Dependency Submission` workflow (enabled via repo-settings → Security → Dependency graph → Automatic dependency submission — no `.github/workflows/*.yml` file for it in our tree). The job scans NuGet deps successfully (full dep-graph visible in failure logs) then POSTs to GitHub's `/repos/.../dependency-graph/snapshots` REST endpoint, which returns *"An error occurred while processing your request. Please try again later."* This is a GitHub-side API intermittent — same class as the `git push` HTTP 500s observed this session. The job's output powers Dependabot alerts + security advisories + SBOM — advisory security surface, not a correctness gate. Clean resolutions: (a) **Remove `submit-nuget` from required checks** — the job still runs and retries naturally on next push; alerts still work when GitHub's API is up; PRs no longer blocked by GitHub-side transient; rationale: advisory check shouldn't gate merges. (b) **Keep required** — current behaviour; merges wait for GitHub's API to recover. (c) **Keep required + add re-run automation** — harder; the workflow is GitHub-managed, no in-tree file to modify. Option (a) is recommended. No deadline but affects every open PR right now. | `gh run view 24847547285 --log-failed` (PR #170's failed submit-nuget run) + the automatic workflow runs across #155-#170 + auto-loop-67 investigation + 2026-04-23 session transcript | Open |  |
