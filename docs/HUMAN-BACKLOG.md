...
| HB-004 | 2026-04-23 | decision / branch-protection | **REVISED + RESOLVED 2026-04-23 same day.** The original row asked Aaron to decide whether `submit-nuget` should stay a required check; initial recommendation was "remove from required." Aaron then delegated branch-protection tuning to the agent *and* sharpened the posture: *"the more checks that gate merges the better as long as for certain PRs we can ignore if need with justification that is peer reviewed by a different named agent or the architect. pr checks keep the quality high and decisions intentional which is what we want."* The sharpening inverts the initial recommendation — maximalist gating posture, with peer-reviewed ignore-justification as the escape valve (not check-removal). Correct resolution: **KEEP submit-nuget required; DO NOT remove.** The forward branch-protection design (separate work, not this row) builds the ignore-with-peer-reviewed-justification workflow; individual PRs blocked by genuine external transients can merge via a named-reviewer-approved justification without relaxing the gate. Interim: currently-blocked PRs wait for GitHub API recovery (waiting is usually cheaper than justification-review overhead). | `gh run view 24847547285 --log-failed` + Aaron's 2026-04-23 branch-protection delegation + same-day sharpening directive + `memory/feedback_branch_protection_settings_are_agent_call_external_contribution_ready_2026_04_23.md` (per-user) | Resolved | Keep submit-nuget required; build ignore-with-peer-reviewed-justification workflow in forward branch-protection design. Row resolved without settings change. |
