...
- `docs/HUMAN-BACKLOG.md` (this file) is **human-facing**:
  work that _requires_ a human decision, credential,
...
Humans _may_ opt into code contribution via the
**teaching-track** (see
...
1. **Aaron's framing (2026-04-20):** _"i think this a
   ...
   anyting from me, i can just look at the human backlog"_.
...
(2026-04-20: _"can you put my tasks at the top of the human
backlog i don't want to have to go digging through it to find
my tasks"_). The `For:` column and the per-addressee sub-table
...
- **Out of scope for names:** any prose _outside_ the row
...
tasks"_); other named humans follow in the order they were
...
_(No open rows. Agents file rows here when a decision /
credential / external action is needed from any human.)_

_(The backlog starts essentially empty. Agents file rows
as blocked-on-human work arises; honest scans of memory
and open PRs may seed additional rows opportunistically.
Do not backfill retroactively for its own sake.)_
