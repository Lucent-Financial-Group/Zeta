...
Next concrete action: review the S6 first-login UX co-design draft — [S6-UX-PLACEHOLDER.md](./S6-UX-PLACEHOLDER.md) — on paper/mock terminal, then verify on physical boot before clearing S6; slice 5 CODEOWNERS when teams confirmed; system mise pinned via Nix overlay (same release as `tools/setup/linux.sh`).
