...
14. ✅ **Lowering as the Kleisli descent** — LANDED (`SoftValue.map`/`bind` + `ParseSoft.lower`):
...
15. 🔀 **ROUTED to math team** — loopy EP + **emotional propagation** on `Zeta.Bayesian` (the
...
16. Parity categories needing a core `DynamicValue` shape first (Decimal / SoftValue / Kleene) —
