...
- **array order preserved** — arrays keep insertion order (only object _keys_ sort) and recurse.
...
TS-only until a non-TS Ace implements them. The golden vectors are the _data the eventual
4-language seam + proof run on_. The new fixture is auto-discovered by
