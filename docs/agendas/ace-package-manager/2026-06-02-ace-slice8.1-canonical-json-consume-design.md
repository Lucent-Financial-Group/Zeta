...
- Ace's current `canonicalize` **sorts keys** (`Object.keys().sort()`), and
  ...
  **sorts object keys** yields sorted-key canonical output — Ace keeps its current
  key-order-independence _and_ consumes the shared primitive. (Pure insertion-order
...
- **Behavior-preservation (the load-bearing gate):** the existing Ace suites
  ...
  publish-side anti-rollback, revocation/quarantine, content_hash. The _values_ change
...
(Ace has none); CBOR (Ace's index is JSON); adding a sorted-key _mode_ to the shared
