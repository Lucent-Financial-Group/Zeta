...
effort: S
ask: Aaron 2026-04-26 — *"this is like the python smell but with python and this one had a real cost it forgot to ignore upstram so it took like a minute to run instead of a few seconds, if it was cononalized in code like in ../scratch it would never forget to exclude directoris like our references"*. The bash pipeline `markdownlint-cli2 "**/*.md" | grep -E 'MD[0-9]{3}'` I composed inline lacked proper repo-aware exclusions for vendored / mirrored directories, ran ~60s instead of expected ~3s. Same Otto-346 pattern (recurring inline composition = missing substrate primitive). Per 081KQ0YZ80008QG0R0012002S4 P2 priority bump: target is TypeScript via Bun, not bash and not Python.
created: 2026-04-26
...
# 081KQ3HBZ0008QG0R000ACTVA6 — extract markdownlint-with-repo-aware-exclusions tool
...
3. 081KQ3HBZ0008QG0R0006Z5EXX — gh-pr-state-summary tool (TS target; awaiting first-migration unblock)
4. **081KQ3HBZ0008QG0R000ACTVA6 (this row)** — lint-with-exclusions tool (TS target)
...
- 081KQ3HBZ0008QG0R0006Z5EXX (`gh-pr-state-summary.ts`) — sibling extraction from same Otto-346 pattern; both target TS
- `tools/hygiene/check-tick-history-order.sh` + `check-no-conflict-markers.sh` — sibling architectural shape (shell now; eventual TS rewrite per 081KQ0YZ80008QG0R0012002S4)

The cumulative `tools/hygiene/` post-install batch awaiting TS migration:

- 081KQ3HBZ0008QG0R003WJ62HM (markdown-table-cell-count fix tool — owed-build, TS target)
- 081KQ3HBZ0008QG0R0006Z5EXX (gh-pr-state-summary — owed-build, TS target)
- 081KQ3HBZ0008QG0R000ACTVA6 (this row — lint-with-exclusions — owed-build, TS target)
- + eventual rewrites of #541, #542

## Why TypeScript

Per Aaron's prior priority bump on 081KQ0YZ80008QG0R0012002S4 (P3 → P2):
...
- **081KQ0YZ80008QG0R0012002S4** (TS-migration P2 priority — first-migration unblock applies)
- **081KQ3HBZ0008QG0R003WJ62HM** (markdown-table-cell-count tool — sibling extraction)
- **081KQ3HBZ0008QG0R0006Z5EXX** (gh-pr-state-summary — sibling extraction)
- **081KQ3HBZ0008QG0R003B2NAA2** (references/ directory rename — paired concern from same Aaron observation)
...
| 3 | Inline Python gh-JSON-parse | 081KQ3HBZ0008QG0R0006Z5EXX (TS owed) |
| 4 | **Bash markdownlint+grep** | **081KQ3HBZ0008QG0R000ACTVA6 (TS owed; this row)** |
...
- 081KQ3HBZ0008QG0R0006Z5EXX gh-pr-state-summary tool → TS (P3)
- 081KQ3HBZ0008QG0R000ACTVA6 lint-with-exclusions tool → TS (P3, this row)
- Rewrites of #541, #542
