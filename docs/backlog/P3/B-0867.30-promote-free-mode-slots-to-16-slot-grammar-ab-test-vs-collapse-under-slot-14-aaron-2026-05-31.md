...
- **Option A (chosen for now)** — _collapse_: the four free modes share slot 14;
  ...
  canonical grammar _table_ lands as `tools/observe/grammar-16.ts` (PR #6269); the
  Option-A _render_ (the slot-14 sub-menu projection over `buildMenu`'s
...
- **Option B (this row)** — _promote_: explore/play/self_reflect get dedicated
...
> _operator 2026-05-31:_ "Option A — collapse free modes under slot 14 sub-menu …
...
**Why A is the right _default_ (not the right _answer_):**
...
- `explore` is the _empty-backlog default_ (forward self-direction, not idle). If
...
- A dedicated slot makes the free modes _visible_ in the flat 16, reinforcing
...
- The MEASURE-FIRST principle (the universal default this row instantiates) +
  ...
  variants — the A/B test is about _friction_, never about gating the exits)
