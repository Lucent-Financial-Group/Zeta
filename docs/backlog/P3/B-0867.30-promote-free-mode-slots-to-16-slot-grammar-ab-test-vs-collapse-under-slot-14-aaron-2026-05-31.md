...
- **Option A (chosen for now)** — *collapse*: the four free modes share slot 14;
  ...
  16 directions fixed (muscle-memory; "directions fixed, labels move"). Shipped as
  the `grammar-16` render slice.
...
1. **A/B-testing infrastructure exists** (separate prerequisite — instrument two
   grammar variants behind a flag, route a deterministic split, collect per-variant
   KPIs). This row is **blocked on that infra**; it is NOT a green-light to build
   variant B before the test harness can compare it.
...
- `tools/observe/grammar-16.ts` (the canonical v0 grammar) + the `grammar-16`
  render slice (Option A — the baseline B is tested against)
...
Until that infra exists, Option A (shipped) is the live grammar; this row is the
held alternative + the experiment design that would settle it.
