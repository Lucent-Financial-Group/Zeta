...
**Required-checks semantics — corrected per repo reality + Aaron 2026-05-03 future-state directive** (post-merge review P1 finding): the repo's current setup (`gate.yml` + `code_quality severity:all`) treats skipped jobs as FAILURE, not SUCCESS. The naive "skipped-but-required passes" claim is INCORRECT for Zeta. **Aaron 2026-05-03 future-state directive**: *"the future will have all and treak skipped or failed steps as fialure to we will have different ruleset per flow so they can stay fully on and true."* Architecture: each parallel-forever-workflow has its OWN ruleset that stays fully-on within its concern-scope; skipped OR failed both = failure within that ruleset's scope; the ruleset isn't asked to gate workflows outside its concern-class, so its gate-checks stay clean. The cheap-no-op-path (required-check name reports SUCCESS via baseline artifact) is a transitional pattern; the long-term pattern is per-flow rulesets that don't over-require. This composes directly with B-0155 (multi-ruleset split): each ruleset configures its required-checks list independently per concern-class, so per-flow gating is precise and per-flow workflows stay fully-on.
