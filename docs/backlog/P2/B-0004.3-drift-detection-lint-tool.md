...
B-0004 explicitly calls out drift detection as owed tooling: _"when English source changes, translations need updating; need a lint that flags stale translations."_ Without it, translations silently diverge from the English source. Per the deployment discipline (Otto-291), retractability requires knowing _which_ translations need refreshing.
