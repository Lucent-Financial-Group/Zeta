...
Two-part vision captured across two messages:
...
plus optional named-dep + optional parent-pr). This row extends the
writer with AUTO-GATHERED CURRENT-STATE fields that the agent doesn't
have to pass explicitly.
...
- `.claude/rules/codeql-no-source-on-docs-only-pr-is-broken-commit-canary.md`
  — dotgit-saturation tier consumed by 5d
