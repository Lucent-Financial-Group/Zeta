---
id: B-0954.1
zetaid: 081KT07NV0008QG0R000QWEKTE
priority: P2
status: open
...
effort: M
ask: aaron + otto 2026-06-01 (multi-agent review of B-0959)
created: 2026-06-01
...
# B-0954.1 — the bus-tip partition-tolerance story

## The gap (multi-agent review of B-0959, 2026-06-01)

Grok (critique) + Amara (sharpen) **converged** on the same un-named hole in the
B-0959 "git + the fold give it for free" framing: the **private** per-agent home +
...
1. **Claim-coordinator (exists: `tools/bus/claim.ts`, B-0400)** — `check / acquire /
...
3. **Co-dominant git mirrors (B-0942)** — multiple co-equal remotes (local, GitHub,
   ...
   bus.
4. **§4 distributed-time primitive (B-0878/B-0683/B-0684)** — HLC + uncertainty for
...
(B-0840 thermal-forgetting) + compaction must run through the claim-coordinator (a
...
- [ ] A concrete, written partition-tolerant bus-tip protocol: claim-coordinator +
      ...
      "CRDT merge" hand-wave in B-0959/B-0954.
...
- [ ] (stretch) A DST scenario (per B-0767/ChaosEnv) that partitions N bus writers
      and verifies the tip heals to the G-Set union on re-sync.

## Composes with

- [B-0954](B-0954-implement-git-native-cross-machine-agent-bus-docs-agent-bus-folder-zetaid-keyed-gset-crdt-no-pr-per-6219-spec-aaron-otto-2026-05-31.md) — the bus this hardens
- [B-0942](B-0942-co-dominant-git-mirrors-git-native-crdt-coordination-no-host-needed-aaron-2026-05-30.md) — co-dominant mirrors (the no-central-remote answer)
- [B-0953](B-0953-git-v2-handshake-fsharp-looks-like-git-negotiates-up-to-dbsp-retraction-algebra-same-objects-agent-speed-upstream-aaron-2026-05-31.md) — Git-V2 negotiation (the protocol layer)
- [B-0400](../P1/B-0400-inter-agent-ephemeral-communication-bus-nats-protocol.md) — the claim-coordinator (`tools/bus/claim.ts`) this reuses
- [B-0892](../P1/B-0892-three-lanes-concurrent-operating-discipline-encryption-plus-zflash-plus-state-machine-substrate-until-each-lane-backlog-drains-per-operator-2026-05-28.md) — the lanes discipline
- B-0878 / B-0683 / B-0684 — the §4 distributed-time primitive
- the `force-push-with-lease-authorization-policy.md` rule — single-row CAS = ref-lease
- [B-0959](../P1/B-0959-zeta-sovereign-distributed-db-and-agent-loop-master-checklist-one-git-native-zset-substrate-aaron-otto-2026-05-31.md) §0 — the "not free" tagging this row resolves
