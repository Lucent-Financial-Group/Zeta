...
Recommendation (defer to maintainer): start with B-0801 autoUpgrade (simpler; existing public-repo state covers auth). When repo goes private OR when we want CI-controlled rollouts, switch to deploy-rs.
...
Self-hosted runner composes with B-0794 homelab gh-auth substrate.
...
- [ ] B-0801 autoUpgrade disabled if this row is the chosen shape (mutually exclusive)

## Out of scope

- Multi-cluster / multi-region rollouts
- Blue-green node-pool patterns
- Distro-upgrade orchestration (B-0804)
