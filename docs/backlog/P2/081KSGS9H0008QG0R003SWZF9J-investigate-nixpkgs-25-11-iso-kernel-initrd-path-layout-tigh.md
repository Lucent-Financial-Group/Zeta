...
`tools/ci/audit-installer-iso-content.ts` REQUIRED_ISO_PATHS asserted `boot/bzImage` + `boot/initrd` as top-level files in the produced ISO. On nixpkgs 24.11 this passed; on nixpkgs 25.11 (post-B-0800 bump), both assertions fail:
...
**B-0823 substrate-layer follow-up (this row)**:
...
4. OR keep the lenient any-of as defense-in-depth (preferred — same discipline as B-0818's audit-glob relaxation)
...
- **[B-0800](../P1/B-0800-bump-nixpkgs-24-11-to-25-11-eol-recovery.md)** — nixpkgs 25.11 EOL recovery that triggered this regression
- **[B-0818](B-0818-investigate-isoname-mkforce-not-sticking-on-nixpkgs-25-11-aaron-2026-05-26.md)** — sibling 25.11 ISO regression (`isoName` mkForce not sticking); both are downstream of B-0800; both use the same audit-glob-relaxation + diagnostic-dump pattern
- **[B-0816](../P1/B-0816-architectural-principle-maximize-argocd-scope-minimize-nixos-native-lock-in-cross-cluster-portability-leverage-aaron-2026-05-26.md)** — cross-distro portability principle; ISO substrate naming + audit are Zeta-internal not portable to non-NixOS distros
...
- Adapting audit for non-NixOS installer ISOs (different layout entirely; B-0816 says ISO substrate is Zeta-internal not portable)
...
Filed P2 — same as B-0818; workaround (lenient any-of) eliminates operator-impact; tighter substrate is cleanliness rather than urgency.
...
- `gh pr list --state all --search "B-0823"` → no in-flight collision
- ID B-0823 next-free per `git ls-tree origin/main` (highest = B-0822 in flight via this batch's other PR)
