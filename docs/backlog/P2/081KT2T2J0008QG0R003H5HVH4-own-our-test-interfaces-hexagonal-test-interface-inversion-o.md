...
3. **Migrate the C1 proof** (B-1007, the first landed proof) behind the ports as the
...
5. Cross-language stub: the same contract shape in one other language (Rust or C#) to
   prove portability (composes B-0982 byte-lock at test scope).
6. Contribute-upstream check (per the rule): if wrapping surfaces a framework
   improvement, file it upstream.

## Composes with

- **B-1000** (the engine whose tests this re-homes) · **B-1006** (registry/BCL — test
  primitives are primitives too; this is the BCL boundary at test scope) · **B-1007**
  (the proofs that sit behind `IProofEngine`) · **B-0982** (four-oracle byte-lock —
  extends to test contracts) · **B-0428** (F# HKT — the port types)
