...
- **Tests.** 9 reader unit tests (exact token sequence, zero-copy borrow vs owned,
  ...
  differential test. So `cargo test` runs 9 unit + 3 integration; `cargo test
  --features serde` runs 9 unit + 4 integration. Zero warnings; clippy-clean.
