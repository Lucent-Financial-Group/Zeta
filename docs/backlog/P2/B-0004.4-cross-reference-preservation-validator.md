...
B-0004 body calls this out explicitly: _"when memory file references another memory file, the translated version must reference the translated version (not break cross-refs)."_ A translated file that links to untranslated targets is worse than no translation for a monolingual reader: they follow a link and hit English content without warning. The validator makes this a measurable gate.
