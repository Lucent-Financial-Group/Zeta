...
- **B-0792** (iter-5 wifi + hostname injection; depends_on; this row picks up where B-0792 sub-target 4 deferred the role-stack decomposition)
- **B-0776** (simplest-first plugin sequence; composes; each plugin's "which roles need it" question becomes cleaner with role-as-capability)
- **B-0789** (iter-4 SSH+password substrate; composes; same ESP-injection pattern)
- **B-0790** (zero-dev-machines cluster-native end-state; this row is load-bearing for multi-role nodes in the homelab persona's typical sub-3-node cluster)
- **B-0759** (first-time-CLI-user broadened to homelab persona; capability auto-detection in sub-target 4 is load-bearing for the no-CLI-typing experience)
- **B-0778** (commodity hardware reference; this row composes with the curated hardware list — each hardware shape maps to a default role-set)

## Out of scope (for this row; tracked elsewhere)

- Cluster orchestration substrate selection (K3S vs Talos vs etc.) — tracked under B-0776
- Worker join token / control-plane discovery — B-0792 sub-target 5; deferred
- Capability auto-detection implementation details — sub-target 4; deferred
- K8s node-label taxonomy schema — sub-target 5; deferred (depends on B-0776 cluster substrate selection)
