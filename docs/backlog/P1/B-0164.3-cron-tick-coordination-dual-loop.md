...
- _Correlated failure_: both loops drift in the same wrong direction → same-cron catches this better (independent perturbations)
- _Cascade error_: loop A's wrong output feeds loop B → staggered catches this better (loop B reviews loop A)
