...
- Path: `docs/agent-events/{trajectory}/YYYY/MM/DD/{eventId}.json` (or similar; composes with B-0867.2 event-sourcing layer's filename convention)
- OR trajectory branches: `agent-events/{trajectory}` (per B-0867.2's branch convention)
- Either way: agents push events directly without per-event PR ceremony

This is the heartbeat-pattern transposed to state-machine events. Heartbeats today go in `docs/agent-heartbeats/` (per B-0858 substrate) and have minimal review-friction; state-machine events get the same treatment on their own path.
...
- Operator's trajectory-async-review (per B-0873) operates on these batches naturally (review the trajectory shape; the batch PR is the visible chunking unit)
...
- **B-0858** heartbeat folder substrate — direct prior art for the fast-lane pattern
- **B-0867.2** event-sourcing layer — fast-lane is the write-side of B-0867.2's event storage; batch-merge is the read-side aggregation
- **B-0867.20** lifecycle DU split (trajectory-push vs PR-review) — fast-lane IS the trajectory-push branch's operational form
- **B-0874** GitHub Actions recursion — batch-merge coordinator could run as a GitHub Action OR as a local job; composes with the runtime substrate
- **B-0887** Zeta-native review substrate — fast-lane events DON'T require review (per the lifecycle DU split); system-changes DO
- **B-0886** ASAP cluster umbrella — this row is supporting infrastructure; should be added to umbrella composition
- **B-0889** symbiotic cross-track self-healing — fast-lane composes with cross-track sync (events on one track propagate to the other via batch-merge coordinator)
...
- Burn GitHub GraphQL budget for PR mutations (the rate-limit topology B-0887 specifically routes around)
