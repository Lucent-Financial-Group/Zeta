...
Order suggestion: 1 (current-inventory mapping) → 2 (workflow engine MVP as compositional substrate) → 3 (boss check-in cadence) → 4 (tradeoff framework) → 5 (cap-awareness) → 7 + 8 (workflow-engine + hats integration) → 6 (evolution tracking).
