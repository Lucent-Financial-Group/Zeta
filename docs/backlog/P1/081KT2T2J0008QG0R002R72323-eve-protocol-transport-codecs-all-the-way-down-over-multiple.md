...
- **on-wire key exchange** (no pre-shared secret) → authenticated encryption of the value/columnar rungs (compose B-0883 PQ-lattice; B-0885 agent private state).
...
- composes B-0289 (Green Lantern / RF mesh transport) + B-0829 (cluster-fork-as-trust-boundary).
...
- **B-1001 `Codec<Codec<T>>`-where-T-is-HKT** = the type; B-1002 = the recursive generalization (`codecs all the way down`) + the concrete transport (multiplexed WS/TCP) + the on-wire negotiation for strangers.
- **B-0428 real HKT** to express the recursive codec tower cleanly; simulate via the `IColumnar`/`IMessage` dictionary + SRTP until the fork lands.
...
- **B-1001** (codec<codec<T>> serialization port; columnar message infra) · **B-1000** (Infer.NET rewrite; the message state being transported) · **B-0638** (Eve Protocol — polymorphic diplomatic language; the negotiation) · **B-0883 / B-0885** (PQ-lattice encryption / agent private state) · **B-0829** (cluster-fork-as-trust-boundary) · **B-0428** (real HKT) · **B-0289** (Green Lantern RF/mesh transport)
