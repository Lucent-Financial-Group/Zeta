...
4. **Signal-blocking as first-class primitive** (Aaron 2026-05-19 sharpening) — Rx subscription has a dual: signal-blocking. Without first-class block primitive, "telepathic" mesh becomes attack surface. Block = receive + immediately retract = net-zero state change (clean dual in DBSP retraction semantics). *(DECOMPOSED to B-0669.1)*
