...
5. **Plugs into existing standards** (per B-0765 ServiceTitan):
...
6. **Substrate-honest minimal**: builds the per-plugin pattern
   ...
   F# + C# facade; per B-0772 polyglot Rx wrapper as v2) on
...
Per B-0765 ServiceTitan route: each Zeta plugin wraps an EXISTING
...
trainable substrate per B-0761.

Per B-0768 Itron route: once Zeta substrate has meaningful
adoption + concrete patterns from the first few plugins, the
Zeta plugin authoring contract itself becomes a candidate
standard for co-creation with incumbents (e.g., per B-0764 CNCF
...
- B-0765 — ServiceTitan route (each plugin wraps existing
  standard; doesn't invent new)
- B-0766 — slow-replace k8s (plugin substrate is the
  Wave-2-operator-surface ramp for B-0766's binary-compat
  endgame)
- B-0772 — observable+controllable cluster fabric (each
  plugin's events become Observables in the fabric; each
  plugin's commands become Observers)
- B-0773 — cluster as digital twin (plugin events feed the
  twin; plugin commands flow through twin)
- B-0774 — etcd-less options (CockroachDB-as-kine backend
...
**Preserves**: B-0763's "operator-in-the-negotiation-high-seat"
principle. Every plugin contract is operator-facing + portable.
Backend swap-out per B-0763 vendor-swap.
...
- After Rank 7-10: full cluster substrate exposed via Zeta
  interfaces; B-0773 digital twin substantively complete

## Out of scope

- Implementing all 10 plugins in one row — handle each as
  separate sub-row when ready (B-0777 = Redis KV; B-0778 =
  NATS PubSub; etc.)
- Substituting alternative backends in v1 (Valkey for Redis,
  YugabyteDB for CockroachDB, etc.) — establish pattern with
  primary backend first; alternatives follow per B-0763
  vendor-swap
- Polyglot Rx wrapping per plugin (per B-0772) — F# + C#
  facade in v1; polyglot SDKs in v2 per B-0772
...
Aaron 2026-05-25 mid-iter-3-CI-wait, after B-0774 etcd-less +
B-0775 scale-architecture: 'okay we already have coackroack in
cluster lets start finding shapes of ontologies and supporing
plugin model and adding one at a time over time starting with
simplest.' Substrate-honest recalibration: B-0763 + B-0772 +
B-0773 + B-0774 + B-0775 named the ABSTRACT pattern; this row
