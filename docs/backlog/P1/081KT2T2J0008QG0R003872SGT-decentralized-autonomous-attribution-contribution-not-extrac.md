...
# AI custom app store over product-decision lanes — web3-shaped, attribution/contribution-based, superadditive
...
1. **App-store-over-lanes spec** — what an "app/contribution" is over the product-decision lanes (B-0867 controller-actions + B-0986 lanes); how it's published/discovered/run via Ace.
2. **Attribution + contribution ledger** — record who contributed what; value flows by attribution (contribution-graph, attention-as-currency); NOT platform-extracted.
3. **Decentralized/autonomous** — runs over Agora (no central owner); composes B-0954 bus + B-0703 multi-oracle.
4. **Superadditive scoring** — model the contribution value as superadditive (attention×memory multiplicative), not linear; verify against the attention-as-currency contribution-graph.
5. **Anti-extractive guarantees** — must-paired-with-can-exit (no lock-in); self-selecting opt-in; attractive-not-coercive (NCI floor).
6. **web3-done-right framing** — decentralized ownership/attribution without the speculative/extractive failure mode; reconcile with B-0623 participation-economy.

## Composes with substrate

- **B-0867** — workflow engine / universal-action-grammar (the product-decision lanes / controller the apps run over)
- **B-0986** — orthogonal lanes + tiles-as-ownership (the lanes; decentralized ownership)
- **B-0985 / B-0543** — hexagonal core seed pair (Pay Attention × Remember When = the multiplicative primitives behind super-additivity)
- **B-0990** — Itron ML-app-store-over-mesh precedent (the "same concept")
- **B-0623** — participation-economy (web3-done-right; only-way-to-lose-is-not-to-play)
- **B-0703 / B-0954** — multi-oracle / relativistic bus (decentralized + autonomous substrate)
- **Ace** (`ace-package-manager` agenda) — the package-manager/app-store distribution layer
- **B-0988 / B-0989 / B-0991** — geospatial cluster (one concrete app-domain over the lanes)
- rules: `attention-as-currency-descriptive-not-proposal-fsharp-uom-memory-as-economy-bias-neutral-contribution-graph` (the contribution-graph + multiplicative components), `additive-not-zero-sum` (upgraded to superadditive here), `honor-those-that-came-before` (attribution), `proud-if-pattern-propagates`, `must-paired-with-can-exit-pattern` (anti-extractive), `only-way-to-lose-is-not-to-play` (self-selecting), `non-coercion-invariant` (attractive-not-coercive floor), `tonal-momentum-equals-meme-emergent-harmonic-coercion` (attractive pull / consensus-is-gravity)
