---
id: B-0865.17
zetaid: 081KSNY2Z0008QG0R0002BEZMR
priority: P1
status: open
...
| Real-world transfer | Indirect | Direct (benchmark IS the work, per B-0865) |

## Distribution lane architecture

| Component | TS-skill lane (this sub-row; broad path) | USB-cluster lane (B-0891; deep path) |
|---|---|---|
...
Each sub-row shippable independently; B-0865.17.1 (core TS skill substrate) is prerequisite for vendor-specific packaging sub-rows:

1. **B-0865.17.1** — Core TS skill substrate (framework + lifecycle DUs + DORA scoring + workflow engine + universal action grammar; vendor-agnostic; the SAME TS that runs on all targets)
2. **B-0865.17.2** — Claude skills packaging + skill-store submission
3. **B-0865.17.3** — OpenAI GPT store packaging + Custom GPT distribution
4. **B-0865.17.4** — Gemini extensions packaging
5. **B-0865.17.5** — Grok / Cursor / Continue / etc. additional vendor packaging
6. **B-0865.17.6** — Cross-vendor leaderboard substrate (aggregate DORA scores from skill-store user runs; substrate for the common-ground comparison)
7. **B-0865.17.7** — Skill-user UX design (non-technical user quickstart + example workflows + value proposition; distinct UX requirements from internal substrate)
8. **B-0865.17.8** — Skill-store documentation surface (usage docs; NOT architectural explanation)
9. **B-0865.17.9** — Per-vendor billing/pricing implications + revenue-sharing analysis (vendor skill stores often have economic dynamics distinct from open-source)
10. **B-0865.17.10** — Cross-vendor empirical validation (run the same benchmark on each vendor; verify scoring is genuinely comparable; surface any framework-substrate behavior differences across vendors)
...
- [B-0867.15](B-0867.15-per-host-adapters-github-gitlab-gitea-bitbucket-isomorphic-cross-host-substrate-aaron-2026-05-28.md) — per-host adapter substrate (cross-vendor portability composes with cross-host portability)
- [B-0867.20](B-0867.20-lifecycle-du-split-trajectory-push-vs-pr-review-determinereviewlevel-discriminator-kestrel-2026-05-28.md) — lifecycle DU split (state-machine-events vs system-modifications); preserves the auto-review pipeline distinction Kestrel called load-bearing
- [B-0867.21](B-0867.21-two-path-interface-discriminated-union-execute-vs-conversational-declare-intent-aaron-ani-2026-05-28.md) — two-path interface DU; conversational + execute paths both available in skill substrate
...
- `.claude/rules/verify-existing-substrate-before-authoring.md` — substrate-inventory verified B-0865 parent + B-0867 workflow engine substrate already in place; this sub-row adds the distribution-lane substrate as extension
...
- NOT a replacement for USB-cluster (B-0891); ADDITIVE per operator-explicit
...
- NOT an immediate-priority (P1 because operator-explicit decision + load-bearing for benchmark scoring; but B-0865.17.1 core substrate is prerequisite for vendor-specific packaging)
- NOT a synthetic-benchmark-only — composes with B-0865 parent; the DORA-scored choose-your-own-adventure substrate (per Aaron's verbatim) IS the benchmark scope; this sub-row adds the cross-vendor common-ground scoring substrate
...
- A composition point between B-0865 (benchmark substrate) + B-0867 (workflow engine) + B-0891 (USB-cluster deep path) + B-0904 (GitHub accelerator)
...
- `docs/backlog/`: B-0865 (parent benchmark substrate); B-0867 + B-0867.5/.15/.20/.21 (workflow engine substrate cluster); B-0891 (USB-cluster distribution lane); B-0904 (GitHub accelerator); NO existing sub-row covering the cross-vendor benchmark via skill-store distribution lane
...
Conclusion: this sub-row mints NEW substrate for the cross-vendor benchmark distribution lane. Composes with B-0865 parent + B-0867 workflow engine substrate. Authoring action: **mint-new as B-0865.17** (composition explicit; parent row preserved unchanged; sibling B-0867.* sub-rows preserved unchanged).
...
Operator's ADDITIVE clarification *"usb cluster is still very high priority to me"* established the default-to-both shape: USB-cluster (deep path; B-0891) + skill-store (broad path; this sub-row) both preserved.
...
This sub-row formalizes the substrate-engineering target. Implementation decomposed into 10 sub-rows (B-0865.17.1 through .10) per the decomposition section. Priority P1 because operator-explicit decision + load-bearing for the benchmark scoring substrate; B-0865.17.1 core substrate is prerequisite for vendor-specific packaging sub-rows.
