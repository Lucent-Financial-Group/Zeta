...
**Date:** 2026-04-23
**Status:** *Decision: adopt. Landed in per-user memory
(`~/.claude/projects/.../memory/CURRENT-aaron.md` +
`CURRENT-amara.md`).*
...
**Per-user memory folder**, not in-repo:

```
~/.claude/projects/<project-slug>/memory/
├── CURRENT-aaron.md        (human maintainer)
├── CURRENT-amara.md        (external AI maintainer)
├── CURRENT-<future>.md     (new maintainer as roster grows)
├── feedback_*.md           (raw memory log, unchanged)
├── project_*.md            (raw memory log, unchanged)
└── MEMORY.md               (index, unchanged)
```

**Not in the repo** because CURRENT files contain
maintainer-specific and potentially company-specific
context that would violate the open-source repo's
"stay generic" rule (see
...
- **Potential for divergence** — raw memory says X,
  ...
  eliminated.
- **Per-user only** — not tracked in git, not
  version-controlled, not peer-reviewable. The raw
  memory files remain the audit trail.

## Related

- `memory/feedback_current_memory_per_maintainer_distillation_pattern_prefer_progress_2026_04_23.md`
  — full memory establishing the rule in per-user scope
...
- `memory/feedback_mission_is_bootstrapped_and_now_mine_aaron_as_friend_not_director_2026_04_23.md`
  ...
  bootstrap
- `memory/feedback_open_source_repo_demos_stay_generic_not_company_specific_2026_04_23.md`
  — why CURRENT files live in per-user memory, not
  in-repo
