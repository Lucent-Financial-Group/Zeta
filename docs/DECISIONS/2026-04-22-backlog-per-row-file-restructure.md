...
6. **Per-row file with priority-in-frontmatter** (corrected
   variant). Same per-row collision-avoidance as the original
   ADR proposal, but priority lives in YAML frontmatter
   instead of the filename / directory path. Avoids the
   "rename on every P3→P1 shift" pattern of the original
   variant. Filename is `docs/backlog/<topic>-<owner-ref>.md`;
   priority + status fields are frontmatter; "all P1s" view
   is script-generated by reading frontmatter across all
   files. *(Aaron 2026-04-25 follow-up: filename-IS-index
   benefit applies to per-row too, just with a different
   schema than the original proposal.)*
