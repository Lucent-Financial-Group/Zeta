...
- **Reticulum-first** — global discovery is RNS destination hashes + hop-by-hop announce (`reticulum-transport.ts`), not a Tor `.onion` hidden service. DHT-like over Reticulum is the path; onion-shaped privacy is later. Fully decentralized identity graphs are discoverable today; frost is the later privacy budget. OK for now.
