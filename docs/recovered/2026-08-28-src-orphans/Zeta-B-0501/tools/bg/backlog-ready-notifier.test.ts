...
---`;
  ...
  describe("assignment history dedup / cooldown (slice 5)", () => {
    ...
    test("writeHistoryFile failure is caught; lastPublishError records it; poll does not throw (Copilot P1)", () => {
      const captured: FakeAssignmentCall[] = [];
      const base = fakeAdapters("2026-05-15T22:00:00Z", [ROW_OPEN_NO_DEPS], captured, "", "");
      const adapters: Adapters = {
        ...base,
        writeHistoryFile: () => { throw new Error("disk full"); },
      };
      const result = pollOnce(DEFAULT_CONFIG, adapters);

      // Assignment succeeded despite the write failure.
      expect(result.publishedEnvelopeIds).toHaveLength(1);
      // Write error surfaced in lastPublishError rather than thrown.
      expect(result.lastPublishError).toMatch(/disk full/);
    });
  });
  ...
  describe("parseArgs", () => {
    ...
    test("--agent vera without --target-agent derives targetAgent from fromAgent (Copilot P2)", () => {
      const config = parseArgs(["--agent", "vera"]);
      expect(config.fromAgent).toBe("vera");
      expect(config.targetAgent).toBe("vera");
    });
  });
});
