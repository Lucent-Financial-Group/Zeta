  ...
  if (!config.noPublish && toPublishRows.length > 0) {
    ...
    if (publishedPairs.length > 0) {
      ...
      try {
        ...
        // History write failure must NOT kill the poll loop. Captured in
        // lastPublishError so the caller can observe and retry next tick.
        if (lastPublishError === null) {
          lastPublishError = `history write failed: ${e instanceof Error ? e.message : String(e)}`;
        }
      }
    }
  }
...
export function parseArgs(argv: string[]): NotifierConfig {
  ...
  // Derive targetAgent from fromAgent when not set explicitly — prevents
  // silent queue-check bypass when --agent is changed but --target-agent is
  // omitted (Copilot review P2: avoid hard-coding otto as default target).
  if (!targetAgentExplicit) {
    config.targetAgent = config.fromAgent;
  }
