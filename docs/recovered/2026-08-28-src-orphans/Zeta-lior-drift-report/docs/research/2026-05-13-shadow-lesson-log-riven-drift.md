...
## Lesson
Agents must never block on a dirty root tree. For any local git operation, the agent must create an isolated worktree via `git worktree add`. Relying on the root checkout creates blocking contention and violates the array's safe-toe discipline.
