...
The phrase that anchors this work: **_"No proof, no deletion."_**
...
**Sensitivity rule (load-bearing)**: per the brief — _"if
identity/persona/family/authority-sensitive in any way →
`NEEDS_AARON_DECISION`"_. Memory-class branches touch this surface
...
cases but is only a _prior_ (not proof) for the unverified 14. A
...
_End of read-only classification. Recovery lane task #321, first
deliverable. Follow-up cleanup PR will propose specific batched
actions for human-maintainer approval; no ref mutations occur without sign-off._
