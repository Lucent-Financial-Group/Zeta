...
| SIMD merge (AVX2/NEON) | **Hold** | 1 | **Retired — the entry was false.** `SimdMerge.fs` never contained a vector instruction; `Vector<int64>.Count` was a loop chunk size and the cited fuzz test did not exist. Renamed to `ScalarMerge.fs`. A sorted merge is data-dependent and has no fixed-stride lane mapping, so this is Hold, not Assess. |
| Vectorised columnar scan (SoA) | Trial | 1 | `ColumnZSet.fs` — struct-of-arrays sibling of `ZSet`; branchless range predicates measured 2.1x (in-cache) to 10.8x (out-of-cache) over the scalar twin on arm64/NEON, gated at 1.5x by `ColumnZSet.Tests.fs`. Unpredicated `SumWeights` measured *slower* vectorised out of cache (0.67x) and is scalar by default — the win is branch elimination, not lane width. |
...
|---|---|---|---|
| Playwright (UI / acceptance automation) | Trial | 2026-09-11 | **The declared default for UI automation** — Aaron 2026-09-11: *"playwright is our goto tool for ui automation."* Pinned `playwright@1.62.1` in `package.json` devDependencies, with `install:browser-smoke` (`playwright install chromium`) as the browser fetch. In-tree users: `uat/playwright.config.ts` and `uat/shortener.spec.ts`, both landed 2026-09-10 in PR #17249 — the spec boots the app out-of-process on an ephemeral port and drives it over HTTP, refusing redirects so the redirect itself is what gets asserted. **Trial, not Adopt, and the gap is the reason:** the legend's Adopt is *"in-tree, we depend on it"*, and **no CI lane runs Playwright today** — measured 2026-09-11, zero files under `.github/workflows/` mention it, so the one spec runs only when a person runs it. A ring of Adopt would assert a dependency nothing exercises, which is the vacuity class the radar exists to catch. **Graduates to Adopt when a workflow lane invokes it on a schedule or a PR trigger** and that lane has been green for a round — a dependency is a thing that can break the build, and until a lane runs it, it cannot. Guarded by `src/Core.TypeScript/hygiene/audit-playwright-specs-excluded-from-bun.ts`: `bun test` collects `*.spec.ts` as readily as `*.test.ts`, so a Playwright spec must be excluded from **both** bun configs or the wrong runner picks it up (which is exactly what happened to `uat/shortener.spec.ts` before that guard existed). |
| `typescript` compiler API (verification fixture) | Trial | 2026-09-06 | Existing `package.json` devDependency pinned at `6.0.3`; directly used by `src/Core.TypeScript/formal-verification/nci-witness-capability.test.ts` for parsed dynamic-import admission and TypeScript type erasure before executing the actual emitter. The focused Bun 1.3.13 suite passes 18 tests and 37 assertions; the file participates in the whole TypeScript hermetic CI tier. This is bounded fixture infrastructure, not an independent type-checking oracle or a JavaScript security boundary. Evidence and limits: `docs/research/2026-09-06-nci-emitter-capability-correction.md`. |
| FsCheck 3 property tests (**.NET only**) | Adopt | 1 | In CI. Measured 2026-08-23: 156 files under `tests/` reference FsCheck, and `dotnet test Zeta.sln -c Release` runs them in `gate.yml`'s blocking `build-and-test` job. **The scope word is load-bearing** — this row is .NET, and it was for a long time the radar's only property-testing row, so it silently read as covering the TypeScript side too. It does not; see the `fast-check` row below, which is a different tool at a different ring. |
| fast-check (**TypeScript** property tests) | Trial | — | **Added 2026-08-23 — it was in use with no ring at all, which is the radar failing at its one job.** Pinned `4.8.0` in `package.json` (devDependency) and imported by 5 tracked test files: `src/Core.TypeScript/cover-acyclicity/gyo.test.ts`, `src/Core.TypeScript/peer-call/_firewall.test.ts`, `src/Core.TypeScript/discovery/udp-lossy-transport.test.ts`, `src/Core.TypeScript/ferry-throttler/priority-ferry-throttler.property.test.ts`, `src/Core.TypeScript/observe/schema-aware-join.test.ts`. **Trial, not Adopt, and the reason is measured**: no workflow runs those five files. Every `bun test` invocation across `.github/workflows/` is path-targeted (`hygiene/`, `ace/`, `inventory/`, `cluster/`, named files), none of the five directories appears in any of them, and there is no bare `bun test`. So the properties exist, are runnable locally, and are **ungated** — the same "live code path, no CI wiring yet" condition that holds the Alloy row at Trial. Graduates to Adopt when a lane executes them: workitem `081M0Q9Y69F087G0R000FBK4JA`. Anchor: Claessen & Hughes, *QuickCheck* (ICFP 2000) — fast-check is the TS descendant, FsCheck the .NET one. |
| TLA+ / TLC model checker | Adopt | 12 | **Ring is Adopt; the lane is dark — both facts, recorded together on purpose.** *Measured 2026-08-23:* the last `tlaps-proof` **success was 2026-07-01** (seven weeks). Over the last 100 runs: **16 success / 22 failure / 62 cancelled**; over the last 12, **zero successes**. The two failure modes are distinct, and separating them meant reading step conclusions rather than run conclusions: (a) the `cancelled` runs are the **60-minute job timeout** (`tlaps-proof.yml` `timeout-minutes: 60`) firing inside **`Install toolchain`** — the opam source-build of `tlapm` — so `Prove all TLAPS obligations` is `skipped` and **never ran**; (b) the `failure` runs die in the prove step with **`FAIL: NciSafetyProofs (exit 127)`** / `NciNonUrgencyProofs (exit 127)` — exit 127 is *command not found*, i.e. `tlapm` was not invocable, not an unproved obligation. **So the honest read is Hold-in-practice** (Soraya 2026-08-22) and nothing has been proved unbounded since 2026-07-01. **The ring is deliberately NOT downgraded**: a ring is a claim about what we would start new work with, and TLA+/TLC still is; a silent downgrade would replace a visible discrepancy with an invisible one. **What restores it** (Soraya's prerequisites, none yet done): (1) decide whether the 60-minute cap is hit by *a model that grew* or *a runner/build that slowed* — opposite fixes, shrink the scope vs. raise the cap; the step-level evidence above already points at the **build**, since the prove step is skipped, but that is a pointer, not the measurement; (2) **one green run on an unchanged spec**, proving the lane can report at all; (3) only then is TLA+ routable again. Tracked: workitem `081M0Q9Y62A087G0R003FZ6TM7`. **The old note on this row ("6 specs validated; 7 more with `.cfg` pending") is retired as unsupportable** — `src/Core.TLA/specs/` now holds 36 `.tla` files and 53 `.cfg` files, and no artifact in the repo records which 6 the number meant. |
| Z3 SMT | Adopt | 1 | Pointwise axioms proved |
| `quantum-circuit` (JS state-vector simulator) | Trial | — | **Added 2026-08-23 by the same audit that found `fast-check` unringed.** Pinned `0.9.247` in `package.json` (latest `0.9.250`). Used as a **second observable oracle** against the Q# reference: `src/Core.QSharp.ReferenceOracle/quantum-circuit.test.ts` ("quantum-circuit simulator (second observable oracle)") + `src/Core.QSharp.ReferenceOracle/generate-treaty-transcript.ts`, with a hand-written `src/Core.QSharp.ReferenceOracle/quantum-circuit.d.ts`. Trial: a live cross-check path which, like `fast-check`, no workflow currently runs. |
| `semver` (npm) as a differential oracle | Trial | — | **Added 2026-08-23 by the same audit.** Pinned `7.8.1` (latest `7.8.5`). Two roles: a working dependency of `src/Core.TypeScript/hygiene/audit-chart-target-revisions.ts`, `src/Core.TypeScript/hygiene/report-chart-currency.ts` and `src/Core.TypeScript/ace/helm-change-detector.ts`, **and** the reference implementation our own `src/Core.TypeScript/ace/semver.ts` is differentially tested against in `src/Core.TypeScript/ace/semver.test.ts` — that second role is what earns a radar row rather than a lockfile line. That test *does* run in CI (`bun test src/Core.TypeScript/ace/` in `gate.yml`). |
| Microsoft Z3 | Adopt | — | `Directory.Packages.props` pins `Microsoft.Z3 4.12.2` — which **is** the newest version on NuGet (checked 2026-08-23 against `api.nuget.org`; the Z3 team's NuGet publishing lags their GitHub releases, so "stale-looking" here is upstream, not us). The **TypeScript** binding is a separate pin: `z3-solver 4.16.0` in `package.json` (latest `5.2.0`), loaded via `require('z3-solver/build/node.js')` in a spawned Node process by `src/Core.TypeScript/ace/solver.z3.test.ts`. Two Z3s at two versions across two oracles is a real skew, named here so it is visible rather than discovered. |
| BenchmarkDotNet | Adopt | — | `bench/` |
| Semgrep | Trial | 12 | **Note corrected 2026-08-23 — the old text understated it in both halves.** Not 12 rules: `.semgrep.yml` carries **15**, plus **2** in `.semgrep-floor.yml` (and per-scaffold rulesets under `src/Core.TypeScript/scaffold/`). Not "runs externally": semgrep runs **inside `gate.yml`** — a blocking floor job over `.semgrep-floor.yml` and a `lint (semgrep drift)` job over `.semgrep.yml` — pinned declaratively as `pipx:semgrep = "1.161.0"` in `.mise.toml`. Ring left at Trial pending a deliberate promotion review; the note is now what the repo can support. |
| CodeQL | Trial | 34 | `.github/workflows/codeql.yml` landed (GitHub-default); tuning drift tracked in codeql-expert skill |
| Stryker.NET | Trial | 10 | Mutation-testing config shipped (`stryker-config.json`) **and a dedicated `.github/workflows/stryker-mutation.yml` lane exists** (measured 2026-08-23; the old note stopped at "config shipped"). Ring stays Trial on its own stated limit, which is real and unchanged: Stryker.NET throws `NotSupportedException: Language not supported: Fsharp`, so the lane targets `src/Core.CSharp/` only — the F# core, which is most of the substrate, is **not** mutation-tested by it. |
| Alloy | Trial | 10 | 6 `.als` specs under `src/Core.Alloy/` (5 in `src/Core.Alloy/specs/` + `src/Core.Alloy/schema_evolution.als`) + `tests/Tests.FSharp/Formal/Alloy.Runner.Tests.fs`. The jar is version 6.2.0 (the latest upstream release), fetched and digest-pinned by the row in `tools/setup/manifests/from-url` — it is **no longer a repo path**, so it is cited here as its pin, and the old src/Core.Alloy/alloy.jar path is un-backticked below by the same retired-citation convention this row already used once (081M23AST90087G0R00150MK76). **Path corrected 2026-08-23**: the jar was cited as tools/alloy/alloy.jar (deliberately un-backticked — a retired citation, not a live claim). **Stated reason corrected 2026-08-23 too**: "no CI wiring yet" was wrong — `Alloy.Runner.Tests.fs` is an xUnit suite inside `Zeta.sln`, so `dotnet test` in `gate.yml` *does* execute it. **The silent-skip half was CLOSED 2026-09-09**: the sentence that used to sit here — "on a runner without the jar the suite passes by not checking" — was the vacuity class, and `the Alloy gate leg actually carries the gate on CI` now makes a missing toolchain a failure on the Linux-x64 leg that carries it. Trial is still the honest ring, now for the narrower reason that the gate rests on one leg. |
| Lean 4 + Mathlib | Adopt | 10 | **Counts and pin re-measured 2026-08-23.** 32 `.lean` files under `src/Core.Lean4/Lean4/` and **57** under `src/Core.Lean4/` overall (the old "26" predates `Zeta23/`, `Gen/`, `Privacy/`, `Safety/`, `ImaginaryStack/`), gated by `.github/workflows/lean-proof.yml`. **The pin is a release candidate**: `src/Core.Lean4/lean-toolchain` is `leanprover/lean4:v4.30.0-rc1` and `src/Core.Lean4/lake-manifest.json` pins Mathlib `rev 0c154d67103f74be3a0f2c509f72ccbf5be9f2a7`, which is *exactly* the Mathlib tag `v4.30.0-rc1` (verified against the upstream tag list, not assumed). Lean **v4.30.0 final shipped 2026-05-26** and current stable is **v4.33.1** (2026-08-21) — so we sit on an RC of a line whose final has been out ~3 months, three minors behind. Not urgent: the `Zeta23/LinAlg` port taken from upstream `v4.33.0-rc2` / Mathlib `51e6992efd06` built at our pin with zero proof edits. But a durable RC pin is drift. Costed in `docs/research/2026-08-23-toolchain-currency-audit-and-tech-radar-ring-drift.md` §2.1; tracked as workitem `081M0Q9Y63V087G0R0007RX4E8`. **Not bumped here on purpose**: a toolchain move needs its own change carrying a full `lake build` + axiom audit as evidence. |
...
| bun + TypeScript (post-setup scripting default) | Trial → **contradicted by this document's own center** | 43 | Round-43 adoption for post-`dotnet`-setup scripting. Replaces bash-per-script drift (the round-43 `tally.sh` pivot exposed the cost). First in-tree artefact: `src/Core.TypeScript/invariant-substrates/tally.ts` (bun Glob + `node:fs/promises`, no third-party runtime deps). Scaffold: `package.json` (pinned `bun@1.3.13`, bun-runtime only — `"type": "module"`, no `tsx`/`ts-node`), `tsconfig.json` cranked-to-11 (strict + `noUncheckedIndexedAccess` + `exactOptionalPropertyTypes` + `verbatimModuleSyntax` + `erasableSyntaxOnly` + `isolatedModules` + `noImplicitOverride`), `eslint.config.ts` strict-type-checked + stylistic-type-checked + sonarjs + `reportUnusedDisableDirectives`, `prettier` + `prettier-plugin-toml`, `markdownlint-cli2`. Excludes hardened across `tsconfig.json` / `bunfig.toml` / `eslint.config.ts` / `.prettierignore` to skip `references/prior-art/**` (~13 GB), `tools/lean4/.lake/**` (~7 GB), `tools/alloy/**`, `tools/tla/**`, `**/BenchmarkDotNet.Artifacts/**`, `**/TestResults/**`, `**/artifacts/**`, `**/bin/**`, `**/obj/**` — SQLSharp lesson: missing the upstream allowlist destroyed language-server perf until the `defaultRepoPathIgnorePatterns` helper shape was adopted. Latest-version audit done at ADR time (`docs/DECISIONS/2026-04-20-tools-scripting-language.md` §Latest-version audit). **Watchlist** (rails for the assumptions): (a) bun on Windows-from-source still maturing — Zeta maintainers are macOS/Linux today, re-audit if Windows becomes a CI target; (b) `erasableSyntaxOnly` presumes bun runs `.ts` without JS emit — re-audit if bun ever ships a JS-compilation pass; (c) `@types/bun 1.3.12` lags the `bun@1.3.13` runtime by one patch (DefinitelyTyped lag, not a bug), exception tracked inline in ADR. Graduates to Adopt once (1) a second in-tree `.ts` script lands, (2) no watchlist triggers fire for 5 rounds, (3) tally.ts + at least one other tool are invoked on round-close. **Re-measured 2026-08-23 — every graduation condition is long past, and the row is now the radar's loudest internal contradiction.** Condition (1) asked for "a second in-tree `.ts` script": the tree holds **1,187 tracked `*.test.ts` files alone**, and `src/Core.TypeScript/hygiene/` alone holds 266 files. The named first artefact moved too — it is `src/Core.TypeScript/invariant-substrates/tally.ts`, **not** tools/invariant-substrates/tally.ts as the row said until today (un-backticked here for the same reason as the Alloy jar: a retired citation is not a live claim). And this same document's *center* section names **TypeScript** as one of the three irreducible essential primitives. A ring of Trial on a center primitive cannot both be true; the ring is left unchanged here only because promoting the radar's own center is a bigger call than a drift-fix PR should make unilaterally. Tracked as workitem `081M0Q9Y6AP087G0R002BV88T7`. |
...
|---|---|---|---|
| Feldera (Rust DBSP) | Assess | 11 | Cloned `references/prior-art/feldera/`; apples-to-apples bench P1 |
