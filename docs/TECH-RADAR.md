...
| Alloy | Trial | 10 | 6 `.als` specs under `src/Core.Alloy/` (5 in `src/Core.Alloy/specs/` + `src/Core.Alloy/schema_evolution.als`) + `src/Core.Alloy/alloy.jar` + `tests/Tests.FSharp/Formal/Alloy.Runner.Tests.fs`. **Path corrected 2026-08-23**: the jar was cited as tools/alloy/alloy.jar (deliberately un-backticked — a retired citation, not a live claim), which does not exist; it is at `src/Core.Alloy/alloy.jar`. **Stated reason corrected too**: "no CI wiring yet" is no longer the right sentence — `Alloy.Runner.Tests.fs` is an xUnit suite inside `Zeta.sln`, so `dotnet test` in `gate.yml` *does* execute it. What it does **not** do is fail when Alloy is absent: the runner returns early on `not (File.Exists alloyJarPath)`, so on a runner without the jar the suite passes by not checking. Trial is still the honest ring — for the *conditional-skip* reason, not the *no-wiring* one. |
