...
| Alloy | Trial | 10 | 6 `.als` specs under `src/Core.Alloy/` (5 in `src/Core.Alloy/specs/` + `src/Core.Alloy/schema_evolution.als`) + `tests/Tests.FSharp/Formal/Alloy.Runner.Tests.fs`. The jar is version 6.2.0 (the latest upstream release), fetched and digest-pinned by the row in `tools/setup/manifests/from-url` — it is **no longer a repo path**, which is why it is cited here as its pin rather than as `src/Core.Alloy/alloy.jar` (081M23AST90087G0R00150MK76). **Path corrected 2026-08-23**: the jar was cited as tools/alloy/alloy.jar (deliberately un-backticked — a retired citation, not a live claim). **Stated reason corrected 2026-08-23 too**: "no CI wiring yet" was wrong — `Alloy.Runner.Tests.fs` is an xUnit suite inside `Zeta.sln`, so `dotnet test` in `gate.yml` *does* execute it. **The silent-skip half was CLOSED 2026-09-09**: the sentence that used to sit here — "on a runner without the jar the suite passes by not checking" — was the vacuity class, and `the Alloy gate leg actually carries the gate on CI` now makes a missing toolchain a failure on the Linux-x64 leg that carries it. Trial is still the honest ring, now for the narrower reason that the gate rests on one leg. |
