...
3. **External actors** — `zeta-external://github/<login>` actors don't have keys until they're promoted from E2 to E3+ (claim approved). At E0/E1 (review-only / patch-only), the GitHub identity itself is the binding (GitHub already authenticates the commit author / PR author).
