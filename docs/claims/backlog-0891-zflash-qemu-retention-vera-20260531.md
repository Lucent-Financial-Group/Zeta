...
- progress: inspected #6220's aggregate CodeQL failure and confirmed it was `js/insecure-temporary-file` from the runtime unit test's predictable `/tmp` inline path, not the `qemu-img`/`mcopy` argv execution path.
- progress: `zflash-file-backed-runtime.ts` now exposes `createNodeFileBackedZflashInlineStagingDirectory`, which allocates future inline staging roots with `mkdtempSync`, and the runtime test no longer routes a fixed `/tmp` path into `writeFileSync`.
- progress: after #6220 returned CLEAN, added `zflash-file-backed.ts`, a non-destructive CLI wrapper that plans and executes the file-backed `qemu-img`/`mcopy` flow and prints `ZFLASH_QEMU_RETENTION_BOOT_IMAGE=<raw image>` for the QEMU retention harness.
- proof: `bun test full-ai-cluster/tools/zflash-file-backed.test.ts full-ai-cluster/tools/zflash-file-backed-runtime.test.ts full-ai-cluster/tools/zflash-lib.test.ts`, `bun --bun tsc --noEmit -p tsconfig.json`, `git diff --check`, and `bun full-ai-cluster/tools/zflash-file-backed.ts --help` pass after the file-backed CLI patch.
- next: run `zflash-file-backed.ts` against an actual ISO, pubkey, and retained credential blob, then feed its printed `ZFLASH_QEMU_RETENTION_BOOT_IMAGE` to `ZFLASH_QEMU_RETENTION_EXECUTE=1`.
