...
| `cross-verify (scanner-cancels-itself)`                 | Evidence workflows must not cancel their own default-branch runs                   |
...
| `cross-verify (bun-pin-parity)`                         | bun pin declared once (mise.lock canonical, every workflow restates)               |
