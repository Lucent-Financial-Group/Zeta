...
| TLA+ + TLC | Concurrency + state-machine safety | committed to git; NOT fetched — the build the gate runs is no longer obtainable upstream (`docs/INSTALLED.md`) | byte-pinned by the diff; banner + sha256 in `registry/tlc-models.json` | [lamport.azurewebsites.net/tla/tla.html](https://lamport.azurewebsites.net/tla/tla.html) | `tla-expert` | Adopt | Specs under `src/Core.TLA/specs/`; 53 pinned model runs in `registry/tlc-models.json` |
| Alloy 6 | Lightweight formal specs | `tools/setup/install.sh` fetches the jar from the digest-pinned row in `tools/setup/manifests/from-url` | sha256 pin on upstream release v6.2.0 | [alloytools.org](https://alloytools.org/) | `alloy-expert` | Adopt | Specs under `src/Core.Alloy/specs/` |
