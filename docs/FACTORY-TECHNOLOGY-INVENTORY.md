...
cross-platform parity status (FACTORY-HYGIENE row #48).
...
**First-pass: 2026-04-23** — bounded subset (~12 rows). The
full footprint is larger; additional rows land on future
cadenced-audit fires or on-touch when a new tech is added.
...
|---|---|---|---|---|---|---|---|
| .NET 10 (F# + C#) | Primary language runtime for `src/Core`, tests, benchmarks | `tools/setup/install.sh` — `dotnet-install.sh` | `global.json` (in-repo pin) | [dotnet.microsoft.com](https://dotnet.microsoft.com/) | `fsharp-expert`, `csharp-expert` | Adopt | F# is the reference implementation per `CURRENT-aaron.md` §5 |
| Rust | Future-Zeta target (not in-tree today) | TBD | TBD | [rust-lang.org](https://www.rust-lang.org/) | none yet | Assess | Anticipated per `CURRENT-aaron.md` §5 |
| bun + TypeScript | Post-setup scripting default (per row #49) | `tools/setup/install.sh` pulls bun | `bun.lock` + TS tooling | [bun.sh](https://bun.sh) | `typescript-expert` | Adopt | Post-setup default per `docs/POST-SETUP-SCRIPT-STACK.md` |
...
| Technology | Role | Install path | Version pin | Auth doc | Expert skill | TECH-RADAR ring | Notes |
|---|---|---|---|---|---|---|---|
| Postgres | Sample app backend (FactoryDemo) | `samples/FactoryDemo.Db/docker-compose.yml` | image pin in compose | [postgresql.org](https://www.postgresql.org/docs/) | `postgresql-expert`, `relational-database-expert` | Adopt | ServiceTitan-shaped demo substrate |
...
| Codex CLI | Secondary agent harness (OpenAI) | Independent install | N/A | [github.com/openai/codex](https://github.com/openai/codex) | referenced in `docs/HARNESS-SURFACES.md` | Trial | Mapped in `docs/research/codex-cli-capability-map.md` |
...
| Playwright | Browser automation for web UI integration (OpenAI, email signup research) | bun + `@playwright/test` | `package.json` + `bun.lock` | [playwright.dev](https://playwright.dev/) | none yet (candidate skill) | Adopt | Constrained by courier protocol + two-layer authorization model; scraping/export only, not primary review signal |
...
| Z3 | SMT solver for pointwise axioms | `tools/setup/install.sh` downloads JARs | pinned in setup | [github.com/Z3Prover/z3](https://github.com/Z3Prover/z3) | `z3-expert` | Adopt | `tools/Z3Verify/` |
...
| Stryker.NET | Mutation testing | NuGet tool | `.config/dotnet-tools.json` | [stryker-mutator.io](https://stryker-mutator.io/docs/stryker-net/introduction/) | `stryker-expert` | Adopt | CI job runs on sub-scope |
...
| Technology | Role | Install path | Version pin | Auth doc | Expert skill | TECH-RADAR ring | Notes |
|---|---|---|---|---|---|---|---|
| GitHub Actions | CI/CD orchestration | `.github/workflows/*.yml` | workflow-level SHA pins per row #43 | [docs.github.com/actions](https://docs.github.com/en/actions) | `github-actions-expert` | Adopt | Gate workflow, CodeQL, auto-merge |
...
5. **Quantum-resistant crypto column** — Aaron 2026-04-23: *"any crypto graphy we decide to use should be quantium resisten, even one place we don't use it could be a place for attack, we really don't have much any encryption yet so this is just a note for the future when we do"*. The factory currently has minimal crypto in-tree; when cryptographic primitives land, every row that uses them MUST be PQC (per NIST FIPS 203/204/205/206 — Kyber / Dilithium / Falcon / SPHINCS+ — already captured in the lattice-based-crypto per-user memory 2026-04-19). A "PQC-clean?" column should be added when crypto becomes a material part of the factory substrate, and classical-crypto adoption requires an explicit exception + ADR. See the per-user feedback memory `feedback_all_cryptography_quantum_resistant_even_one_gap_is_attack_vector_2026_04_23.md` for the mandate's scope.
...
- `docs/FACTORY-HYGIENE.md` row #48 (cross-platform parity), row #49 (post-setup stack), row #54 (backlog-refactor cadenced audit — this doc itself may surface rows for that audit)
- `memory/MEMORY-AUTHOR-TEMPLATE.md` — absorb-time lint hygiene (used for authoring this and similar docs)
- Per-user memory: `project_factory_technology_inventory_first_class_support_openai_playwright_hard_2026_04_23.md` (captures OpenAI web UI caveats)
