...
3. Unsealer extraContainer (close to Google, rewritten): `valuesObject` only; fetch-at-unseal from Lucent; threshold-many keys; cannot init; amend `full-ai-cluster/k8s/applications/openbao/TOPOLOGY.md` §5 in the **same commit as the sidecar**. Not ESO-into-etcd for shares. Not HA joiners until three-node.
