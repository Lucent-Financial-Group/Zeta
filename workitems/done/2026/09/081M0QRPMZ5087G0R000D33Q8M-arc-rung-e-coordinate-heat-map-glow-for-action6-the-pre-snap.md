...
**Register: `built` (toy environment).** Design: `docs/design/2026-08-23-arc-agi-3-integration-design-chip8-chip9-atari-and-the-arena.md` §8.
...
- The seed precondition had already moved by implementation time:
  `BnnSocietyPredictor` defaults to `COMMON_SEED` on current main.
- `ZetaClickTarget` is a source-owned ACTION6 environment with three visible
  candidates and one real level-completing click.
- `ClickPolicy.forecast` exposes normalized sparse probability mass without
  spending its selected point. The deterministic lattice fallback is reported
  as a point mass rather than pretending to have wider uncertainty.
- The recorded contract carries the forecast on the pre-action frame. The
  browser refuses it unless coordinates are unique and bounded, mass sums to
  one, the selection has maximal mass, and the following action commits that
  exact ACTION6 coordinate.
- The arena renders probability alpha in red, outlines the selected pre-action
  coordinate, and renders the committed coordinate in cyan on the following
  frame.

## Verification

- ARC Python suite: 145 passed.
- Browser replay contract: 9 passed.
- App typecheck, ESLint, and production Vite build: clean.
- Playwright at 1440x1000 and 390x844: zero page errors, horizontal overflow,
  or external requests; canvas bytes contain the expected red probability and
  cyan commit values.
- `bun run preflight`: all 18 checks passed, including Release build and full
  .NET tests.
