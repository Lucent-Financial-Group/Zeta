...
completed: 2026-08-09T18:43:27.734Z
depends_on: []
composes_with: []
---
...
`TS2307 Cannot find module 'playwright'` in
`src/Core.TypeScript/browser-node/browser-pwa-smoke.ts`, while CI's
`lint (TS)` on the _same commit_ was **green**. `playwright` is correctly declared in
...
2. **Add a lint-preflight check** — have `lint-typescript.ts` detect missing `node_modules` /
   ...
   read like real type errors. Smallest blast radius; fixes the _misleading_ half directly.
3. **Both** — (2) is the honest-error guard, (1) is the parity fix.

Recommendation: at minimum (2), because the failure mode is _misdiagnosis_, not inconvenience —
