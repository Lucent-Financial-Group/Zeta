...
The objection to random draws is that an LLM asked *"does tradition X connect to Zeta?"* will
...
> *"this is the single request/response failure — iterated density connections over time is how
> you find the weak connections over the dense ones in an infinite iterated game"*
...
- `src/Core.TypeScript/tradition-density/msc2020-corpus.ts` — the complete 63-class MSC2020
  ...
  names because those are partitions of *activity*, and a uniform draw over them is weighted by
...
- `density.ts` — the fold. **Depth = distinct drawn traditions that named a target.** The
  ...
  tau, and attaches **no threshold and no verdict** (same reasoning as `chip9/consult-census.ts`).
