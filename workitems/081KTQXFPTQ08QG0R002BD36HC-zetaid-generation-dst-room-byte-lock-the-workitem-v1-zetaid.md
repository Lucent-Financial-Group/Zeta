...
- **No hand-faked hex.** Each oracle must *compute* the id; faking outputs defeats the byte-lock. The
...
- **MUMPS as an oracle** is new work (a MUMPS ZetaId routine byte-locked to the codec) — design with the
  MUMPS/globals owner.
- **Governed:** the WorkItem ZetaId stays minted by the common generator; this room *verifies* it across
