...
`agencysignature-block.test.ts` §MAINTENANCE COMMITS.
...
- `src/Core.TypeScript/hygiene/agencysignature-block.ts` — `findAllSignatureBlocks` · `detectBlockDisagreement` · `GOVERNANCE_KEYS` (the five keys a disagreement is loud on)
