...
2. **Every `memory/<role>/<persona>/NOTEBOOK.md`** — are they human-readable?
...
2. **Spawn a dedicated agent in a fresh worktree** with no
   access to `.claude/skills/` or `memory/<role>/<persona>/`.
...
- Skill notebooks (`memory/<role>/<persona>/NOTEBOOK.md`) are per-agent; a
...
- `memory/<role>/<persona>/` — agent notebooks he audits
