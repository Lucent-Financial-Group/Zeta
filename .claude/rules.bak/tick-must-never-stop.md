...
The session-exit mechanism is the more common cause of "sentinel missing" at fresh-session cold-boot, because Claude Code sessions cycle on a much shorter cadence (minutes to hours) than the 3-day auto-expire window. Per `CronCreate`'s own documentation: _"Jobs live only in this Claude session — nothing is written to disk, and the job is gone when Claude exits."_ The session-exit non-persistence is by design at the harness level, NOT a bug in the cron mechanism.
