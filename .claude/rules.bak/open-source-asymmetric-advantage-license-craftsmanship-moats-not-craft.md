...
> externally-checkable line is **moats vs craftsmanship**: open-source the _moats_
> (advantage from a position others can't fairly reach — privileged information,
> structural lock-in), license the _craftsmanship_ (advantage from having done the
...
let you win _unfairly_, and you only keep (for licensing) what competes _fairly_.
...
| Would let you win by others being _unable_ to compete                       | Lets you win in a game others can still play |
...
Public verification is _stronger_ verification (more eyes, independent checks — the
...
- **Contractual / agreement-bound** content (e.g. a customer's data, a partner's
  sector-specific code under agreement) stays private because you're _honoring an
  obligation_, not holding a moat.
- The **responsible-disclosure private window** (per
  ...
  is private for _safety_ during the disclosure pipeline, not for advantage.
...
- [`responsible-disclosure-private-window-prompt-to-vendor-never-hoard-kid-floor-escalation.md`](responsible-disclosure-private-window-prompt-to-vendor-never-hoard-kid-floor-escalation.md) — the disclosure window is a _safety_ exception, not a moat
...
should be _open the moat_ rather than _hold it_. Future-Otto cold-booting needs the
