...
const readlinkSafe = (p: string) => { try { return readlinkSync(p); } catch { return ""; } };
...
for (const root of [...VIEWS, ...CANON]) for (const e of walk(join(VOCAB, root)))
  if (e.link && !existsSync(realpathSafe(e.path))) v(`dangling symlink: docs/${relative(VOCAB, e.path)}`);
...
for (const home of CANON) for (const e of walk(join(VOCAB, home))) {
  if (e.link) continue;
  const term = basename(e.path, ".md");
  homeRealTerms.add(term);
  const arr = canonTerms.get(term) ?? []; arr.push(relative(VOCAB, e.path)); canonTerms.set(term, arr);
}
...
for (const view of ["grams", "travelers"]) for (const e of walk(join(VOCAB, view))) {
  if (!e.link) continue;
  const immediate = realpathSafe(join(dirname(e.path), readlinkSafe(e.path)));
  if (!existsSync(immediate)) { v(`DAG: symlink resolves nowhere: ${relative(VOCAB, e.path)}`); continue; }
  if (lstatSafe(immediate)?.isSymbolicLink()) v(`DAG: symlink points to another symlink (chain, not an edge into canonical): ${relative(VOCAB, e.path)}`);
}
