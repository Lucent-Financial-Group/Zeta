...
const EXPECTED_IDS = (() => {
  ...
  for (let i = 1; i <= 211; i++) if (i !== 8) s.add(i); // 1..211 skip 8 == 210 ids
  return s;
})();
...
function validate(items: Item[]): void {
  ...
  if (items.length !== EXPECTED_COUNT) die(`expected ${EXPECTED_COUNT} items, got ${items.length}`);
  ...
  if (missing.length) die(`missing ids vs {1..211}\\{8}: ${JSON.stringify(missing)}`);
  if (extra.length) die(`unexpected ids not in {1..211}\\{8}: ${JSON.stringify(extra)}`);
  console.log(`PRE-CHECK ok: ${items.length} items; ids === {1..211}\\{8}; statuses valid; qty ints.`);
}

async function getAccessToken(url: string, anon: string, email: string, password: string): Promise<string> {
...
async function rpcRole(url: string, anon: string, access: string): Promise<string | null> {
...
async function fetchByIds(url: string, anon: string, access: string, ids: number[]): Promise<Record<number, Item>> {
...
async function main() {
  ...
  if (!file) die("usage: bun seed-import.ts <seed.json> [--dry-run]");
  if (!file.includes("_seed_tmp/")) die(`seed file must be under _seed_tmp/ (git-ignored). Got: ${file}`);

  const items = JSON.parse(await Bun.file(file).text()) as Item[];
  ...
  if (post !== EXPECTED_COUNT) die(`post-import count ${post} != ${EXPECTED_COUNT}.`);

  const spotIds = [1, 2, 10, 14, 18, 211].filter((id) => items.some((it) => it.id === id));
  const dbRows = await fetchByIds(url, anon, access, spotIds);
  const fields: (keyof Item)[] = ["name", "brand", "model_pn", "qty", "device_type", "category", "status", "notes"];
  let mismatches = 0;
  ...
  if (mismatches) die(`${mismatches} field mismatch(es) in spot-check.`);
