...
type Item = {
  ...
  status: string | null;
  notes: string | null;
  // location / assignment_purpose / value / serial: not in sheet -> omitted (null in DB)
  // is_archived / custom_fields: DB defaults (false / {})
};
...
const colLetters = (ref: string) => ref.replace(/[0-9]+$/, "");
...
function buildNotes(notesRaw: string | null, sourcesRaw: string | null): string | null {
  ...
  if (sourcesBlock) return sourcesBlock; // empty-notes edge: begins with "SOURCES:\n"
  return null;
}
