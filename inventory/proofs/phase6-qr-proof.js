...
async function independentDecode(rgba, dim) {
...
async function main() {
    ...
    if (!Number.isInteger(id) || id <= 0)
        die(`bad item id: ${String(process.argv[2])}`);
    ...
    if (decoded !== url)
        die(`decoded payload != expected\n  got : ${decoded}\n  want: ${url}`);
    const routedId = IE.parseItemId(decoded);
    if (routedId !== id)
        die(`parseItemId(decoded) = ${String(routedId)}, expected ${String(id)}`);
