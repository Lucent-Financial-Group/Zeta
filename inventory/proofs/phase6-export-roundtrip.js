...
function show(label, s) { console.log(`\n--- ${label} ---\n${s}`); }
// Mirrors seed-import.ts STEP-3: field-by-field spot-check, count, abort+report.
function stagingCheck(kind, reimported, source) {
    console.log(`\n[${kind}] STAGING CHECK`);
    if (reimported.length !== source.length)
        die(`[${kind}] count ${reimported.length} != ${source.length}`);
    ...
    for (const id of TRICKY_IDS) {
        ...
        const got = byId[id];
        if (!got) {
            console.error(`  [MISS] id ${id} absent after re-import`);
            mismatches++;
            continue;
        }
        let rowOk = true;
        for (const f of fields) {
            ...
            if (a !== b) {
                mismatches++;
                rowOk = false;
                console.error(`  [DIFF] id ${id}.${f}: src=${JSON.stringify(a)} got=${JSON.stringify(b)}`);
            }
        }
        console.log(`  SPOT id ${id}: ${rowOk ? "VALUE-IDENTICAL across all fields" : "see diffs above"}`);
    }
    if (mismatches)
        die(`[${kind}] ${mismatches} field mismatch(es) — round trip NOT identical.`);
...
function main() {
    ...
    if (!jsonBack.ok)
        die(`JSON re-import failed: ${jsonBack.error}`);
