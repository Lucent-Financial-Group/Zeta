...
const cspMeta = html.match(/<meta\s+http-equiv=["']Content-Security-Policy["']\s+content=["']([\s\S]*?)["']\s*\/?>/i);
...
for (const part of csp.split(";")) {
    ...
    const name = toks[0];
    if (name)
        directives.set(name.toLowerCase(), toks.slice(1));
}
const scriptSrc = directives.get("script-src") ?? [];
const styleSrc = directives.get("style-src") ?? [];
check("CSP script-src is exactly 'self'", scriptSrc.length === 1 && scriptSrc[0] === "'self'", `script-src = [${scriptSrc.join(" ")}]`);
check("CSP style-src is exactly 'self'", styleSrc.length === 1 && styleSrc[0] === "'self'", `style-src = [${styleSrc.join(" ")}]`);
...
check("no inline script body (all script tags are src=)", inlineScripts.length === 0, `${inlineScripts.length} inline script block(s)`);
...
check("supabase-js loaded same-origin from lib/", sbTag !== null, sbTag ? "" : "no lib/supabase-js* script tag found");
if (sbTag) {
    ...
    if (integrity) {
        ...
        check("SRI integrity matches the vendored file on disk", integrity === want, `attr=${integrity} file=${want}`);
    }
}
// ---- Verdict -----------------------------------------------------------------
console.log("");
