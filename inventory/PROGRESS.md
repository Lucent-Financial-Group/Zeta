...
## Phase 3 evidence (Claude, 2026-05-31)

**Phase 3 gate = PASSED.** Build: `inventory/index.html` (read path added; standalone; all Phase-2
auth behavior preserved — sign-in-only, getUser() trust, current_user_role(), sign-out; Phase-2
probe + sample-item folded into a collapsed Diagnostics panel, not deleted). Committed on
`claude/nifty-cray-6Xap1` (PR #6236).

### Pre-seed cleanup (owner-run)

Owner ran `inventory/sql/phase3_cleanup.sql` in the Supabase SQL editor -> exact expected grid:
`items_remaining 0 / change_log_total 1 / admin_cleanup_rows 1 / next_item_id (seq+1) 212`. The
documentary `ADMIN_CLEANUP` row (item_id NULL) is permanently in change_log (immutability
re-enabled). Deletion audited in git (committed script + Decisions log) AND in-DB (append-only row).

### Seed import (Claude, REST as editor, NO service_role; DIRECT container TLS)

`bun inventory/seed/seed-import.ts inventory/_seed_tmp/seed.json` (publishable/anon key + editor
creds via env; never logged):

```
PRE-CHECK ok: 210 items; ids === {1..211}\{8}; statuses valid; qty ints.
auth ok; current_user_role() = "editor"
items count BEFORE import: 0
bulk INSERT: HTTP 201 (atomic array POST)
items count AFTER import: 210
  spot id 1/2/10/14/18/211: match   (6-record field-by-field, 0 mismatches)
POST-CHECK ok: count=210. SEED COMPLETE.
```

Converter (`xlsx-to-json.ts`) validated first: 210 rows, ids 1..211 skip 8, 54 Needs Attention /
156 Active/In Use, 160 SOURCES blocks; an independent 6-record re-derivation from the raw xlsx XML
= 0 mismatches. Source xlsx + seed.json live ONLY in git-ignored `_seed_tmp/`. NOTE: item id=82's
product name is literally `(no items visible)` in the source sheet (a Generic/WiFi entry) — verified
verbatim from the xlsx; it correctly sorts first under name-asc (leading `(`), not a bug.

Key-test-fails-on-broken-code: importer `--dry-run` on the real file -> PRE-CHECK ok; on a
deliberately-broken 209-row copy -> `ABORT: expected 210 items, got 209` (exit 1).

### Browser-proof method (HONEST NOTE — two obstacles, neither faked)

1. The MCP Playwright browser tool could NOT launch (`Chromium distribution 'chrome' is not found at
   /opt/google/chrome/chrome`). Resolved by installing Playwright's own Chromium
   (`bunx playwright install chromium`) and driving it headless via a git-ignored script
   (`inventory/_proof_tmp/run-proofs.ts`, `--no-sandbox`). This is a REAL browser rendering the REAL
   committed page DOM.
2. Transport = SAME flagged same-origin proxy as Phase 2: the in-environment browser blocks the
   jsdelivr CDN and has unreliable direct egress to Supabase, so the page is served by a tiny local
   server (`_proof_tmp/serve.ts`, git-ignored) that serves a container-downloaded `supabase-js` from
   `/vendor/` and reverse-proxies `/auth/*`+`/rest/*` to the REAL Supabase project over container
   TLS. Backend / RLS / auth / data REAL + unmodified — only the transport hop rerouted. The
   COMMITTED `index.html` is unchanged (real CDN + real Supabase URL). Phase 7 Auditor re-verifies
   (a)(b)(c) end-to-end on the live deployed site (no proxy).

### Read-path proofs — observed JSON (real headless Chromium)

```json
{
  "proof_a_count": { "role_badge": "role: viewer", "who": "test2@test.com",
    "state_items": 210, "table_rows": 210, "card_rows": 210, "count_pill": "210 items" },
  "proof_b_searches": [
    { "query": "asus",   "rendered": 7,  "expected": 7,  "ids_match": true, "cards": 7,  "pill": "7 of 210 items" },
    { "query": "kindle", "rendered": 3,  "expected": 3,  "ids_match": true, "cards": 3,  "pill": "3 of 210 items" },
    { "query": "z-wave", "rendered": 13, "expected": 13, "ids_match": true, "cards": 13, "pill": "13 of 210 items" }
  ],
  "proof_b_sorts": {
    "name_asc": { "ok": true, "n": 210, "first3": ["(no items visible)", "2.5\" SATA SSD enclosure", "8BitDo Ultimate 2.4G controller"] },
    "qty_desc": { "ok": true, "n": 210, "top3": ["100","60","48"] },
    "category_then_qty_desc": { "ok": true, "n": 210, "arrows": ["Category ▲1","Qty ▼2"] }
  },
  "filters": { "category": {"value":"Alexa Devices","n":12,"ok":true},
    "status_needs_attention": {"n":54,"ok":true}, "attn_toggle": {"n":54,"ok":true}, "after_reset": 210 },
  "xss": { "rendered_as_text": true, "img_or_script_nodes": 0, "xss_flag_unset": true, "child_elements": 0 },
  "proof_c_phone": { "viewport":"390x844", "table_display":"none", "cards_display":"block",
    "cards":210, "controls_cols":1, "no_h_overflow":true, "first_card":"(no items visible)" },
  "signout": { "before": {"rows":210,"mem":210,"token":true},
    "after": {"signin_visible":true,"rows":0,"cards":0,"mem":0,"who":"","token_left":[],"status":"Signed out."} },
  "proof_d_demo": { "title":"Zeta Factory Dashboard", "h1":"Zeta Factory",
    "nav_tabs":["Agent Array","Human Anchors","External Reviewers","Alignment ▸","Circuit Breaker","Hamiltonian ▸","UX of Math ▸"],
    "panels":18, "switchTab":true }
}
```

`console_errors: []`. Phone screenshot: `_proof_tmp/phase3-phone-viewport.png` (git-ignored).

### (d) existing dashboard unchanged — non-browser checks

- `git diff --name-only origin/main..HEAD` = only `inventory/*`.
- `git diff --quiet origin/main -- demo/index.html demo/metrics.json demo/circuit-breaker-snapshot.json index.html` -> rc=0 (byte-identical).
- LIVE `https://lucent-financial-group.github.io/Zeta/demo/index.html` -> HTTP 200, title "Zeta Factory Dashboard". Live `/inventory/` -> HTTP 404 (not deployed; Phase 7).

### Post-seed RLS default-deny re-check (anon, direct TLS)

After the 210-row seed, unauthenticated anon REST still returns `[]` on every sensitive table:
`items [] · change_log [] · profiles [] · field_definitions []`. RLS default-deny intact; the seed
loosened nothing. (Owner/Auditor re-runs as the authority per spec.)

### Honesty ledger (proxy vs direct)

| Proof | Transport |
|---|---|
| seed import + DB count/spot-check + anon re-check | DIRECT container TLS (real Supabase, anon key) |
| (a)(b)(c) browser read path | real headless Chromium -> same-origin proof server -> reverse-proxy -> real Supabase (FLAGGED) |
| (d-i) git diff scope | local git (no network) |
| (d-ii) demo render | real headless Chromium -> local static server |
| (d-iii) live dashboard URL + live /inventory 404 | DIRECT container TLS to GitHub Pages |

Phase 7 Auditor re-verifies (a)(b)(c) end-to-end on the deployed live site (no proxy).
