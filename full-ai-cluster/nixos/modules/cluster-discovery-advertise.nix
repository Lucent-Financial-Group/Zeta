...
let
  ...
  advertiseScript = pkgs.writeShellScript "zeta-cluster-advertise" (lib.concatStringsSep "\n" [
    ...
    "  exit 1"
    "fi"
    "# sha256 of the CA PEM. UNVERIFIED that this equals the digest k3s embeds"
    "# in its own K10 token prefix; if it does not, a joiner that supplies the"
    "# pin refuses every join, which is the fail-closed direction and the first"
