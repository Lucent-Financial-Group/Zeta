...
const SFTP = { name: "sftp", image: "atmoz/sftp:alpine", mountDataAt: "/home/zeta/data" };
const steam = (appId, dir = "/data") => `/opt/steamcmd/steamcmd.sh +force_install_dir ${dir} +login anonymous +app_update ${appId} validate +quit`;
const GAMES = [
    {
...
const num = (s) => (s.match(/\b(\d+(?:gi|mi|g|m)?)\b/i) ?? [])[1];
/**
 ...
 */
export function build(message, draft) {
    ...
    if (draft) {
        ...
        if (/expos|publi|reachable|\blan\b|cluster|internal|private|make it (public|private)/.test(t)) {
            ...
            if (/publi/.test(t))
                e = "public";
            else if (/\blan\b/.test(t))
                e = "lan";
            else if (/cluster|internal/.test(t))
                e = "cluster";
            else if (/private|none|no expos/.test(t))
                e = "none";
            if (e) {
                d.defaultExpose = e;
                return { reply: `Set exposure to ${e}.`, spec: d };
            }
        }
        if (/memory|ram/.test(t)) {
            let m = num(t);
            if (!m && /\bmore\b/.test(t))
                m = `${parseInt(d.resources?.memory ?? "4", 10) + 2}Gi`;
            if (m) {
        ...
        if (/\bcpu\b|\bcores?\b/.test(t)) {
            const c = num(t);
            if (c) {
                d.resources = { ...d.resources, cpu: c.replace(/[a-z]+/i, "") };
                return { reply: `CPU limit set to ${d.resources.cpu}.`, spec: d };
            }
        }
        const v = t.match(/add (?:a )?(?:variable|var|knob)\s+([a-z_][a-z0-9_]*)/i);
        if (v) {
            d.variables = [...(d.variables ?? []), { name: v[1].toUpperCase() }];
            return { reply: `Added variable ${v[1].toUpperCase()}. Fill its default in the form.`, spec: d };
        }
        const port = t.match(/add (?:a )?port\s+(\d+)(?:\s*(udp|tcp))?/i);
        if (port) {
            d.ports = [...(d.ports ?? []), { name: `port-${port[1]}`, port: Number(port[1]), protocol: port[2]?.toUpperCase() ?? "TCP" }];
            return { reply: `Added ${port[2]?.toUpperCase() ?? "TCP"} port ${port[1]}.`, spec: d };
        }
        const rn = t.match(/(?:rename|call|name)\s+(?:it\s+)?(?:to\s+)?([a-z0-9][a-z0-9-]*)/i);
        if (rn && rn[1] !== "it" && rn[1] !== "to") {
            d.name = rn[1];
            return { reply: `Renamed the blueprint to "${d.name}".`, spec: d };
        }
        if (/sftp|file|ftp/.test(t) && !d.sidecars?.length) {
            d.sidecars = [SFTP];
            return { reply: "Added an SFTP sidecar for file access on the data volume.", spec: d };
        }
