...
SEED = {
    "version": "fixture-v1",
    "entries": [
        {"id": "big", "exponent": "big", "category": "descriptor", "allolexes": [], "valencyFrames": []},
        {"id": "small", "exponent": "small", "category": "descriptor", "allolexes": [], "valencyFrames": []},
        {"id": "walk", "exponent": "walk", "category": "action", "allolexes": [], "valencyFrames": []},
        {"id": "spinning", "exponent": "while spinning", "category": "adverb", "allolexes": [], "valencyFrames": []},
    ],
}
...
def _dataset(commands: list[str]) -> dict[str, object]:
    return {"examples": {"situational_1": [{"command": command} for command in commands]}}


def _inspect(tmp_path: Path, commands: list[str], seed: object = SEED) -> dict[str, object]:
    tmp_path.mkdir(parents=True, exist_ok=True)
    dataset_path = tmp_path / "dataset.json"
    seed_path = tmp_path / "seed.json"
    expected_hash = _write_json(dataset_path, _dataset(commands))
    _write_json(seed_path, seed)
    return inspect_gscan_lexical_preflight(str(dataset_path), str(seed_path), "situational_1", expected_hash)


def test_retains_every_unresolved_form_and_refuses_lexical_admission(tmp_path: Path) -> None:
...
def test_independent_event_stream_oracle_matches_object_stream_receipt(tmp_path: Path) -> None:
    dataset_path = tmp_path / "dataset.json"
    seed_path = tmp_path / "seed.json"
    expected_hash = _write_json(dataset_path, _dataset(["walk,big,blue", "small,while spinning"]))
    _write_json(seed_path, SEED)
    production = inspect_gscan_lexical_preflight(str(dataset_path), str(seed_path), "situational_1", expected_hash)
    oracle = inspect_gscan_lexical_preflight_oracle(str(dataset_path), str(seed_path), "situational_1", expected_hash)
...
def test_independent_oracle_retains_the_seed_removal_mutation(tmp_path: Path) -> None:
    ...
    _write_json(seed_path, {**SEED, "entries": [entry for entry in SEED["entries"] if entry["id"] != "walk"]})
    oracle = inspect_gscan_lexical_preflight_oracle(str(dataset_path), str(seed_path), "situational_1", expected_hash)
...
def test_input_reordering_keeps_aggregate_counts_and_canonical_forms(tmp_path: Path) -> None:
    ...
    for key in ("example_count", "command_form_count", "resolved_form_count", "unresolved_form_count", "resolved_seed_ids", "unresolved_forms"):
...
def test_seed_entry_removal_changes_the_admission_receipt(tmp_path: Path) -> None:
    ...
    removed = _inspect(tmp_path / "removed", ["walk,big"], {**SEED, "entries": [entry for entry in SEED["entries"] if entry["id"] != "walk"]})
...
def test_hash_mismatch_and_malformed_inputs_refuse_partial_counts(tmp_path: Path) -> None:
    dataset_path = tmp_path / "dataset.json"
    seed_path = tmp_path / "seed.json"
    _write_json(dataset_path, _dataset(["walk,big"]))
    _write_json(seed_path, SEED)
    mismatch = inspect_gscan_lexical_preflight(str(dataset_path), str(seed_path), "situational_1", "0" * 64)
    assert mismatch["status"] == "dataset-hash-mismatch"
    assert "example_count" not in mismatch

    malformed_seed = _inspect(tmp_path / "bad-seed", ["walk,big"], {"version": "bad", "entries": [{"id": "walk"}]})
    ...
    missing_split_hash = _write_json(missing_split_path, {"examples": {"train": [{"command": "walk,big"}]}})
    missing_split = inspect_gscan_lexical_preflight(str(missing_split_path), str(seed_path), "situational_1", missing_split_hash)
