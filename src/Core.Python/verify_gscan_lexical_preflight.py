...
def _comparable(value: Mapping[str, object]) -> dict[str, object]:
    return {
        key: value[key] for key in sorted(value) if key not in VOLATILE_RESULT_FIELDS
    }


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify a committed gSCAN lexical-admission receipt"
    )
    parser.add_argument(
        "dataset", help="explicit path to the pinned gSCAN JSON carrier"
    )
    ...
    if production_comparable != oracle_comparable:
        print(
            json.dumps(
                {"oracle": oracle_comparable, "production": production_comparable},
                indent=2,
                sort_keys=True,
            ),
            file=sys.stderr,
    ...
    if production_comparable != expected_comparable:
        print(
            json.dumps(
                {"expected": expected_comparable, "production": production_comparable},
                indent=2,
                sort_keys=True,
            ),
            file=sys.stderr,
    ...
    print(
        json.dumps(
            {
                "status": "verified",
                "receipt_sha256": arguments.expected_sha256,
