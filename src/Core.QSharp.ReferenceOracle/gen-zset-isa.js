 ...
 */
function extractOperatorBodies(source) {
...
function checkFixpoint(generated, committed) {
    ...
    for (const [name] of comBodies) {
        if (name === "VerifyIdentity" || name === "JoinWeighted")
            continue; // verification entrypoint + bonus op
...
export { generateQSharp, checkFixpoint, extractOperatorBodies };
