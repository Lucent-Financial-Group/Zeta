...
const singleQubitActions = new Map([
    [
...
const machZehnderActions = new Map([
    [
...
const singleQubitGateActions = new Map([
    [
        "H",
        (circuit) => {
            append(circuit, "h", 0);
...
const pauliProductActions = new Map([
    [
...
function append(circuit, gate, wire, params) {
    if (params === undefined) {
...
function applyGates(circuit, first, second) {
    append(circuit, first, 0);
    append(circuit, second, 0);
}
function applyClosedPhaseMachZehnder(circuit, phase) {
    append(circuit, "h", 0);
    append(circuit, "rz", 0, [phase]);
    append(circuit, "h", 0);
}
function applyKnownOperation(circuit, operation, actions) {
...
function probabilityOne(circuit) {
    return expectDefined(circuit.probabilities()[0], "missing |1> probability");
}
function amplitudeProbability(circuit, basisIndex) {
    const amplitude = normalizeComplex(expectDefined(circuit.state[basisIndex], `missing basis amplitude ${String(basisIndex)}`));
    return amplitude.real * amplitude.real + amplitude.imag * amplitude.imag;
}
function bellProbabilities(circuit) {
...
function bellPhiPlusCircuit(a, b) {
...
function bellSingletCircuit(a, b) {
    const circuit = new QuantumCircuit(2);
    append(circuit, "h", 0);
    append(circuit, "cx", [0, 1]);
    append(circuit, "x", 1);
...
function phiPlusCorrelator(a, b) {
    const { p00, p01, p10, p11 } = bellProbabilities(bellPhiPlusCircuit(a, b));
    return p00 + p11 - (p01 + p10);
}
function singletCorrelatorProbabilities(a, b) {
...
function makeGateCircuit(gateName) {
...
function circuitMatrix(circuit) {
...
describe("quantum-circuit simulator (second observable oracle)", () => {
    ...
    test("Bell coincidence probability outcomes match Q# observables", () => {
        for (const v of golden.vectors.bellCoincidence) {
            const circuit = v.operation === "Zeta.ReferenceOracle.ApplyBellSingletAnalyzers"
    ...
    test("treaty transcript integrity: TS, Q#, and F#/Analytic values match within tolerance", () => {
        ...
        for (const res of chsh.results) {
            ...
            const qsharp = chsh.qsharpCircuits[res.id];
        ...
        for (const res of transcript.jobs.bellCoincidence.results) {
            ...
            const qsharp = transcript.jobs.bellCoincidence.qsharpCircuits[res.id];
            expect(qsharp).toBeDefined();
            expect(qsharp).toContain("operation Circuit()");
        }
        ...
        for (const res of transcript.jobs.interferenceGrid.results) {
            ...
            const qsharp = transcript.jobs.interferenceGrid.qsharpCircuits[res.id];
            expect(qsharp).toBeDefined();
            expect(qsharp).toContain("operation Circuit()");
        }
    });
