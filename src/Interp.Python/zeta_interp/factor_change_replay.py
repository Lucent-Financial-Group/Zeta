...
def likelihoods(tokens, positions=(64, 128, 192, 256)):
    ...
    alternatives = [full_sequence(np.where(t >= start, p, 0.75))
                    for start in positions for p in (0.5, 0.25)]
...
def alarm_summary(first):
    ...
    delay = post - 128
    return {"Alarms": alarms, "Rate": p, "Wilson95": [float(center - radius), float(center + radius)],
            "Pre128": pre, "Post128": len(post), "Post128Rate": len(post) / n,
            "Post128AtRiskRate": len(post) / (n - pre), "Misses": int((first < 0).sum()),
            "ConditionalDelayQuantiles": np.quantile(delay, [0, 0.5, 0.9, 1]).tolist() if len(delay) else None}


def verify_change(root):
    data = read(root, "change-detection-results.json")
    assert data["Complete"] and data["MarginalOnlyRatio"] == 1
    expected = [("unchanged", 0.75, 0, 41), ("permanent-fair", 0.5, 384, 42),
                ("permanent-anticopy", 0.25, 384, 43), ("transient-fair", 0.5, 16, 44)]
    assert len(data["Panels"]) == 4
    summaries = []
    for row, (name, probability, duration, tag) in zip(data["Panels"], expected, strict=True):
        assert (row["Panel"], row["Probability"], row["Duration"], row["Domain"]) == (name, probability, duration, tag)
        tokens = change_streams(probability, duration, tag)
        assert hashlib.sha256(tokens.astype(np.uint8).tobytes()).hexdigest().upper() == row["StreamsSha256"]
        known, wrong = likelihoods(tokens)
        for values, first, final in [(known, "FirstKnown", "FinalKnownLogRatio"), (wrong, "FirstWrongIid", "FinalWrongLogRatio")]:
            np.testing.assert_array_equal(crossing(values), row[first])
            np.testing.assert_allclose(values[:, -1], row[final], atol=2e-10, rtol=1e-10)
        summaries.append({"Panel": name, "KnownNull": alarm_summary(row["FirstKnown"]), "WrongIidNull": alarm_summary(row["FirstWrongIid"])})
    return summaries
...
def factor_operators(n, epsilon):
    ...
    for token in range(2**n):
        ...
        flipped = reduce(np.kron, [local[1 - ((token >> f) & 1)] for f in reversed(range(n))])
    ...
    for row in tokens:
        ...
        for t in range(64):
            ...
            for f in range(n):
                ...
                choice = min(int(np.searchsorted(cumulative, rng.next(), side="right")), 3)
...
def get_marginals(state, n):
    columns = [state[:, [s for s in range(2**n) if ((s >> f) & 1) == bit]].sum(axis=1)
               for f in range(n) for bit in range(2)]
    return np.stack(columns, axis=1)


def product(marginals, n):
    return np.stack([np.prod(marginals[:, [2 * f + ((s >> f) & 1) for f in range(n)]], axis=1)
                     for s in range(2**n)], axis=1)
...
def verify_factors(root):
    ...
    assert data["Complete"] and len(data["Scores"]) == 54 and len(data["Measurements"]) == 270
    expected = {(n, e, s, m) for n in (2, 3, 4) for e in (0, 0.2) for s in (41, 53, 67) for m in ("Dense", "TensorJoint", "ProjectedProduct")}
    assert {(r["Factors"], r["Epsilon"], r["Seed"], r["Mode"]) for r in data["Scores"]} == expected
    outputs, summaries = {}, []
    for n in (2, 3, 4):
        for epsilon in (0, 0.2):
            for seed in (41, 53, 67):
                tokens = factor_streams(n, epsilon, seed)
                context_hash = hashlib.sha256(tokens.astype(np.uint8).tobytes()).hexdigest().upper()
                true_state, true_p = factor_outputs(n, epsilon, tokens)
                reduced_state, reduced_p = factor_outputs(n, epsilon, tokens, True)
                corr = divergence(true_state, product(get_marginals(true_state, n), n)).mean()
                for row in data["Scores"]:
                    if (row["Factors"], row["Epsilon"], row["Seed"]) != (n, epsilon, seed):
                        continue
                    projected = row["Mode"] == "ProjectedProduct"
                    state, p = (reduced_state, reduced_p) if projected else (true_state, true_p)
                    values = {"NextKlBits": divergence(true_p, p).mean(), "MaximumStateError": np.max(np.abs(true_state - state)), "JointVsMarginalsKlBits": corr}
                    assert row["ContextSha256"] == context_hash
                    for key, value in values.items():
                        np.testing.assert_allclose(value, row[key], atol=3e-12, rtol=1e-9)
                    s = 2**n
                    assert row["ParameterBytes"] == 8 * (s**3 + s if row["Mode"] == "Dense" else 11)
                    assert row["StateBytes"] == 8 * (2 * n if projected else s)
                    outputs[row["Model"]] = output_checksum(get_marginals(state, n) if projected else state, p)
                    summaries.append({k: row[k] for k in ["Model", "NextKlBits", "MaximumStateError", "JointVsMarginalsKlBits"]})
    assert {(r["Model"], r["Repetition"]) for r in data["Measurements"]} == {(i, r) for i in outputs for r in range(5)}
