...
class PixelAgent:
    ...
    def act(self, grid: Grid) -> GameAction:
        ...
        # The target is the nearest component that is not me. On ZetaChase that
        # is the goal; the rule is "something else worth reaching", not "the
        # blob painted 4".
        target = min((c for c in now if self._key(c) != self._key(me)), key=me.distance_to)
