...
export function canonicalJson(n) {
    try {
 ...
 */
export function fromCanonicalJson(json) {
    let pos = 0;
    const fail = (e) => {
    ...
    const expectLiteral = (lit) => {
        if (json.slice(pos, pos + lit.length) !== lit)
            fail("UnexpectedEnd");
        pos += lit.length;
    };
    ...
    const readEscape = () => {
        ...
        if (rep === undefined)
            fail("UnexpectedEnd"); // invalid escape / EOF after backslash
        pos += 1;
    ...
    const parseNumber = () => {
        ...
        if (isFloat)
            return fail("Unsupported"); // Float deferred in v1 JSON
    ...
    const parseObject = (depth) => {
        ...
        while (pos < json.length) {
            skipWs();
            if (json.charAt(pos) !== '"')
                return fail("UnexpectedEnd"); // key must be a string
