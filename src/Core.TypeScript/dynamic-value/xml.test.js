...
test("XML float corners are distinct + round-trip exactly (bit-pattern form)", () => {
    ...
    const posZero = { t: "float", v: "0000000000000000" };
    const negZero = { t: "float", v: "8000000000000000" };
    const nan = { t: "float", v: "7ff8000000000000" };
    const posInf = { t: "float", v: "7ff0000000000000" };
    ...
    for (const t of corners) {
        ...
        expect(enc.ok).toBe(true);
        if (enc.ok)
            expect(fromCanonicalXml(enc.value)).toEqual({ ok: true, value: t });
    }
