...
function f16ToValue(bits: number): number {
...
export function fromCanonicalCbor(bytes: number[]): DecodeResult {
  ...
  const readBE = (n: number): bigint => {
    let v = 0n;
    for (let i = 0; i < n; i++) v = (v << 8n) | BigInt(bytes[pos + i] ?? 0);
    pos += n;
    return v;
  };

  const argWidth = (ai: number): number => {
    if (ai === 24) return 1;
    if (ai === 25) return 2;
    if (ai === 26) return 4;
    if (ai === 27) return 8;
  ...
  const readArg = (ai: number): bigint => {
    if (ai < 24) return BigInt(ai);
    const n = argWidth(ai);
    if (n < 0) fail("Unsupported");
    need(n);
    return readBE(n);
  };

  const readSimpleOrFloat = (ai: number): Tagged => {
    switch (ai) {
      case 20:
      ...
      case 26: {
        need(4);
        const dv = new DataView(new ArrayBuffer(4));
        for (let i = 0; i < 4; i++) dv.setUint8(i, bytes[pos + i] ?? 0);
        pos += 4;
      ...
      case 27: {
        need(8);
        const dv = new DataView(new ArrayBuffer(8));
        for (let i = 0; i < 8; i++) dv.setUint8(i, bytes[pos + i] ?? 0);
        pos += 8;
  ...
  const remaining = (): bigint => BigInt(bytes.length - pos);

  const readInt = (major: number, arg: bigint): Tagged => {
    if (arg > I64_MAX) return fail("IntegerOverflow");
    return { t: "int", v: major === 0 ? arg.toString() : (-1n - arg).toString() };
  };

  const readByteString = (arg: bigint): Tagged => {
  ...
  const readTextString = (arg: bigint): Tagged => {
    if (arg > remaining()) return fail("UnexpectedEnd");
    const n = Number(arg);
  ...
  function readValue(depth: number): Tagged {
    ...
    if (major === 7) return readSimpleOrFloat(ai);
