...
export function canonicalXml(n) {
    try {
...
export function fromCanonicalXml(xml) {
    let pos = 0;
    const fail = (e) => {
    ...
    const readTag = () => {
        ...
        // tag name = letters
        const nameStart = pos;
        while (pos < xml.length && /[a-z]/.test(xml.charAt(pos)))
            pos += 1;
    ...
    const expectClose = (name) => {
        const tag = readTag();
        if (!tag.close || tag.name !== name || tag.selfClose)
            fail("MalformedXml");
    };
    ...
    const parseValue = (depth) => {
        ...
        switch (tag.name) {
            ...
            case "str": {
                if (tag.selfClose)
                    return { t: "str", v: "" }; // tolerated; fixed-point rejects (canonical is <str></str>)
            ...
            case "bytes": {
                if (tag.selfClose)
                    return { t: "bytes", v: "" }; // tolerated; fixed-point rejects (canonical is <bytes></bytes>)
                const text = readTextRun();
                expectClose("bytes");
            ...
            case "arr": {
                if (tag.selfClose)
                    return { t: "arr", v: [] }; // tolerated; fixed-point rejects
                const items = [];
            ...
            case "obj": {
                if (tag.selfClose)
                    return { t: "obj", v: [] }; // tolerated; fixed-point rejects
                const pairs = [];
                while (!(at() === "<" && xml.charAt(pos + 1) === "/")) {
                    const eTag = readTag();
                    if (eTag.close || eTag.name !== "e" || eTag.attrK === null || eTag.selfClose)
                        fail("MalformedXml");
    ...
    try {
        ...
        if (!enc.ok || enc.value !== xml)
            return { ok: false, error: "NonCanonical" };
        return { ok: true, value };
    }
    catch (e) {
        if (e instanceof XmlDecodeError)
            return { ok: false, error: e.error };
        if (e instanceof XmlEncodeError)
            return { ok: false, error: "NonCanonical" };
        if (e instanceof XmlEncodeErrorCarrier)
