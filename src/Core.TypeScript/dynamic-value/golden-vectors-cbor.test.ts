...
import { type Tagged, canonicalCbor, toHex, f64ToBitsHex } from "./cbor";

// DynamicValue canonical-CBOR ENCODE byte-lock — the TS oracle agrees on the shared seed
// (golden-vectors-cbor.json). CBOR is the TOTAL form (all 8 shapes), so this is where Float
// (RFC 8949 §4.2.2 shortest-float) and Bytes (major-type-2) lock — the two cases canonical
// JSON deferred. The seed was generated + RFC-8949-Appendix-A-anchored independently; the
// `cbor float matches RFC 8949 Appendix A` test re-anchors the float logic against the RFC
// directly so the lock is not circular. The codec lives in ./cbor.ts (shared with the decode
// test). "The compilers don't lie."
