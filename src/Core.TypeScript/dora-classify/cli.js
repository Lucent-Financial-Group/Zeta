...
import { aggregateAuthorRatios, classifyCommit, } from "./classify";
...
function gitLogShasInRange(args) {
    let gitArgs;
    if (args.sha) {
        return [args.sha];
    }
...
function gitCommitMetadata(sha) {
    // Header line with %H|%an|%ae|%cI|%s; then blank line; then changed files
    const hdrOut = execFileSync("git", ["log", "-1", "--pretty=format:%H|%an|%ae|%cI|%s", sha], { encoding: "utf8" }).trim();
    ...
    const filesOut = execFileSync("git", ["diff-tree", "--no-commit-id", "--name-only", "-r", sha], { encoding: "utf8" }).trim();
...
async function main() {
    ...
    if ("error" in parsed) {
        if (parsed.error === "help") {
            process.stdout.write("Usage: bun src/Core.TypeScript/dora-classify/cli.ts [--sha <ref> | --since <git-time> | --range <gitref..gitref>] [--aggregate]\n" +
