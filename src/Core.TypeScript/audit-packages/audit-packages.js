...
function classify(pinned, latest) {
    if (latest === pinned)
        return { marker: "✓ up-to-date", failed: false };
    if (latest === "?")
        return { marker: "? couldn't query", failed: false };
...
export function main() {
    ...
    if (packages.length === 0) {
        process.stderr.write(`error: parsed 0 PackageVersion entries from ${propsPath} — regex may be stale relative to file format\n`);
        return 1;
    }
