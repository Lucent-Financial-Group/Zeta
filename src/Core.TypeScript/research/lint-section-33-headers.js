...
function isSection33File(content, maxHeader) {
...
function findMissingLabels(content, maxHeader) {
    const lines = content.split("\n");
...
function lintFile(path, maxHeader, errorOnUnreadable) {
    const findings = [];
    let content;
    try {
    ...
    if (!isSection33File(content, maxHeader))
        return findings;
...
function walkResearch(baseDir, files, isRoot = false) {
    let entries;
    try {
