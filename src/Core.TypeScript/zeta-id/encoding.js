...
export function format(id) {
    let v = id & MASK_128;
    const out = new Array(ZETAID_BASE32_LEN);
...
export function parse(s) {
    if (s.length !== ZETAID_BASE32_LEN) {
        throw new Error(`ZetaId.parse: expected ${ZETAID_BASE32_LEN} Crockford-base32 chars, got ${s.length} (${JSON.stringify(s)})`);
    }
    let v = 0n;
...
export function toHex(id) {
    return (id & MASK_128).toString(16).padStart(ZETAID_HEX_LEN, "0");
}
