...
import { format, parse, toHex, fromHex, isCanonical, CROCKFORD_ALPHABET, ZETAID_BASE32_LEN, ZETAID_HEX_LEN, } from "./encoding";
...
function sampleIds(n, seed = 0x9e3779b97f4a7c15n) {
    const g = lcg(seed);
    return Array.from({ length: n }, () => g.next().value);
}
...
test("format is always exactly 26 chars (fixed width)", () => {
    for (const id of [0n, MASK_128, ...sampleIds(500)]) {
...
test("sort-preserving holds at the boundaries (0, max, single-bit deltas)", () => {
    const a = 0n;
    const b = 1n;
    const c = (1n << 75n); // a timestamp-region bit (high)
    const d = MASK_128;
...
test("parse accepts Crockford lenient aliases (i/l→1, o→0) and lowercase", () => {
    ...
    expect(id).toBe(31n);
    // lowercase z == Z
    expect(parse("0000000000000000000000000z")).toBe(31n);
    // 'O'/'o' alias for 0, 'I'/'L'/'i'/'l' alias for 1 — both decode without error
    expect(parse("OOOOOOOOOOOOOOOOOOOOOOOOOO")).toBe(0n);
    ...
    const vectors = [
        {
            ...
            obs: {
                version: 1, timestamp: 1747780809123, chromosome: 7, category: 0,
        ...
        },
        {
            ...
            obs: {
                version: 1, timestamp: 1749200000000, chromosome: 0, category: 8,
...
test("two ids minted close in time sort by time (the workitems/ ls-ordering guarantee)", () => {
    const base = {
        version: 1, timestamp: 0, chromosome: 0, category: 8, firefly: 1,
        authority: { type: "Standard" }, persona: 1, momentum: { type: "Normal" }, location: 1,
    };
    const earlier = pack({ ...base, timestamp: 1749200000000 }, DEFAULT_ENV);
    const later = pack({ ...base, timestamp: 1749200001000 }, DEFAULT_ENV);
