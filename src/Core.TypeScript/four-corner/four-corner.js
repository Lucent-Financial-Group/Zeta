...
export function ofLine(line) {
    ...
    if (parts.length !== 5 || parts[0] !== "fourcorner1")
        return null;
    const tOut = optOfText(parts[2]);
    const tOutFeedback = optOfText(parts[3]);
    const tInFeedback = optOfText(parts[4]);
    if (tOut === undefined || tOutFeedback === undefined || tInFeedback === undefined)
        return null;
