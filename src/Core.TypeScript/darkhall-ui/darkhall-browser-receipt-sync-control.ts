...
export const DARK_HALL_BROWSER_RECEIPT_SYNC_CONTROL_SCHEMA =
  "zeta.darkhall.browser-receipt-sync-control.v1" as const;
...
export interface DarkHallBrowserReceiptSyncControlReadout {
  ...
  readonly active: "submit" | "poll" | null;
...
function submitGesture(event: unknown): boolean {
  ...
  try {
    return Reflect.apply(closest, target, ["[data-receipt-sync-submit]"]) !== null;
  } catch {
    return false;
  }
}
...
export function startDarkHallBrowserReceiptSyncControl(
  ...
  let stopped = false;
  let active: "submit" | "poll" | null = null;
  let submissions = 0;
  let polls = 0;
  ...
  const publish = (): DarkHallBrowserReceiptSyncControlResult<DarkHallBrowserReceiptSyncControlReadout> => {
    ...
    try {
      ...
      return setText(output, text)
        ? succeeded(readout)
        : failed("receipt-sync-control-observer-failed", "The browser refused the receipt synchronization text readout.");
    } catch {
      return failed(
        "receipt-sync-control-observer-failed",
  ...
  const recordFailure = (
    ...
    return rendered.ok
      ? failed("receipt-sync-control-operation-failed", observedCode, observedSeverity)
      : rendered;
  };
  ...
  const clicked: NativeListener = (event) => {
    if (!submitGesture(event)) return;
    preventDefault(event);
    void operate("submit", "user-activation");
  };
