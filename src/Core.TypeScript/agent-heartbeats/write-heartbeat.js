#!/usr/bin/env bun
// tools/agent-heartbeats/write-heartbeat.ts — 081KDWYRBE008QG0R002M1DH07 heartbeat writer.
//
...
export function parseArgs(argv, env = process.env) {
    ...
    let authority = env.ZETA_AGENT_AUTHORITY ?? "TrustedAgent";
    let momentum = env.ZETA_AGENT_MOMENTUM ?? "Normal";
    ...
    let namedDep = env.ZETA_AGENT_NAMED_DEP ?? null;
    let disposition = env.ZETA_AGENT_DISPOSITION ?? "bounded-wait";
    let parentPr = env.ZETA_AGENT_PARENT_PR ? parseIntStrict(env.ZETA_AGENT_PARENT_PR) : null;
    ...
    let writeLocalExplicit = null; // null=auto (push→false, no-push→true)
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        try {
            if (arg === "--persona-slot") {
                ...
                if (v === null)
                    return { error: "--persona-slot must be integer" };
                personaSlot = v;
            }
            else if (arg === "--persona-name")
                personaName = next();
                ...
                const v = parseIntStrict(next());
                if (v === null)
                    return { error: "--chromosome must be integer" };
                chromosome = v;
            }
            else if (arg === "--location") {
                const v = parseIntStrict(next());
                if (v === null)
                    return { error: "--location must be integer" };
                location = v;
            }
            else if (arg === "--named-dep")
                namedDep = next();
                ...
                const v = parseIntStrict(next());
                if (v === null)
                    return { error: "--parent-pr must be integer" };
                parentPr = v;
            }
                ...
                push = false;
            else if (arg === "--write-local")
                writeLocalExplicit = true;
            else if (arg === "--no-write-local")
                writeLocalExplicit = false;
            else if (arg === "--repo")
                repo = next();
            else if (arg === "--branch")
                branch = next();
    ...
    if (chromosome < 0 || chromosome > 31)
        return { error: "--chromosome must be 0..31" };
    if (location < 0 || location > 255)
        return { error: "--location must be 0..255" };
    if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repo))
        return { error: "--repo must match owner/name" };
    if (!KNOWN_AUTHORITIES.includes(authority))
        return { error: `--authority must be one of ${KNOWN_AUTHORITIES.join(",")}` };
    if (!KNOWN_MOMENTUM.includes(momentum))
        return { error: `--momentum must be one of ${KNOWN_MOMENTUM.join(",")}` };
 ...
 */
export function pushHeartbeatViaRest(repo, branch, filePath, // repo-relative path (e.g., "docs/agent-heartbeats/otto/2026/05/27/abc.md")
fileContent, commitMessage, maxRetries = 5) {
    ...
    const blobReq = gh(["api", "-X", "POST", `repos/${repo}/git/blobs`, "--input", "-"], JSON.stringify({ content: fileContent, encoding: "utf-8" }));
    if (blobReq.status !== 0)
        return { error: `blob create failed: ${blobReq.stderr || blobReq.stdout}` };
    let blobSha;
    try {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        if (refReq.status !== 0)
            return { error: `ref read failed: ${refReq.stderr || refReq.stdout}` };
        let parentCommitSha;
        try {
        ...
        if (commitReq.status !== 0)
            return { error: `commit read failed: ${commitReq.stderr || commitReq.stdout}` };
        let parentTreeSha;
        try {
        ...
        // eslint-disable-next-line sonarjs/no-os-command-from-path
        const treeReq = gh(["api", "-X", "POST", `repos/${repo}/git/trees`, "--input", "-"], JSON.stringify({
            base_tree: parentTreeSha,
            tree: [{ path: filePath, mode: "100644", type: "blob", sha: blobSha }],
        }));
        if (treeReq.status !== 0)
            return { error: `tree create failed: ${treeReq.stderr || treeReq.stdout}` };
        let newTreeSha;
        try {
        ...
        // eslint-disable-next-line sonarjs/no-os-command-from-path
        const newCommitReq = gh(["api", "-X", "POST", `repos/${repo}/git/commits`, "--input", "-"], JSON.stringify({
        ...
        // eslint-disable-next-line sonarjs/no-os-command-from-path
        const refUpdateReq = gh(["api", "-X", "PATCH", `repos/${repo}/git/refs/heads/${branch}`, "--input", "-"], JSON.stringify({ sha: newCommitSha, force: false }));
...
export function buildHeartbeatObservation(args, timestampMs) {
    return {
        version: 1,
...
export function renderHeartbeat(args, idHex, timestampMs) {
    ...
    if (args.namedDep)
        lines.push(`named-dep: ${JSON.stringify(args.namedDep)}`);
    if (args.parentPr !== null)
        lines.push(`parent-pr: ${args.parentPr}`);
    lines.push(`---`);
    lines.push(``);
...
export function heartbeatRepoRelPath(personaName, timestampMs, idHex) {
    const d = new Date(timestampMs);
    const yyyy = d.getUTCFullYear().toString();
    const mm = (d.getUTCMonth() + 1).toString().padStart(2, "0");
    const dd = d.getUTCDate().toString().padStart(2, "0");
...
async function main() {
    ...
    if (parsed.dryRun) {
        ...
        if (parsed.writeLocal)
            console.log(`DRY RUN — would write local: ${localPath}`);
        if (parsed.push)
            console.log(`DRY RUN — would push: ${parsed.repo} branch=${parsed.branch} path=${repoRelPath}`);
        if (!parsed.writeLocal && !parsed.push)
            console.log(`DRY RUN — both --no-write-local + --no-push = no-op`);
        return 0;
    }
    if (parsed.writeLocal) {
        try {
