#!/usr/bin/env bun
// tools/agent-heartbeats/merge-heartbeats-to-main.ts — 081KDWYRBE008QG0R001G14TE3: periodic
// merge of agent-heartbeats branch back into main.
//
// Composes:
//   - tools/agent-heartbeats/write-heartbeat.ts (081KDWYRBE008QG0R002M1DH07; the per-tick writer)
 ...
 */
export function isUpToDate(repo, base, head) {
    const result = gh(["api", `repos/${repo}/compare/${base}...${head}`]);
    if (result.status !== 0)
        return { error: `compare failed: ${result.stderr || result.stdout}` };
    try {
 ...
 */
export function findExistingPR(repo, head, base) {
    const owner = repo.split("/")[0];
    const result = gh(["api", `repos/${repo}/pulls?state=open&head=${owner}:${head}&base=${base}`]);
    if (result.status !== 0)
        return { error: `list pulls failed: ${result.stderr || result.stdout}` };
    try {
        const parsed = JSON.parse(result.stdout);
...
export function openMergePR(repo, head, base, title, body) {
    ...
    if (existing.found) {
        ...
        const createResult = gh(["api", "-X", "POST", `repos/${repo}/pulls`, "--input", "-"], JSON.stringify({ title, body, head, base }));
