...
function extractStep(stepHeader, nextStepHeader) {
...
const ITER_540_BLOCK = extractStep("Step 6.8: iter-5.4.0 homelab gh-auth + operator pubkey copy", "Step 6.9: iter-5.4.1 self-registration commit+push");
const ITER_541_BLOCK = extractStep("Step 6.9: iter-5.4.1 self-registration commit+push", 
...
const ITER_42_BLOCK = extractStep("Step 6.5: iter-4.2 probe boot USB for operator SSH pubkey", "Step 6.56: 081KSKBP80008QG0R003AX2A69.3b cred-blob passphrase prompt");
...
describe("iter-5.4.0 — gh auth + ssh-key flow (081KSGS9H0008QG0R00120EEHM Bug 2a + 2b)", () => {
    ...
    test("081KSGS9H0008QG0R00120EEHM Bug 2a fix: gh auth setup-git is inside the auth-success branch", () => {
        ...
        const successBranchMatch = ITER_540_BLOCK.match(/if gh auth login; then\n([\s\S]*?)\n {4}else\n {6}echo\n {6}echo "\[iter-5\.4\.0\] {3}gh auth login FAILED/);
    ...
    test("081KSGS9H0008QG0R00120EEHM Bug 2b fix: discriminates scope-error from empty-list", () => {
        ...
        expect(ITER_540_BLOCK).toMatch(/grep -qE "\(scope\|insufficient\|admin:public_key\|read:public_key\)" "\$SSH_KEY_ERR_FILE"/);
    });
...
describe("081KSNY2Z0008QG0R0008PN7RQ retained zflash credential preseed", () => {
    ...
    test("Step 6.95 skips picker when a retained blob already exists", () => {
        expect(SCRIPT).toMatch(/if \[ "\$\{BOOT_USB_CREDS_PRESEEDED:-0\}" = "1" \] && \[ -f \/mnt\/boot\/zeta-creds\.enc \]; then\n\s+PICKER_OPT_OUT=1/);
...
describe("iter-5.1 hardware-configuration copy (081KSNY2Z0008QG0R0008PN7RQ phase-2 initrd)", () => {
    test("nixos-generate-config output is copied into flake host tree before nixos-install", () => {
        ...
        expect(SCRIPT).toContain('HW_DST="/mnt/etc/zeta/full-ai-cluster/nixos/hosts/${HOST}/hardware-configuration.nix"');
    });
});
