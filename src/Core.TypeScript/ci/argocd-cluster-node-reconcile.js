...
export function planClusterNodeReconcile(input) {
    ...
    const pathMatch = input.treePath.match(/^maintainers\/([^/]+)\/cluster-nodes\/([^/]+)\/node\.yaml$/);
    if (!pathMatch) {
        ...
        if (crName && crName !== hostFromPath) {
            errors.push(`metadata.name '${crName}' must match hostname segment in tree-path '${hostFromPath}'`);
        }
        if (input.serial) {
            if (input.serial.maintainer !== maintainer) {
                errors.push(`serial maintainer '${input.serial.maintainer}' != tree-path maintainer '${maintainer}'`);
            }
            if (input.serial.nodeHostname !== hostFromPath) {
                errors.push(`serial node '${input.serial.nodeHostname}' != tree-path host '${hostFromPath}'`);
            }
