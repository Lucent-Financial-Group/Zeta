...
function expectMiseTool(name, version) {
...
function miseToolPattern(name, version) {
...
const WINDOWS_EXCEPTIONS = {
    ...
    "hermes-agent": "NousResearch agent; upstream install is a Linux/macOS curl script (WebSearch 2026-05-30); Windows + cross-platform (uv/npm?) install TBD — deferred from manifests/windows pending an install-graph decision",
    ...
    mtools: "file-backed zflash ESP-image writer for Unix/NixOS QEMU proof; no scoop/winget/choco package source is declared yet, so Windows keeps QEMU-only coverage until a Windows package source is selected",
...
test("Codex CLI setup migrates the deprecated service_tier default value", () => {
    const installPs1 = readFileSync(join(setupDir, "install.ps1"), "utf8");
    const agentClisTs = readFileSync(join(repoRoot, "src", "Core.TypeScript", "ace", "setup-realizers", "from-bun-global.ts"), "utf8");
...
test("NixOS install shield validates from-bun-global manifest, not one hardcoded CLI", () => {
    const dockerfile = readFileSync(join(repoRoot, "src", "Core.TypeScript", "ci", "dockerfiles", "nixos-install-sh-test", "Dockerfile"), "utf8");
...
test("from-ollama install defaults to skip outside interactive/full install contexts", () => {
    const localLlm = readFileSync(join(repoRoot, "src", "Core.TypeScript", "ace", "setup-realizers", "from-ollama.ts"), "utf8");
    const ubuntuDockerfile = readFileSync(join(repoRoot, "src", "Core.TypeScript", "ci", "dockerfiles", "ubuntu-install-sh-test", "Dockerfile"), "utf8");
    ...
    const jammyDockerfile = readFileSync(join(repoRoot, "src", "Core.TypeScript", "ci", "dockerfiles", "ubuntu-jammy-install-sh-test", "Dockerfile"), "utf8");
...
test("NixOS and USB installer surfaces delegate agent/runtime drift to install graph", () => {
    ...
    const installerNix = readFileSync(join(repoRoot, "full-ai-cluster", "usb-nixos-installer", "nixos", "installer", "configuration.nix"), "utf8");
