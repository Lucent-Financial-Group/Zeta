...
import { validateSelfRegCiCoherent } from "./self-reg-serial.js";
import { firstSessionPhase3Enabled, firstSessionMarkersSatisfied, } from "./qemu-first-session-phase3.js";
...
const CONSOLE_MIRROR_HINT = "serial log shows idle installer shell without install progress — " +
...
export function detectPhase2Success(serialOutput, expectedHostname, requireFirstSession = false) {
...
async function waitForInstalledLogin(serialLogPath, expectedHostname, requireFirstSession) {
    ...
    const content = readSerial(serialLogPath);
    const emptySerialHint = content.trim().length === 0
...
async function main() {
    ...
    const writeArtifactSerialLog = (phase1, phase2) => {
    ...
    const phase1 = await runQemuUntil(buildQemuInstallArgs(isoPath, diskPath, phase1SerialLogPath), phase1SerialLogPath, () => waitForInstallComplete(phase1SerialLogPath), "phase 1 (ISO install)");
