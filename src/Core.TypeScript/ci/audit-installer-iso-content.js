...
function isAuditError(r) {
    return !Array.isArray(r);
}
function auditIsoContent(isoPath) {
    ...
    try {
        ...
        return {
            kind: "missing-file",
            detail: `cannot stat ISO path ${isoPath}: ${e.message}`,
        };
    }
...
function dumpIsoEntriesForDiagnostic(isoPath, limit = 80) {
...
function main() {
    ...
    if (failures.length === 0) {
        process.stdout.write(`audit-installer-iso-content: PASS — ${parsed.isoPath} contains all ${REQUIRED_ISO_PATHS.length} expected top-level files\n`);
        return 0;
    }
    process.stderr.write(`audit-installer-iso-content: FAIL — ${failures.length} assertion(s) failed for ISO ${parsed.isoPath}\n\n`);
