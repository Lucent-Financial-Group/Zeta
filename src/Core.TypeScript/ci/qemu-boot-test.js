...
function checkDependencies(arch) {
...
function buildQemuArgs(arch, bootMedia, serialLogPath) {
...
function timeoutSecondsFor(arch) {
    const kvm = existsSync(KVM_PATH);
...
async function waitForLoginPrompt(serialLogPath, timeoutSeconds, abandoned) {
