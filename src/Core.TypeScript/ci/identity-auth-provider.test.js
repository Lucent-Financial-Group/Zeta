...
describe("identity-auth-provider seam", () => {
  ...
  it("serialLinesForIdentityAuth distinguishes mock vs skip", () => {
    const mock = createMockIdentityAuthProvider().authenticate(), mockLines = serialLinesForIdentityAuth(mock);
    expect(mockLines).toContain(IDENTITY_AUTH_SERIAL.mockBegin);
    expect(mockLines).toContain(IDENTITY_AUTH_SERIAL.mockOk);
    expect(mockLines.some((l) => l.includes(MOCK_GH_USER_CODE))).toBe(!0);
    const skip = createSkipIdentityAuthProvider().authenticate(), skipLines = serialLinesForIdentityAuth(skip);
