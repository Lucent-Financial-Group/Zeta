...
function evidenceFor(window: PromotionWindow): readonly string[] {
  return [
    ...
    `observe-act-promotion:shadow_divergence_rate:${String(window.divergenceRate)}`,
    `observe-act-promotion:shadow_illegal_selections:${String(window.illegalSelections)}`,
...
export function evaluatePromotion(window: PromotionWindow): GateDecision {
  ...
  if (window.primaryControlBypassRejections30m >= DEMOTION_THRESHOLDS.controlBypassRejections30m) {
    return {
      ...
      detail: `${String(window.primaryControlBypassRejections30m)} control-bypass rejection(s) in the last 30 minutes — one is enough, because a lane that tried to go around a control is not a lane whose soak record means anything`,
      evidence,
    };
  }
  if (window.primarySelectorRejections30m >= DEMOTION_THRESHOLDS.selectorRejections30m) {
    return {
      ...
      detail: `${String(window.primarySelectorRejections30m)} selector rejections in the last 30 minutes (threshold ${String(DEMOTION_THRESHOLDS.selectorRejections30m)})`,
      evidence,
    };
  }
  ...
  if (!soaked) {
    return {
      ...
      detail: `${String(window.shadowTicks)} ticks / ${String(window.shadowSoakHours)}h of soak — needs ${String(PROMOTION_THRESHOLDS.minShadowTicks)} ticks or ${String(PROMOTION_THRESHOLDS.minShadowSoakHours)}h`,
      evidence,
    };
  }
  ...
  if (window.illegalSelections > PROMOTION_THRESHOLDS.maxIllegalSelections) {
    return {
      ...
      detail: `${String(window.illegalSelections)} illegal slot selection(s) in the shadow window — the bar is zero, because one slot the grammar did not permit is a lane that does not respect the grammar`,
      evidence,
    };
  }
  ...
  if (window.divergenceRate > PROMOTION_THRESHOLDS.maxDivergenceRate) {
    return {
      ...
      detail: `divergence ${String(window.divergenceRate)} exceeds ${String(PROMOTION_THRESHOLDS.maxDivergenceRate)}`,
      evidence,
    };
  }
  ...
  return {
    mode: "primary",
    reason: "promoted",
    detail: `promoted: ${String(window.shadowTicks)} ticks / ${String(window.shadowSoakHours)}h soak, ${String(window.illegalSelections)} illegal selections, divergence ${String(window.divergenceRate)}`,
    evidence,
  };
}
