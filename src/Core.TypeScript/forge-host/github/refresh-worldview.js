...
function runGitOrExit(args, context) {
...
// --- Data fetchers ---
function fetchOpenPRs(owner, repo, limit) {
    ...
    return parseJsonOrExit(stdout, "fetchOpenPRs");
}
function fetchRecentMerges(owner, repo, limit) {
    const stdout = runGhOrExit([
        "pr",
        "list",
        "--repo",
    ...
    return parseJsonOrExit(stdout, "fetchRecentMerges");
}
function fetchOpenIssues(owner, repo, limit) {
    const stdout = runGhOrExit([
        "issue",
        "list",
        "--repo",
    ...
    return parseJsonOrExit(stdout, "fetchOpenIssues");
}
...
function fetchBacklogDelta() {
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    const result = spawnSync("find", ["docs/backlog", "-name", "B-*.md", "-type", "f"], { encoding: "utf8", maxBuffer: SPAWN_MAX_BUFFER });
...
function fetchBranchState() {
    const branchStdout = runGitOrExit(["branch", "--show-current"], "fetchBranchState.branch");
    const current = branchStdout.trim() || "HEAD (detached)";
    let tracking = null;
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    const trackResult = spawnSync("git", ["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], { encoding: "utf8", maxBuffer: SPAWN_MAX_BUFFER });
    ...
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    const countResult = spawnSync("git", ["rev-list", "--left-right", "--count", `${compareRef}...HEAD`], { encoding: "utf8", maxBuffer: SPAWN_MAX_BUFFER });
...
function fetchPendingCI(owner, repo) {
    const runs = [];
    for (const status of ["in_progress", "queued"]) {
        ...
        const parsed = parseJsonOrExit(stdout, `fetchPendingCI(${status})`);
...
function parseArgs(argv) {
    ...
    const requirePositiveInt = (flag, v) => {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--owner") {
            ...
            process.stdout.write("Usage: refresh-worldview.ts [--owner X] [--repo Y]\n" +
...
function buildSummary(openPRs, recentMerges, openIssues, gitState, backlogDelta, claims, branchState, pendingCI) {
    const parts = [];
    ...
    if (branchState.behind > 0)
        parts.push(`${branchState.behind} behind`);
    if (branchState.ahead > 0)
        parts.push(`${branchState.ahead} ahead`);
    return parts.join(", ");
}
// --- Main ---
export function main(argv) {
    ...
    const summary = buildSummary(openPRs, recentMerges, openIssues, gitState, backlogDelta, claims, branchState, pendingCI);
    const snapshot = {
        owner: args.owner,
        repo: args.repo,
