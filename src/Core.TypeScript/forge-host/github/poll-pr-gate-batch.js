...
import { allActiveClaims } from "../../bus/claim.js";
import { PriorityFerryThrottlerWithResult } from "../../ferry-throttler/priority-ferry-throttler-with-result.js";
import { readPRState } from "../../observe/world-infra.js";
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--owner") {
            ...
            process.stdout.write("Usage: poll-pr-gate-batch.ts <PR1> <PR2> ...\n" +
...
function pollOne(prNumber, owner, repo) {
...
export async function pollAllBounded(prs, owner, repo, concurrency, pollFn = pollOne, urgentPrs) {
    ...
    const resultPromises = prs.map((pr) => {
...
export async function main(argv, busClaimsFn = allActiveClaims, pollFn = pollOne) {
