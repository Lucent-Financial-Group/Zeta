...
import { computeMetrics, computeSinceFromWindow, formatResult, interpretMetrics, parseArgs, } from "./accelerator-measure";
describe("parseArgs", () => {
...
describe("computeMetrics", () => {
    const baseWindow = { humanReadable: "test", since: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), author: "any" };
    ...
    test("all merged → compression ratio 1.0", () => {
        const prs = [
            { number: 1, state: "MERGED", createdAt: "2026-05-28T00:00:00Z", mergedAt: "2026-05-28T01:00:00Z", closedAt: null },
            { number: 2, state: "MERGED", createdAt: "2026-05-28T00:00:00Z", mergedAt: "2026-05-28T01:00:00Z", closedAt: null },
        ];
    ...
    test("half merged half closed-no-merge → compression ratio 0.5", () => {
        const prs = [
            { number: 1, state: "MERGED", createdAt: "2026-05-28T00:00:00Z", mergedAt: "2026-05-28T01:00:00Z", closedAt: null },
            { number: 2, state: "CLOSED", createdAt: "2026-05-28T00:00:00Z", mergedAt: null, closedAt: "2026-05-28T01:00:00Z" },
        ];
        const metrics = computeMetrics(prs, baseWindow);
    ...
    test("open PRs don't affect compression ratio", () => {
        const prs = [
            { number: 1, state: "MERGED", createdAt: "2026-05-28T00:00:00Z", mergedAt: "2026-05-28T01:00:00Z", closedAt: null },
            { number: 2, state: "OPEN", createdAt: "2026-05-28T00:00:00Z", mergedAt: null, closedAt: null },
        ];
        const metrics = computeMetrics(prs, baseWindow);
        expect(metrics.merged).toBe(1);
    ...
    test("throughputPerHour computed correctly", () => {
        const prs = [
            { number: 1, state: "MERGED", createdAt: "2026-05-28T00:00:00Z", mergedAt: "2026-05-28T01:00:00Z", closedAt: null },
            { number: 2, state: "MERGED", createdAt: "2026-05-28T00:00:00Z", mergedAt: "2026-05-28T02:00:00Z", closedAt: null },
        ];
        const metrics = computeMetrics(prs, baseWindow);
