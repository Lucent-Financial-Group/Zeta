...
export function computeSinceFromWindow(window) {
...
export function fetchPRs(window) {
    ...
    try {
        ...
        return { kind: "gh-cli-error", stderr: `JSON parse failed: ${e.message}` };
    }
    // Filter to window
...
export function computeMetrics(prs, window) {
...
export function formatResult(result) {
    switch (result.kind) {
        ...
        case "no-data-in-window":
            return {
                stdout: JSON.stringify({ kind: "no-data-in-window", window: result.window, suggestion: "try a wider window" }, null, 2),
                exitCode: 0,
            };
