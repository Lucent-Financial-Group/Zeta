...
function classifyOverall(indicator) {
...
function classifyDecision(overall, factoryRelevant) {
    if (overall === "outage")
        return "halt";
...
function buildReport(summary) {
    ...
    const factoryNameSet = new Set(FACTORY_RELEVANT_COMPONENTS);
    const factoryRelevant = {};
...
function parseArgs(argv) {
    const out = { quiet: false, strict: false };
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--fixture") {
            ...
            process.stdout.write("Usage: check-github-status.ts [--component <name>] [--quiet] [--strict]\n" +
...
function decisionExitCode(decision) {
    if (decision === "halt")
        return 9;
    if (decision === "proceed-with-verify")
        return 8;
    return 0;
}
