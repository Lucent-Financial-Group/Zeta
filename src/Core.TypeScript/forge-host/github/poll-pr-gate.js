...
function classifyChecks(rollup, filter) {
    let ok = 0;
    let inProgress = 0;
    let pending = 0;
    let failed = 0;
...
function nonRequiredFailures(rollup, requiredNames) {
    const failures = [];
    for (const c of rollup) {
        if (c.name && requiredNames.has(c.name))
            continue;
        if (c.conclusion && BLOCKING_CONCLUSIONS.has(c.conclusion)) {
            if (c.name)
...
function nextAction(report) {
    ...
    if (report.unresolvedThreads > 0)
        return "resolve-threads";
    if (report.requiredChecks.inProgress > 0 ||
        report.requiredChecks.pending > 0) {
        return "wait-ci";
    }
    return "none";
}
export function buildReport(pr) {
...
function fetchPR(owner, repo, number) {
    ...
    const pr = parseJsonOrExit(prStdout, "fetchPR.gh-pr-view");
    ...
    while (hasNextPage) {
        ...
        const parsed = parseJsonOrExit(pageStdout, "fetchPR.gh-graphql-threads.page");
    ...
    const requiredResult = spawnSync("gh", [
        "pr",
        "checks",
        String(number),
        "--repo",
    ...
    if (requiredStatus === 0 || requiredStatus === 4 || requiredStatus === 8) {
        try {
            const required = JSON.parse(requiredResult.stdout);
    ...
    const prNarrowed = pr;
    const rollup = (prNarrowed.statusCheckRollup ?? []);
    const result = {
        ...prNarrowed,
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--fixture") {
            ...
            process.stdout.write("Usage: poll-pr-gate.ts <PR_NUMBER> [--owner X] [--repo Y]\n" +
