...
import { appendAttribution } from "./ai-attribution";
...
const NAME_ATTRIBUTION_DIRECT_PATTERNS = [
...
const NAME_ATTRIBUTION_FUZZY_NAME = ["name attribution", "contributor names", "no name"];
const NAME_ATTRIBUTION_FUZZY_RULE = ["rule", "standing", "policy", "conflicts with", "prohibits"];
function classifyBody(bodyLower) {
    ...
    if (NAME_ATTRIBUTION_FUZZY_NAME.some((p) => bodyLower.includes(p)) &&
...
function classifyThreads(threads) {
    ...
    for (const t of threads) {
        ...
        if (classification === "dangling-ref")
            dangling.push(t.id);
        else if (classification === "name-attribution")
            nameAttribution.push(t.id);
        else
            unknown.push(t.id);
    }
...
`;
function resolveThread(threadId, replyBody) {
    ...
    const replyParsed = JSON.parse(replyResult.stdout);
    ...
    const resolveArgs = ["api", "graphql", "-F", `thread_id=${threadId}`, "-f", `query=${RESOLVE_MUTATION}`];
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    ...
    const resolveParsed = JSON.parse(resolveResult.stdout);
...
function emitSummary(args) {
    ...
    if (c.truncationWarnings > 0) {
        process.stderr.write(`warning: ${String(c.truncationWarnings)} thread(s) have >50 comments; only first 50 inspected for classification\n`);
    }
    process.stdout.write(`PR #${String(args.prNumber)} (${args.owner}/${args.name}) unresolved thread classification:\n` +
...
function applyResolutions(classified) {
