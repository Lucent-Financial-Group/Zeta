...
// PR. Sibling to the iter-5.4.1 (B-0812) self-registration flow that
// CREATES that tree.
//
...
// Tracks as B-0814; this tool is the implementation.
//
// Usage:
...
//   3 — git/push/gh error
//
// Composes with B-0812 (iter-5.4.1 self-registration; this tool is the
// inverse) + B-0813 (iter-5.4.2 ArgoCD reconciliation; ArgoCD will
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--host") {
            const v = argv[i + 1];
            if (!v || v.startsWith("-"))
                return { error: "--host requires a value" };
            host = v;
            i++;
        }
            ...
            const v = argv[i + 1];
            if (!v || v.startsWith("-"))
                return { error: "--maintainer requires a value" };
            maintainer = v;
            i++;
        }
        else if (a === "--reason") {
            ...
            return {
                error: "Usage: bun tools/cluster/deregister-node.ts --host <hostname> " +
                    "[--maintainer <name>] [--reason \"...\"] [--push-direct]",
            };
        }
        else {
    ...
    if (host === "")
        return { error: "--host <hostname> is required" };
    ...
    if (!/^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$/.test(host)) {
        return {
            error: `--host '${host}' is not a valid DNS label hostname. ` +
...
function nodeExistsOnMain(operator, host) {
...
function main() {
    ...
    if (maintainer === null) {
        ...
        if (maintainer === null) {
            process.stderr.write("deregister-node: could not resolve operator via 'gh api /user'. " +
    ...
    if (!nodeExistsOnMain(maintainer, host)) {
        process.stderr.write(`deregister-node: maintainers/${maintainer}/cluster-nodes/${host}/ not found on origin/main; ` +
    ...
    // Step 6: commit
    const commitMsg = `deregister(cluster-node): ${host} — ${reason || "no reason given"}\n\n` +
        `Removes maintainers/${maintainer}/cluster-nodes/${host}/ subtree.\n` +
        `Sibling to iter-5.4.1 self-registration flow (B-0812 substrate).\n` +
    ...
    if (!pushDirect) {
        ...
        const body = `Removes \`maintainers/${maintainer}/cluster-nodes/${host}/\` subtree.\n\n` +
            `${reason ? `**Reason**: ${reason}\n\n` : ""}` +
            `On merge, ArgoCD reconciles the K8s state to drop the node's labels + taints + role-specific workload memberships.\n\n` +
            `Generated by \`tools/cluster/deregister-node.ts\` (B-0814 substrate; sibling to iter-5.4.1 self-registration).\n`;
        const prCreate = run("gh", ["pr", "create", "--head", branch, "--base", "main", "--title", title, "--body", body], wt);
        if (!prCreate.ok) {
