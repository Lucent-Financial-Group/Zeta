...
import { DEV_CLUSTER_SUBSTRATE_DIR } from "./lib.js";
export function bringUpKindCiCluster(ports, options) {
    ...
    if (localCluster.list().includes(options.clusterName)) {
        console.log(`Cluster ${options.clusterName} already exists. Use bun src/Core.TypeScript/cluster/dev-cluster/kind-down.ts --cluster-name ${options.clusterName} to recreate.`);
    }
    else {
    ...
    controlPlane.applyRemoteManifest("https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.2.0/standard-install.yaml", true);
...
export function tearDownKindCluster(ports, clusterName) {
...
export function bringUpK3dDevCluster(ports, options) {
    const { localCluster, controlPlane, packages, appCatalog } = ports;
    const context = localCluster.contextName(options.clusterName);
    if (localCluster.list().includes(options.clusterName)) {
        console.log(`Cluster ${options.clusterName} already exists. Use bun src/Core.TypeScript/cluster/dev-cluster/k3d-down.ts --config ${options.configPath} to recreate.`);
    }
    else {
...
export function tearDownK3dDevCluster(ports, clusterName) {
