 ...
 *
 * This is a TypeScript string on purpose. The in-cluster payload is still
 * busybox ash (ConfigMap `serve.sh`, image `busybox:1.37.0`), but a repo
 * `.sh` is a new bash-retirement inventory entry. The inventory exists so
 * that surface does not grow. The pod has no bun; the checkout must not
 * grow a shell entrypoint to feed it.
 *
 ...
 */
export const LANE_TREE_SERVE_SH =
  [
    "#!/bin/sh",
    "PORT=8080",
    "ROOT=/srv/tree.git",
    ...
    '    if ! read -r sha _unused < "$ROOT/info/refs"; then',
    ...
    '      if pack_size=$(wc -c < "$f" 2>/dev/null); then',
    "        pack=$f",
    "        break",
    "      fi",
    "    done",
    ...
    "      exit 0",
    "    fi",
    "    pack_size=$(printf '%s' \"$pack_size\" | tr -d ' \\t\\n')",
