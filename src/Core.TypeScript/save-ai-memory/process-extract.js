...
function parseArgs(argv) {
    ...
    if (!args.aiName || !args.platform || !args.topic) {
        console.error("Missing required args. Need: --ai-name <name> --platform <grok|chatgpt|claudeai|gemini|deepseek> --topic <slug>");
        printHelp();
        process.exit(1);
    }
    return args;
...
function extractText(raw, platform) {
...
function tryPlatformExtractor(parsed, _platform) {
    const candidates = [];
    if (parsed && typeof parsed === "object") {
        ...
        if (conv) {
            ...
            if (Array.isArray(conv.chat_messages))
                candidates.push(conv.chat_messages);
            if (Array.isArray(conv.responses))
                candidates.push(conv.responses);
        }
        if (Array.isArray(root.messages))
            candidates.push(root.messages);
        if (Array.isArray(root.responses))
            candidates.push(root.responses);
...
function getPersonaPath(aiName) {
...
function generateOutputPath(args, isoDate) {
...
function buildArchive(args, extractedBody, sourceKind) {
    ...
    const extractionDesc = sourceKind === "json"
...
function gitCommit(filePath, aiName, topic) {
    const today = new Date().toISOString().slice(0, 10);
    ...
    const message = "feat(save-ai-memory): §33 archive " +
        aiName +
        " " +
        topic +
    ...
    console.error('  gh pr create --base main --head ' +
        branchName +
        ' --title "..." --body "..."');
...
async function main() {
    ...
    if (args.commit) {
        ...
        const relPath = relative("memory", personaPath).replace(/\\/g, "/");
        console.error("\nNext steps:");
        console.error("  1. Update memory/" +
            relPath +
            "/MEMORY.md with pointer to " +
            outputPath);
        console.error("  2. Optionally update memory/" +
            relPath +
