...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        function parseIntOrDie(name, raw) {
            if (!/^[0-9]+$/.test(raw)) {
                console.error(`Invalid value for ${name}: "${raw}" (expected positive integer with no trailing chars; saw partial-numeric or non-numeric input)`);
                process.exit(1);
            }
        ...
        switch (a) {
            ...
            case "--help":
            case "-h":
                console.error(`Usage: bun ${argv[1] ?? "extract-grok-conversation.ts"} [options]\n\n` +
                    "Options:\n" +
...
function log(cfg, msg) {
    if (cfg.logToStderr)
        console.error(`[extract-grok] ${msg}`);
}
function sleep(ms) {
...
let secureTmpDir = null;
function getSecureTmpDir() {
    if (secureTmpDir === null) {
        secureTmpDir = mkdtempSync(join(tmpdir(), "extract-grok-"));
        process.on("exit", () => {
            try {
                rmSync(secureTmpDir, { recursive: true, force: true });
            }
            catch {
...
function escapeAppleScriptString(s) {
    if (s.includes("\n") || s.includes("\r")) {
        throw new Error("AppleScript string literal cannot contain raw newlines (would break the source); refusing to interpolate");
    }
 ...
 */
function runJs(cfg, js, timeoutSec = 60, treatErrorPrefixAsAbort = true) {
...
async function main() {
    ...
    const JS_SCROLL_TOP_AND_HEIGHT = '(function() { var c = document.querySelector("div.w-full.h-full.overflow-y-auto.overflow-x-hidden"); if (!c) return "ERROR: container not found"; c.scrollTop = 0; return c.scrollHeight.toString(); })()';
    ...
    const JS_SCROLL_HEIGHT = 'document.querySelector("div.w-full.h-full.overflow-y-auto.overflow-x-hidden").scrollHeight.toString()';
