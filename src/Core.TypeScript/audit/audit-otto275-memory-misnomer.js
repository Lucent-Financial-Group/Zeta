#!/usr/bin/env bun
/**
 * 081KDVHQA5008QG0R0018D0MX2 smallest safe slice: Pure-TS audit of references to the
...
function walk(dir, files = []) {
    ...
    for (const e of entries) {
        if (e === 'node_modules' || e === '.git' || e === 'bin' || e === 'obj')
            continue;
        const p = join(dir, e);
        const st = statSync(p);
        if (st.isDirectory())
            walk(p, files);
        else if (['.md', '.ts', '.fs', '.cs', '.json'].includes(extname(e)))
            files.push(p);
    }
    return files;
}
...
console.log(JSON.stringify({
    slice: '081KDVHQA5008QG0R0018D0MX2',
    oldName: OLD_NAME,
    framing: OLD_FRAMING,
