...
function classifyCommit(files) {
    ...
    if (src > 0)
        return "EXT ";
    if (meta > 0 && research <= meta)
        return "INTL";
    if (research > 0)
        return "SPEC";
    return "OTHR";
}
function tallyCommits(shas) {
    let ext = 0;
    let intl = 0;
    let spec = 0;
    let other = 0;
...
function emitReport(counts, window, threshold) {
    ...
    for (const line of counts.lines)
