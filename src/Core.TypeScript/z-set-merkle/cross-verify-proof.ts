...
import { ofBytes, toHex, type MerkleHash } from "../merkle/merkle";
...
const encodeKey = (s: string): Uint8Array => encoder.encode(s);
...
function combine(a: MerkleHash, b: MerkleHash): MerkleHash {
...
interface Proof {
  leafKeyBytes: Uint8Array;
  leafWeight: bigint;
  steps: Step[];
}

/** Net-consolidate raw entries into a canonical, byte-sorted, weight-nonzero run. */
function consolidate(entries: VectorEntry[]): Array<{ kb: Uint8Array; w: bigint }> {
  const acc = new Map<string, { kb: Uint8Array; w: bigint }>();
...
function proofFor(entries: VectorEntry[], probe: string): Proof | null {
  ...
  let level: MerkleHash[] = sorted.map((e) => ofBytes(leafBytes(e.kb, e.w)));
  const steps: Step[] = [];
  let idx = leafIdx;
...
function rootOfProof(p: Proof): MerkleHash {
...
function root(entries: VectorEntry[]): MerkleHash {
  const sorted = consolidate(entries);
  if (sorted.length === 0) return ofBytes(new Uint8Array(0));
  let level: MerkleHash[] = sorted.map((e) => ofBytes(leafBytes(e.kb, e.w)));
  while (level.length > 1) {
    const parents: MerkleHash[] = [];
...
function proofString(rootHex: string, p: Proof): string {
...
const yamlPath = join(
  import.meta.dirname,
  "../../../tests/cross-verification/zset-merkle-proof/vectors.yaml",
);
...
const outputPath = join(
  import.meta.dirname,
  "../../../tests/cross-verification/zset-merkle-proof/ts-output.json",
);
