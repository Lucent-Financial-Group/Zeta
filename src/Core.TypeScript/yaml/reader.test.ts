...
import type { YamlEvent } from "./reader";
...
test("decline: unterminated double quote -> UnterminatedQuote", () => {
  ...
  expect(result.ok).toBe(false);
  if (!result.ok) expect(result.feedback).toBe("UnterminatedQuote");
});
...
test("decline: anchor value -> UnsupportedConstruct", () => {
  ...
  expect(result.ok).toBe(false);
  if (!result.ok) expect(result.feedback).toBe("UnsupportedConstruct");
});
...
test("decline: unknown double-quote escape -> UnexpectedCharacter", () => {
  ...
  expect(result.ok).toBe(false);
  if (!result.ok) expect(result.feedback).toBe("UnexpectedCharacter");
});

test("tryReadEvents ok path mirrors readEvents", () => {
  const result = tryReadEvents("name: zeta\n");
  expect(result.ok).toBe(true);
  if (result.ok) expect(result.events).toEqual(readEvents("name: zeta\n"));
});
