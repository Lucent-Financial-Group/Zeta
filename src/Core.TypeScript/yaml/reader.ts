...
export type ScalarKind = "Null" | "Bool" | "Int" | "Float" | "Str";

export type ScalarStyle = "Plain" | "SingleQuoted" | "DoubleQuoted";

export type YamlEvent =
  | { e: "StreamStart" }
  | { e: "StreamEnd" }
  | { e: "MappingStart" }
  | { e: "MappingEnd" }
  | { e: "SequenceStart" }
  | { e: "SequenceEnd" }
  | { e: "Scalar"; raw: string; kind: ScalarKind; style: ScalarStyle };

export type YamlFeedback =
  | "TabIndentation"
  | "UnterminatedQuote"
  | "UnexpectedCharacter"
  | "UnexpectedIndent"
...
function resolvePlainKind(raw: string): ScalarKind {
  if (NULL_LITERALS.has(raw)) return "Null";
  if (BOOL_LITERALS.has(raw)) return "Bool";
  if (INT_RE.test(raw)) return "Int";
...
function stripTrailingComment(token: string): string {
...
function decodeSingleQuoted(token: string): string {
  // token[0] === "'"
  let out = "";
...
function decodeDoubleQuoted(token: string): string {
  ...
  for (let i = 1; i < token.length; i++) {
    ...
    if (ch === "\\") {
      ...
      if (next === undefined) throw new DeclineError("UnterminatedQuote");
      switch (next) {
        case "\\":
          out += "\\";
          break;
...
function parseValue(token: string): ParsedScalar {
  const first = token[0];
  if (first === '"') {
...
type ContainerKind = "Mapping" | "Sequence";
...
function toContentLines(text: string): ContentLine[] {
  ...
  for (let raw of text.split("\n")) {
    ...
    if (body.length === 0) continue; // blank line — never a tab-indent error
    if (body[0] === "#") continue; // full-line comment
    if (sawTab) throw new DeclineError("TabIndentation");
...
function splitMappingEntry(text: string): { key: string; value: string | null } | null {
  let inSingle = false;
  let inDouble = false;
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === ":" && (text[i + 1] === " " || text[i + 1] === undefined)) {
      ...
      if (value.length === 0 || value.startsWith("#")) return { key, value: null };
      return { key, value };
    }
  }
  return null;
...
function scan(text: string): YamlEvent[] {
  const lines = toContentLines(text);
  const events: YamlEvent[] = [{ e: "StreamStart" }];
  const stack: Frame[] = [];

  const pushContainer = (indent: number, kind: ContainerKind): void => {
    stack.push({ indent, kind });
    events.push({ e: kind === "Mapping" ? "MappingStart" : "SequenceStart" });
  };

  const popGreaterThan = (indent: number): void => {
  ...
  const childIndentAt = (idx: number): number => {
  ...
  const peekChildKind = (idx: number, childIndent: number): ContainerKind => {
    const next = lines[idx + 1];
    if (!next || next.indent !== childIndent) return "Mapping";
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (isDocumentMarker(body)) throw new DeclineError("UnsupportedConstruct");
    ...
    if (stack.length === 0) {
      ...
      if (!top || top.indent !== indent) throw new DeclineError("UnexpectedIndent");
      if (isSeqItem !== (top.kind === "Sequence")) throw new DeclineError("UnexpectedIndent");
    }
...
export function tryReadEvents(text: string): ReadResult {
  try {
    return { ok: true, events: scan(text) };
  } catch (err) {
    if (err instanceof DeclineError) return { ok: false, feedback: err.feedback };
    throw err;
  }
}

export function readEvents(text: string): YamlEvent[] {
  const result = tryReadEvents(text);
  if (!result.ok) throw new DeclineError(result.feedback);
