...
import { audit } from "./audit_clause_coverage.js";
...
function defaultBranch() {
    const result = spawnSync("git", // eslint-disable-line sonarjs/no-os-command-from-path
    ["rev-parse", "--abbrev-ref", "origin/HEAD"], { encoding: "utf8" });
...
function parseArgs(argv) {
    ...
    while (i < argv.length) {
        ...
        if (arg === "--base") {
            const next = argv[i + 1];
            if (next === undefined)
                return { kind: "error", message: "audit_clause_drift: --base requires a ref" };
            state.base = next;
            i += 2;
            continue;
        }
        if (arg === "--head") {
            const next = argv[i + 1];
            if (next === undefined)
                return { kind: "error", message: "audit_clause_drift: --head requires a ref" };
            state.head = next;
            i += 2;
            continue;
        }
...
function computeDrift(baseRef, headRef) {
...
export function main(argv) {
    ...
    if (parsed.kind === "help") {
        process.stdout.write("Usage: audit_clause_drift.ts [--base REF] [--head REF] [--json | --md]\n" +
