...
function readDirentsOrEmpty(dir) {
    try {
...
function isExcludedReferences(full, refsRoot) {
...
// the resulting paths.
function listMarkdownFilesUnder(targetPath) {
...
export function audit(targetPath) {
...
function writePerFileJson(outDir, targetPath, finding) {
...
function emitJsonRollup(args, total, filesOk, filesMissing) {
    const payload = {
        tool: "audit_archive_headers",
        v: 0,
...
function emitHumanSummary(targetPath, total, filesOk, filesMissing, findings) {
...
export function main(argv) {
    ...
    if (args.json)
        emitJsonRollup(args, total, filesOk, filesMissing);
    else
        emitHumanSummary(args.targetPath, total, filesOk, filesMissing, result.findings);
    if (filesMissing > 0 && args.enforce)
        return 1;
    return 0;
}
if (import.meta.main) {
