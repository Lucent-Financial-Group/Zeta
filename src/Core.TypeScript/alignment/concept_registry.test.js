...
import { buildRegistry, extractByRegex, extractGlassHaloDoctrines, } from "./concept_registry.js";
describe("extractByRegex", () => {
    test("extracts alignment clause IDs from the canonical source", () => {
        const concepts = extractByRegex("docs/ALIGNMENT.md", /\b(HC-[1-7]|SD-[1-9]|DIR-[1-5])\b/g, "alignment-clause");
    ...
    test("deduplicates repeated best-practice IDs", () => {
        const concepts = extractByRegex("docs/AGENT-BEST-PRACTICES.md", /\b(BP-\d+)\b/g, "best-practice");
...
describe("buildRegistry", () => {
    test("builds a schema-v1.1 registry with all expected concept classes", () => {
        ...
        expect(Object.values(registry.summary).reduce((sum, count) => sum + count, 0)).toBe(registry.concepts.length);
        expect(registry.anchoredCount).toBe(registry.concepts.filter((concept) => concept.anchorState && concept.anchorState !== "factory-native").length);
        expect(registry.concepts.every((concept) => concept.id && concept.conceptClass && concept.source && concept.label)).toBe(true);
