...
function parseArgs(argv) {
    ...
    while (i < argv.length) {
        ...
        if (arg === "--out") {
            const next = argv[i + 1];
            if (next === undefined)
                return { kind: "error", message: "audit_clause_coverage: --out requires a directory" };
            state.outDir = next;
            i += 2;
            continue;
        }
        if (arg === "--gate") {
            const next = argv[i + 1];
            if (next === undefined)
                return { kind: "error", message: "audit_clause_coverage: --gate requires a number" };
            const n = Number(next);
...
export function extractClauses(content) {
    const found = new Set();
    ...
    if (args.gate !== null) {
        ...
        if (failing.length > 0) {
            for (const f of failing) {
                process.stderr.write(`audit_clause_coverage: ${f.name} (${f.kind}) has ${String(f.clauseCount)} clauses < gate ${String(gateN)}\n`);
            }
            return 1;
        }
    }
