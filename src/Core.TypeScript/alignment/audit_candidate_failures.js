...
import { logFilePath } from "./filter_gate_log.js";
...
function nonEmptyString(entry, key) {
...
function readDecision(entry) {
    ...
    if (typeof value !== "string")
        return null;
    return VALID_DECISIONS.includes(value) ? value : null;
}
function readClauses(entry) {
...
function finding(line, severity, code, message, candidate, decision) {
    return { line, severity, code, message, candidate, decision };
}
function validateEntry(line, entry) {
    const findings = [];
    ...
    if (entry.schema !== "filter-gate-v1") {
        findings.push(finding(line, "violation", "invalid-schema", "entry schema must be filter-gate-v1", candidate, decision));
    }
    for (const key of ["timestamp", "candidate", "source", "rationale", "author"]) {
        if (nonEmptyString(entry, key) === null) {
            findings.push(finding(line, "violation", `missing-${key}`, `${key} must be a non-empty string`, candidate, decision));
        }
    }
    if (decision === null) {
        findings.push(finding(line, "violation", "invalid-decision", "decision must be pass, fail, or defer", candidate, null));
    }
    if (clauses === null) {
        findings.push(finding(line, "violation", "invalid-clauses", "clauses must be an array of non-empty strings", candidate, decision));
    }
    else if ((decision === "fail" || decision === "defer") && clauses.length === 0) {
        findings.push(finding(line, "warning", "non-pass-without-clauses", "fail/defer entry has no clause IDs; make the non-adoption rationale reconstructable", candidate, decision));
    }
    return { decision, findings };
}
export function auditCandidateFailures(path) {
    const findings = [];
    let totalLines = 0;
    let totalEntries = 0;
    let passEntries = 0;
...
function emitHuman(audit) {
    const lines = [];
    lines.push(`candidate-failure-audit entries=${String(audit.totalEntries)} nonpass=${String(audit.nonPassEntries)} violations=${String(audit.violations)} warnings=${String(audit.warnings)}`);
...
function emitMarkdown(audit) {
    ...
    lines.push("| --- | --- | --- | --- | --- | --- |");
    for (const f of audit.findings) {
        lines.push(`| ${String(f.line)} | ${f.severity} | ${f.code} | ${f.candidate ?? ""} | ${f.decision ?? ""} | ${f.message} |`);
    }
    return lines.join("\n");
}
const HELP = `Usage:
