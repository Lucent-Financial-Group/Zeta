...
export function parseArgs(argv) {
    ...
    while (i < argv.length) {
        ...
        if (arg === "--gate") {
            const next = argv[i + 1];
            if (next === undefined)
                return { kind: "error", message: "audit_retractibility: --gate requires a number" };
            const n = Number(next);
...
function isGitTracked(filePath) {
    const result = spawnSync("git", // eslint-disable-line sonarjs/no-os-command-from-path
    ["ls-files", "--error-unmatch", filePath], { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] });
    return result.status === 0;
}
export function countInboundRefs(filePath, root) {
    const relPath = relative(root, `${root}/${filePath}`);
    const result = spawnSync("git", // eslint-disable-line sonarjs/no-os-command-from-path
    ["grep", "-l", "--fixed-strings", relPath], { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] });
    const lines = (result.stdout ?? "").trim().split("\n").filter((l) => l.length > 0);
    const selfExcluded = lines.filter((l) => l !== relPath).sort();
    return { count: selfExcluded.length, from: selfExcluded };
}
function classifyStatus(gitTracked, inboundRefs) {
    if (!gitTracked)
        return "untracked";
    if (inboundRefs >= ENTANGLEMENT_THRESHOLD)
        return "entangled";
    return "retractible";
}
...
export function auditRetractibility(paths) {
    const root = process.cwd();
    const surfaces = [];
    for (const p of paths) {
        const gitTracked = isGitTracked(p);
        const { count, from } = gitTracked ? countInboundRefs(p, root) : { count: 0, from: [] };
...
export function main(argv) {
    ...
    if (parsed.kind === "help") {
        process.stdout.write("Usage: audit_retractibility.ts [--json | --md] [--gate N] [path ...]\n" +
    ...
    if (args.gate !== null) {
        ...
        if (failing.length > 0) {
            for (const f of failing) {
                process.stderr.write(`audit_retractibility: ${f.name} (${f.kind}) has ${String(f.inboundRefs)} inbound refs >= gate ${String(gateN)}\n`);
            }
            return 1;
        }
    }
