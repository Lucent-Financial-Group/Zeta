...
function startsFenceDelimiter(line) {
    const trimmed = line.trimStart();
    const first = trimmed.at(0);
    if (first !== "`" && first !== "~")
        return false;
...
function stripMarkdownStructuralPrefix(text) {
...
function isSupersededByMarkerLine(text) {
    return /^\*\*Superseded by\*\*(?:\s|$)/i.test(stripBlockquotePrefix(text));
}
function pushUnique(claims, seen, claim) {
...
function pushMatchesForLine(regex, line, lineNumber, claims, seen) {
    regex.lastIndex = 0;
    let match;
...
function logicalSearchLines(lines) {
    const searchLines = [];
    const fence = { char: null, len: 0 };
    const physicalLines = [];
    for (let i = 0; i < lines.length; i++) {
        ...
        if (trimmed.length > 0)
            physicalLines.push({ line: i + 1, text: trimmed });
    }
...
function resolveTarget(target, fileDir, fileParentDir, repoRoot) {
    if (!isRepoRelativeTarget(target))
        return null;
...
function supersededByMarkerLines(targetContent) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!isSupersededByMarkerLine(line))
            continue;
...
function markerNamesSupersedingAdr(markerLines, supersedingFile) {
...
function checkFile(filePath) {
    ...
    for (const claim of claims) {
        ...
        if (resolved === null)
            continue; // existence drift belongs to check-existence.ts
