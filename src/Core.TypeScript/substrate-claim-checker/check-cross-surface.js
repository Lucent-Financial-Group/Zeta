...
import { findTables } from "./check-counts.js";
const TRACKED_NOUNS = [
    "drift instances",
    "drift instance",
    "rows",
...
export function extractClaims(text) {
    ...
    for (const m of text.matchAll(pattern)) {
        ...
        const plus = m[2];
        const noun = m[3];
        if (numStr === undefined || noun === undefined)
            continue;
        claims.push({
            raw: m[0] ?? "",
...
export function checkFile(filePath) {
    ...
    try {
        ...
        if (code === "ENOENT") {
            ...
            console.error(`error: read failed for ${filePath}: ${err.message}`);
        }
...
export function main() {
    ...
    for (const arg of args) {
        ...
        for (const f of findings) {
            ...
            console.log(`${f.file}: cross-surface count drift — frontmatter.${f.field} claims "${f.claim}" (expected ${op} ${f.claimedCount}); body tables have [${actual}] rows`);
            totalFindings++;
        }
    }
    if (inputErrors > 0) {
