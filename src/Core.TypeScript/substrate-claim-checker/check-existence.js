...
function findRepoRoot(startPath) {
    ...
    if (statExists(cur).isFile)
        cur = dirname(cur);
    while (cur !== "/" && cur !== "") {
        const gitPath = join(cur, ".git");
        if (statExists(gitPath).exists)
            return cur;
...
function looksLikePath(s) {
    ...
    if (isAbsolute(s))
        return false;
    if (/^[A-Za-z]:[\\/]/.test(s))
        return false; // Windows drive (C:\, C:/)
    if (/^\\\\/.test(s))
        return false; // Windows UNC (\\server\share)
    if (/^\$/.test(s))
        return false;
    ...
    const knownExt = /\.(md|markdown|ts|tsx|js|jsx|fs|fsi|fsx|fsproj|csproj|sln|sh|bash|zsh|yaml|yml|json|toml|tla|alloy|lean|lean4|py|rs|go|java|kt|kts|c|h|cpp|cc|hpp|fish|gradle|conf|ini|cfg|env|html|css|scss|less|sql|graphql|proto)$/i;
    if (/^\.{0,2}\//.test(s))
        return true;
    if (s.includes("/") && !/^\d+(\.\d+)+(-[\w.]+)?$/.test(s))
        return true;
    if (knownExt.test(s))
        return true;
    return false;
}
function isFutureStateContext(line, contextBefore, contextAfter) {
...
export function main() {
    ...
    if (totalDrift > 0 || totalWarnings > 0) {
        const parts = [];
        if (totalDrift > 0)
            parts.push(`${totalDrift} drift finding(s)`);
        if (totalWarnings > 0)
