...
function parseArgs() {
    ...
    const parseNonNegativeNumber = (flag, raw) => {
        const n = Number(raw);
    ...
    const maxLineBytes = parseNonNegativeNumber("--max-line-bytes", get("--max-line-bytes", String(DEFAULT_MAX_LINE_BYTES)));
...
function scrub(blocks, maxImageBytes) {
...
function annotate(container, stamp, dropped, bytes) {
    const tag = ` [${dropped} image(s) stripped ${stamp}: ~${Math.round(bytes / 1024).toLocaleString()} KB base64 removed to recover session]`;
...
function processLine(raw, stamp, maxImageBytes) {
    let obj;
    try {
        obj = JSON.parse(raw);
    }
    ...
    let keptImages = 0;
    const droppedSizes = [];
    let firstModified; // track which container actually lost images
...
function scanFile(path, maxLineBytes) {
    const flagged = [];
    ...
    for (let i = 0; i <= buf.length; i++) {
        if (i === buf.length || buf[i] === 0x0a) {
            ...
            const len = i - start;
            if (len > maxLineBytes)
                flagged.push({ lineNo, length: len });
            start = i + 1;
        }
    }
    return flagged;
...
function runScan(args) {
    ...
    console.log(`scanning ${sessions.length} session(s) in ${dir} (threshold ${args.maxLineBytes.toLocaleString()} bytes)`);
    let anyFlagged = 0;
    for (const f of sessions) {
        ...
        console.log(`    repair: bun tools/claude-code-recovery/repair-jsonl-strip-images.ts --session ${f.replace(/\.jsonl$/, "")} --apply`);
    }
    if (anyFlagged === 0) {
...
function runRepair(args) {
    ...
    if (reports.length === 0) {
        console.log(`no oversize images found in ${linesInspected} oversize line(s) (line threshold ${args.maxLineBytes.toLocaleString()} bytes, image threshold ${args.maxImageBytes.toLocaleString()} bytes)`);
        return 0;
    }
    console.log(`${args.apply ? "stripping" : "would strip"} from ${reports.length} line(s) (image threshold ${args.maxImageBytes.toLocaleString()} bytes):`);
    for (const r of reports) {
        ...
        console.log(`  line ${r.lineNo}: dropped ${r.drops} oversize image(s) [${sizesMb}], ~${Math.round(r.bytes / 1024).toLocaleString()} KB freed${keptNote}`);
    }
    ...
    for (let i = 0; i < out.length; i++) {
        ...
        try {
            ...
            bad++;
            console.error(`BAD line ${i + 1} after edit: ${e.message.slice(0, 100)}`);
        }
    }
    if (bad > 0) {
    ...
    console.log(`size: ${sizeBefore.toLocaleString()} → ${newSize.toLocaleString()} bytes (saves ${(sizeBefore - newSize).toLocaleString()})`);
    if (!args.apply) {
