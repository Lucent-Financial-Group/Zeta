...
import { formatBypassMessage, formatRejectionMessage, KIRO_SUBSTANTIVE_TRIGGERS, peerFirewallCheck, } from "./_firewall";
...
function classifyFlag(a, next, state) {
    ...
    if (a === "--context-cmd") {
        if (next === undefined)
            return { kind: "error", message: "error: --context-cmd requires COMMAND" };
    ...
    if (a === "--output-file") {
        if (next === undefined)
            return { kind: "error", message: "error: --output-file requires PATH" };
        if (next.startsWith("-"))
...
function parseArgs(argv) {
    ...
    while (i < argv.length) {
        ...
        if (step.kind === "help")
            return { help: true };
        if (step.kind === "error")
            return { error: step.message, exitCode: 1 };
        if (step.kind === "stop") {
...
function resolveOutputFile(explicitPath) {
...
function emitHelp() {
    process.stdout.write(`kiro.ts — agent-side caller for invoking Kiro as a peer\n` +
        `specification reviewer via the kiro CLI.\n` +
        `\n` +
        `Usage:\n` +
        `  bun tools/peer-call/kiro.ts "prompt text"\n` +
...
function kiroCliAvailable() {
    const result = spawnSync(
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    KIRO_CLI, ["--help"], { stdio: "ignore" });
    return result.error === undefined;
}
// Confirm the kiro.dev headless AI CLI supports non-interactive chat.
function isKiroHeadlessCli() {
    const result = spawnSync(
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    KIRO_CLI, ["chat", "--help"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
...
function splitContextCmd(input) {
    if (/[\u0000\r\n|&;<>`$]/u.test(input)) {
    ...
    if (escaped)
        current += "\\";
    if (quote.length > 0)
        return { error: "error: --context-cmd has an unterminated quote" };
    if (current.length > 0)
        words.push(current);
    return words;
}
function parseContextCmd(contextCmd) {
    const words = splitContextCmd(contextCmd);
    if ("error" in words)
        return words;
    if (words.length === 0)
        return { error: "error: --context-cmd requires COMMAND" };
...
function runContextCmd(command) {
    const options = {
        encoding: "utf8",
...
function writeOutput(path, data, exclusive) {
    let fd;
    try {
...
function buildFullPrompt(args) {
    ...
    if (args.contextCmd.length > 0) {
        ...
        if ("error" in parsedContextCmd)
            return { ok: false, value: parsedContextCmd.error };
...
export function main(argv) {
    ...
    if (!kiroCliAvailable()) {
        ...
        process.stderr.write("install the kiro.dev headless AI CLI from official docs:\n" +
    ...
    if (!isKiroHeadlessCli()) {
        process.stderr.write(`error: ${KIRO_CLI} does not expose Kiro headless chat mode\n` +
    ...
    const result = spawnSync(
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    KIRO_CLI, ["chat", "--no-interactive", "--trust-all-tools", fullPromptResult.value], {
