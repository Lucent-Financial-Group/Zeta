...
const VALID_ACTORS = ["vera", "amara", "ani", "otto", "aaron"];
const VALID_KINDS = ["ingress", "egress"];
const VALID_DOCTRINE = ["mechanism-only", "identity-doctrine-changing"];
function repoRoot() {
...
function parseArgs(argv) {
    ...
    if (files.length === 0)
        missing.push("--files");
    if (!oldCommitProvided)
        missing.push("--old-commit");
    if (newCommit === null || newCommit.length === 0)
        missing.push("--new-commit");
    if (testEvidence === null)
        missing.push("--test-evidence");
    if (doctrineClass === null)
        missing.push("--doctrine-class");
    if (summary === null)
    ...
    const root = repoRoot();
    const finalPath = receiptsPath !== null
...
function checkNoCollision(path, receiptId) {
    if (!existsSync(path))
        return;
    ...
    if (contents.includes(needle)) {
        throw new Error(`receipt_id ${receiptId} already present in ${path}; refusing to append (this would violate the no-overwrite invariant). Re-run with --receipt-id <unique-id> if this is intentional, or vary one of (ts|actor|kind|files|new_commit|summary) so the content-hash differs.`);
    }
}
function main() {
    ...
    try {
        ...
        process.stderr.write(`append-identity-receipt: ${e.message}\n\n${usage()}`);
        return 1;
    }
    if ("help" in parsed) {
    ...
    try {
        ...
        process.stderr.write(`append-identity-receipt: append failed for ${parsed.receiptsPath}: ${e.message}\n`);
        return 2;
    }
