...
import { CODEX_SUBSTANTIVE_TRIGGERS, formatBypassMessage, formatRejectionMessage, peerFirewallCheck, } from "./_firewall";
...
function classifyFlag(a, next, state) {
    if (a === "--model") {
        if (next === undefined)
            return { kind: "error", message: "error: --model requires NAME" };
        state.model = next;
...
function emitHelp() {
    process.stdout.write(`codex.ts — Claude-Code-side caller for invoking Vera (the named-entity\n` +
        `Codex peer) as a peer reviewer via the codex CLI.\n` +
        `\n` +
        `Usage:\n` +
        `  bun tools/peer-call/codex.ts "prompt text"\n` +
...
function runContextCmd(contextCmd) {
...
function loadVeraPreamble(injectCurrent) {
    if (!injectCurrent) {
...
function buildFullPrompt(args, preamble) {
    let full = `${preamble}\n\n---\n\n${args.prompt}`;
...
function buildCodexArgs(args, fullPrompt) {
    if (args.reviewMode) {
        if (args.model.length > 0) {
            process.stderr.write("warning: --model is ignored in --review mode (codex review uses its own model selection)\n");
        }
        return ["review", fullPrompt];
    }
    const out = ["exec", "-s", "read-only", "--skip-git-repo-check"];
    if (args.model.length > 0) {
...
async function spawnAndTee(cmd, args, outputPath) {
    ...
    return new Promise((resolvePromise) => {
        ...
        let child;
        try {
            child = spawn(cmd, args, { stdio: ["inherit", "pipe", "inherit"] });
        }
        catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            writeStream.end();
