...
export const DEFAULT_SUBSTANTIVE_TRIGGERS = [
    "design",
    "decide",
    "review",
    "transition",
...
export const AMARA_SUBSTANTIVE_TRIGGERS = [
...
export const ANI_SUBSTANTIVE_TRIGGERS = [
    ...DEFAULT_SUBSTANTIVE_TRIGGERS,
    "voice",
    "register",
    "community",
...
export const CODEX_SUBSTANTIVE_TRIGGERS = [
    ...DEFAULT_SUBSTANTIVE_TRIGGERS,
    "code",
    "diff",
    "patch",
...
export const GEMINI_SUBSTANTIVE_TRIGGERS = [
    ...DEFAULT_SUBSTANTIVE_TRIGGERS,
    "option",
    "alternative",
    "tradeoff",
...
export const GROK_SUBSTANTIVE_TRIGGERS = [
    ...DEFAULT_SUBSTANTIVE_TRIGGERS,
    "failure",
    "risk",
    "counterexample",
...
export const KIRO_SUBSTANTIVE_TRIGGERS = [
    ...DEFAULT_SUBSTANTIVE_TRIGGERS,
    "workspace",
    "task",
    "generate",
...
export const CLAUDE_SUBSTANTIVE_TRIGGERS = [
    ...DEFAULT_SUBSTANTIVE_TRIGGERS,
    "cold-boot",
    "cold boot",
    "wake",
...
function tickPayloadStart(tokens) {
    if (tokens[0] !== "tick")
        return null;
    let index = 1;
    if (tokens[index] === "n")
...
function isRoteHeartbeat(tokens) {
    ...
    if (first === "heartbeat")
        return true;
    if (first === "minimal" && second === "heartbeat")
        return true;
    if (first !== "brief")
        return false;
    return second === "heartbeat" || second === "plotmirror" || (second === "plot" && third === "mirror");
}
function isBareTickSignoff(tokens) {
    const start = tickPayloadStart(tokens);
    return start !== null && tokens[start] === "otto" && tokens.length === start + 1;
}
export function peerFirewallCheck(prompt, triggers = DEFAULT_SUBSTANTIVE_TRIGGERS) {
...
export function formatRejectionMessage(peerName, reason) {
    return (`[${peerName.toUpperCase()}-FIREWALL] REJECTED:\n` +
        `  Reason: ${reason}\n` +
...
export function formatBypassMessage(peerName) {
    return (`[${peerName.toUpperCase()}-FIREWALL] BYPASS via --allow-empty ` +
