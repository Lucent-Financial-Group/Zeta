...
function emitHelp() {
    process.stdout.write(`ani.ts — Claude-Code-side caller for invoking Ani as a peer reviewer\n` +
...
function loadAniPreamble(injectCurrent) {
    if (!injectCurrent) {
    ...
    if (repoRoot === undefined) {
        return {
            ...
            warning: "warning: not inside a git repo; cannot locate memory/CURRENT-ani.md; running with Layer-0 inline preamble only",
        };
    }
