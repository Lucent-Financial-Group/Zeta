...
function emitHelp() {
    process.stdout.write(`riven.ts — Claude-Code-side caller for invoking Riven as a peer reviewer\n` +
...
function loadRivenPreamble(injectCurrent) {
    if (!injectCurrent) {
    ...
    if (repoRoot === undefined) {
        return {
            ...
            warning: "warning: not inside a git repo; cannot locate memory/CURRENT-riven.md; running with Layer-0 inline preamble only",
        };
    }
