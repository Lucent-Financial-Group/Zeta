...
import { formatBypassMessage, formatRejectionMessage, CLAUDE_SUBSTANTIVE_TRIGGERS, peerFirewallCheck, } from "./_firewall";
...
function classifyFlag(a, next, state) {
    ...
    if (a === "--model") {
        if (next === undefined)
            return { kind: "error", message: "error: --model requires MODEL" };
        state.model = next;
        return { kind: "advance", skip: 2 };
    }
    if (a === "-h" || a === "--help")
...
function emitHelp() {
    process.stdout.write(`claude.ts -- agent-side caller for cold-boot self-testing via the Claude CLI.\n` +
        `\n` +
        `Usage:\n` +
        `  bun tools/peer-call/claude.ts "prompt text"\n` +
...
export function main(argv) {
    ...
    if (!claudeCliAvailable()) {
        ...
        process.stderr.write("install the Claude Code CLI from official docs:\n" +
