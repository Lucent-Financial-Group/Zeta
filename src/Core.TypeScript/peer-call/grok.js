...
function emitHelp() {
    process.stdout.write(`grok.ts — Claude-Code-side caller for invoking Grok as a peer\n` +
        `reviewer via cursor-agent.\n` +
        `\n` +
        `Usage:\n` +
        `  bun tools/peer-call/grok.ts "prompt text"\n` +
...
export function main(argv) {
    ...
    const spawnSignal = result.signal ?? null;
    const rawStatus = result.status ?? null;
    ...
    const exitCodeDisplay = spawnSignal !== null
    ...
    const isFailureCase = stdoutBuf.length === 0 &&
        (exitCode !== 0 || spawnError !== undefined || spawnSignal !== null);
    let fileContent;
    if (isFailureCase) {
        const stderrText = stderrBuf.length > 0
