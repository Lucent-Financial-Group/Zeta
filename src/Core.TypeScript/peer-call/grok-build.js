...
function emitHelp() {
    process.stdout.write(`grok-build.ts — Claude-Code-side caller for invoking Grok as a peer\n` +
...
function runContextCmd(contextCmd, maxBytes) {
...
function writeOutputExclusive(path, data) {
...
function writeOutputTruncate(path, data) {
...
export function main(argv) {
    ...
    if (useExplicitPath) {
        try {
            ...
            process.stderr.write(`grok-build.ts: failed to create output dir ${dirname(outPath)}: ${e instanceof Error ? e.message : String(e)}\n`);
            return 1;
        }
    }
    ...
    if (r.error) {
        process.stderr.write(`grok-build.ts: failed to spawn ${GROK_CLI}: ${r.error.message}\n` +
    ...
    try {
        ...
        process.stderr.write(`grok-build.ts: failed to write output file ${outPath}: ${e instanceof Error ? e.message : String(e)}\n`);
        return 1;
    }
