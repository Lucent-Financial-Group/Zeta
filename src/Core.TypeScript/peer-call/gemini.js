...
import { formatBypassMessage, formatRejectionMessage, GEMINI_SUBSTANTIVE_TRIGGERS, peerFirewallCheck, } from "./_firewall";
...
function classifyFlag(a, next, state) {
    if (a === "--model") {
        if (next === undefined)
            return { kind: "error", message: "error: --model requires NAME" }; // exitCode set in parseArgs
        state.model = next;
...
function emitHelp() {
    process.stdout.write(`gemini.ts — Claude-Code-side caller for invoking Gemini as a peer\n` +
        `proposer via the gemini CLI.\n` +
        `\n` +
        `Usage:\n` +
        `  bun tools/peer-call/gemini.ts "prompt text"\n` +
...
if (import.meta.main) {
    void main(process.argv.slice(2)).then((code) => process.exit(code), (err) => {
        const message = err instanceof Error ? err.message : String(err);
