...
import { DEFAULT_SUBSTANTIVE_TRIGGERS, formatBypassMessage, formatRejectionMessage, peerFirewallCheck, } from "./_firewall";
const FILE_HEAD_BYTES = 20000;
const CTX_HEAD_BYTES = 20000;
export class PersonaSummoner {
    async summon(persona, prompt, options = {}) {
        ...
        if (existsSync(wrapperPath) && persona !== "summon") {
            ...
            if (options.contextCmd)
                args.push("--context-cmd", options.contextCmd);
        ...
        if (!cliAvailable) {
            ...
            process.stderr.write(`[summon] ${persona}'s CLI '${personaConfig.harness.command}' not on PATH — falling back to local-LLM\n`);
        ...
        const execArgs = harness.args.map(arg => arg.replace("{{PROMPT}}", fullPrompt).replace("{{MODEL}}", resolvedModel));
        ...
        if (runResult.error) {
            success = false;
            exitCode = 1;
            const errCode = runResult.error.code ?? "";
            if (errCode === "ENOENT") {
                stderr = `error: command "${harness.command}" for persona "${persona}" not found on PATH\n`;
            }
            else {
     ...
     */
    async summonViaLocalLlm(config, fullPrompt, outputFile) {
...
export function emitHelp() {
    process.stdout.write(`summon.ts — generic launcher and ISummon interface implementation.\n` +
        `\n` +
        `Usage:\n` +
        `  bun src/Core.TypeScript/peer-call/summon.ts <persona> "prompt text"\n` +
        `  bun src/Core.TypeScript/peer-call/summon.ts <persona> --model NAME "prompt text"\n` +
