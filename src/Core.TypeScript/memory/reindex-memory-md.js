...
async function collectEntriesRecursive(baseDir, currentDir, entries) {
    ...
    for (const item of items) {
        ...
        if (!item.name.endsWith(".md"))
            continue;
        if (item.name === "MEMORY.md" || item.name === "README.md")
            continue;
...
function truncateDescription(desc, maxLen = 240) {
...
function renderIndex(entries, autoDreamMarker) {
    const now = new Date().toISOString().slice(0, 10);
    const lines = [];
    lines.push(autoDreamMarker ?? "[AutoDream last run: 2026-04-23]");
    lines.push("");
    lines.push("**📌 Fast path: read `CURRENT-aaron.md`, `CURRENT-amara.md`, " +
        "`CURRENT-ani.md`, `CURRENT-vera.md`, `CURRENT-riven.md`, " +
        "and `CURRENT-otto.md` first.**");
    lines.push("");
    lines.push("> **Stack-vs-heap framing (Aaron 2026-05-12):** This file is the " +
    ...
    if (entries.length > MAX_STACK_ENTRIES) {
        lines.push("");
        lines.push(`_Stack truncated at ${MAX_STACK_ENTRIES} most-recent entries. ` +
