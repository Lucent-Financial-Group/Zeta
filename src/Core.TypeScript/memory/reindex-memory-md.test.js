...
import { collectEntries, MAX_STACK_ENTRIES, parseFrontmatter, PREAMBLE_END, PREAMBLE_MARKER, renderIndex, } from "./reindex-memory-md.js";
...
describe("renderIndex", () => {
    const makeEntry = (name, description, created) => ({
        filename: `${name}.md`,
        fm: { name, description, type: "feedback", created },
        date: created,
        mtime: 0,
    });
    ...
    test("truncates stack at MAX_STACK_ENTRIES and appends overflow note", () => {
        const entries = Array.from({ length: MAX_STACK_ENTRIES + 5 }, (_, i) => makeEntry(`entry-${i}`, `desc-${i}`, `2026-05-01`));
        const output = renderIndex(entries);
