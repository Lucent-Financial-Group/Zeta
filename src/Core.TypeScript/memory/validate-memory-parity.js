...
import { collectEntries } from "./reindex-memory-md.js";
...
function extractLinkedFilenames(content) {
    ...
    for (const m of content.matchAll(pattern)) {
        ...
        if (SKIP_PREFIXES.some((p) => raw.startsWith(p)))
            continue;
        ...
        if (parts.slice(0, -1).some((p) => p.endsWith(".md")))
            continue;
        results.push(raw);
    }
...
async function main() {
    ...
    const filenameToSource = new Map();
    ...
    for (const indexFile of OLD_INDEX_FILES) {
        ...
        if (content === null) {
            if (!jsonMode)
                console.warn(`⚠  ${indexFile} not found — skipping`);
            continue;
        }
