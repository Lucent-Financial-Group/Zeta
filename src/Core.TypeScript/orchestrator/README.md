# src/Core.TypeScript/orchestrator/ — Otto containment substrate
...
bun src/Core.TypeScript/orchestrator/validate-otto-diff.ts

# Or wire as pre-commit hook (manual; harness-side):
# .husky/pre-commit or .git/hooks/pre-commit:
#   bun src/Core.TypeScript/orchestrator/validate-otto-diff.ts || exit 1
```

## Constraint receipts

When Otto's diff touches gated surfaces (memory/, peer-call/, orchestrator/, governance, alignment, CLAUDE.md, AGENTS.md), the commit message must contain Vera and Riven `OUTPUT-FILE:` lines from `src/Core.TypeScript/peer-call/codex.ts` (Vera) and `src/Core.TypeScript/peer-call/riven.ts` (Riven) dispatches that PASSed the proposed delta.
