...
function resolveRepoRoot() {
    ...
    while (dir !== "/" && !existsSync(resolve(dir, ".git")))
        dir = dirname(dir);
    return dir;
}
function loadState() {
...
function hasReceipts(commitMsg) {
...
function classify(path, status, state) {
    const violations = [];
    ...
    if (c.no_otto_behavior_narrative &&
    ...
    if (c.memory_cron_governance_alignment_commits_require_pass &&
