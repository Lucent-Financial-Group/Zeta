...
describe("parseImagesManifest", () => {
  it("parses the repo images.manifest", () => {
    const text = readFileSync(REPO_MANIFEST, "utf8"), result = parseImagesManifest(text);
  ...
  it("rejects unknown kind and bad sha256", () => {
    expect(parseImagesManifest("x weird-kind a b").ok).toBe(!1);
    expect(parseImagesManifest("other grub-iso https://example.com/a.iso deadbeef").ok).toBe(!1);
  });
...
`), result = parseImagesManifest(text);
...
`), result = resolveLatestFromSha256Sums(sums, "mynode_amd64_*.img.gz");
    expect(result.ok).toBe(!0);
    if (!result.ok)
      return;
    expect(result.entry.filename).toBe("mynode_amd64_0-3-34.img.gz");
    expect(result.entry.sha256.startsWith("bbbb")).toBe(!0);
  });
...
describe("planMultibootUsb", () => {
  it("plans repo manifest with zeta /boot and mynode /payloads namespaces", () => {
    ...
    if (!planned.ok)
      return;
    const zeta = planned.plan.items.find((i) => i.name === "zeta-installer"), mynode = planned.plan.items.find((i) => i.name === "mynode-model-two");
  ...
  it("rejects manifest with only flash payloads (no GRUB boot)", () => {
    const parsed = parseImagesManifest(`only flash-img https://example.com/x.img.gz aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
`);
    expect(parsed.ok).toBe(!0);
    if (!parsed.ok)
      return;
    const planned = planMultibootUsb({ entries: parsed.entries });
    expect(planned.ok).toBe(!1);
    if (!planned.ok)
      expect(planned.error).toContain("grub-iso");
  });
});
describe("resolveIsoKernelInitrdPaths + renderGrubCfgTemplate", () => {
  it("resolves nixos-25.11 store paths and fills placeholders", () => {
    ...
    expect(resolved.ok).toBe(!0);
    if (!resolved.ok)
      return;
    const rendered = renderGrubCfgTemplate(`linux (loop)/@KERNEL@
initrd (loop)/@INITRD@
`, resolved);
