 ...
 */
export function classifyGpu(lspciOutput) {
    ...
    if (lower.match(/(vga|3d|display).*nvidia/))
        return "nvidia";
    if (lower.match(/(vga|3d|display).*(amd|advanced micro devices)/)) {
        return "amd";
    }
    if (lower.match(/(vga|3d|display).*intel.*arc/))
        return "intel-arc";
    return "none";
}
/**
 ...
 */
export function classifyStorage(lsblkOutput) {
    ...
    for (const line of lines) {
        ...
        if (name === undefined || rotaStr === undefined)
            continue;
 ...
 */
export function parseCpuVendor(procCpuinfo) {
 ...
 */
export function parseMemoryGb(procMeminfo) {
