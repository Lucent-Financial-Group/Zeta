...
describe("parsePersistArgs", () => {
    it("accepts well-formed args with --passphrase-env", () => {
        const result = parsePersistArgs(["--usb-uuid", UUID, "--output", "/tmp/blob", "--passphrase-env", "TP", "--bake-cred", "gh-cli=ghp_x"], { TP: PASS });
        if ("error" in result)
...
describe("persist → restore round-trip via tmpdir", () => {
    ...
    it("keeps ESP blob usable across root reformat and skips already-present restores", () => {
        ...
        if ("error" in firstPlan)
            throw new Error(firstPlan.error);
        ...
        if ("error" in reformatPlan)
            throw new Error(reformatPlan.error);
        ...
        if ("error" in idempotentPlan)
            throw new Error(idempotentPlan.error);
    ...
    it("planRestore reports wrong passphrase as code 5", () => {
        ...
        if (!("error" in plan))
            throw new Error("expected error from wrong passphrase");
    ...
    it("planRestore reports wrong UUID as code 5 (defeats copy-to-different-USB)", () => {
        ...
        if (!("error" in plan))
            throw new Error("expected error from wrong UUID");
        expect(plan.code).toBe(5);
    });
    it("planRestore reports tampered blob as code 5", () => {
        ...
        if (!("error" in plan))
            throw new Error("expected error from tampered blob");
        expect(plan.code).toBe(5);
    });
    it("planRestore reports invalid magic header as code 4", () => {
        const plan = planRestore(Buffer.from("not a valid blob at all just text bytes" + "x".repeat(200)), UUID, PASS, null, "/tmp/dontcare");
        if (!("error" in plan))
            throw new Error("expected error from invalid header");
