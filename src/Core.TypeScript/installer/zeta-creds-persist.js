...
//
// Composes:
//   - tools/installer/zeta-creds-crypto.ts (081KDWYDBW008QG0R001H6HTJK; encrypt)
//   - tools/installer/zeta-creds-manifest.ts (081KDWYDBW008QG0R002QYAWPV; cred catalog)
//   - tools/installer/zeta-cred-handlers.ts (081KDWYDBW008QG0R0002PTY26; parse + validate args)
...
// value-source contents per 081KDWYDBW008QG0R0002PTY26 P0 security discipline.
import { writeFileSync, readFileSync, existsSync } from "node:fs";
import { encrypt } from "./zeta-creds-crypto";
import { DEFAULT_HANDLERS, resolveBakeCred } from "./zeta-cred-handlers";
import { encodeBundle, serializeEnvelope } from "./zeta-creds-envelope";
...
export function composeBundle(args) {
    ...
    for (const arg of args.bakeCredArgs) {
        ...
        if ("error" in result)
            return { error: `bake-cred parse failed: ${result.error}` };
        ...
        if (!manifestEntry)
            return { error: `cred id "${result.ok.id}" not declared in manifest` };
        if (manifestEntry.personaScoped) {
            if (!args.persona)
                return { error: `cred "${result.ok.id}" is personaScoped; --persona required` };
...
export function buildBlob(bundle, usbUuid, passphrase) {
