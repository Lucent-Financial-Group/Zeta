...
const keyDec = (t: Tagged): number => {
  if (t.t !== "int") throw new Error(`Expected int key`);
  return parseInt(t.v, 10);
};

const codec = new CborDeltaCodec<number>(numCompare, keyEnc, keyDec);
...
function generateRandomZSet(): [number, number][] {
...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  const rustDecodedPairs = JSON.parse(rustDecodedStr) as [number, number][];
