...
const entry = (e: string, n: number): BagEntry<string> => ({ e, n });
const bag = (...es: readonly BagEntry<string>[]): Bag<string> => ofEntries(cmp, es);
...
describe("Bag — commutative-monoid laws (and NON-idempotence)", () => {
  ...
  it("identity: union(a, empty) == a and union(empty, a) == a", () => {
    expect(equals(cmp, union(cmp, bagA, empty<string>()), bagA)).toBe(true);
    expect(equals(cmp, union(cmp, empty<string>(), bagA), bagA)).toBe(true);
  });
...
interface GoldenBagEntry {
  e: string;
  n: number;
}
type GoldenOp =
  | { op: "add"; arg: string }
  | { op: "addN"; arg: string; n: number }
  | { op: "union"; arg: readonly GoldenBagEntry[] };
interface GoldenVector {
  readonly initialBag: readonly GoldenBagEntry[];
  readonly ops: readonly GoldenOp[];
  readonly expectedReplayStates: readonly (readonly GoldenBagEntry[])[];
  readonly expectedFinalState: readonly GoldenBagEntry[];
}
