...
type MarkupToken =
  ...
  | { kind: "open"; channel: Channel; spec: string }
  | { kind: "close"; channel: Channel };
...
function basicForegroundName(code: number): string | undefined {
...
function basicBackgroundName(code: number): string | undefined {
...
function parseMarkupTokens(markup: string): MarkupToken[] {
...
function lastReal(stack: readonly (string | null)[]): string | undefined {
...
function currentSgr(foreground: readonly (string | null)[], background: readonly (string | null)[]): string {
...
function renderToken(token: MarkupToken, foreground: (string | null)[], background: (string | null)[]): string {
  if (token.kind === "text") {
    return token.value;
  }
...
function closeForeground(state: ParseState): void {
  if (state.fgOpen) {
    state.out += "{/c}";
    state.fgOpen = false;
  }
...
function closeBackground(state: ParseState): void {
  if (state.bgOpen) {
    state.out += "{/bg}";
    state.bgOpen = false;
  }
...
function closeOpen(state: ParseState): void {
  closeBackground(state);
  closeForeground(state);
}

function openForeground(state: ParseState, spec: string): void {
  closeForeground(state);
  state.out += `{c:${spec}}`;
  state.fgOpen = true;
}

function openBackground(state: ParseState, spec: string): void {
  closeBackground(state);
  state.out += `{bg:${spec}}`;
  state.bgOpen = true;
}

function parseExtendedSpec(params: readonly string[], index: number): { spec: string | null; next: number } {
...
function applyColorCode(state: ParseState, code: number, params: readonly string[], index: number): number | null {
  if (code === 38 || code === 48) {
...
function applySgrParam(state: ParseState, params: readonly string[], index: number): number {
...
function parseSgrParams(rawParams: string): string[] {
...
export function parseFromAnsi(input: string): string {
  ...
  const state: ParseState = { out: "", fgOpen: false, bgOpen: false };
  let last = 0;
  let match: RegExpExecArray | null;
