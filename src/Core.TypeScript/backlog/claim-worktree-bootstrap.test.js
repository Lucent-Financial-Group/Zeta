...
import { assertNoPathOverlaps, buildBootstrapPlan, findPathOverlaps, pathsOverlap, } from "./claim-worktree-bootstrap";
...
describe("buildBootstrapPlan", () => {
    ...
    test("rejects non-numeric backlog suffix slugs", () => {
        expect(() => buildBootstrapPlan(request({ slug: "backlog-0164-alpha" }), "/repo/Zeta/.git")).toThrow("invalid claim slug");
    });
    test("rejects absolute or parent-traversal path claims", () => {
        expect(() => buildBootstrapPlan(request({ paths: ["/tmp/outside"] }), "/repo/Zeta/.git")).toThrow("unsafe repo-relative path");
        expect(() => buildBootstrapPlan(request({ paths: ["../outside"] }), "/repo/Zeta/.git")).toThrow("unsafe repo-relative path");
    });
});
