...
function getStubContent(resolved) {
...
function rewriteCitingFile(filePath, oldTarget, newTarget) {
    try {
        ...
        console.error(`Failed to rewrite link in ${filePath}: ${e.message}`);
    }
}
...
export function processAll(checkOnly = false, customFiles) {
    let scannedCount = 0;
    ...
    for (const [path, info] of pendingWrites) {
        ...
        try {
            ...
            console.error(`Failed to auto-vivify ${path}: ${e.message}`);
        }
    }
            ...
            watch(dirPath, { recursive: true }, (_, filename) => {
                if (filename && filename.endsWith(".md")) {
                    ...
                    try {
                        ...
                        console.error(`Watcher execution error: ${e.message}`);
                    }
                }
            });
        }
