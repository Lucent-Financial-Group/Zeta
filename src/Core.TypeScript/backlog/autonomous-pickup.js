...
function usage() {
    return [
        "Usage:",
        "  bun src/Core.TypeScript/backlog/autonomous-pickup.ts [--json] [--repo-root DIR]",
...
function isYamlKey(key) {
    if (key.length === 0) {
        return false;
    }
    for (const char of key) {
        const valid = (char >= "A" && char <= "Z") ||
            (char >= "a" && char <= "z") ||
            (char >= "0" && char <= "9") ||
            char === "_" ||
            char === "-";
...
function setFrontmatterValue(out, key, value) {
    const trimmed = value.trim();
    if (trimmed.length === 0) {
        out[key] = [];
        return key;
...
function parseFrontmatterLine(out, line, currentListKey) {
...
export function readBacklogItems(repoRoot) {
...
function dependencyBlocker(item, byId) {
...
function claimMatchesItem(claim, item) {
    ...
    if (normalized.includes(idLower))
        return true;
    ...
    if (item.legacyId) {
        ...
        if (normalized.includes(legacyLower) ||
...
function openChildrenByParent(items) {
...
function itemBlocker(item, byId, childrenByParent, activeClaims) {
    return (decomposedParentBlocker(item, childrenByParent) ??
...
function promptFor(item, action) {
    const lead = action === "decompose-first"
...
export function selectNextBacklogItem(items, activeClaims, maxPriority = "P2") {
    ...
    for (const item of items) {
        ...
        if (item.legacyId)
            byId.set(item.legacyId, item); // legacy B-xxxx alias
    }
    const blocked = [];
...
function readOpenPrClaims(repoRoot) {
    const gh = ghBin();
    if (gh === null) {
        return [];
    }
    const result = spawnSync(gh, ["pr", "list", "--state", "open", "--limit", "1000", "--json", "number,headRefName,title"], {
        cwd: repoRoot,
        encoding: "utf8",
...
export function main(argv) {
    ...
    const activeClaims = [...new Set([...gitClaims, ...openPrClaims, ...args.activeClaim])].sort((a, b) => a.localeCompare(b));
