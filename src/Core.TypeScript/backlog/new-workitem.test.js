...
const detEnv = (ms = FIXED_MS) => ({ nowMs: () => ms, nextInt64: () => 0n });
const cryptoEnv = (ms = FIXED_MS) => ({ nowMs: () => ms, nextInt64: () => DEFAULT_ENV.nextInt64() });
...
test("frontmatter carries id/type/state/priority/slug/title/created/cross-refs", () => {
    const m = mintWorkItem({ title: "Fix the login bug", type: "bug", priority: "P1", dependsOn: ["081KSXN940008QG0R002FWR9B2", "081KS3X9Y0008QG0R000W00V73"] }, detEnv());
