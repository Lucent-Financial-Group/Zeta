...
export function listRoutines(repoRoutinesDir) {
    try {
...
export function readSchedule(srcDir) {
...
export function readCloudSchedule(srcDir) {
...
export function syncRoutine(taskId, repoRoutinesDir, runtimeTasksDir) {
...
export function main(repoRoutinesDir = DEFAULT_REPO_ROUTINES_DIR, runtimeTasksDir = DEFAULT_RUNTIME_TASKS_DIR) {
    ...
    const needsCloudRegistration = results.filter((r) => r.cloudSchedule && !r.cloudSchedule.missing && r.cloudSchedule.trigger !== undefined);
