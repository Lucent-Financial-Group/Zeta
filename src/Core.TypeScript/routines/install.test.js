...
import { readCloudSchedule } from "./install.js";
...
describe("readCloudSchedule", () => {
    test("happy path - api trigger", () => {
        writeFileSync(join(dir, "cloud-schedule.json"), JSON.stringify({
            taskId: "test-1",
            trigger: { type: "api" },
        }));
    ...
    test("parse error - scheduled trigger with empty cronExpression", () => {
        writeFileSync(join(dir, "cloud-schedule.json"), JSON.stringify({ trigger: { type: "scheduled", cronExpression: "   " } }));
        const result = readCloudSchedule(dir);
        expect(result.missing).toBe(false);
        expect(result.parseError).toContain("scheduled missing required field: cronExpression");
    });
    test("parse error - github_event trigger missing event", () => {
        writeFileSync(join(dir, "cloud-schedule.json"), JSON.stringify({ trigger: { type: "github_event", repos: ["foo/bar"] } }));
        const result = readCloudSchedule(dir);
        expect(result.missing).toBe(false);
    ...
    test("parse error - github_event trigger missing repos", () => {
        writeFileSync(join(dir, "cloud-schedule.json"), JSON.stringify({ trigger: { type: "github_event", event: "issues.opened" } }));
        const result = readCloudSchedule(dir);
        expect(result.missing).toBe(false);
    ...
    test("parse error - github_event trigger with empty repos array", () => {
        writeFileSync(join(dir, "cloud-schedule.json"), JSON.stringify({ trigger: { type: "github_event", event: "issues.opened", repos: [] } }));
        const result = readCloudSchedule(dir);
        expect(result.missing).toBe(false);
        expect(result.parseError).toContain("github_event missing required field: repos");
    });
