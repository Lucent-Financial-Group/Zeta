...
import { scanSpecs, scanModules, buildGapReport, evaluateInventoryGate, parseCliArgs, CAPABILITY_MODULE_MAP, CAPABILITY_ARTIFACT_MAP, EXCLUDED_MODULES, } from "./inventory.js";
function makeTempDir() {
    return mkdtempSync(join(tmpdir(), "openspec-inventory-test-"));
}
function makeGateReport(overrides = {}) {
    return {
...
describe("buildGapReport", () => {
    ...
    test("reports missing mapped modules", () => {
        const modules = [{ name: "ZSet.fs", path: "src/Core/ZSet.fs", namespace: "Zeta.Core" }];
        const report = buildGapReport([], modules);
