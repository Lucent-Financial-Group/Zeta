...
export interface Fat12Geometry {
  readonly partOffset: number;
  readonly bps: number;
  readonly spc: number;
  readonly rsv: number;
  readonly fatCnt: number;
  readonly fatSz16: number;
  readonly rootEnt: number;
  readonly totSec16: number;
  readonly totSec32: number;
  readonly fatStart: number;
...
export function parseFat12Bpb(bpb: Buffer, partOffset: number): Fat12Geometry | { error: string } {
...
export function resolveEspGeometry(isoHead: Buffer): Fat12Geometry | { error: string } {
  const espOffset = detectIsohybridEspOffsetBytes(isoHead);
  if (espOffset === null) return { error: "could not locate ESP partition in ISO MBR" };
...
export function buildShortDirEntry(shortName: string, cluster: number): Buffer {
...
export interface AuthorizedKeysInjectPlan {
  readonly geom: Fat12Geometry;
  readonly fat: Buffer;
  readonly root: Buffer;
  readonly dirEntries: Buffer[];
  readonly dirSlotOffset: number;
...
export function planAuthorizedKeysInject(params: {
  ...
  longName: string;
  shortName: string;
}): AuthorizedKeysInjectPlan | { error: string } {
  const geomOrErr = resolveEspGeometry(params.isoHead);
  if ("error" in geomOrErr) return geomOrErr;
...
export function verifyAuthorizedKeysInRoot(
  root: Buffer,
  rootBytes: number,
  longName: string,
  shortName: string,
  ...
  for (let i = 0; i < rootBytes; i += 32) {
    ...
    const lfnPieces: Buffer[] = [];
    for (let j = i - 32; j >= 0; j -= 32) {
      const lfn = root.subarray(j, j + 32);
      if (lfn[11] !== 0x0f) break;
      lfnPieces.unshift(lfn);
    }
    if (lfnPieces.length > 0 && reconstructLongName(lfnPieces) === longName) return true;
  }
  return false;
}
