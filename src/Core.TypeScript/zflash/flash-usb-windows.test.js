...
import { MIN_USB_BYTES, MAX_USB_BYTES, MIN_ISO_BYTES, MAX_ISO_BYTES, parseGetDiskJson, selectUsbCandidate, validateIso, physicalDrivePath, buildShortChallenge, makeNonce, psGetDiskScript, psIsAdminScript, psSetReadonlyScript, psSetOfflineScript, psListVolumesScript, copyImageToDevice, autoDiscoverIso, human, 
// SSH-key injection surface
ESP_PUBKEY_FILENAME, ESP_GPT_TYPE_GUID, validatePubkeyContent, pubkeyFileContent, resolveSshPubkey, parseGetPartitionJson, isEspLike, selectEspPartition, firstFreeDriveLetter, psGetPartitionScript, diskpartAssignScript, diskpartRemoveLetterScript, psAddAccessPathAssignScript, injectPubkeyIntoEsp, } from "./flash-usb-windows.js";
...
describe("selectUsbCandidate — safety rails", () => {
    ...
    test("REFUSES when MORE THAN ONE USB candidate is present", () => {
        ...
        expect(sel.ok).toBe(false);
        if (!sel.ok)
            expect(sel.message).toContain("multiple USB candidates");
    });
...
describe("copyImageToDevice — the actual write path (byte-exact)", () => {
    ...
    test("rejects a chunkSize that is not a multiple of the sector size", () => {
        expect(() => copyImageToDevice({ isoPath: "x", destPath: "y", chunkSize: 1000, sectorSize: 512 })).toThrow();
    });
});
...
describe("resolveSshPubkey()", () => {
    ...
    test("explicit --ssh-key path wins and is validated", () => {
        ...
        if (r.ok)
            expect(r.path).toBe("/keys/mine.pub");
    });
    ...
    test("default search prefers id_ed25519.pub over id_rsa.pub", () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.path).toBe(join(home, ".ssh/id_ed25519.pub"));
    });
    test("falls back to id_rsa.pub when ed25519 absent", () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.path).toBe(join(home, ".ssh/id_rsa.pub"));
    });
...
describe("parseGetPartitionJson() + isEspLike() + selectEspPartition()", () => {
    ...
    test("selectEspPartition chooses the tiny FAT ESP, never the 1.5 GiB data region", () => {
        ...
        if (g.ok)
            expect(g.partition.number).toBe(2);
        const m = selectEspPartition(parseGetPartitionJson(mbrUsb));
        expect(m.ok).toBe(true);
        if (m.ok)
            expect(m.partition.number).toBe(2);
    });
    test("size-heuristic fallback when no explicit EFI signal", () => {
        ...
        if (s.ok)
            expect(s.partition.number).toBe(2);
    });
...
describe("ESP-mount script builders", () => {
    ...
    test("psGetPartitionScript selects the fields the parser reads", () => {
        ...
        for (const f of ["PartitionNumber", "GptType", "MbrType", "DriveLetter"])
...
describe("injectPubkeyIntoEsp() — full orchestration via a fake runner", () => {
    ...
    function makeFakeRunner(opts) {
        const files = new Map();
        const calls = [];
        let assigned = opts.autoLetter ?? null;
        const partJson = () => JSON.stringify(opts.partitions.map((p) => ({
        ...
        const isEsp = (p) => isEspLike(p);
        const runner = {
            ps(script) {
                calls.push(`ps:${script.slice(0, 28)}`);
                if (script.startsWith("Get-Partition"))
                    return partJson();
                if (script.includes("Get-Volume"))
                    return "C"; // used letters
            ...
            diskpart(script) {
                ...
                if (!opts.diskpartWorks)
                    throw new Error("diskpart refused");
                if (script.includes("assign letter="))
                    assigned = script.split("assign letter=")[1].trim();
                if (script.includes("remove letter="))
                    assigned = null;
                return "";
            },
