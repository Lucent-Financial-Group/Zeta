...
describe("081KSNY2Z0008QG0R0008PN7RQ QEMU state-preservation planner", () => {
  ...
  test("can boot from a zflash-prepared raw USB image instead of a plain CD-ROM ISO", () => {
    ...
    expect(result.ok.restartFromIsoWithDisk.args).toContain("file=/tmp/zflash-boot.img,if=none,format=raw,readonly=on,id=zflashboot");
...
`), result = assertFirstSessionSerialMarkers(serial);
    expect("ok" in result).toBe(!0);
  });
...
`), result = assertHostnameInjectionSerialMarkers(serial);
    expect("ok" in result).toBe(!0);
    if ("ok" in result)
...
`), result = assertHostnameAutogenerationSerialMarkers(serial);
    expect("ok" in result).toBe(!0);
    if ("ok" in result)
...
`), result = assertSkipGhFirstSessionSerial(serial);
  ...
  test("executes the QEMU retention command sequence and asserts serial markers", () => {
    const observedSteps = [], result = executeQcow2SnapshotRetentionPlan(retentionPlan(), {
...
`)
  ...
  test("fails closed when serial-gated QEMU steps lack a lifecycle executor", () => {
    const observedSteps = [], result = executeQcow2SnapshotRetentionPlan(retentionPlan(), {
      ...
      readSerialOutput: () => {
        throw Error("serial should not be read when lifecycle executor is missing");
      }
    });
  ...
  test("adapts planned QEMU commands to a timeout-bound process executor", () => {
    const observed = [], managedObserved = [], stoppedPids = [], serialPaths = [];
...
`);
    ...
    }), result = executeQcow2SnapshotRetentionPlan(retentionPlan(), executor);
    expect("ok" in result).toBe(!0);
    if ("ok" in result) {
      ...
      expect(observed.every((entry) => entry.options.cwd === "/tmp/zeta-worktree")).toBe(!0);
      expect(observed.every((entry) => entry.options.timeoutMs === 1234)).toBe(!0);
      expect(managedObserved.every((entry) => entry.options.cwd === "/tmp/zeta-worktree")).toBe(!0);
      expect(managedObserved.every((entry) => entry.options.timeoutMs === 1234)).toBe(!0);
  ...
  test("scans only new serial output for each lifecycle-managed QEMU phase", () => {
    const managedObserved = [], staleSerial = ["ZETA CLUSTER NODE INSTALL COMPLETE", "nixos@zeta-installer:~]$"].join(`
`), readsByManagedCount = new Map, executor = createSpawnSyncQcow2RetentionExecutor({
      pollIntervalMs: 1,
      ...
      readSerialOutput: () => {
        const managedCount = managedObserved.length, nextRead = (readsByManagedCount.get(managedCount) ?? 0) + 1;
...
`);
  ...
  test("waits for managed QEMU stop before the next qcow2 step", () => {
    let initialQemuRunning = !0, initialQemuStopIssued = !1, initialQemuRunningPollsAfterStop = 0, snapshotSawStoppedQemu = !1;
    const managedObserved = [], executor = createSpawnSyncQcow2RetentionExecutor({
      pollIntervalMs: 1,
...
`);
    ...
    expect("ok" in result).toBe(!0);
    expect(snapshotSawStoppedQemu).toBe(!0);
  ...
  test("stops before serial assertion when a QEMU command fails", () => {
    const result = executeQcow2SnapshotRetentionPlan(retentionPlan(), {
      ...
      readSerialOutput: () => {
        throw Error("serial should not be read after command failure");
      }
    });
    expect("error" in result).toBe(!0);
    if ("error" in result) {
