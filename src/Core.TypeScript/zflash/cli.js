...
import { buildBlob, composeBundle, parseArgs as parsePersistArgs, } from "../installer/zeta-creds-persist";
import { composeAuthorizedKeysFileContent, parseUuidFromDiskutilInfo, resolveZetaTestInfraPubkeyFromZflashModule, } from "./lib.js";
import { deviceKeyPath, userKeyringPublicPath, realEffects as realMachineEffects, } from "../../../tools/setup/persona-keys/machine.js";
import { realEffects as realPublishEffects } from "../../../tools/setup/persona-keys/publish.js";
import { realEffects as realTrustEffects } from "../../../tools/setup/persona-keys/github-trust.js";
import { onboard, formatOnboard } from "../../../tools/setup/persona-keys/onboard.js";
...
function autoDiscoverIso() {
    ...
    if (candidates.length === 0) {
        bail(2, `no Zeta installer ISO found under ~/Downloads/${ISO_GLOB_PREFIX}*.iso\n` +
    ...
    if (chosen === undefined)
        bail(2, "internal: candidates list non-empty but [0] is undefined");
    return chosen;
}
...
function checkLocalCheckoutFreshness(repoRoot) {
    ...
    try {
        ...
        const execCode = e && typeof e === "object" && "code" in e
            ? String(e.code)
            : "";
        ...
        if (looksNetwork) {
            process.stderr.write(`zflash: iter-4.3 freshness fetch failed (looks network-related; proceeding offline):\n` +
        ...
        bail(2, `iter-4.3 freshness fetch FAILED${looksAuth ? " (AUTH-flavored)" : " with non-network error"}:\n` +
            `  ${causeDetail}\n\n` +
            `  Common causes:\n` +
    ...
    for (const file of INSTALL_SUBSTRATE_FILES) {
        try {
            execFileSync("git", ["-C", repoRoot, "diff", "--quiet", "HEAD", "origin/main", "--", file], { stdio: ["ignore", "ignore", "pipe"] });
            // exit 0 = no diff
        }
        catch (e) {
    ...
    if (errored.length > 0) {
        bail(2, `iter-4.3 freshness check FAILED — ${errored.length} file(s) produced unexpected git diff errors:\n` +
    ...
    if (stale.length > 0) {
        ...
        bail(2, `iter-4.3 freshness check FAILED — local checkout differs from origin/main on ${stale.length} install-substrate file(s):\n` +
            stale.map((f) => `    ${f}`).join("\n") +
...
function autoDownloadFreshIsoIfNeeded(localIso) {
    ...
    try {
        ...
        if (ciMtime <= localMtime) {
            process.stdout.write(`zflash: local ISO is current with latest CI build (run ${latest.id}, ${latest.updated_at}) ✓\n`);
            return localIso;
        }
        process.stdout.write(`zflash: CI has fresher ISO — run ${latest.id} updated ${latest.updated_at}, ` +
        ...
        let dlDest = localIso;
        try {
            execFileSync("gh", ["run", "download", String(latest.id), "--dir", dlDir, "-R", ZETA_REPO_GH], { stdio: "inherit" });
            // gh run download puts artifact into a directory NAMED after the artifact.
            // Walk dlDir to find the .iso file.
            const findIsoUnder = (d) => {
                if (!existsSync(d))
                    return null;
            ...
            dlDest = join(homedir(), "Downloads", `zeta-installer-25.11-ci${latest.id}-${latest.updated_at.slice(0, 10)}.iso`);
            ...
            try {
                ...
                process.stderr.write(`zflash: (could not clean up ${dlDir}: ${cleanupErr instanceof Error ? cleanupErr.message : String(cleanupErr)}; harmless)\n`);
            }
        }
        return dlDest;
    }
    catch (e) {
        process.stderr.write(`zflash: (iter-4.3 CI ISO pull failed: ${e instanceof Error ? e.message : String(e)}; falling back to local)\n`);
        return localIso;
    }
}
function findFlashUsbPath() {
...
function findFatPartition(device) {
...
function unmountEsp(espPart, result) {
...
function getPartitionUuid(partition) {
    try {
        const out = execFileSync("diskutil", ["info", partition], { encoding: "utf8" });
...
function dumpDiagnostics(context) {
    ...
    process.stderr.write(`--- what to do next ---\n` +
...
function writeCredBlobToEsp(mountPoint, espPart, credBake) {
...
function readAuthorizedKeysContent(pubkeyPath, testMode) {
...
async function injectPubkeyToUsb(pubkeyPath, hostOverride, credBake, testMode) {
    ...
    if (externalDisks.length > 1) {
        ...
        bail(3, `iter-4.2 inject failed: expected 1 external USB post-flash, saw ${externalDisks.length}. Unplug all but the flashed USB and re-run.`);
    }
    ...
    if (hostOverride !== null) {
        if (!/^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$/.test(hostOverride)) {
            unmountEsp(espPart, mountResult);
            bail(3, `iter-5.2 inject failed: hostname '${hostOverride}' fails RFC1123 validation at write time (should have caught at flag-parse; internal bug)`);
        }
        ...
        try {
            ...
            unmountEsp(espPart, mountResult);
            bail(3, `iter-5.2 inject failed: sudo tee ${hostnameTarget} failed: ${e instanceof Error ? e.message : String(e)}`);
        }
...
async function main() {
    ...
    let testMode = false;
    const bakeCredArgs = [];
    let bakePassphraseFile = null;
    let bakePassphraseEnv = null;
    let bakePersona = null;
    const rawFlags = [];
    const positional = [];
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--host") {
            ...
            if (!/^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$/.test(next)) {
                bail(2, `--host '${next}' is not a valid RFC1123 hostname (alphanumeric + hyphens, 1-63 chars, no leading/trailing hyphen)`);
            }
            hostOverride = next;
            i++;
            continue;
    ...
    if (positional.length > 1) {
        bail(2, `too many positional arguments: ${positional.length} provided; expected at most 1 ISO path.\n` +
            `  got: ${positional.join(" ")}\n` +
    ...
    if (isHelp) {
        process.stdout.write("Usage: bun src/Core.TypeScript/zflash/cli.ts [flags] [iso-path]\n" +
    ...
    if (willInject) {
        ...
        if (hostOverride !== null) {
            process.stdout.write(`(iter-5.2 hostname inject ALSO skipped — --host ${hostOverride} requires --no-inject NOT set;\n` +
