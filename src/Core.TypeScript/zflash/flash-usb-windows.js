 ...
 */
import { closeSync, existsSync, fstatSync, mkdtempSync, openSync, readdirSync, readFileSync, readSync, rmSync, statSync, unlinkSync, writeFileSync, writeSync, fsyncSync, } from "node:fs";
 ...
 */
export function parseGetDiskJson(jsonText) {
    ...
    return arr.map((d) => {
        ...
        const flat = (v) => v != null && typeof v === "object" && "value" in v
            ? String(v.value)
            : str(v);
        return {
            number: num(o.Number),
 ...
 */
export function selectUsbCandidate(disks) {
    const candidates = disks.filter((d) => d.busType.toUpperCase() === "USB" &&
        !d.isBoot &&
        !d.isSystem &&
    ...
    if (candidates.length === 0) {
        ...
        return {
            ok: false,
            code: 2,
            message: `no eligible USB target found (need: BusType=USB, not boot/system disk, size in ` +
    ...
    if (candidates.length > 1) {
        ...
        return {
            ok: false,
            code: 2,
            message: `multiple USB candidates — refusing to pick one. Unplug all but the target USB and re-run:\n${list}`,
        };
    }
    return { ok: true, disk: candidates[0] };
}
export function validateIso(isoPath, sizeBytes, isFile) {
    if (!isoPath.toLowerCase().endsWith(".iso"))
        return { ok: false, message: `expected a *.iso file, got: ${isoPath}` };
    if (!isFile)
        return { ok: false, message: `ISO path is not a file: ${isoPath}` };
...
export function physicalDrivePath(diskNumber) {
 ...
 */
export function validatePubkeyContent(text) {
    const normalized = text.replace(/\r\n/g, "\n").trim();
    if (normalized.length === 0)
        return { ok: false, message: "SSH key file is empty" };
    ...
    if (fields.length < 2)
        return { ok: false, message: "malformed public key (need '<type> <base64> [comment]')" };
    const type = fields[0];
...
export function pubkeyFileContent(content) {
 ...
 * not a silent skip.
 */
export function resolveSshPubkey(explicitPath, home, fs) {
    if (explicitPath) {
        if (!fs.exists(explicitPath))
            return { ok: false, message: `--ssh-key not found: ${explicitPath}` };
        const content = fs.read(explicitPath);
        const v = validatePubkeyContent(content);
        if (!v.ok)
            return { ok: false, message: `${explicitPath}: ${v.message}` };
    ...
    for (const rel of DEFAULT_SSH_KEY_CANDIDATES) {
        ...
        const v = validatePubkeyContent(content);
        if (!v.ok)
            return { ok: false, message: `${p}: ${v.message}` };
        return { ok: true, path: p, content: content.replace(/\r\n/g, "\n").trim() };
    }
    return {
        ok: false,
        message: `no SSH public key found (looked for: ${tried.join(", ")}).\n` +
...
export function parseGetPartitionJson(jsonText) {
    ...
    return arr.map((d) => {
        ...
        const driveLetter = dl == null || dl === 0 || dl === "0" || dl === " " ? "" : String(dl).trim().replace(/[:\\]/g, "");
...
export function isEspLike(p) {
    if (p.gptType === ESP_GPT_TYPE_GUID)
        return true;
    if (p.mbrType === ESP_MBR_TYPE)
        return true;
 ...
 */
export function selectEspPartition(parts) {
    const bySizeAsc = (a, b) => a.size - b.size;
    const explicit = parts.filter(isEspLike).slice().sort(bySizeAsc);
    if (explicit.length > 0)
        return { ok: true, partition: explicit[0] };
    const small = parts
        .filter((p) => p.size > 0 && p.size < 512 * 1024 * 1024)
        .slice()
        .sort(bySizeAsc);
    if (small.length > 0)
        return { ok: true, partition: small[0] };
...
export function firstFreeDriveLetter(used) {
...
export function buildShortChallenge(nonceHex4) {
    if (!/^[0-9a-f]{4}$/.test(nonceHex4))
        throw new Error(`nonce must be 4 lowercase hex chars, got: ${nonceHex4}`);
    return `yes ${nonceHex4}`;
}
export function makeNonce(rng = Math.random) {
    return Math.floor(rng() * 0x10000)
        .toString(16)
        .padStart(4, "0");
}
// PowerShell command builders (pure → assert-able in tests).
export function psGetDiskScript() {
    return ("Get-Disk | Select-Object Number,FriendlyName,SerialNumber,BusType,Size," +
        "IsBoot,IsSystem,IsReadOnly,OperationalStatus,PartitionStyle | ConvertTo-Json -Depth 3");
}
export function psIsAdminScript() {
    return ("[bool](([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent())" +
        ".IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator))");
}
export function psSetReadonlyScript(diskNumber, ro) {
    return `Set-Disk -Number ${diskNumber} -IsReadOnly $${ro ? "true" : "false"}`;
}
export function psSetOfflineScript(diskNumber, offline) {
    return `Set-Disk -Number ${diskNumber} -IsOffline $${offline ? "true" : "false"}`;
}
export function psListVolumesScript(diskNumber) {
 ...
 */
export function diskpartAssignScript(diskNumber, partitionNumber, letter) {
    return [
...
export function diskpartRemoveLetterScript(diskNumber, partitionNumber, letter) {
    return [
        `select disk ${diskNumber}`,
        `select partition ${partitionNumber}`,
 ...
 */
export function copyImageToDevice(o) {
    const chunk = o.chunkSize ?? 4 * 1024 * 1024;
    const sector = o.sectorSize ?? 4096;
    if (chunk % sector !== 0)
        throw new Error(`chunkSize ${chunk} must be a multiple of sectorSize ${sector}`);
...
export function autoDiscoverIso(downloadsDir) {
 ...
 */
export function injectPubkeyIntoEsp(runner, diskNumber, pubkeyContent, log = () => { }) {
    const fail = (message, partitionNumber = -1, driveLetter = "", assignedLetter = false) => ({
        ok: false,
        partitionNumber,
        driveLetter,
        assignedLetter,
    ...
    if (!letter) {
        ...
        try {
            ...
            if (letter)
                log(`inject: mounted ESP at ${letter}: via Add-PartitionAccessPath`);
        }
        catch (e) {
    ...
    if (!letter) {
        return fail(`could not mount the ESP (no drive letter could be assigned). The USB is bootable but has NO operator key.\n` +
            `  Manually mount the small FAT partition on disk ${diskNumber} and copy your .pub key to it as\n` +
            `  ${ESP_PUBKEY_FILENAME}, or re-run after closing anything holding the disk.`, esp.number);
    }
...
async function main(runner = realRunner) {
    const argv = process.argv.slice(2);
    const short = argv.includes("--short");
    const dryRun = argv.includes("--dry-run");
    const noInject = argv.includes("--no-inject");
    if (argv.includes("-h") || argv.includes("--help")) {
        process.stdout.write("usage: bun full-ai-cluster\\tools\\flash-usb-windows.ts [flags] [iso-path]\n" +
    ...
    if (skIdx !== -1) {
        ...
        if (!sshKeyPath || sshKeyPath.startsWith("-"))
            bail(2, "--ssh-key requires a path argument");
    }
    const positional = argv.filter((a, i) => !a.startsWith("-") && !(skIdx !== -1 && i === skIdx + 1));
    if (positional.length > 1)
        bail(2, `at most one ISO path expected, got ${positional.length}`);
    ...
    if (!admin && !dryRun) {
        bail(2, "not elevated. Re-run from an Administrator PowerShell (right-click → " +
    ...
    if (!isoPath)
        bail(2, `no zeta-installer-*.iso under %USERPROFILE%\\Downloads; pass an ISO path explicitly`);
    ...
    const realFs = { exists: existsSync, read: (p) => readFileSync(p, "utf8") };
    let pubkey = null;
    if (!noInject) {
        const r = resolveSshPubkey(sshKeyPath, homedir(), realFs);
        if (!r.ok)
            bail(2, `SSH key injection required (default). ${r.message}`);
    ...
    if (typed !== phrase)
        bail(2, `confirmation mismatch — aborting (no write performed).`);
    ...
    if (pubkey) {
        ...
        if (!inj.ok) {
            bail(1, `SSH-KEY INJECTION FAILED — ${inj.message}\n` +
