...
import { realRing, complexRing, doubled, quaternionRing, consolidate, } from "./star-ring";
// ─── Ring law helpers ────────────────────────────────────────────────────────
function checkRingLaws(name, ring, eq, samples) {
...
// ─── Real ring ───────────────────────────────────────────────────────────────
const realEq = (a, b) => Math.abs(a - b) < 1e-10;
const realSamples = [0, 1, -1, 2.5, -3.7, 0.001];
checkRingLaws("real", realRing, realEq, realSamples);
// ─── Complex ring ────────────────────────────────────────────────────────────
const cEq = (a, b) => Math.abs(a.re - b.re) < 1e-10 && Math.abs(a.im - b.im) < 1e-10;
const cSamples = [
...
describe("Cayley-Dickson doubled(real) ≅ complex", () => {
    ...
    const dEq = (a, b) => Math.abs(a.real - b.real) < 1e-10 && Math.abs(a.imag - b.imag) < 1e-10;
...
describe("quaternion ring (doubled(complex))", () => {
    test("quaternion multiplication is non-commutative", () => {
        ...
        const eq = (a, b) => Math.abs(a.real.re - b.real.re) < 1e-10 &&
...
describe("consolidate — ring-generic merge", () => {
    ...
    const isZeroC = (z) => magSq(z) < 1e-12;
    const isZeroR = (w) => Math.abs(w) < 1e-12;
