...
import { specialize, createSpecializationCache, createSpecializationRegistry } from "./specialization-cache";
import { readFileSync } from "node:fs";
import { join } from "node:path";
// Load splitmix64 IR
const goldenFile = JSON.parse(readFileSync(join(import.meta.dir, "../../../tests/cross-verification/zeta-ir-v1/zeta-ir-v1.golden.json"), "utf-8"));
const irRaw = goldenFile["rng.splitmix64"];
const irSafe = irRaw.replace(/"k"\s*:\s*(-?\d+)/g, '"k_bigint": "$1"');
const sm64 = JSON.parse(irSafe);
...
describe("createSpecializationRegistry — multi-IR cache", () => {
    const fmix32Raw = goldenFile["hash.fmix32"];
    const fmix32Safe = fmix32Raw.replace(/"k"\s*:\s*(-?\d+)/g, '"k_bigint": "$1"');
    const fmix32 = JSON.parse(fmix32Safe);
...
describe("error handling — NEVER cache errors", () => {
    test("specialization error is NOT cached — retries on next call", () => {
        let callCount = 0;
        const failingThenSucceeding = (ir) => {
            callCount++;
            if (callCount === 1)
