...
export interface Game {
  ...
  readonly vx: number;
  readonly vy: number;
  readonly paddleA: number;
  readonly paddleB: number;
...
function clampPaddle(p: number): number {
  return Math.max(0, Math.min(HEIGHT - PADDLE_LEN, p));
}

function serve(g: Game, vx: number): Game {
...
export function step(inputA: number, inputB: number, g0: Game): Game {
  const g: Game = {
    ...g0,
...
export function parseInputs(payload: string): readonly [number, number] | null {
  if (!payload.startsWith("pong:")) return null;
  const parts = payload.slice(5).split(",");
  if (parts.length !== 2) return null;
  const a = Number(parts[0]);
  const b = Number(parts[1]);
  if (!Number.isInteger(a) || !Number.isInteger(b) || parts[0] === "" || parts[1] === "") return null;
  return [a, b];
}

/** Serialize to the canonical treaty line (byte-identical to the F# oracle). */
export function toLine(g: Game): string {
...
export function ofLine(line: string): Game | null {
  const parts = line.split("\t");
  if (parts.length !== 9 || parts[0] !== "ponggame1") return null;
  const vals: number[] = [];
  for (let i = 1; i < 9; i++) {
    const v = Number(parts[i]);
    if (!Number.isInteger(v) || parts[i] === "") return null;
    vals.push(v);
  }
  return {
    ballX: vals[0]!,
