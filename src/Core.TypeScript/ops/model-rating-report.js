...
export function main(argv = process.argv) {
    ...
    for (const row of rows)
        console.log(padRow(row, colWidths));
    ...
    if (includeCosts) {
        ...
        for (const row of costRows)
            console.log(padRow(row, costWidths));
        const rateLines = Object.entries(RATE_CARD).map(([tier, r]) => `${tier}=$${(r.input * 1_000_000).toFixed(2)}/$${(r.output * 1_000_000).toFixed(2)}`).join(" ");
    ...
    if (includeReviews) {
        ...
        for (const r of ratings) {
            if (!r.pr_number || prsSeen.has(r.pr_number))
                continue;
        ...
        if (allFindings.length === 0) {
            ...
            for (const row of catRows)
                console.log(padRow(row, catWidths));
            console.log("\n--- Severity breakdown ---\n");
            const sevByModel = new Map();
            for (const model of models)
                sevByModel.set(model, new Map());
            for (const f of allFindings) {
            ...
            for (const row of sevRows)
                console.log(padRow(row, sevWidths));
        }
    }
    // === Recent entries ===
    ...
    const flags = [];
    if (!includeReviews)
        flags.push("--reviews (PR review failure categories)");
    if (!includeCosts)
        flags.push("--costs (token cost derivation from current rate card)");
    if (flags.length > 0)
