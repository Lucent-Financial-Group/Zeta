    ...
    for (const entry of raw.datfiles) {
        for (const field of REQUIRED_FIELDS) {
            if (typeof entry[field] !== "string" || entry[field] === "") {
                throw new Error(`manifest entry missing required string field "${field}": ` +
 ...
 */
export function fetchBlockReason(pin) {
    if (isPlaceholder(pin.downloadUrl)) {
        return (`downloadUrl for "${pin.platform}" is an unverified placeholder ` +
    ...
    if (isPlaceholder(pin.sha256)) {
        return (`sha256 for "${pin.platform}" is an unverified placeholder (${pin.sha256}). ` +
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--platform" || arg === "-p") {
            ...
            process.stdout.write("Usage: fetch-datfile.ts [--platform <slug>] [--manifest <path>] [--out <dir>] [--list]\n\n" +
...
export async function main(argv) {
    ...
    if (!verifyChecksum(bytes, pin.sha256)) {
        process.stderr.write(`checksum MISMATCH for ${pin.datfileName}: ` +
    ...
    process.stderr.write(`verified + wrote ${bytes.length} bytes to ${outPath}\n` +
