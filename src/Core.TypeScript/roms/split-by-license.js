...
export function splitRoms(romDir, safeDir, unsafeDir, allowed, apply) {
    if (apply) {
    ...
    for (const dirent of readdirSync(romDir, { withFileTypes: true })) {
        ...
        if (!ROM_EXTENSIONS.has(extname(file).toLowerCase()))
            continue;
...
function parseArgs(argv) {
    ...
    let allowlist;
    let apply = false;
    function readOptionValue(index, flag) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--rom-dir") {
            ...
            process.stdout.write("Usage: split-by-license.ts --rom-dir <dir> --safe-dir <dir> --unsafe-dir <dir> --allowlist <path> [--apply]\n\n" +
    ...
    if (!romDir)
        throw new ArgError("--rom-dir is required", 64);
    if (!safeDir)
        throw new ArgError("--safe-dir is required", 64);
    if (!unsafeDir)
        throw new ArgError("--unsafe-dir is required", 64);
    if (!allowlist)
        throw new ArgError("--allowlist is required", 64);
