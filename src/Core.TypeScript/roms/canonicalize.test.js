...
import { main, parseDatfile, hashFileSha1, scanRomFiles, matchAndReport, } from "./canonicalize.js";
...
describe("matchAndReport", () => {
    ...
    test("does not apply unsafe canonical names with path separators", () => {
        ...
        const file = join(tmp, "wrong-name.bin");
        writeFileSync(file, "hello");
        const lookup = parseDatfile(`<rom name="../escaped.a26" size="5" sha1="aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d" />`);
        const results = matchAndReport(lookup, [file], true);
        expect(results[0]?.matched).toBe(true);
        expect(results[0]?.renamed).toBe(false);
    ...
    test("does not apply unsafe canonical names with Windows separators", () => {
        ...
        const file = join(tmp, "wrong-name.bin");
        writeFileSync(file, "hello");
        const lookup = parseDatfile(`<rom name="nested\\\\escaped.a26" size="5" sha1="aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d" />`);
        const results = matchAndReport(lookup, [file], true);
        expect(results[0]?.matched).toBe(true);
        expect(results[0]?.renamed).toBe(false);
        expect(existsSync(file)).toBe(true);
