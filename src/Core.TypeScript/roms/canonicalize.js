...
function isSafeCanonicalName(name) {
...
export function parseDatfile(xml) {
...
// --- File hashing ---
export function hashFileSha1(path) {
...
// --- Directory scanning ---
export function scanRomFiles(dir) {
    const files = [];
...
export function matchAndReport(datLookup, romFiles, apply) {
    const results = [];
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--datfile" || arg === "-d") {
            ...
            process.stdout.write("Usage: canonicalize.ts --datfile <path.dat> --dir <rom-dir> [--apply]\n\n" +
