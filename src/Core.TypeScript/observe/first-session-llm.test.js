...
import { buildFirstSessionMenu, defaultNodeSession, firstSessionOracle, firstSessionWithLlm, firstSessionActionsEqual, foldFirstSession, runFirstSessionLoop, simulateFirstSession, } from "./first-session";
...
function backendChoosing(session, target) {
    ...
    if (idx < 0)
        throw new Error(`menu has no ${JSON.stringify(target)} for this session`);
...
describe("first-session closed-loop — fold reconstructs adventure trace", () => {
    it("one tick: simulate matches fold of appended action", async () => {
        ...
        const pick = await firstSessionWithLlm(initial, backendChoosing(initial, { kind: "setup_credential", vendor: "gh", reason: "x" }));
