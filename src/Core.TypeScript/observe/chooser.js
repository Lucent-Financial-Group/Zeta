 ...
 */
function oracleChoose(world, menu) {
    if (menu.length === 0)
        return null;
    ...
    const viableCount = menu.filter(a => a.kind === "do_item" || a.kind === "decompose" || a.kind === "explore").length;
    if (viableCount <= 1) {
 ...
 */
async function composerChoose(world, menu, backend) {
 ...
 */
async function deliberatorChoose(world, participant) {
 ...
 */
export async function choose(world, config = {}) {
