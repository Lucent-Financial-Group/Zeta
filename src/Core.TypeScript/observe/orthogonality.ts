...
export function assessOrthogonality(
  surfaces: ReadonlyArray<{ path: string; text: string }>,
  th: Thresholds,
): Pair[] {
...
const THRESHOLDS: Thresholds = { jaccard: 0.2, containment: 0.5 };
...
if (import.meta.main) {
  ...
  if (args.has("--check") && offenders.length > 0) process.exit(1);
  process.exit(0);
}
