...
export interface Directive {
  readonly id: string; // filename stem, e.g. "no-directives"
  readonly title: string; // the "# " heading
  readonly carved: string; // the carved sentence (blockquote after "Carved sentence:")
  readonly pointers: readonly string[]; // doc/memory/rule links the detail lives behind
}

const stem = (path: string): string => path.replace(/^.*\//, "").replace(/\.md$/, "");

/** Parse one surface into a Directive (pure, deterministic — no model). */
export function parseSurface(path: string, text: string): Directive {
  ...
  for (let i = start; i < lines.length; i++) {
    const l = lines[i]!;
    if (l.trim().startsWith(">")) quote.push(l.trim().replace(/^>\s?/, ""));
    else if (quote.length > 0) break; // blockquote ended
  }
  const carved = quote.join(" ").replace(/\s+/g, " ").trim();

  // Pointers = markdown link targets + bare backtick paths under a Pointers section.
  const pointers = new Set<string>();
  for (const m of text.matchAll(/\]\(([^)]+\.md)\)/g)) pointers.add(m[1]!);
  for (const m of text.matchAll(/`([a-zA-Z0-9_./-]+\.md)`/g)) pointers.add(m[1]!);

  return { id: stem(path), title, carved, pointers: [...pointers].sort() };
}

/** Compile a set of surfaces into the DSL (sorted by id; pure). */
export function compile(surfaces: ReadonlyArray<{ path: string; text: string }>): Directive[] {
...
export function toChoosePrompt(directives: readonly Directive[]): string {
...
if (import.meta.main) {
  ...
  for (const d of dsl) console.log(`  ${d.id} (${d.pointers.length} pointers): ${d.carved.slice(0, 72)}${d.carved.length > 72 ? "…" : ""}`);
