...
export function readCellState(repoRoot) {
    ...
    if (existsSync(path)) {
        for (const line of readFileSync(path, "utf8").split("\n")) {
            ...
            if (cellId && agent)
                declared.push({ cellId, harness, agent, interval, forward });
        }
    }
...
export function readPRState(_forge) {
    ...
    const result = spawnSync("gh", ["pr", "list", "--state", "open", "--limit", "20", "--json", "number,title,mergeStateStatus"], { encoding: "utf8", timeout: 30000 });
    if (result.status !== 0) {
 ...
 */
export async function readPRStateAsync(forge) {
