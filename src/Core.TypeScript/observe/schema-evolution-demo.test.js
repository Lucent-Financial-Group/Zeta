...
import { schemaZSet, applyDelta, deltaFromSchemas, currentSchema, resolveField, FS_METADATA_SCHEMA_V1, FS_METADATA_SCHEMA_V2, } from "./schema-zset";
import { emitSchemaEvent, parseSchemaEvent, addedFields, } from "./schema-cdc";
import { overlapStatus, tryConsolidate, migrateReader, } from "./schema-overlap";
describe("schema evolution worked example: FS metadata v1 → v2", () => {
    ...
    test("complete lifecycle: define → delta → emit → apply → overlap → migrate → consolidate", () => {
        ...
        const event = emitSchemaEvent(FS_METADATA_SCHEMA_V1, FS_METADATA_SCHEMA_V2, delta, { source: "zeta://schema/fs-metadata", actor: "alexa", mint: () => "b".repeat(32), now: () => Date.now() });
    ...
    test("lifecycle with field TYPE CHANGE (retract old + insert new)", () => {
        ...
        const after = FS_METADATA_SCHEMA_V1.map(f => f.name === "created" ? { ...f, type: "number" } : f);
