...
const INITIAL_FIELDS: readonly SchemaField[] = [
...
const DELTAS: readonly SchemaEvolutionDelta[] = [
...
function computeGoldenVectors() {
  ...
  const replayStates: Array<{ activeFields: readonly SchemaField[]; entryCount: number }> = [];
  ...
  return {
    description:
      "schema-zset cross-language conformance (zero-downtime-schema-evolution). " +
