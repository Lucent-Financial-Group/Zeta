...
async function main(): Promise<void> {
  ...
  for (const producer of producers) {
    ...
    console.log(`    When producer CORRECT: verifier approves ${verifierApprovesCorrect}/${producerCorrect} (${(verifierApprovesCorrect/Math.max(1,producerCorrect)*100).toFixed(0)}%), rejects ${verifierRejectsCorrect} (${(falseNegativeRate*100).toFixed(1)}%)`);
