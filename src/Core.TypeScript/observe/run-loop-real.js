...
import { realWorkspacePort } from "./workspace-port";
import { observeWithParticipant, oracleParticipant, localLlmParticipant, cloudPersonaParticipant, } from "./participant";
 ...
 */
function resolveParticipant(spec) {
    if (spec === "oracle")
        return oracleParticipant();
    if (spec === "local-llm")
        return localLlmParticipant();
...
async function main() {
    ...
    if (forgeResult.ok) {
        ...
        if (prState.ok) {
            ...
            console.error(`[forge] PR state read FAILED: ${prState.error instanceof Error ? prState.error.message : String(prState.error)} ` +
    ...
    const port = realWorkspacePort(args.repoRoot);
    const executor = {
        tier: "just-bash",
        run: async (_spec) => {
            if (action.kind !== "do_item") {
                return { ok: true, stdout: "no-op (non-do_item)", exitCode: 0 };
            }
