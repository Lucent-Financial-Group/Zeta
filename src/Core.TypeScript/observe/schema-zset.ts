...
/** The value types a schema field can hold. */
export type FieldType = "boolean" | "string" | "uint8array" | "string[]" | "number" | "zetaid";
...
/** A schema Z-set entry: a field with a weight (positive = active, negative = retracted). */
export interface SchemaEntry {
  readonly field: SchemaField;
  readonly weight: number;
}

/** The schema Z-set — a collection of weighted field definitions. */
export type SchemaZSet = readonly SchemaEntry[];

/** A schema evolution delta — what to retract and what to insert. */
export interface SchemaEvolutionDelta {
  readonly retract: readonly SchemaField[];  // fields being removed (weight -1)
  readonly insert: readonly SchemaField[];   // fields being added (weight +1)
}

// ─── Construction ────────────────────────────────────────────────────────────

/** Create a schema Z-set from field definitions (each at weight +1). */
export function schemaZSet(fields: readonly SchemaField[]): SchemaZSet {
 ...
 */
export function applyDelta(schema: SchemaZSet, delta: SchemaEvolutionDelta): SchemaZSet {
  // Build a mutable map: fieldName → { field, weight }
  const map = new Map<string, { field: SchemaField; weight: number }>();
 ...
 */
export function consolidate(schema: SchemaZSet): SchemaZSet {
...
export function currentSchema(schema: SchemaZSet): readonly SchemaField[] {
...
export function resolveField(schema: SchemaZSet, fieldName: string): SchemaField | undefined {
...
export function isInOverlap(schema: SchemaZSet, fieldName: string): boolean {
 ...
 */
export function deltaFromSchemas(before: readonly SchemaField[], after: readonly SchemaField[]): SchemaEvolutionDelta {
  const beforeMap = new Map(before.map(f => [f.name, f]));
  const afterMap = new Map(after.map(f => [f.name, f]));

  const retract: SchemaField[] = [];
  const insert: SchemaField[] = [];
...
export const FS_METADATA_SCHEMA_V1: readonly SchemaField[] = [
...
export const FS_METADATA_SCHEMA_V2: readonly SchemaField[] = [
