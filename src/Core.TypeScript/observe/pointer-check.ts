...
export interface PointerResult {
  readonly target: string; // as written (anchor stripped)
  readonly kind: "file" | "external" | "anchor";
...
export interface FilePointers {
  ...
  readonly checked: number; // file-kind pointers (external/anchor excluded)
  readonly broken: ReadonlyArray<PointerResult>;
}
...
export function extractTargets(text: string): string[] {
  ...
  for (const m of text.matchAll(/`([a-zA-Z0-9_./-]+\.[a-z]{2,4})`/g)) {
    const t = m[1]!;
    if (t.includes("/") || t.endsWith(".md")) out.push(t);
  }
  return [...new Set(out)];
}

/** Classify + resolve a single target relative to its file and the repo root. */
export function resolveTarget(target: string, fromFile: string, repoRoot: string): PointerResult {
  if (/^(https?:|mailto:|#)/.test(target)) {
    return { target, kind: target.startsWith("#") ? "anchor" : "external", resolved: true };
  }
  const path = target.split("#")[0]!.trim();
  if (path === "") return { target, kind: "anchor", resolved: true };
...
export function checkFile(path: string, text: string, repoRoot: string): FilePointers {
...
if (import.meta.main) {
  ...
  for (const p of paths) {
    ...
    for (const b of fp.broken) console.log(`    BROKEN → ${b.target}`);
  }
  console.log(`\n${paths.length} files, ${totalPtrs} pointers, ${totalBroken} broken.`);

  if (flags.has("--check") && totalBroken > 0) process.exit(1);
  process.exit(0);
}
