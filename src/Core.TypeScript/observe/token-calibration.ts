...
export interface Sample {
  ...
  readonly observedTokens: number; // measured ONCE with the real tokenizer
}

export interface Calibration {
  readonly model: string;
  readonly bytesPerToken: number;
  readonly calibrated: boolean; // false => bytesPerToken is the placeholder default
  readonly provenance: string;
}

/** Estimate tokens from a proven byte count and the calibration ratio. */
export function estimateTokens(bytes: number, bytesPerToken: number): number {
...
export interface Fit {
  ...
  readonly meanAbsPctError: number; // fit quality on the samples themselves
}

/** Fit bytesPerToken from samples (least-squares through the origin). */
export function fitRatio(samples: readonly Sample[]): Fit {
  if (samples.length === 0) throw new Error("fitRatio: no samples");
  const totalBytes = samples.reduce((s, x) => s + x.bytes, 0);
  const totalTokens = samples.reduce((s, x) => s + x.observedTokens, 0);
  if (totalTokens <= 0) throw new Error("fitRatio: observed tokens must be positive");
...
// ── CLI ─────────────────────────────────────────────────────────────────────
interface CalibrationFile extends Calibration {
  samples: Sample[];
}
