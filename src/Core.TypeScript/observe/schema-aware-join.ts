...
/** A raw data row -- field name to value. The data Z-set the schema describes. */
export type DataRow = Readonly<Record<string, unknown>>;

/** A projected output row -- a row resolved against the live schema. */
export type ProjectedRow = Readonly<Record<string, unknown>>;

/** A materialized view: the data rows projected against the current schema. */
export interface MaterializedView {
  readonly fields: readonly string[]; // active field names, canonical (sorted)
 ...
 */
export function projectRow(fields: readonly SchemaField[], row: DataRow): ProjectedRow {
...
export function materialize(schema: SchemaZSet, data: readonly DataRow[]): MaterializedView {
...
// ─── SchemaCell (input A: subscribe to deltas) ────────────────────────────────

/** A subscriber notified of each schema evolution: (newSchema, theDelta). */
export type SchemaSubscriber = (schema: SchemaZSet, delta: SchemaEvolutionDelta) => void;
...
export class SchemaCell {
  ...
  private readonly subscribers: SchemaSubscriber[] = [];

  constructor(initial: SchemaZSet) {
    this.schema = initial;
  }
  ...
  subscribe(fn: SchemaSubscriber): () => void {
  ...
  evolve(delta: SchemaEvolutionDelta): SchemaZSet {
    this.schema = applyDelta(this.schema, delta);
    // Deterministic order; one notification per evolve -> glitch-free.
    for (const fn of [...this.subscribers]) fn(this.schema, delta);
    return this.schema;
  }
}

// ─── SchemaAwareJoin (the Rx join) ────────────────────────────────────────────

/** A subscriber to the materialized view -- fired once per input change. */
export type ViewSubscriber = (view: MaterializedView) => void;
...
export class SchemaAwareJoin {
  private readonly schemaCell: SchemaCell;
  private data: readonly DataRow[];
  private view: MaterializedView;
  private readonly subscribers: ViewSubscriber[] = [];
  private readonly unsubscribeSchema: () => void;
  private emitCount = 0;

  constructor(schemaCell: SchemaCell, initialData: readonly DataRow[] = []) {
  ...
  subscribe(fn: ViewSubscriber): () => void {
    this.subscribers.push(fn);
    return () => {
      const i = this.subscribers.indexOf(fn);
      if (i >= 0) this.subscribers.splice(i, 1);
  ...
  setData(rows: readonly DataRow[]): void {
    this.data = rows;
 ...
 */
export function canonicalSchema(schema: SchemaZSet): string {
  const rows = schema
    .map(e => ({
      name: e.field.name,
      type: e.field.type,
...
export function schemaEqual(a: SchemaZSet, b: SchemaZSet): boolean {
  return canonicalSchema(a) === canonicalSchema(b);
}

/** Canonical form of a materialized view (rows already key-sorted by projectRow). */
export function canonicalView(view: MaterializedView): string {
