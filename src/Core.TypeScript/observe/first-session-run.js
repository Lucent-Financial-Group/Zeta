...
import { buildFirstSessionMenu, defaultNodeSession, firstSessionLabel, firstSessionOracle, firstSessionWithLlm, simulateFirstSession, } from "./first-session";
import { SERIAL_PREFIX, defaultShellRunner, executeSetupCredential, probeAllCredentials, } from "./first-session-executor";
...
export function sessionFromProbe(runner, home, complete = false) {
...
export function actionFromDemoToken(token, session) {
    ...
    if (alias.includes(":")) {
        ...
        return menu.find((a) => a.kind === kind &&
            ("vendor" in a ? a.vendor === vendor : true)) ?? null;
    }
