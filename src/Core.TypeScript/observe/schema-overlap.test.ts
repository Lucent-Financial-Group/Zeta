...
describe("schema-overlap — state machine", () => {
  ...
  test("writer_switched after delta applied (no readers migrated)", () => {
    ...
    const withOverlap = [...evolved, { field: { name: "modified", type: "string" as const, required: false }, weight: -1 }];
...
describe("schema-overlap — consolidation", () => {
  test("canDropOldSchema is false when readers pending", () => {
    const schema = [...schemaZSet(FS_METADATA_SCHEMA_V1), { field: { name: "modified", type: "string" as const, required: false }, weight: -1 }];
