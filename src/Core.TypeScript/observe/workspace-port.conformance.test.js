...
import { simulatedWorkspacePort, realWorkspacePort, emptySimulatedState, } from "./workspace-port";
const factories = [
    {
...
for (const factory of factories) {
    describe(`WorkspacePort conformance [${factory.name}]`, () => {
        ...
        test("writeFile + readFile round-trips text content", () => {
            ...
            if (r.ok)
                expect(r.value).toBe("export const x = 1;");
        });
        ...
        test("git checkout -B creates a branch", () => {
            ...
            if (branch.ok)
                expect(branch.value).toBe("test/branch");
        });
        ...
        test("schema evolution: old files readable after schema change (default fills missing)", () => {
            ...
            if (readResult.ok)
                expect(readResult.value).toContain("old");
        });
        test("schema evolution: new files with new fields work alongside old", () => {
            ...
            if (fresh.ok)
                expect(fresh.value).toContain("081KOWNER000001");
        });
        // ── History operations ────────────────────────────────────────────
