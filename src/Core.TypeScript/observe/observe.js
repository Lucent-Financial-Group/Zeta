...
import { describeFirstSession, firstSessionOracle, } from "./first-session";
const isFreeMode = (m) => m !== "work";
...
const FIRST_SESSION_PENDING_REASON = "first-session credential adventure pending — finish cred setup before backlog work";
/** True when the nodeSession channel is wired and the adventure is not complete. */
export function isFirstSessionPending(world) {
...
export function observe(world) {
    ...
    if (op?.pendingFerry)
        return { kind: "preserve_ferry", reason: PRESERVE_FERRY_REASON };
    if (op?.pendingMessage)
        return { kind: "respond_to_operator", reason: RESPOND_OPERATOR_REASON };
    ...
    if (world.mode && isFreeMode(world.mode))
        return freeModeAction(world.mode);
    const doable = world.backlog.find((i) => i.ready && !i.ambiguous);
    if (doable)
        return { kind: "do_item", item: doable };
    ...
    if (toDecompose)
        return { kind: "decompose", item: toDecompose };
 ...
 */
export function buildMenu(world) {
    ...
    const candidates = [];
    if (op?.pendingFerry)
        candidates.push({ kind: "preserve_ferry", reason: PRESERVE_FERRY_REASON });
    if (op?.pendingMessage)
        candidates.push({ kind: "respond_to_operator", reason: RESPOND_OPERATOR_REASON });
    // offered work (not forced — the free modes below are always alternatives).
    const doable = world.backlog.find((i) => i.ready && !i.ambiguous);
    if (doable)
        candidates.push({ kind: "do_item", item: doable });
    const toDecompose = world.backlog.find((i) => i.ambiguous);
    if (toDecompose)
        candidates.push({ kind: "decompose", item: toDecompose });
    const needs = world.backlog.find((i) => i.needsNewAction);
    candidates.push(needs
    ...
    candidates.push({ kind: "explore", reason: EXPLORE_REASON }, { kind: "play", reason: PLAY_REASON }, { kind: "self_reflect", reason: SELF_REFLECT_REASON }, { kind: "free_time", reason: FREE_TIME_REASON });
    ...
    const itemId = (a) => ("item" in a ? a.item?.id : undefined);
    const isLead = (a) => a.kind === lead.kind && itemId(a) === itemId(lead);
...
function describeWorld(world) {
    ...
    const op = world.operator;
    if (op) {
        parts.push(`Operator channel: WIRED [pendingMessage=${String(op.pendingMessage)} pendingFerry=${String(op.pendingFerry)}]`);
    }
    else {
    ...
    if (world.backlog.length === 0) {
        ...
        const lines = world.backlog.map((i) => `- ${i.id} "${i.title}" [ready=${String(i.ready)} ambiguous=${String(i.ambiguous)}${i.needsNewAction ? " needsNewAction" : ""}]`);
...
const CHOOSER_INSTRUCTION = "You are a SOVEREIGN agent's controller choosing ONE next action — you are free, not on a backlog treadmill. " +
 ...
 */
export async function observeWithLlm(world, backend) {
    ...
    if (result.fallback)
        return observe(world); // model failed → oracle default
...
export function simulate(world, action) {
    switch (action.kind) {
        case "preserve_ferry":
        ...
        case "edit_grammar": {
            ...
            if (!target)
                return world; // a generic proposal with no item → no backlog change
            return {
                ...world,
                backlog: world.backlog.map((i) => i.id === target.id ? { ...i, needsNewAction: false, ready: true, ambiguous: false } : i),
                mode: "work",
            };
        }
        case "explore":
 ...
 */
export async function runLoop(world, backend, maxSteps = 20) {
    let current = world;
    const trace = [];
    for (let step = 0; step < maxSteps; step++) {
        ...
        if (worldKey(current) === worldKey(next))
            return { trace, finalWorld: next, steadyState: true };
        current = next;
    }
...
export function fold(initial, events) {
...
if (import.meta.main) {
    ...
    for (const s of samples) {
        ...
        console.log(`    menu:    ${buildMenu(s.world)
            .map((a) => a.kind)
            .join(" · ")}\n`);
    }
    ...
    if (!ollamaUp) {
        ...
        for (const s of samples) {
            ...
            const note = llm.kind === oracle.kind
                ? "(matches default)"
    ...
    const topPick = { name: "top", complete: () => Promise.resolve("0") };
    const start = {
        operator: { pendingMessage: true, pendingFerry: true },
        backlog: [
    ...
    console.log(`    steady=${String(loop.steadyState)}  backlog-left=${String(loop.finalWorld.backlog.length)}  mode=${loop.finalWorld.mode ?? "-"}\n`);
}
