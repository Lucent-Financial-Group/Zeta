...
import { expandHome, manifestPathsForVendor, probeAllCredentials, probeVendorStatus, } from "./first-session-executor";
import { actionFromDemoToken, parseArgs, runFirstSession, sessionFromProbe, } from "./first-session-run";
...
describe("first-session-executor — probe", () => {
    ...
    it("probeVendorStatus gh ready when auth status succeeds", () => {
        const runner = fakeRunner({
            run: (cmd, args) => cmd === "gh" && args[0] === "auth" && args[1] === "status"
                ? { exitCode: 0 }
                : { exitCode: 1 },
        });
