...
import { simulatedWorkspacePort, emptySimulatedState, GATED_PERMISSIONS, DEFAULT_PERMISSIONS, } from "./workspace-port";
describe("simulatedWorkspacePort — in-memory filesystem", () => {
    test("writeFile + readFile round-trips", () => {
        ...
        if (readResult.ok)
            expect(readResult.value).toBe("export const x = 1;");
    });
...
describe("simulatedWorkspacePort — pre-seeded state", () => {
    test("pre-seeded files are readable immediately", () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok)
            expect(result.value).toContain("081KTEST");
    });
});
