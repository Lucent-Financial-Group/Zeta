 ...
 */
import { buildMenu, observe, actionLabel, isFirstSessionPending } from "./observe";
...
import { T, F, N } from "../tri-boolean/index";
...
const FREE_MODE_KINDS = ["explore", "play", "self_reflect", "free_time"];
/**
 ...
 */
export function renderGrammar16(world) {
    const menu = buildMenu(world);
    const find = (...kinds) => menu.find((a) => kinds.includes(a.kind));
    ...
    const freeModes = FREE_MODE_KINDS.map((k) => menu.find((a) => a.kind === k)).filter((a) => a !== undefined);
    const overrides = {
 ...
 */
export function leadSlot(world) {
