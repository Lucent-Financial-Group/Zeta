...
export function byHarness(records) {
    const m = new Map();
    for (const r of records)
        (m.get(r.harness) ?? m.set(r.harness, []).get(r.harness)).push(r);
    return m;
}
// ── CLI (I/O at the edge) ────────────────────────────────────────────────────
...
if (import.meta.main) {
    ...
    if (args.has("--record")) {
        const cal = JSON.parse(readFileSync("src/Core.TypeScript/observe/token-calibration.json", "utf8"));
        const ts = new Date().toISOString();
        const rows = measureNow().map((m) => ({ ts, harness: m.harness, bytes: m.bytes, estTokens: Math.round(estimateTokens(m.bytes, cal.bytesPerToken)) }));
