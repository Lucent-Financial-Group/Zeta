...
import { measureText, sum } from "../byte-cost/byte-cost";
...
export function assessDrift(cost, baseline, budget) {
...
if (import.meta.main) {
    ...
    for (const c of costs) {
        ...
        if (v.overBudget)
            over++;
        if (args.has("--kpi")) {
            console.log(`context_cost_bytes harness=${v.harness} resident=${v.current} ondemand=${c.onDemand.bytes} baseline=${v.baseline} delta=${v.delta} budget=${v.budget} over=${v.overBudget}`);
        }
        else {
            const sign = v.delta >= 0 ? "+" : "";
            console.log(`${v.overBudget ? "✗" : "✓"} ${v.harness}: ${v.current}B resident (${sign}${v.delta} vs baseline ${v.baseline}; budget ${v.budget}) + ${c.onDemand.bytes}B on-demand`);
            if (args.has("--verbose"))
                for (const f of c.files)
                    console.log(`    ${f.resident}B/${f.total}B ${f.mode === "description" ? "desc" : "whole"}  ${f.path}`);
        }
    }
