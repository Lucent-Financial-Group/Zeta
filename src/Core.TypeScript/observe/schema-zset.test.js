...
import { schemaZSet, applyDelta, consolidate, currentSchema, resolveField, isInOverlap, deltaFromSchemas, FS_METADATA_SCHEMA_V1, FS_METADATA_SCHEMA_V2, } from "./schema-zset";
...
describe("schema-zset — deltaFromSchemas (automatic delta computation)", () => {
    ...
    test("detects removed fields", () => {
        const before = [{ name: "a", type: "string", required: true }, { name: "b", type: "number", required: false }];
        const after = [{ name: "a", type: "string", required: true }];
    ...
    test("detects type changes (retract old + insert new)", () => {
        const before = [{ name: "x", type: "string", required: true }];
        const after = [{ name: "x", type: "number", required: true }];
        const delta = deltaFromSchemas(before, after);
        expect(delta.retract.length).toBe(1);
...
describe("schema-zset — overlap window", () => {
    ...
    test("isInOverlap detects retracted-but-not-consolidated fields", () => {
        ...
        const withRetraction = [...zset, { field: { name: "old", type: "string", required: true }, weight: -1 }];
