...
import { refCounts, retractedFieldRefCounts, checkQuorum, registerConsumer, deregisterConsumer, updateConsumerRefs, consumersOfField, buildReviewPrompt, } from "./schema-refcount";
...
describe("schema-refcount — reference counting", () => {
    ...
    test("retractedFieldRefCounts only counts fields no longer in active schema", () => {
        ...
        const schema = applyDelta(schemaZSet(FS_METADATA_SCHEMA_V1), { retract: [{ name: "modified", type: "string", required: false }], insert: [] });
