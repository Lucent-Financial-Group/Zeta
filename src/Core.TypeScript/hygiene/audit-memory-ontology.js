 ...
 */
function suggestRename(filename, currentPrefix, correctType) {
...
function applyFixes(memoryDir, mismatches, limit) {
    const toFix = mismatches.slice(0, limit);
    const fixed = [];
    const errors = [];
    // Detect duplicate suggestedName values in the fix set
    const suggestedNameCounts = new Map();
    for (const entry of toFix) {
        if (entry.error)
            continue; // skip entries with invalid types
    ...
    for (const entry of toFix) {
        ...
        if ((suggestedNameCounts.get(entry.suggestedName) ?? 0) > 1) {
            errors.push(`Skipped ${entry.file}: duplicate target '${entry.suggestedName}' (${suggestedNameCounts.get(entry.suggestedName)} files would collide)`);
            continue;
        }
