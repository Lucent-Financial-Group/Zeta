...
export function extractBody(row) {
    const fields = row.split("|");
    if (fields.length <= BODY_FIELD_INDEX)
        return null;
...
function findLatestTimestamp(content) {
    const timestamps = [];
...
export function main(argv) {
    ...
    if (body !== null) {
        ...
        if (verdict.isRut) {
            ...
            process.stderr.write("A degenerate-output tick must not be logged as a heartbeat. Fix the\n" +
