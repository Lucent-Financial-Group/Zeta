...
function emitReportSection(title, items, hint) {
    const lines = [];
    lines.push(`## ${title}`);
    if (items.length > 0) {
...
function emitReport(b) {
    ...
    lines.push("");
    for (const l of emitReportSection(`Exempt (${String(b.exempt.length)})`, b.exempt))
        lines.push(l);
    lines.push("");
    for (const l of emitReportSection(`Labelled exceptions (${String(b.labelled.length)})`, b.labelled))
        lines.push(l);
    lines.push("");
