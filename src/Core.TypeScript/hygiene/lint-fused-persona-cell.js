 ...
 */
export const PENDING_REGISTRY_SURFACES = ["cowork", "browser-tab", "chat"];
export function surfaceVocabulary() {
    const s = new Set(CELL_SURFACES);
    for (const extra of PENDING_REGISTRY_SURFACES)
        s.add(extra);
    return s;
}
...
function buildMatchers(personas, surfaces) {
 ...
 */
export function findFusedLiterals(source, filePath, personas = [...VALID_PERSONAS], surfaces = [...surfaceVocabulary()]) {
...
export const PARSER_MODULE_ALLOWLIST = [
...
export function isInScope(filePath) {
    ...
    if (!/^(src|tools|tests)\//.test(p))
        return false;
    if (!SCAN_EXTENSIONS.has(path.extname(p)))
        return false;
...
export function main(argv = process.argv.slice(2)) {
    ...
    if (updateBaseline) {
        ...
        const doc = {
            description: "Grandfathered fused persona-cell literals (pre-phase-8). Ratchet: shrink-only — never add entries by hand; phase-8 contract drives this to [].",
            entries,
        };
    ...
    for (const f of violations) {
        console.log(`VIOLATION ${f.file}:${f.line}  "${f.literal}" (${f.form}) — store the record (ActorRef), not the projection; parse/project only via identity/actor-ref.ts (treaty Art. 1)`);
    }
    ...
    console.log(`summary: ${files.length} files scanned, ${grandfathered.length} legacy (baseline), ${violations.length} new, ${stale.length} stale baseline entries`);
    if (violations.length > 0)
        return 1;
    if (strict && grandfathered.length > 0)
        return 1;
    return 0;
}
