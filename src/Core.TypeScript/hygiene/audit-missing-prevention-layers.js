...
function extractHygieneRows(content) {
    const rows = [];
    let inMain = false;
...
function extractClassifications(content) {
    const out = [];
    for (const line of content.split("\n")) {
        if (!ROW_RE.test(line))
            continue;
...
function audit(args, root) {
    ...
    for (const r of rows) {
        ...
        if (label === undefined || label === "")
            unclassified.push(r);
        else if (label.includes("detection-only (gap)"))
            gaps.push(r);
        else
            okCount += 1;
    }
...
function emitReport(args, r) {
    ...
    if (r.unclassified.length > 0) {
        ...
        for (const row of r.unclassified)
            lines.push(`- ${row.num} — ${row.title}`);
    }
    if (r.gaps.length > 0) {
        lines.push("");
        lines.push("## Gap rows (prevention layer candidate)");
        for (const row of r.gaps)
            lines.push(`- ${row.num} — ${row.title}`);
        lines.push("");
...
export function main(argv) {
    ...
    if (r.unclassified.length > 0 || r.gaps.length > 0)
        return 2;
    return 0;
}
if (import.meta.main) {
