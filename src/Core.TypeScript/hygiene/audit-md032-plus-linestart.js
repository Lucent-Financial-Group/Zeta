...
const EXCLUDE_RE = /^(docs\/ROUND-HISTORY\.md|docs\/hygiene-history\/|docs\/DECISIONS\/|tools\/hygiene\/audit-md032-plus-linestart\.(sh|ts))/;
...
function isBlankAfterStrip(line) {
...
function auditFile(file) {
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? "";
        if (!PLUS_MARKER_RE.test(line))
            continue;
        if (i === 0)
            continue;
        const prev = lines[i - 1] ?? "";
        if (isBlankAfterStrip(prev))
            continue;
        if (PLUS_MARKER_RE.test(prev))
            continue;
