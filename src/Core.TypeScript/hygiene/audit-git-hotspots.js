...
function reduceFlag(acc, flag, value) {
    if (acc.kind === "error")
        return acc;
    if (value === undefined) {
    ...
    if (flag === "--window")
        return { ...acc, window: value };
    if (flag === "--report")
        return { ...acc, report: value };
    if (flag === "--top") {
...
function parseArgs(argv) {
    ...
    while (i < argv.length) {
        ...
        if (acc.kind === "error")
            return { kind: "error", message: acc.message };
        i += 2;
    }
    return {
        kind: "args",
...
function expandWindowShorthand(window) {
    const match = /^(\d+)([dwmy])$/.exec(window);
    if (!match)
        return window;
    const n = match[1];
    const suffix = match[2];
    const unit = suffix === "d" ? "days" :
        suffix === "w" ? "weeks" :
            suffix === "m" ? "months" :
                "years";
    return `${n} ${unit} ago`;
}
function tallyTouches(window) {
...
function summariseFile(window, file, touches) {
    const since = expandWindowShorthand(window);
...
export function auditRepo(args) {
...
export function renderReport(result, clock) {
    const lines = [];
