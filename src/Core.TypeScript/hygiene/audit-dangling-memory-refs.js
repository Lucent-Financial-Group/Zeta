...
export function findEdgesInFile(absPath) {
    let content;
    try {
...
export function isDangling(ref) {
    const abs = join(repoRoot(), ref);
    return !isFile(abs);
}
export function auditSurface(surfaceRel) {
...
export function runAudit(surfaces) {
    const reports = surfaces.map((s) => auditSurface(s));
    const allDanglingRefs = new Set();
    let danglingEdges = 0;
    let filesScanned = 0;
...
function renderHuman(out) {
    ...
    for (const r of out.surfaces) {
        lines.push(`${r.surface}: ${r.edges.length} dangling edges / ${r.uniqueDanglingRefs} unique dangling refs (${r.scanned} files scanned, ${r.uniqueRefs} unique refs total)`);
        for (const e of r.edges) {
            lines.push(`  ${e.citing}:${e.line} → ${e.ref}`);
        }
    }
    lines.push("");
    lines.push(`TOTAL: ${out.totals.danglingEdges} dangling edges / ${out.totals.uniqueDanglingRefs} unique dangling refs across ${out.totals.surfacesScanned} surfaces (${out.totals.filesScanned} files scanned)`);
    return lines.join("\n");
}
...
export function main(argv) {
    ...
    if (strayPositional.length > 0) {
        process.stderr.write(`error: unexpected positional argument(s): ${strayPositional.join(", ")}; paths only allowed after --surfaces\n`);
        return 2;
    }
    ...
    if (surfacesFlagSeen && customSurfaces.length === 0) {
        process.stderr.write("error: --surfaces specified but no paths provided; refusing to fall back to DEFAULT_SURFACES\n");
        return 2;
    }
    ...
    if (missingSurfaces.length > 0) {
        process.stderr.write(`error: requested surface(s) do not exist under ROOT=${repoRoot()}: ${missingSurfaces.join(", ")}\n`);
        return 2;
    }
