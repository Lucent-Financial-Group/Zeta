...
function emitUsageError(message) {
...
function classifyArg(arg, next) {
    if (arg === "--cadence") {
        if (next === undefined)
            return { kind: "error", message: "--cadence requires a value" };
    ...
    if (arg === "--count") {
        if (next === undefined)
            return { kind: "error", message: "--count requires a value" };
...
function listCounterweights(memoryDir) {
    let entries;
    try {
...
function extractOttoId(filename) {
    const m = OTTO_ID_RE.exec(filename);
    if (m === null)
        return "(no Otto-ID in filename)";
    return `Otto-${m[1] ?? ""}`;
}
...
function nameValueOrNull(line) {
    if (!line.startsWith(NAME_FIELD_PREFIX))
        return null;
...
function extractNameField(content) {
...
function emitHeader(cadence, shown, total) {
...
function emitCounterweight(ottoId, rel, nameLine) {
    process.stdout.write("---\n");
    process.stdout.write("\n");
...
export function main(argv) {
    ...
    if (parsed.args === null)
        return emitUsageError(parsed.errorMessage);
