...
export function parseShardPath(absPath) {
...
export function alreadyCompliant(content, info) {
...
export function buildHeader(iso) {
...
export function processOne(absPath, write) {
...
export function main(argv) {
    ...
    if (strayPositional.length > 0) {
        process.stderr.write(`error: unexpected positional argument(s): ${strayPositional.join(", ")}; paths only allowed after --files\n`);
        return 1;
    }
    ...
    if (filesFlagSeen) {
        ...
        for (const { input, abs } of resolvedInputs) {
            ...
            const keep = rel.startsWith(SHARD_PREFIX) &&
                rel.endsWith(".md") &&
        ...
        if (droppedInputs.length > 0) {
            process.stderr.write(`error: --files paths filtered out (missing, outside shard tree, or not .md): ${droppedInputs.join(", ")}\n`);
            return 1;
        }
    }
    else {
    ...
    process.stderr.write(`[${mode}] scanned ${shards.length}; compliant=${counts["skip-already-compliant"]} ` +
