...
function stripHeadingPunctuation(line) {
    ...
    if (i === trimmed.length)
        return null; // no trailing punctuation
...
function isListOrContinuation(line) {
...
function frontmatterRatioOk(lines, fmEnd) {
    let yamlCount = 0;
    let nonBlank = 0;
    for (let j = 1; j < fmEnd; j += 1) {
        const lj = lines[j];
        if (lj === undefined || lj.trim() === "")
            continue;
        nonBlank += 1;
        if (YAML_KEY_LINE.test(lj) || isYamlContinuation(lj))
            yamlCount += 1;
    }
...
function findFrontmatterRegion(lines) {
    ...
    if (line0.trimEnd() !== "---")
        return -1;
    if (!YAML_KEY_LINE.test(line1))
        return -1;
    const fmEnd = findFrontmatterEnd(lines);
    if (fmEnd <= 0)
        return -1;
    if (!isYamlOk(lines[fmEnd - 1] ?? ""))
        return -1;
    if (!frontmatterRatioOk(lines, fmEnd))
        return -1;
    return fmEnd;
}
...
function processFenceLine(line, i, inside, state) {
    ...
    const isClose = parsed.fenceChar === state.openChar &&
...
function insertBlanksBefore(lines, inside) {
    const out = [];
    const outInside = [];
    for (let i = 0; i < lines.length; i += 1) {
        const line = lines[i];
...
function insertBlanksAfter(lines, inside) {
    const out = [];
    for (let i = 0; i < lines.length; i += 1) {
        const line = lines[i];
        if (line === undefined)
...
async function fixFile(path, dryRun) {
    let original;
    try {
...
function parseArgs(argv) {
    ...
    if (args.files.length === 0)
        fail("at least one FILE is required", 2);
    return args;
}
function printHelp() {
    console.log([
...
async function processOneFile(path, dryRun) {
