...
const INLINE_CROSSREF_PATTERNS = [
 ...
 * deliverables).
 */
export function extractPrimaryArtifacts(body) {
    const lines = body.split("\n");
    const artifacts = new Set();
    let sectionMode = "other";
    for (const line of lines) {
        ...
        for (const match of extractableSegment.matchAll(PATH_REGEX)) {
            ...
            if (candidate.startsWith("docs/backlog/"))
                continue;
...
export function enumerateOpenRows(backlogDir = "docs/backlog") {
    const rows = [];
...
export function findDriftCandidates(rows) {
    return rows.filter((row) => {
        if (row.primaryArtifacts.length === 0)
            return false;
        return row.primaryArtifacts.every((p) => existsSync(p));
    });
}
function reportMarkdown(candidates) {
    ...
    console.log();
    console.log("**Reminder**: Each candidate requires manual verification before close-row — every Acceptance bullet must have a corresponding merged PR, not just the file present. See `.claude/rules/backlog-item-start-gate.md` step 0 for the partial-vs-drift discriminator.");
}
function reportJson(candidates) {
...
function main() {
    ...
    try {
        ...
        process.stderr.write(`audit-backlog-status-drift: unable to chdir to repo root: ${err.message}\n`);
        return 64;
    }
    ...
    for (const arg of args) {
        if (!KNOWN_FLAGS.has(arg)) {
            ...
            console.error("Usage: bun tools/hygiene/audit-backlog-status-drift.ts [--json] [--check]");
            return 64;
        }
    }
    if (args.includes("--help") || args.includes("-h")) {
        console.log("Usage: bun tools/hygiene/audit-backlog-status-drift.ts [--json] [--check]\n\n" +
