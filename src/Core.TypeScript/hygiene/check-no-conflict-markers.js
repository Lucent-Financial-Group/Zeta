...
function grepMarkerLines() {
    ...
    if (result.status === GREP_NO_MATCH)
        return [];
    const failure = classifyFailure("git", ["grep", "-n", "-I", "-E", MARKER_GREP_PATTERN, "--", "."], result);
    if (failure !== null)
...
export function main() {
    ...
    if (violations.length > 0) {
        ...
        process.stderr.write(`FAIL: ${String(violations.length)} git merge-conflict marker line(s) found in committed files\n`);
        process.stderr.write("\n");
