...
import { snapshot } from "./snapshot-github-settings.js";
...
export async function main(argv) {
    ...
    let expectedObj;
    try {
        liveObj = JSON.parse(liveContent);
    ...
    function isSkipped(v) {
        return (v !== null &&
            typeof v === "object" &&
            "_skipped" in v &&
            v._skipped === "insufficient-token-scope");
    }
    function stripSkippedSentinels(live, exp, pathPrefix, report) {
        for (const key of Object.keys(live)) {
            ...
            if (isSkipped(val)) {
                ...
                stripSkippedSentinels(val, exp[key], pathPrefix.length > 0 ? `${pathPrefix}.${key}` : key, report);
            }
        }
    }
    const skippedPaths = [];
    ...
    if (skippedPaths.length > 0) {
        process.stderr.write(`github-settings-drift: skipping ${skippedPaths.length} field(s) not readable with current token: ${skippedPaths.join(", ")}\n`);
