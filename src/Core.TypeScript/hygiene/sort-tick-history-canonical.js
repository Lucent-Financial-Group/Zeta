...
function findSeparatorLine(lines) {
    let sepIdx = null;
    const sepPattern = /^\|[-|\s]+\|$/;
...
function bucketDataLines(data) {
    const dataRows = [];
    const unmatchedTableRows = [];
...
function computeTrailingLines(data, dataRows, unmatchedTableRows) {
    const tableIndices = [
...
function enforceRowGuards(rowsIn, unmatchedTableRows, sepFileLine) {
    ...
    if (rowsIn === 0) {
        throw new Error(`schema drift: ${String(unmatchedTableRows.length)} table-shaped row(s) ` +
    ...
    throw new Error(`refusing to drop ${String(unmatchedTableRows.length)} unmatched ` +
...
function sortAndDedupe(dataRows) {
...
function assembleOutput(header, uniqueRows, trailingLines) {
...
function detectReordering(uniqueRows, originalOrder, rowsIn) {
    if (uniqueRows.length !== rowsIn)
        return true;
    for (let i = 0; i < uniqueRows.length; i += 1) {
        if (uniqueRows[i] !== originalOrder[i])
            return true;
    }
    return false;
}
