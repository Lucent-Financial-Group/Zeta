...
function readDeclLine(path) {
    ...
    for (const line of head) {
        if (LABEL_DECL_RE.test(line))
            return line;
    }
    return null;
}
function classify(path) {
    if (path.startsWith("tools/setup/"))
        return "pre-setup";
    if (path.startsWith("tools/_deprecated/"))
        return "exempt-deprecated";
...
function classifyShFile(file, buckets) {
    const stem = file.slice(0, -3);
    const hasTwin = gitTracksFile(`${stem}.ps1`);
    const klass = classify(file);
    if (klass === "pre-setup") {
        if (hasTwin)
            buckets.paired.push(file);
        else
            buckets.preSetupGapBash.push(file);
        return;
    }
...
function classifyPs1File(file, buckets) {
    if (file.startsWith("tools/setup/")) {
        const stem = file.slice(0, -4);
        if (!gitTracksFile(`${stem}.sh`))
            buckets.preSetupGapPs1.push(file);
        return;
    }
...
export function auditRepo() {
    ...
    for (const file of all) {
        if (file.endsWith(".sh"))
            classifyShFile(file, buckets);
        else if (file.endsWith(".ps1"))
            classifyPs1File(file, buckets);
    }
    return {
...
export function gapCount(buckets) {
    return (buckets.preSetupGapBash.length +
...
function renderBulletList(items, suffix) {
    if (items.length === 0)
        return ["- (none)"];
    return items.map((i) => (suffix === "" ? `- ${i}` : `- ${i} ${suffix}`));
}
export function renderReport(buckets, mode, clock) {
    const lines = [];
    ...
    lines.push("");
    for (const l of renderBulletList(buckets.paired, ""))
        lines.push(l);
    lines.push("");
    ...
    lines.push("");
    for (const l of renderBulletList(buckets.preSetupGapBash, buckets.preSetupGapBash.length > 0 ? "(missing .ps1)" : "")) {
    ...
    for (const l of renderBulletList(buckets.transitional, ""))
        lines.push(l);
    lines.push("");
    lines.push("Exempt:");
    for (const l of renderBulletList(buckets.exempt, ""))
        lines.push(l);
    lines.push("");
    lines.push("## Summary");
    lines.push("");
