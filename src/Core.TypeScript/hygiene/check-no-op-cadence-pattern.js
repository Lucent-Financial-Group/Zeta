 ...
 */
export function parsePositiveInt(envName, fallback) {
 ...
 */
export function collectShards(dir, dateFlat) {
    if (!dateFlat || !existsSync(dir))
        return [];
    let files;
    try {
 ...
 * (#1366 P1 finding).
 */
export const OBSERVATION_CLASS_REGEX = /minimal observation|within-basin observation|observe-only|minimal[ -]not[ -]idle|same\.[ \t]*stopping/i;
export function isMinimalObservation(path) {
    let body = "";
    let content = "";
    try {
...
export function runCheck(repoRoot, args) {
    ...
    for (const shard of recent) {
        if (isMinimalObservation(shard.path))
            minObsCount++;
    }
...
export function main() {
    ...
    console.error(`[no-op-check] Recent ${result.totalShards} shards across today+yesterday; ${result.minObsCount} match minimal-observation pattern (threshold: ${args.threshold}).`);
    if (result.thresholdHit) {
        console.error("");
        console.error(`WARNING: no-op-cadence pattern detected — ${result.minObsCount}/${result.totalShards} recent ticks are minimal-observation.`);
        console.error("");
    ...
    if (result.gapMinutes === null) {
        ...
        console.error(`[no-op-check] Most recent shard ${result.gapMinutes} minutes old (gap-threshold: ${args.gapThresholdMinutes}).`);
        if (result.gapHit) {
            console.error("");
            console.error(`WARNING: missing-shard-cadence detected — most recent shard is ${result.gapMinutes} minutes old, exceeding threshold ${args.gapThresholdMinutes} minutes.`);
            console.error("");
            ...
            console.error("  - Repeated 'standing by' / minimal-acknowledgment chat output WITHOUT writing shards IS the failure mode");
            console.error("  - Body-length check above doesn't catch this because it requires shards to exist");
            console.error("  - Shard-density check (this) catches the gap structurally without needing chat-transcript scanning");
            console.error("");
