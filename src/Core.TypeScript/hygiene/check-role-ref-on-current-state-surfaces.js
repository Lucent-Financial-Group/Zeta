...
function expandSurfaces(patterns) {
    const result = [];
    ...
    for (const name of allNames) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (name === "Claude.ai") {
                ...
                pattern = new RegExp(`\\b${name}( 2026| 2027|'s [a-z]| said| grants| proposed| asked| corrected| confirmed| disclosed)`);
            }
            if (pattern.test(line)) {
                const lineNum = i + 1;
                process.stderr.write(`VIOLATION: ${surface}:${lineNum}: direct name attribution '${name}' on current-state surface\n` +
                    `  ${line.trim()}\n` +
