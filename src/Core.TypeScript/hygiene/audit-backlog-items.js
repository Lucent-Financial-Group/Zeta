...
function tierStatusBreakdown(rows) {
...
function buildAgedRows(rows, nowEpoch) {
    const aged = [];
    for (const r of rows) {
...
function reportBrokenEdges(rows) {
...
function reportOrphans(rows, edges) {
    ...
    for (const [, target] of edges.depends)
        incoming.add(target);
    for (const [, target] of edges.composes)
        incoming.add(target);
...
function reportTopBlocked(rows, edges) {
...
async function reportMergedCandidates(rows, ghAvailable) {
    ...
    if (r.exitCode !== 0) {
        console.log(`SKIP: gh pr list (merged) exited ${r.exitCode}; merged-candidate scan unreliable. stderr: ${r.stderr.trim()}`);
        console.log("");
        return;
    }
    let prs = [];
...
function reportDuplicateIds(rows) {
    ...
    if (duplicates.length > 0) {
        ...
        console.log("`renumbered_from:` breadcrumb. Per src/Core.TypeScript/backlog/README.md, backlog");
...
function reportParentChildStatusMismatch(rows) {
    ...
    for (const parent of rows) {
        if (!isClosedStatus(parent.status))
            continue;
        if (parent.childrenRefs.length === 0)
            continue;
        const openChildren = [];
...
async function main() {
    ...
    let brokenDepends = 0;
    let brokenComposes = 0;
    for (const [, ref] of edges.depends)
        if (!allIds.has(ref))
            brokenDepends++;
    for (const [, ref] of edges.composes)
        if (!allIds.has(ref))
            brokenComposes++;
