...
function isHistorySurface(f) {
    if (f.startsWith("memory/"))
        return true;
    if (f.startsWith("docs/research/"))
        return true;
    if (f.startsWith("docs/aurora/"))
        return true;
    if (f === "docs/ROUND-HISTORY.md")
        return true;
    if (f.startsWith("docs/DECISIONS/"))
        return true;
    if (f.startsWith("docs/hygiene-history/"))
        return true;
    if (f.startsWith("docs/pr-preservation/"))
        return true;
    if (f.startsWith("docs/pr-discussions/"))
        return true;
    if (f === "docs/active-trajectory.md")
        return true;
    if (f.startsWith("docs/backlog/"))
        return true;
    if (f.startsWith("docs/lost-substrate/"))
        return true;
    if (f === "docs/CURRENT-ROUND.md")
        return true;
    if (f.startsWith("docs/amara-full-conversation/"))
        return true;
    if (f.startsWith(".git/"))
        return true;
    ...
    if (f.startsWith("third_party/"))
        return true;
    if (f.startsWith("references/prior-art/"))
        return true;
    if (f.startsWith("tools/lean4/.lake/"))
        return true;
    if (f.startsWith("tools/setup/build/"))
        return true;
    return false;
}
// Code-surface inclusion check
function isCodeSurface(f) {
    ...
    if (f.startsWith("docs/"))
        return true;
    if (/^\.claude\/skills\/[^/]+\/SKILL\.md$/.test(f))
        return true;
    if (/^\.claude\/agents\/[^/]+\.md$/.test(f))
        return true;
    if (/^\.claude\/rules\/[^/]+\.md$/.test(f))
        return true;
    if (/^\.claude\/commands\/[^/]+\.md$/.test(f))
        return true;
    if (f.startsWith("src/"))
        return true;
    if (f.startsWith("tests/"))
        return true;
    if (f.startsWith("openspec/specs/"))
        return true;
    if (f.endsWith(".fsproj") || f.endsWith(".csproj"))
        return true;
    // Root-level *.md (README, AGENTS, GOVERNANCE, CLAUDE.md, etc.)
    if (!f.includes("/") && f.endsWith(".md"))
        return true;
    return false;
}
...
export function main(argv) {
    ...
    process.stdout.write(`Found ${allFindings.length} orphan role-ref / un-stripped-name-attribution findings on code-surface files.\n`);
    process.stdout.write("\n");
