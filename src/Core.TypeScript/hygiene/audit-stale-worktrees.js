...
function parseWorktreePorcelain(stdout) {
    ...
    for (const block of blocks) {
        ...
        if (path)
            entries.push({ path, head, branch, prunable });
    }
    return entries;
}
function audit(root) {
    ...
    const stalePathExists = [];
    const stalePathMissing = [];
    let healthy = 0;
...
function renderReport(result, now, pruned) {
    const lines = [];
...
function main(argv) {
    ...
    let pruneOutput = null;
    if (parsed.args.prune && r.stalePathMissing.length > 0) {
        const p = runPrune(parsed.args.root);
        pruneOutput = p.output;
        if (!p.ok)
            console.error("git worktree prune exited non-zero (continuing — some entries may have pruned)");
    }
