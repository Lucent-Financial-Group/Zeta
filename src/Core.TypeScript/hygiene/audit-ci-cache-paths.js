 ...
 */
function parseCachePaths(workflow, content) {
    const out = [];
 ...
 * overlap.
 */
function overlapsTrackedFile(cachePath, trackedFiles) {
...
function main() {
    ...
    for (const wf of workflows) {
        ...
        try {
            ...
            process.stderr.write(`ERROR: cannot read ${wf}: ${err.message}\n`);
            readFailures++;
            continue;
        }
        const cachePaths = parseCachePaths(wf, content);
        for (const cp of cachePaths) {
            if (isUserHomePath(cp.path))
                continue;
    ...
    if (violations.length === 0) {
        process.stdout.write(`OK: ${String(workflows.length)} workflow(s) audited; no cache paths overlap git-tracked files\n`);
        return 0;
    }
    for (const v of violations) {
        process.stderr.write(`VIOLATION: ${v.workflow} step "${v.stepName}" caches "${v.cachePath}" which contains git-tracked file "${v.trackedFile}"\n`);
    }
    process.stderr.write("\n");
    process.stderr.write(`FAIL: ${String(violations.length)} cache-path violation(s) — actions/cache paths overlap git-tracked files\n`);
    process.stderr.write("\n");
