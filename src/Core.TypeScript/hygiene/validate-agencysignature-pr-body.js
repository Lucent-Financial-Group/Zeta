...
function parseTrailers(stripped) {
    // eslint-disable-next-line sonarjs/no-os-command-from-path
...
function findExactLineNumber(input, target) {
...
function findKeyPrefixLineNumber(input, key) {
...
function trailerLineKey(line) {
    const idx = line.indexOf(":");
    if (idx <= 0)
        return "";
    return line.slice(0, idx);
}
function nonBlankLinesAfter(input, lineNum) {
    const lines = input.split("\n");
...
function getValue(trailers, key) {
    const prefix = `${key.toLowerCase()}:`;
...
function emitNonTrailerAfterFailure(after) {
...
function emitMissingKeys(missing) {
...
function emitEnumFailure(key, found, allowed) {
...
function checkTerminalBlock(stripped, trailers) {
    ...
    if (tailLine === 0) {
        ...
        if (key !== "")
            tailLine = findKeyPrefixLineNumber(stripped, key);
    }
    if (tailLine === 0)
        return emitLookupFailure();
    const after = nonBlankLinesAfter(stripped, tailLine);
    if (after.length > 0)
        return emitNonTrailerAfterFailure(after);
    return null;
}
function checkRequiredKeys(trailers) {
    const missing = [];
...
function checkEnums(trailers) {
    for (const { key, allowed } of ENUMS) {
        const value = getValue(trailers, key);
        if (!allowed.includes(value))
            return emitEnumFailure(key, value, allowed);
    }
    return null;
}
function checkTaskPattern(trailers) {
...
function checkHumanReviewConsistency(trailers) {
...
export function main() {
    ...
    if (input === "") {
        ...
        process.stderr.write("usage: gh pr view N --json body --jq '.body' | bun tools/hygiene/validate-agencysignature-pr-body.ts\n");
        return 2;
    }
    const stripped = stripCodeFences(input);
    const trailers = parseTrailers(stripped);
    if (trailers === "")
        return emitParseFailure();
