...
function loadBaseline(path) {
    ...
    try {
        ...
        process.stderr.write(`baseline parse failed: ${path}: ${e.message}\n`);
        process.exit(64);
    }
...
function isInBaseline(f, baseline) {
    for (const b of baseline) {
        if (b.file === f.file && b.line === f.line && b.target === f.target)
            return true;
    }
    return false;
}
...
function isRelativeTarget(target) {
    if (URI_SCHEME_RE.test(target))
        return false; // http(s)://, mailto:, ftp:, file:, tel:, data:, etc.
...
function buildCodeFenceFlags(lines) {
...
export function stripInlineCodeSpans(line) {
    ...
    while (i < line.length) {
        ...
        while (j < line.length) {
            if (line[j] === "`") {
                let close = 0;
                while (j + close < line.length && line[j + close] === "`")
                    close++;
                if (close === run) {
                    found = j;
                    break;
...
export function main(argv) {
    ...
    if (args.files) {
        ...
        for (const f of args.files) {
            ...
            try {
                ...
                bad.push({ path: f, reason: `stat failed (${e.message})` });
            }
        }
        if (bad.length > 0) {
            for (const b of bad)
                process.stderr.write(`input ${b.reason}: ${b.path}\n`);
            return 64;
        }
    }
    ...
    for (const f of findings) {
        if (isInBaseline(f, baseline))
            baselineMatched.push(f);
        else
            newFindings.push(f);
    }
    if (args.json) {
