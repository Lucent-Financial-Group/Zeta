...
import { baselineKey, findFusedLiterals, isAllowlisted, isInScope, splitByBaseline, surfaceVocabulary, } from "./lint-fused-persona-cell";
...
describe("findFusedLiterals — hyphen (legacy composite) form", () => {
    ...
    test("does not flag a composite embedded in a longer slug (doc filenames)", () => {
        const line = 'const doc = "docs/research/2026-05-15-ferry-lior-gemini-cascade-close.md";';
...
describe("baseline ratchet", () => {
    test("splits grandfathered from new violations by file:literal key", () => {
        const findings = findFusedLiterals(['const a = "otto-cli";', 'const b = "riven-cursor";'].join("\n"), FILE);
