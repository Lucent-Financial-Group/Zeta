...
function classify(relPath) {
    ...
    if (relPath.startsWith("tools/Z3Verify/") && (ext === ".fs" || ext === ".fsproj"))
        return "z3";
    if (relPath.startsWith("tools/alloy/") && ext === ".java")
        return "alloy";
...
function findRefsInIndex(relPath, index) {
    const refs = [];
...
function emitMarkdown(artifacts) {
    ...
    console.log("");
    const categories = ["lean4", "tla+", "z3", "alloy", "formal-test"];
...
async function main() {
    ...
    if (jsonMode) {
        const summary = {
            ...
            byCategory: Object.fromEntries(["lean4", "tla+", "z3", "alloy", "formal-test"].map((cat) => {
                const items = artifacts.filter((a) => a.category === cat);
                return [
                    cat,
                    {
