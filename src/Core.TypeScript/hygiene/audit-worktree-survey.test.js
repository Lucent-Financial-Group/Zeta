...
import { classifyWorktrees, formatSurveyOutput, inspectWorktreeEntry, makeSurvey, parseArgs, parseWorktreePorcelain, renderMarkdown, } from "./audit-worktree-survey.js";
...
describe("renderMarkdown", () => {
    test("surveys only locked or prunable worktrees", () => {
        ...
        ], { inspect: () => inspection() }, new Date("2026-05-31T14:32:00Z"), null);
    ...
    test("renders totals and bucket tables", () => {
        const survey = makeSurvey([entry({ path: "/repo/dirty" }), entry({ path: "/repo/stale", head: null, prunable: true })], {
            inspect: (e) => e.path.endsWith("stale")
                ? inspection({ pathExists: false, dirty: null, headReachableFromMain: null })
                : inspection({ dirty: true, headReachableFromMain: false }),
        }, new Date("2026-05-31T14:32:00Z"), "`/repo`");
    ...
    test("escapes markdown table cells", () => {
        const survey = makeSurvey([entry({ path: "/repo/has|pipe\\slash", branch: "refs/heads/has`tick" })], {
            inspect: () => inspection({
                dirty: null,
                headReachableFromMain: null,
                statusError: "fatal: reason|with\nnewline",
            }),
        }, new Date("2026-05-31T14:32:00Z"), null);
        const md = renderMarkdown(survey);
    ...
    test("uses longer code fences for values with edge backticks", () => {
        const survey = makeSurvey([entry({ path: "`/repo/edge`", branch: "refs/heads/``edge" })], { inspect: () => inspection() }, new Date("2026-05-31T14:32:00Z"), null);
        const md = renderMarkdown(survey);
