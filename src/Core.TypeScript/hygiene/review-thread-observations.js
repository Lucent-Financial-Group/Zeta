...
import { fileReviewThreadDisagreement, } from "./divergence-shard.js";
...
function threadKey(observation) {
...
export function validateReviewThreadObservation(observation) {
...
export function emptyObservationStore() {
...
function parseStoredObservation(value, index) {
    if (!isRecord(value)) {
...
function parseStoredDisagreement(value, index) {
    if (!isRecord(value)) {
    ...
    if (evidenceFingerprints !== undefined &&
        ...
        throw new Error(`observation store filedDisagreements entry ${index}.evidenceFingerprints must be a 2-item string array`);
    }
    return {
...
export function parseObservationStore(text) {
...
export function loadObservationStore(repoRoot, storeRelPath = DEFAULT_OBSERVATION_STORE_REL_PATH) {
...
export function writeObservationStore(repoRoot, storeRelPath, store) {
    const relPath = normalizeRelPath(storeRelPath);
    const absPath = join(repoRoot, relPath);
...
function acquireObservationStoreLock(repoRoot, storeRelPath) {
...
function withObservationStoreLock(repoRoot, storeRelPath, fn) {
...
export function recordReviewThreadObservation(input) {
    ...
    return withObservationStoreLock(input.repoRoot, storeRelPath, () => {
        ...
        const priorSameThread = store.observations.filter((prior) => sameReviewThread(prior.observation, input.observation) &&
            differentLoopIdentity(prior.observation, input.observation));
        const filedDisagreements = [...(store.filedDisagreements ?? [])];
        const filed = [];
        const noDisagreements = [];
        const writeStore = () => writeObservationStore(input.repoRoot, storeRelPath, {
            schemaVersion: SCHEMA_VERSION,
            observations,
            filedDisagreements,
        });
        ...
        for (const prior of priorSameThread) {
            if (normalizedConclusion(prior.observation.conclusion) !== normalizedConclusion(input.observation.conclusion) &&
...
export function recordReviewThreadObservationBatch(input) {
    return input.observations.map((observationInput) => recordReviewThreadObservation({
        ...observationInput,
...
function readRequired(flags, key) {
...
const OPTIONAL_FLAGS = ["--repo-root", "--store", "--observed-at"];
