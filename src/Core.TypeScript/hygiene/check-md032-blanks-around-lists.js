 ...
 */
function isListFriendlyLeading(line, inList) {
...
export function findMd032Violations(content) {
 ...
 *   refresh).
 */
export function checkFiles(files, surfaceReadErrors = false) {
    const all = [];
    for (const f of files) {
        ...
        try {
            ...
            if (surfaceReadErrors) {
                throw Object.assign(new Error(`Cannot read '${f}': ${e.message ?? String(e)}`), { cause: e });
            }
 ...
 */
export function loadMarkdownlintIgnores(repoRoot) {
    ...
    try {
        ...
        if (Array.isArray(json.ignores)) {
            return json.ignores.filter((g) => typeof g === "string");
        }
        return [];
    }
    catch {
 ...
 */
export function stagedMarkdownFiles(repoRoot) {
    ...
    const r = spawnSync("git", ["-C", repoRoot, "diff", "--name-only", "--cached", "--diff-filter=AMR"], { encoding: "utf-8" });
 ...
 */
export function readStagedBlob(repoRoot, repoRelPath) {
    // eslint-disable-next-line sonarjs/no-os-command-from-path -- git invoked as explicit args array; no shell, no user input on the command line.
 ...
 */
export function checkStagedFiles(repoRoot, paths) {
...
export function main() {
    ...
    if (isStaged) {
        ...
        try {
            ...
            console.error(`check-md032: ${e.message}`);
            return 1;
        }
    }
