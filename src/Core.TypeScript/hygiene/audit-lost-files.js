...
async function runCmd(cmd, cwd = REPO_ROOT) {
...
async function classClosedNotMergedPRs(ghAvailable) {
    ...
    if (r.exitCode !== 0) {
        console.log(`SKIP: gh pr list (closed) exited ${r.exitCode}; closed-not-merged class unreliable. stderr: ${r.stderr.trim()}`);
        console.log("");
        return;
    }
    let prs = [];
    ...
    if (closedNotMerged.length > 0) {
        console.log("Triage: per Otto-262 + Otto-257 + Otto-254 -- recover via roll-forward on fresh short-lived branch OR prune.");
...
async function classOrphanBranches(ghAvailable) {
    ...
    if (refsResult.exitCode !== 0) {
        console.log(`SKIP: git for-each-ref exited ${refsResult.exitCode}; orphan-branch ref scan unreliable. stderr: ${refsResult.stderr.trim()}`);
        console.log("");
        return;
    }
    const unmerged = new Set(refsResult.stdout
        .split("\n")
    ...
    if (prResult.exitCode !== 0) {
        console.log(`SKIP: gh pr list exited ${prResult.exitCode}; orphan-branch detection unreliable. stderr: ${prResult.stderr.trim()}`);
        console.log("");
        return;
    }
    let prBranches = new Set();
    try {
        const arr = JSON.parse(prResult.stdout || "[]");
...
async function classDeletedFilesRecent() {
...
async function classUntracked() {
    console.log("## 6. Untracked working-directory artifacts (db/drop/, .playwright-mcp/, *.tmp, *.log)");
...
async function classDraftPRs(ghAvailable) {
    ...
    if (r.exitCode !== 0) {
        console.log(`SKIP: gh pr list (drafts) exited ${r.exitCode}; draft-PR count unreliable. stderr: ${r.stderr.trim()}`);
        console.log("");
        return;
    }
    let drafts = [];
    try {
        drafts = JSON.parse(r.stdout || "[]");
    }
    catch {
        drafts = [];
    }
...
function classDeferred() {
    console.log("## 9-14. Closed-PR threads / squash intermediates / force-pushed / courier-ferry / external exports / deleted-PR-descriptions");
...
async function classMemoryRefs(bunAvailable) {
