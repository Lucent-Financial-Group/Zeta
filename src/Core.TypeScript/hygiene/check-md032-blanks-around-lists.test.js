...
import { findMd032Violations, checkFiles, globToRegex, loadMarkdownlintIgnores, stagedMarkdownFiles, readStagedBlob, checkStagedFiles, } from "./check-md032-blanks-around-lists.js";
/**
...
const git = (args, cwd) => spawnSync("git", args, { cwd });
...
describe("loadMarkdownlintIgnores + stagedMarkdownFiles", () => {
    ...
    test("loadMarkdownlintIgnores parses JSONC with line comments", () => {
        const dir = mkdtempSync(join(tmpdir(), "md032-test-"));
        try {
            writeFileSync(join(dir, ".markdownlint-cli2.jsonc"), `// header comment\n{\n  "ignores": [\n    // ignore agent logs\n    "memory/**",\n    "docs/pr-preservation/**"\n  ]\n}\n`);
            expect(loadMarkdownlintIgnores(dir)).toEqual([
                "memory/**",
                "docs/pr-preservation/**",
            ]);
