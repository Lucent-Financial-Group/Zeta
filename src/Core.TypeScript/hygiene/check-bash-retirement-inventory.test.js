...
import { buildInventoryReport, hasDrift, renderReport, EXPECTED_RETAINED_SHELL, RETAINED_SHELL_SCOPE, trackedNonLeanShellFilesFromGit, } from "./check-bash-retirement-inventory";
...
function splitExpectedRetained() {
    const [missing, ...rest] = EXPECTED_RETAINED_SHELL;
    if (missing === undefined)
        throw new Error("expected retained shell allowlist must be non-empty");
    return [missing, rest];
}
function firstTwoExpectedRetained() {
...
describe("package.json wiring", () => {
    test("keeps the package wiring pointed at the enforcing inventory guard", () => {
        ...
        expect(packageJson.scripts?.["hygiene:check-bash-retirement-inventory"]).toBe("bun ./src/Core.TypeScript/hygiene/check-bash-retirement-inventory.ts --enforce");
    });
});
describe("buildInventoryReport", () => {
    ...
    test("summarizes retained shell files by explicit category", () => {
        ...
        expect(report.retainedCategories.slice(1)).toEqual([
            {
                category: "git hooks",
                files: ["githooks/pre-push", "scripts/hooks/commit-msg", "scripts/hooks/install-git-hooks.sh"],
            },
            {
    ...
    test("enumerates tracked shell-family files while excluding Lean vendor scripts", () => {
        ...
        try {
            ...
            writeFileSync(join(repo, "scripts", "extensionless-bash-env-s-quoted-assignment"), "#!/usr/bin/env -S 'NAME=two words' bash -eu\n");
            ...
            writeFileSync(join(repo, "scripts", "extensionless-zsh-env-split-string-quoted"), '#!/usr/bin/env --split-string "zsh -eu"\n');
...
describe("renderReport", () => {
    test("renders an OK summary for a matching inventory", () => {
        ...
        expect(renderReport(report)).toContain("- git hooks: 3");
    ...
    test("renders uncategorized allowlist entries as allowlist integrity errors", () => {
        const uncategorized = "tools/setup/zz-new-bootstrap.sh";
        const rendered = renderReport(buildInventoryReport(EXPECTED_RETAINED_SHELL, [...EXPECTED_RETAINED_SHELL, uncategorized]));
        expect(rendered).toContain("## Retained shell allowlist integrity errors");
