...
function inspect(path: string): void {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]!;
    if (line.includes(MARKER)) continue; // the declaration line itself may name the banned doors
    if (line.includes("SEAL-WAIVER:")) continue; // explicit, visible, per-line waiver (falsifiers PROVE the boundary rejects these tokens — the waiver names why)
    for (const [re, why] of BANNED) {
      if (re.test(line)) findings.push(`${path}:${i + 1} — ${why}: ${line.trim().slice(0, 90)}`);
    }
  }
}
