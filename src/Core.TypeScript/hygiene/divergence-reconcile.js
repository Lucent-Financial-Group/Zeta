...
import { closeSync, constants, fstatSync, ftruncateSync, lstatSync, openSync, readFileSync, readdirSync, writeSync, } from "node:fs";
 ...
 */
export function reconciliationBody(markdown) {
 ...
 */
export function isReconciliationPending(markdown) {
 ...
 */
export const RECONCILIATION_DECISIONS = ["accept-loop-a", "accept-loop-b", "accept-both", "escalate"];
/** True when `value` is one of the four canonical reconciliation decisions. */
export function isReconciliationDecision(value) {
    return RECONCILIATION_DECISIONS.includes(value);
}
/**
 ...
 */
export function fillReconciliation(markdown, decision, note) {
    if (!isReconciliationDecision(decision)) {
        throw new Error(`invalid reconciliation decision "${decision}": expected one of ${RECONCILIATION_DECISIONS.join(" | ")}`);
    }
    const heading = /^## Reconciliation[ \t]*$/m.exec(markdown);
    if (!heading) {
    ...
    if (!isReconciliationPending(markdown)) {
        throw new Error("refusing to fill reconciliation: section is already reconciled (overwriting would erase a prior decision)");
    }
...
function nestedAgent(frontmatter, parentKey) {
    ...
    for (let i = start + 1; i < lines.length; i++) {
        const l = lines[i];
        if (/^[A-Za-z_]/.test(l))
            break; // next unindented top-level key ends the block
 ...
 */
export function parseShardMeta(markdown) {
    const fm = /^---\n([\s\S]*?)\n---/.exec(markdown);
    if (!fm)
        return null;
    const block = fm[1];
    const typeM = /^type:[ \t]*(\S+)/m.exec(block);
    if (!typeM || typeM[1] !== "divergence")
        return null;
 ...
 */
export function findPendingShards(files) {
    const pending = [];
    for (const { relPath, content } of files) {
        const meta = parseShardMeta(content);
        if (meta === null)
            continue; // not a divergence shard
        if (!isReconciliationPending(content))
            continue; // already reconciled
        pending.push({ relPath, ...meta });
    }
    pending.sort((a, b) => a.tick < b.tick ? -1 : a.tick > b.tick ? 1 : a.relPath < b.relPath ? -1 : a.relPath > b.relPath ? 1 : 0);
    return pending;
}
/**
 ...
 */
export function scanDivergenceDir(repoRoot, divergenceRel = DIVERGENCE_ROOT) {
    const root = join(repoRoot, divergenceRel);
    if (!assertRealDivergenceRoot(root, divergenceRel, "scan divergences"))
        return [];
    const files = [];
    const walk = (dir) => {
...
export function parseArgs(argv) {
    if (argv.length === 0)
        return { kind: "ok", command: { kind: "list", format: "text" } };
    let relPath;
    let decision;
    let note;
    let sawList = false;
    ...
    const command = note === undefined ? { kind: "reconcile", relPath, decision } : { kind: "reconcile", relPath, decision, note };
    return { kind: "ok", command };
}
export function regularShardOpenFlags(noFollowFlag) {
    if (typeof noFollowFlag !== "number" || noFollowFlag === 0) {
        throw new Error("cannot reconcile: O_NOFOLLOW is unavailable on this platform; refusing write-back without symlink protection");
    }
    return constants.O_RDWR | noFollowFlag;
}
function openRegularShardForReadWrite(abs, relPath) {
    try {
...
function assertRealDivergenceRoot(absRoot, divergenceRel, action) {
    ...
    for (const part of relParts) {
        ...
        try {
            ...
            if (stat.isSymbolicLink()) {
                throw new Error(`cannot ${action}: ${divergenceRel} is a symbolic link; divergence root must be a real directory`);
            }
            if (!stat.isDirectory()) {
...
function rejectSymlinkedAncestors(divergenceRoot, abs, relPath) {
    if (!assertRealDivergenceRoot(divergenceRoot, DIVERGENCE_ROOT, "reconcile"))
        return;
    ...
    for (const part of parts.slice(0, -1)) {
        ...
        try {
            const stat = lstatSync(cursor);
            if (stat.isSymbolicLink()) {
                throw new Error(`cannot reconcile: ${relPath} contains a symbolic link; shard ancestors must be real directories`);
            }
            if (!stat.isDirectory()) {
                throw new Error(`cannot reconcile: ${relPath} is not a regular file`);
            }
...
function writeAllUtf8Sync(fd, content) {
 ...
 * B-0160).
 */
export function reconcileDivergenceShard(repoRoot, relPath, decision, note) {
...
function renderPendingReport(pending) {
    if (pending.length === 0) {
...
export function pendingShardListJson(pending) {
    return { schemaVersion: 1, pending };
}
export function renderPendingJsonReport(pending) {
