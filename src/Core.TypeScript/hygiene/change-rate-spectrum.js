...
for (const line of raw.split("\n")) {
    ...
    if (s === undefined)
        stats.set(line, { commits: 1, first: epoch });
    else {
        s.commits++;
        if (epoch < s.first)
            s.first = epoch;
...
const live = new Set(execSync("git ls-files", { maxBuffer: 1024 * 1024 * 64 }).toString().split("\n").filter(Boolean));
...
for (const r of rows.slice(0, topN)) {
    console.log(`  ${r.rate.toFixed(2)}/wk  ${String(r.commits).padStart(4)} commits  age ${r.ageWeeks.toFixed(1)}w  ${r.path}`);
}
...
for (const g of sorted.slice(0, 5)) {
    console.log(`  gap ${g.size.toFixed(3)} between ${(10 ** g.below).toFixed(4)}/wk and ${(10 ** g.above).toFixed(4)}/wk`);
}
console.log(`gappedness (largest gap / median positive gap): ${gappedness.toFixed(1)} — ${gappedness > 10 ? "GAPPED (canonical hub cuts exist in this region)" : "weakly gapped / continuum (cut placement is partly authored)"}`);
...
console.log(`  reading: research ferries are written once and frozen (rate → 0 with age); rules are the\n` +
