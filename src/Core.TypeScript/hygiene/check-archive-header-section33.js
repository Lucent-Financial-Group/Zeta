...
function isCourierFerryImport(file, headerRegion) {
    if (FILENAME_HINT_RE.test(file))
        return true;
    return CONTENT_HINT_RE.test(headerRegion);
}
function findMissingLabels(headerRegion) {
    const missing = [];
...
function findBadOpStatusValue(headerRegion) {
    if (!/^Operational status:/m.test(headerRegion))
        return "";
    const match = OP_STATUS_LINE_RE.exec(headerRegion);
    if (match === null)
        return "";
    const opLine = match[0];
    if (OP_STATUS_VALID_RE.test(opLine))
        return "";
    return opLine;
}
export function audit(researchDir) {
    ...
    for (const file of files) {
        ...
        if (!isCourierFerryImport(file, header))
            continue;
...
export function main() {
    ...
    for (const v of violations) {
        ...
        if (v.badValue !== "") {
            process.stderr.write(`VIOLATION: ${rel} 'Operational status:' value not enum-strict (must be 'research-grade' or 'operational' alone): ${v.badValue}\n`);
        }
    }
    if (violations.length > 0) {
        process.stderr.write("\n");
        process.stderr.write(`FAIL: ${String(violations.length)} courier-ferry research-doc(s) missing GOVERNANCE.md §33 archive header(s)\n`);
        process.stderr.write("\n");
