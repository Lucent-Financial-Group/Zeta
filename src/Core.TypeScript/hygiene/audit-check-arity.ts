...
export const AUDIT_PATH = "src/Core.TypeScript/hygiene/audit-check-arity.ts";

/**
 * The fix, runnable as printed, driving the SAME code path as the check so the two cannot disagree.
 * It CANNOT raise a count -- see `check-arity-census.ts` for why a frictionless raise is a silencer.
 */
export const CENSUS_FIX_COMMAND = `bun ${AUDIT_PATH} --write`;
/** The deliberate second half: record an ADJUDICATION that a new self-comparison is correct. */
 ...
 * not a second approximate one that can disagree with it. Pinned by the falsifier suite.
 */
export function isCorpusPath(rel: string): boolean {
  if (!SCAN_ROOTS.some((d) => rel === d || rel.startsWith(`${d}/`))) return false;
  return /\.fsx?$/.test(rel) || /\.(?:test|spec)\.(?:ts|tsx|mts)$/.test(rel);
}

/** The paths whose change can move THIS census, beyond the corpus itself. */
export const OWN_PATHS: readonly string[] = [CENSUS_PATH, AUDIT_PATH];

/** One line per site, so a count becomes an adjudication a reader can do at a glance. */
function describeComparisons(cs: readonly Comparison[]): string {
  return cs
    .map(
      (c) =>
        `      ${c.path}:${c.line}  ${c.kind}\n` +
        `          in test: ${c.unit}\n` +
        `          lhs: ${c.lhs.slice(0, 80)}\n` +
        `          rhs: ${c.rhs.slice(0, 80)}   (both normalize to: ${c.normalized.slice(0, 80)})`,
    )
    .join("\n");
}
...
export function main(argv: readonly string[]): number {
  ...
  // SELF-SCOPING, before `collect`: the value of a preflight guard is that it costs ~0.05 s on a
  // change that cannot move the census. Walking the corpus first and then skipping would not be.
  if (argv.includes("--drift-check")) {
    ...
    if (changed !== undefined) {
      ...
      if (touched.length === 0) {
        console.log(`check-arity drift-check: no scanned test file in ${changed.length} changed path(s) — skipped.`);
        return 0;
      }
      console.log(`check-arity drift-check: ${touched.length} corpus file(s) touched, e.g.`);
      for (const p of touched.slice(0, 3)) console.log(`  ${p}`);
    } else {
      // Unresolvable base: CHECK, never skip. A guard that goes quiet because it could not scope
      // itself is a check that did not run wearing a check that passed — this file's own subject.
      console.log(`check-arity drift-check: cannot scope against '${base}' — checking anyway.`);
    }
  }
  ...
  const allComparisons = (): Comparison[] => {
    const cs: Comparison[] = [];
    for (const s of sources) cs.push(...scanSource(s.path, s.text));
    return cs;
  };

  if (argv.includes("--emit-census") && !argv.includes("--write")) {
    console.log(serializeCensus(censusNote("audit-check-arity.ts"), tally(allComparisons())).trimEnd());
    return 0;
  }

  if (argv.includes("--write")) {
    const acceptRaises = argv.includes("--accept-raises");
    const path = join(repoRoot, CENSUS_PATH);
    const read = readCensusText(readFileSync(path, "utf8"));
    if (!read.ok) {
      console.error(`FAIL: ${CENSUS_PATH} ${read.error}`);
      return 1;
    }
    const comparisons = allComparisons();
    const plan = planWrite(read.census.counts, tally(comparisons), acceptRaises);
    const byPath = new Map<string, Comparison[]>();
    for (const c of comparisons) {
      const bucket = byPath.get(c.path);
      if (bucket === undefined) byPath.set(c.path, [c]);
      else bucket.push(c);
    }
    ...
    for (const d of plan.accepted) {
      console.log(
        `  ACCEPTED ${d.path}: ${d.was} -> ${d.now} — recorded as ADJUDICATED CORRECT.\n` +
          describeComparisons(byPath.get(d.path) ?? []),
      );
    }
    const next = serializeCensus(censusNote("audit-check-arity.ts"), plan.next);
    if (next === readFileSync(path, "utf8")) console.log(`${CENSUS_PATH} already current.`);
    else {
      writeFileSync(path, next);
      console.log(`${CENSUS_PATH} updated.`);
    }
    ...
    if (plan.refused.length > 0) {
      console.error(
        `\nREFUSED ${plan.refused.length} count raise(s). A raise is an ADJUDICATION, not a formatting fix,\n` +
          ...
          `  1. STRENGTHEN the test so the claim rides on a check that can fail (this is what #15446 did).\n` +
          `  2. If the self-comparison is genuinely correct — two executions that really differ — record\n` +
          `     that judgement: ${CENSUS_ACCEPT_COMMAND}\n`,
      );
  ...
  try {
    censusText = readFileSync(join(repoRoot, CENSUS_PATH), "utf8");
  } catch (e) {
  ...
  if (!read.ok) {
    console.error(annotation({ file: CENSUS_PATH, title: "census is unreadable", message: read.error }));
    console.error(`FAIL: ${CENSUS_PATH} ${read.error}`);
    return 1;
  }
  ...
  // a word, so an audit that reads only the parsed census can never see a duplicated row -- which is
  // how one lived undetected in the sibling census. Round-tripping is the cheapest sound detector.
  if (!read.canonical) {
    const msg =
      `${CENSUS_PATH} is not the canonical rendering of its own content. A duplicated key, a row ` +
      `inserted out of ordinal order, or formatting drift — all invisible to JSON.parse. Run: ${CENSUS_FIX_COMMAND}`;
    console.error(annotation({ file: CENSUS_PATH, title: "census is not canonical", message: msg }));
  ...
  // Workflow commands alongside the human text. Without them the check-run's only annotation is
  // `Process completed with exit code 1` — measured on every 2026-08-26 episode of this job.
  const annotations: string[] = [];
  const comparisonsByPath = new Map<string, Comparison[]>();
  for (const c of r.comparisons) {
    const bucket = comparisonsByPath.get(c.path);
    if (bucket === undefined) comparisonsByPath.set(c.path, [c]);
    else bucket.push(c);
  }
  ...
  for (const d of r.censusRose) {
    const cs = comparisonsByPath.get(d.path) ?? [];
    annotations.push(
      annotation({
        file: d.path,
        line: cs[0]?.line,
        title: `R2: ${d.now - d.was} unadjudicated self-comparison(s)`,
        message:
          `count rose ${d.was} -> ${d.now}. Both sides normalize to one expression` +
          `${cs[0] ? `: ${cs[0].normalized.slice(0, 80)}` : ""}. Strengthen the test, or — if the two ` +
          `executions really do differ — record that judgement with: ${CENSUS_ACCEPT_COMMAND}`,
      }),
    );
    problems.push(
      `${d.path}: R2 -- self-comparison count rose ${d.was} -> ${d.now}.\n` +
        `    A new check whose two sides normalize to one expression. The sites, so the adjudication is\n` +
        `    one glance and not a hunt:\n` +
        `${describeComparisons(cs)}\n` +
        `    Two remedies, and the FIRST is usually right:\n` +
        `      1. STRENGTHEN the test so the claim rides on a check that can fail. #15446 did exactly\n` +
        `         this: FreeTimeAllocation.Tests.fs appears in no census because FTA-5 was FIXED.\n` +
        `      2. If it is genuinely correct -- \`f x = f x\` evaluated twice IS two executions, a\n` +
        `         mutation-mediated pair differs in what happened between them -- record that judgement:\n` +
  ...
  for (const d of r.censusFell) {
    annotations.push(
      annotation({
        file: CENSUS_PATH,
        title: "R2: a stale census row constrains nothing",
