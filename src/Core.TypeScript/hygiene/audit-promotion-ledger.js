...
function parseLedger() {
    ...
    for (let i = 0; i < rawLines.length; i++) {
        ...
        transitions.push({
            ...
            operational_artifact: obj["operational_artifact"] === null
                ? null
                : String(obj["operational_artifact"]),
            wont_do_pointer: obj["wont_do_pointer"] === null
                ? null
    ...
    for (const t of sortedByTs) {
        ...
        if (t.to_state === "Declined") {
            if (t.wont_do_pointer === null || t.wont_do_pointer === "") {
                ...
                else if (pathOnly.endsWith("WONT-DO.md") &&
                    wontDoText !== null &&
                    anchor.length > 0 &&
...
function projectCurrentState(transitions) {
...
function reportSchemaDoc(schemaDoc) {
...
function reportProjection(latest) {
    ...
    for (const state of [
        ...
        for (const t of list.sort((a, b) => a.id.localeCompare(b.id))) {
            const artifact = t.operational_artifact !== null
...
function reportActionables(failures, warnings) {
    ...
    for (const f of failures) {
        switch (f.kind) {
            ...
            case "promoted-artifact-missing":
                console.log(`  - Promoted id=${f.id ?? "?"}: create the operational_artifact OR demote to Pending-* (line ${f.lineNo}).`);
                break;
    ...
    for (const w of warnings) {
        if (w.kind === "impossible-move") {
            console.log(`  - id=${w.id}: append a Pending-NOW or Pending-LATER re-open transition before the Promoted transition on line ${w.lineNo}.`);
        }
    }
    console.log("");
}
async function main() {
    ...
    console.log("  docs/backlog/P1/081KQNJ500008QG0R003SCWBDV* substrate-claim-checker (this audit IS the checker).");
