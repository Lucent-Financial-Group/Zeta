...
import { diffPageSnapshots, diffSnapshotSets, loadSnapshotSet, main, MONITORED_PAGES, renderDiffReport, saveSnapshotSet, } from "./feature-diff";
// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------
const tempDirs = [];
...
async function captureStderr(action) {
    let stderr = "";
    const io = { stderr: (chunk) => void (stderr += chunk) };
    return { code: await action(io), stderr };
}
async function captureStreams(action) {
    let stdout = "";
    let stderr = "";
    const io = {
...
function makeSnapshot(url, overrides = {}) {
    return {
        url,
...
describe("diffSnapshotSets", () => {
    ...
    test("priorDate and currentDate flow through from sets", () => {
        const currentSet = { date: "2026-05-08", pages: {} };
        const report = diffSnapshotSets(priorSet, currentSet);
...
describe("saveSnapshotSet / loadSnapshotSet", () => {
    ...
    test("saved file is named <date>.json", () => {
        const dir = tempDir();
        const set = { date: "2026-05-08", pages: {} };
