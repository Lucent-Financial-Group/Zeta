...
export function resolveStorageStatePath(options = {}) {
    const env = options.env ?? process.env;
    const path = options.storageStatePath ?? env[GITHUB_STORAGE_STATE_ENV] ?? env[GITHUB_STORAGE_STATE_FALLBACK_ENV];
    const trimmedPath = path?.trim();
    if (!trimmedPath) {
        throw new GitHubSessionAuthError(`Set ${GITHUB_STORAGE_STATE_ENV} (or ${GITHUB_STORAGE_STATE_FALLBACK_ENV}) to a Playwright storage-state JSON file before using GitHub UI auth.`);
    }
    return trimmedPath;
}
export async function validateStorageStateFile(path) {
...
export async function validateGitHubSession(page, options = {}) {
    ...
    if (isUnauthenticatedGitHubPage(finalUrl, html)) {
        throw new GitHubSessionAuthError(`GitHub session is expired or unauthenticated; refresh the configured storage-state file (${GITHUB_STORAGE_STATE_ENV} or ${GITHUB_STORAGE_STATE_FALLBACK_ENV}).`);
    }
    const username = extractGitHubUsername(html);
    if (!username) {
        throw new GitHubSessionAuthError("Could not extract authenticated GitHub username from profile page; the session may be invalid or GitHub markup may have changed.");
    }
    if (options.expectedUsername && username !== options.expectedUsername) {
        throw new GitHubSessionAuthError(`GitHub session user mismatch: expected ${options.expectedUsername}, got ${username}.`);
    }
    return username;
}
export async function withGitHubSession(fn, options = {}) {
...
function hasGitHubCookie(value) {
    if (!isRecord(value))
        return false;
...
function isGitHubCookieDomain(domain) {
    const normalized = domain.toLowerCase().replace(/^\./, "");
    return normalized === "github.com" || normalized.endsWith(".github.com");
}
function isUnauthenticatedGitHubPage(url, html) {
...
function extractGitHubUsername(html) {
    const patterns = [
...
async function loadPlaywright() {
    ...
    try {
        ...
        throw new GitHubSessionAuthError("Playwright is unavailable; install the project Playwright runtime or pass a test driver.", { cause: err });
    }
