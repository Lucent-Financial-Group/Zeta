...
import { appendEntry, DEFAULT_LOG_PATH, listPending, revert } from "./drain-log";
// ---------------------------------------------------------------------------
// Temp dir helpers
// ---------------------------------------------------------------------------
const tempDirs = [];
...
function tempSurfacesFile(surfaces) {
...
function appendRevertMarker(logPath, entry) {
    const marker = { ...entry, status: "reverted" };
    appendFileSync(logPath, JSON.stringify(marker) + "\n", "utf8");
}
function appendIndeterminateMarker(logPath, entry) {
    const marker = { ...entry, status: "indeterminate" };
    appendFileSync(logPath, JSON.stringify(marker) + "\n", "utf8");
}
// ---------------------------------------------------------------------------
// Fake mutable driver for revert tests
// ---------------------------------------------------------------------------
class FakeRevertPage {
    currentUrl = "";
    onClick;
    username;
    clickedSelectors = [];
    constructor(username, onClick) {
    ...
    content() {
        ...
        return Promise.resolve("<html><body><input type='checkbox' name='dependabot-security-updates' checked></body></html>");
    }
    url() {
...
function makeRevertOpts(surfacesPath, page = new FakeRevertPage("octocat")) {
    return {
...
describe("appendEntry", () => {
    test("defaults to the repository hygiene-history log path", () => {
        expect(DEFAULT_LOG_PATH).toBe(resolve(import.meta.dir, "..", "..", "..", "docs/hygiene-history/playwright-mutations/log.jsonl"));
    });
...
describe("revert", () => {
    ...
    test("records durable indeterminate marker when inverse mutation throws", async () => {
        ...
        const result = await revert(entry.id, {
    ...
    test("serializes concurrent revert attempts for the same entry id", async () => {
        ...
        const clickBlock = new Promise((resolveBlock) => {
