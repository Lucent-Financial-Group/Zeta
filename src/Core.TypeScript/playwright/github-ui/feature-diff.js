...
// ---------------------------------------------------------------------------
// Monitored pages (Phase 3 of 081KQ8P5D0008QG0R0010FP5SY — weekly UI observation targets)
// ---------------------------------------------------------------------------
...
function emptyPageSnapshot(url) {
    return {
        url,
        timestamp: "",
        username: "",
...
export function diffPageSnapshots(prior, current) {
    ...
    for (const key of allToggleKeys) {
        ...
        if (!inPrior && inCur) {
            ...
            changedToggles.push({ key, prior: priorToggles[key], current: curToggles[key] });
        }
    }
    ...
    for (const key of allFormKeys) {
        ...
        if (!inPrior && inCur) {
            ...
            changedFormFields.push({ key, prior: priorForm[key], current: curForm[key] });
        }
    }
    newToggles.sort();
    removedToggles.sort();
...
export function diffSnapshotSets(prior, current) {
    ...
    const pageDiffs = currentUrls.map((url) => {
        const currentPage = current.pages[url];
...
export function saveSnapshotSet(set, outDir = SNAPSHOT_DIR) {
...
export function loadSnapshotSet(path) {
...
export function renderDiffReport(report) {
    ...
    const totalNew = pageDiffs.reduce((n, d) => n + d.newToggles.length + d.newFeatures.length + d.newFormFields.length, 0);
    const totalRemoved = pageDiffs.reduce((n, d) => n + d.removedToggles.length + d.removedFeatures.length + d.removedFormFields.length, 0);
    lines.push(``, `## Summary`, ``, `| Category | Count |`, `|---|---|`, `| New feature candidates (toggles + headings + form fields) | ${String(totalNew)} |`, `| Removed elements | ${String(totalRemoved)} |`, `| Pages added to monitoring | ${String(pagesAdded.length)} |`, `| Pages removed from monitoring | ${String(pagesRemoved.length)} |`);
    if (pagesAdded.length > 0) {
        lines.push(``, `## New pages added to monitoring`, ``);
        for (const url of pagesAdded)
            lines.push(`- ${url}`);
    }
    if (pagesRemoved.length > 0) {
        lines.push(``, `## Pages removed from monitoring`, ``);
        for (const url of pagesRemoved)
            lines.push(`- ${url}`);
    }
    lines.push(``, `## Page-by-page diffs`, ``);
    for (const diff of pageDiffs) {
        const hasChanges = diff.newToggles.length > 0 ||
        ...
        if (diff.newToggles.length > 0) {
            ...
            for (const t of diff.newToggles)
                lines.push(`- \`${t}\` ★ new`);
            lines.push(``);
        }
        ...
        if (diff.removedToggles.length > 0) {
            ...
            for (const t of diff.removedToggles)
                lines.push(`- \`${t}\` ✗ removed`);
            lines.push(``);
        }
        ...
        if (diff.newFormFields.length > 0) {
            ...
            for (const k of diff.newFormFields)
                lines.push(`- \`${k}\` ★ new`);
            lines.push(``);
        }
        if (diff.removedFormFields.length > 0) {
            lines.push(`**Removed form fields:**`, ``);
            for (const k of diff.removedFormFields)
    ...
    lines.push(`---`, ``, `_Generated by \`tools/playwright/github-ui/feature-diff.ts\` (081KR2E4K0008QG0R003E09GMM). ` +
...
function printHelp(writeStdout) {
    writeStdout(`feature-diff.ts — diff two GitHub UI snapshot sets and report new feature candidates\n` +
...
export async function main(argv, io = {}) {
    const writeStdout = io.stdout ?? ((chunk) => process.stdout.write(chunk));
    const writeStderr = io.stderr ?? ((chunk) => process.stderr.write(chunk));
    ...
    const hasNewCandidates = report.pageDiffs.some((d) => d.newToggles.length > 0 || d.newFeatures.length > 0 || d.newFormFields.length > 0);
    if (hasNewCandidates) {
