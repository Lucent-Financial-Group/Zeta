...
import { snapshotGitHubPage } from "./snapshot";
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function getJsonValue(obj, dotPath) {
...
function toExpectedBool(raw, statusToBoolean) {
...
// Core reconciliation
// ---------------------------------------------------------------------------
export function reconcile(snapshot, expected, mapping = SETTINGS_MAPPING) {
    const matchedUiKeys = new Set();
    const match = [];
    const drift = [];
    for (const entry of mapping) {
        const uiValue = snapshot.extracted.toggles[entry.uiKey];
        if (uiValue === undefined)
            continue; // toggle not present in this snapshot
        ...
        if (expectedValue === null)
            continue; // can't compare — type mismatch
        const item = {
            uiKey: entry.uiKey,
...
// ---------------------------------------------------------------------------
export async function reconcileSettings(url, expectedJsonPath, options = {}, mapping = SETTINGS_MAPPING) {
...
function printHelp() {
    process.stdout.write(`reconcile-settings.ts — compare live GitHub UI snapshot to expected settings\n` +
...
export async function main(argv) {
    ...
    if (result.drift.length > 0) {
        ...
        for (const d of result.drift) {
            process.stderr.write(`  ${d.uiKey} (${d.jsonPath}): ui=${String(d.uiValue)} expected=${String(d.expectedValue)}\n`);
        }
        return 2;
    }
    return 0;
