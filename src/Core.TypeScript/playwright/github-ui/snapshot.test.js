...
import { extractToggles, extractFormValues, extractVisibleFeatures, snapshotGitHubPage, } from "./snapshot";
// ---------------------------------------------------------------------------
// Test fixtures
// ---------------------------------------------------------------------------
const tempDirs = [];
...
function makeMultiPage(username, targetUrl, targetHtml) {
...
describe("snapshotGitHubPage", () => {
    ...
    test("returns snapshot with url, timestamp, and username", async () => {
        ...
        const opts = { storageStatePath: tempStorageState(), driver: new FakeDriver(page) };
