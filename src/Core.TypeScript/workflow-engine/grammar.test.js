 ...
 *
 * 081KDWZ8TS008QG0R003BD6345 minimal parser/composer coverage.
 */
import { describe, expect, it } from "bun:test";
import { composeActionGrammarLine, parseActionGrammarLine } from "./grammar";
import { SEED_ACTION_CATALOG } from "./types";
describe("081KDWZ8TS008QG0R003BD6345 action grammar parser/composer", () => {
    ...
    it("parses a grammar-extension action with explicit review gate", () => {
        const parsed = parseActionGrammarLine("propose-rank | grammar-extension | pr-gated | propose-rank-action | add ranking action | 081KDWZ8TS008QG0R003BD6345,081KSNY2Z0008QG0R001YK61JQ | GrammarExtensionProposed");
        expect(parsed.ok).toBe(true);
        if (parsed.ok) {
            expect(parsed.action.class).toBe("grammar-extension");
            expect(parsed.action.gate).toBe("pr-gated");
            expect(parsed.action.composesWith).toEqual(["081KDWZ8TS008QG0R003BD6345", "081KSNY2Z0008QG0R001YK61JQ"]);
    ...
    it("rejects unknown classes and gates as grammar errors", () => {
        expect(parseActionGrammarLine("x | unknown-class | append-only | x | x | 081KDWZ8TS008QG0R003BD6345 | X")).toEqual({
            ok: false,
            error: "unknown action class: unknown-class",
        });
        expect(parseActionGrammarLine("x | transition | unknown-gate | x | x | 081KDWZ8TS008QG0R003BD6345 | X")).toEqual({
            ok: false,
    ...
    it("rejects malformed field counts and missing feedback variants", () => {
        ...
        expect(parseActionGrammarLine("x | transition | append-only | x | x | 081KDWZ8TS008QG0R003BD6345 | ")).toEqual({
            ok: false,
            error: "feedbackVariants requires at least one item",
        });
    });
    it("rejects empty CSV items instead of silently normalizing ambiguity", () => {
        expect(parseActionGrammarLine("x | transition | append-only | x | x | 081KDWZ8TS008QG0R003BD6345,,081KSNY2Z0008QG0R001YK61JQ | X")).toEqual({
            ok: false,
            error: "composesWith contains an empty item",
        });
        expect(parseActionGrammarLine("x | transition | append-only | x | x | 081KDWZ8TS008QG0R003BD6345 | X,")).toEqual({
            ok: false,
    ...
    it("composer rejects non-canonical whitespace before parse can normalize it", () => {
        ...
        const listWhitespace = {
            ...SEED_ACTION_CATALOG[0],
            composesWith: ["081KDWZ8TS008QG0R003BD6345", " 081KSNY2Z0008QG0R001YK61JQ"],
        };
