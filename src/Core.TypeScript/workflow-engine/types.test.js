 ...
 *
 * 081KDWZ8TS008QG0R0020NJ9D0 PoC — invariant tests for declarative type substrate.
 *
 * Run via: bun test src/Core.TypeScript/workflow-engine/
 */
import { describe, expect, it } from "bun:test";
import { SEED_ACTION_CATALOG, SEED_STATES, determineReviewLevel, validateCatalog, validateStateOtto5Mods, } from "./types";
describe("081KDWZ8TS008QG0R0020NJ9D0 workflow-engine scaffold invariants", () => {
    ...
    it("validateCatalog catches duplicate action id", () => {
        ...
        if (!first)
            throw new Error("SEED_ACTION_CATALOG unexpectedly empty");
...
describe("081KSNY2Z0008QG0R003WFDCJ9 determineReviewLevel lifecycle DU discriminator", () => {
    ...
    it("transition + append-only → trajectory-push (state-machine-event direct push; cheap; heartbeat-pattern)", () => {
        ...
        if (!seedAdvance)
            throw new Error("seed catalog missing 'advance'");
    ...
    it("menu-contribution + append-only → trajectory-push (Mod 5 safe at append-only scope)", () => {
        ...
        if (!seedMenu)
            throw new Error("seed catalog missing 'menu-contribute'");
