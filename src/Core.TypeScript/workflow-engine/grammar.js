...
const ACTION_GATES = new Set(["append-only", "pr-gated"]);
function splitCsv(field) {
    if (field.trim() === "")
        return [];
    return field.split(",").map((part) => part.trim());
}
function hasIllegalDelimiter(value) {
...
function validateList(name, values) {
    for (const value of values) {
        if (value.trim().length === 0)
            return `${name} contains an empty item`;
        if (value.trim() !== value)
            return `${name} item must be trimmed`;
...
export function parseActionGrammarLine(line) {
...
export function composeActionGrammarLine(action) {
    ...
    const listError = validateList("composesWith", action.composesWith) ?? validateList("feedbackVariants", action.feedbackVariants);
    if (listError !== undefined) {
        return { ok: false, error: listError };
    }
