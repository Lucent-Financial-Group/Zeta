...
// bootstrap-only CLAUDE.md (081KDW1XG6008QG0R001M0723V, smallest safe slice of 081KR50HA0008QG0R001CNS20T).
//
...
//   081KDW1XG6008QG0R002VWB0H7 (document findings + file gap children). 081KDW1XG6008QG0R001M0723V shipped the
//   harness skeleton + checks #1-#3 + #5; 081KDW1XG6008QG0R001D9AC3S (this slice) adds check
...
//
// Origin: 081KDW1XG6008QG0R001M0723V. Convention-matched to tools/cold-start-check.ts
...
export const REQUIRED_STEP_NUMBERS = [1, 2, 3, 4, 5, 6];
...
export function checkClaudeMdExists(claudeMdPresent) {
 ...
 */
export function checkSixStepProcess(claudeMd) {
    const found = new Set();
 ...
 */
export function checkRulesAutoLoad(ruleFileNames) {
 ...
 * #3 cannot give (081KDW1XG6008QG0R001D9AC3S — execute minimal validation, no Claude spawn).
 *
 ...
 */
export function checkReferencedPointers(refs, fileExists) {
 ...
 */
export function extractReferencedPointers(claudeMd) {
    const refs = new Set();
    ...
    for (const m of claudeMd.matchAll(/\]\(([^)]+)\)/g)) {
        ...
        if (hash >= 0)
            target = target.slice(0, hash); // strip #anchor; keep path
        if (target === "")
            continue; // pure-anchor link (#section)
        if (/^[a-z][a-z0-9+.-]*:/i.test(target))
            continue; // URL scheme (http:, mailto:)
        if (/[*<>\s]/.test(target))
            continue; // glob / template / not a clean token
        if (target.startsWith("~") || target.startsWith("/"))
            continue; // home/absolute
        refs.add(target);
    }
    ...
    for (const m of claudeMd.matchAll(/`([^`\n]+)`/g)) {
        ...
        if (/^[a-z][a-z0-9+.-]*:/i.test(target))
            continue; // URL scheme
        if (/[*<>\s]/.test(target))
            continue; // glob / template / command (has spaces)
        if (target.startsWith("~") || target.startsWith("/"))
            continue; // home/absolute
        if (!target.includes("/"))
            continue; // must be a path, not a bare identifier
        if (!/\.[a-z0-9]+$/i.test(target))
            continue; // must end in a file extension
        refs.add(target);
    }
    return [...refs];
}
 ...
 */
export function checkConciseness(claudeMd, maxLines) {
 ...
 */
export function runValidation(root, maxLines) {
    ...
    const checks = [checkClaudeMdExists(claudeMdPresent)];
...
function parseArgs(argv) {
    const args = { json: false, help: false, root: process.cwd(), maxLines: DEFAULT_MAX_LINES };
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--json")
            ...
            if (!v || v.startsWith("-"))
                return { error: "--max-lines requires a positive integer" };
            const n = Number(v);
