    ...
    for (let attempt = 1; attempt <= validated.maxAttempts; attempt++) {
        ...
        if (TRANSIENT_5XX_RE.test(stderr)) {
            if (attempt < validated.maxAttempts) {
                process.stderr.write(`push-with-retry: transient 5xx on attempt ${String(attempt)}/${String(validated.maxAttempts)}; retrying in ${String(backoff)}s...\n`);
                if (backoff > 0)
                    sleepSeconds(backoff);
                backoff *= 2;
                continue;
            }
            process.stderr.write(`push-with-retry: failed after ${String(validated.maxAttempts)} attempts on transient 5xx\n`);
            return 1;
        }
