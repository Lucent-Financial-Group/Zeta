...
function lanePath(root, lane) {
    const parent = dirname(root);
    const name = basename(root);
    return resolve(parent, `${name}-${lane}-lane`);
}
function parseLane(s) {
    if (s === "doc" || s === "code")
        return s;
    return null;
}
...
function allocate(root, lane, branch) {
