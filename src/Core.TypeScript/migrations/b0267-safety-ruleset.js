...
export async function main() {
    ...
    const defaultRuleset = (await ghApi("GET", `repos/${OWNER}/${REPO}/rulesets/${DEFAULT_RULESET_ID}`));
    ...
    if (dryRun) {
        ...
        if (defaultIsEmpty) {
            ...
            console.log(`[DRY RUN] Would update Default ruleset (${DEFAULT_RULESET_ID}) to keep ${remainingRules.length} rule(s):`);
            console.log(JSON.stringify(remainingRules.map((r) => r.type), null, 2));
        }
        console.log();
    ...
    if (defaultIsEmpty) {
        ...
        console.log(`  Default ruleset updated (${remainingRules.length} rule(s) remaining: ${remainingRules.map((r) => r.type).join(", ")})`);
    }
    console.log();
