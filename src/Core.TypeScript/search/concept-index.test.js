...
import { buildIndex, lookup } from "./concept-index.js";
...
describe("lookup — multi-word AND semantics", () => {
    test("AND result is a subset of each single-word result", () => {
        ...
        const inA = (e) => rA.some((x) => x.term === e.term && x.conceptClass === e.conceptClass);
        const inB = (e) => rB.some((x) => x.term === e.term && x.conceptClass === e.conceptClass);
        for (const r of rAnd) {
