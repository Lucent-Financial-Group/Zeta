...
import { readdirSync, fstatSync, statSync, readFileSync, openSync, closeSync } from "node:fs";
...
export function isExcludedDir(root, absDir) {
...
export function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--repo") {
            const v = argv[++i];
            if (!v)
                return { error: "--repo requires a directory" };
            repo = resolve(v);
        }
        else if (a === "--ext") {
            const v = argv[++i];
            if (!v)
                return { error: "--ext requires a comma-separated list" };
