...
import { extractPaceInstructions } from "./pace-extractor.js";
function instructionAt(instructions, index) {
...
describe("extractPaceInstructions", () => {
    test("single pace instruction from memory file → extracted with source attribution", async () => {
        ...
        writeFileSync(join(root, "memory", "feedback_go_hard_aaron_2026_05_02.md"), [
            "---",
    ...
    test("two instructions from maintainer → both returned in chronological order", async () => {
        const root = makeTempRoot();
        writeFileSync(join(root, "memory", "feedback_go_hard_aaron_2026_05_01.md"), [
            "---",
            "name: Go hard first",
        ...
        writeFileSync(join(root, "memory", "feedback_rest_now_aaron_2026_05_02.md"), [
            "---",
            "name: Rest now",
    ...
    test("peer-AI framing → extracted with peer-AI source tag", async () => {
        const root = makeTempRoot();
        writeFileSync(join(root, "memory", "feedback_cooling_period_claudeai_2026_05_02.md"), [
            "---",
        ...
        writeFileSync(join(root, "memory", "feedback_codex_pace_note_codex_2026_05_03.md"), [
            "---",
    ...
    test("peer-authored file mentioning Aaron uses issuer line for source", async () => {
        const root = makeTempRoot();
        writeFileSync(join(root, "memory", "feedback_cooling_claudeai_2026_05_04.md"), [
            "---",
    ...
    test("no pace instruction in substrate → empty array", async () => {
        const root = makeTempRoot();
        writeFileSync(join(root, "memory", "feedback_some_unrelated_topic_2026_05_01.md"), [
            "---",
    ...
    test("instruction with rescind language → extracted with raw text preserved", async () => {
        const root = makeTempRoot();
        writeFileSync(join(root, "memory", "feedback_stop_grinding_aaron_2026_05_03.md"), [
            "---",
            "name: Stop grinding",
    ...
    test("mixed sources across surfaces → all extracted", async () => {
        ...
        writeFileSync(join(root, "memory", "feedback_cooling_claudeai_2026_05_02.md"), [
            "---",
            "name: Cooling",
    ...
    test("Amara framing → extracted with source amara", async () => {
        const root = makeTempRoot();
        writeFileSync(join(root, "memory", "feedback_amara_hold_amara_2026_05_01.md"), [
            "---",
            "name: Amara hold",
