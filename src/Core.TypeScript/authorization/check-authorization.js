...
import { resolveAuthorization, } from "./resolve-authorization.js";
export async function checkAuthorization(rootPath) {
...
export function formatInterpretation(output) {
    const { result } = output;
...
export function formatShardField(output) {
    const { result } = output;
    if (result.operative === null) {
