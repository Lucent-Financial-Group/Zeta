...
const FILENAME_SOURCE_PATTERNS = [
    [/(?:^|[/_.-])aaron(?:[/_.-]|$)/i, "aaron"],
...
function containsPaceInstruction(line) {
...
function inferIssuerSource(text) {
...
function extractTimestamp(raw, previousLine, filename) {
    return (extractInlineTimestamp(raw) ??
...
function previousAttributionLine(lines, beforeIndex) {
    let seen = 0;
    for (let i = beforeIndex - 1; i >= 0 && seen < MAX_ATTRIBUTION_LOOKBACK; i -= 1) {
        ...
        if (candidate === undefined || !NON_EMPTY_RE.test(candidate))
            continue;
        seen += 1;
        if (inferIssuerSource(candidate) !== "unknown" ||
...
function extractFromFile(filePath, rootPath) {
    ...
    for (const [index, line] of lines.entries()) {
        if (!containsPaceInstruction(line))
            continue;
...
export async function extractPaceInstructions(rootPath) {
    const results = [];
    // Surface 1: CLAUDE.md
