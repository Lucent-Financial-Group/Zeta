...
import { AGENT_BUS_ROOT, envelopePath, serializeEnvelope, makeEnvelope, isCanonicalTimestamp, isCanonicalBusId, } from "./types";
import { SENDER_IDS, AGENT_IDS, TTL_MS } from "../bus/types";
export { makeEnvelope };
/**
 ...
 */
export function writeEnvelope(env, root = AGENT_BUS_ROOT, 
...
at = new Date(env.timestamp)) {
    ...
    if (!isCanonicalTimestamp(env.timestamp) || !isCanonicalBusId(env.id)) {
        throw new Error(`agent-bus: refusing to write malformed envelope (timestamp=${JSON.stringify(env.timestamp)}, id=${JSON.stringify(env.id)})`);
    }
...
function gitPushEnvelope(path, from, topic) {
    ...
    if (ahead !== "0") {
        throw new Error(`agent-bus publish: local main is ${ahead} commit(s) ahead of origin/main; reconcile before publishing (refusing to push unrelated commits to main)`);
    }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            try {
                ...
                throw new Error("agent-bus publish: rebase conflict — a peer published the same ZetaId path with different content (same-path collision). Re-mint with DEFAULT_ENV (crypto) and retry.");
            }
        }
    }
    // eslint-disable-next-line sonarjs/no-os-command-from-path
...
if (import.meta.main) {
    ...
    const env = makeEnvelope(from, to, message);
