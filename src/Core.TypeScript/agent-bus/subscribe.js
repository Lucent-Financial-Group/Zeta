...
import { AGENT_BUS_ROOT, isCanonicalTimestamp, isCanonicalBusId } from "./types";
/**
 ...
 */
export function envelopeCursor(env) {
...
function matchesRecipient(env, recipient) {
...
function collect(files, cursor, recipient) {
    const envs = [];
    for (const { path, json } of files) {
        ...
        if (!matchesRecipient(parsed, recipient))
            continue;
        if (cursor === undefined || envelopeCursor(parsed) > cursor)
            envs.push(parsed);
    }
 ...
 */
export function readEnvelopesSince(root = AGENT_BUS_ROOT, cursor, recipient) {
    return collect(walkJson(root).map((path) => ({ path, json: readFileSync(path, "utf-8") })), cursor, recipient);
}
/**
 ...
 */
export function readEnvelopesFromGitRef(ref, root = AGENT_BUS_ROOT, cursor, recipient) {
    let listing;
    try {
    ...
    return collect(
    // eslint-disable-next-line sonarjs/no-os-command-from-path
    paths.map((path) => ({ path, json: execFileSync("git", ["show", `${ref}:${path}`], { encoding: "utf-8" }) })), cursor, recipient);
}
/** The next compound cursor = the last (newest) envelope's `<timestamp>|<id>`, else prior. */
export function nextCursor(envs, prior) {
 ...
 */
export function parseSubscribeArgs(args) {
    const forIdx = args.indexOf("--for");
    const forValueIdx = forIdx >= 0 ? forIdx + 1 : -1;
    return {
        recipient: forIdx >= 0 ? args[forIdx + 1] : undefined,
