...
import { deserializeSecretBundle, looksLikeSecretBundle, encryptBytes, } from "./files";
...
export function listInputs(inDir, inExt = ".txt") {
...
export function outPathFor(inFile, outDir, inExt = ".txt") {
    let stem = basename(inFile);
    if (stem.toLowerCase().endsWith(inExt.toLowerCase()))
        stem = stem.slice(0, stem.length - inExt.length);
 ...
 */
export function encryptDir(self, inDir, outDir, opts = {}) {
...
function loadSelf(keysPath) {
    if (!existsSync(keysPath))
        throw new Error(`--keys not found: ${keysPath}`);
    let parsed;
    try {
        parsed = JSON.parse(readFileSync(keysPath, "utf8"));
    }
    catch (e) {
        throw new Error(`--keys is not valid JSON: ${keysPath} (${e.message})`);
    }
    if (!looksLikeSecretBundle(parsed)) {
        throw new Error(`--keys is not a secret bundle (need the SECRET key file, e.g. <identity>.secret.json — ` +
            `NOT the .recipient.json public file): ${keysPath}`);
    }
    return deserializeSecretBundle(parsed);
}
...
async function main(argv) {
    ...
    if (!keys) {
        process.stderr.write(`--keys is required for a real run (only the key owner can run this — the secret bundle is the sender / sole decryptor).\n` +
    ...
    for (const s of result.skipped)
        process.stdout.write(`skipped    ${s.in}  (${s.reason})\n`);
    for (const er of result.errors)
        process.stderr.write(`ERROR      ${er.in}  (${er.feedback})\n`);
