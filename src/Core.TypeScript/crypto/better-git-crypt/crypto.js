...
import { determineEncryptionPath, validateEnvelopeStructure, } from "./types";
 ...
 */
function firstInvalidIdentity(recipients) {
    const seen = new Set();
    for (const r of recipients) {
        if (r.identity.length === 0)
            return { identity: r.identity, reason: "empty recipient identity" };
        if (seen.has(r.identity))
            return { identity: r.identity, reason: "duplicate recipient identity" };
        seen.add(r.identity);
    }
    return null;
}
 ...
 */
export function generateRecipientKeyPair(identity, seedSource = "random-bytes") {
 ...
 */
function kemSecretUnwrapsSlot(slot, kemSecretKey, expectedCek) {
    try {
 ...
 */
export function encrypt(context, senderSecretKeys) {
 ...
 */
export function decrypt(envelope, recipientSecretKeys, senderPublicSigKey) {
...
function decodeRecipientSlots(arr) {
    const isBytes = (v) => v instanceof Uint8Array;
    const isStr = (v) => typeof v === "string";
    const slots = [];
    for (const r of arr) {
        if (typeof r !== "object" || r === null)
            return { error: "recipient slot is not an object" };
        const rr = r;
...
export function decodeEnvelope(bytes) {
    ...
    const recipientsResult = decodeRecipientSlots(o.recipients);
