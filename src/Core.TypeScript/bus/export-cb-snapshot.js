...
function toIdentity(from) {
    for (const id of IDENTITIES) {
        if (from === id || from.startsWith(id + "-"))
            return id;
    }
    return null;
}
async function readEnvelopes(busDir) {
    const now = Date.now();
    ...
    for (const file of files) {
        if (!file.endsWith(".json"))
            continue;
        try {
            const raw = JSON.parse(await readFile(join(busDir, file), "utf8"));
...
function deriveEntry(identity, meta, envelopes) {
    ...
    const hasWorkSignal = own.some(e => (e.topic === "claim" && e.payload.action === "claim") ||
...
async function main() {
    ...
    const entries = Object.entries(AGENT_META).map(([id, meta]) => deriveEntry(id, meta, envelopes));
    const snapshot = {
