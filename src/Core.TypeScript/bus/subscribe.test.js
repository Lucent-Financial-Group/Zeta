...
describe("bus subscribeOnce (B-0459 slice 1)", () => {
    ...
    test("calls handler for unseen envelopes and records seen state", async () => {
        ...
        await subscribeOnce("work-assignment", "test-surface", handler, { list: fakeList });
    ...
    test("propagates seen-file write failures to caller", async () => {
        ...
        try {
            ...
            const handler = mock(async (_env) => { });
            await expect(subscribeOnce("work-assignment", surface, handler, { list: fakeList })).rejects.toThrow();
        }
        finally {
    ...
    test("does not mark as seen if handler throws", async () => {
        ...
        await subscribeOnce("work-assignment", "test-surface", handlerFailing, { list: fakeList });
