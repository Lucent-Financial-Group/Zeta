...
describe("bus — publish + list", () => {
    ...
    test("list --to filters by recipient (includes broadcast)", () => {
        ...
        const topics = forVera.map((m) => m.topic).sort();
...
describe("bus — watch", () => {
    ...
    test("watch emits messages whose timestamp is after ZETA_WATCH_INITIAL_CURSOR", () => {
        ...
        expect(msgs[0].topic).toBe("heartbeat");
        expect(msgs[0].payload.check).toBe("live");
    });
    ...
    test("watch does not replay messages already at cursor timestamp (cursor seeding)", () => {
        ...
        const msgs = JSON.parse(listResult.stdout);
        expect(msgs.length).toBeGreaterThan(0);
...
describe("bus — status", () => {
    ...
    test("status --json includes review-request messages", () => {
        run("publish", "--from", "otto", "--to", "vera", "--topic", "review-request", "--payload", JSON.stringify({ artifact: "tools/bus/bus.ts", question: "is the status command correct?" }));
        const r = run("status", "--json");
        expect(r.exitCode).toBe(0);
        const out = JSON.parse(r.stdout);
    ...
    test("status human-readable shows review-requests section when present", () => {
        run("publish", "--from", "alexa", "--to", "otto", "--topic", "review-request", "--payload", JSON.stringify({ artifact: "src/Foo.fs" }));
        const r = run("status");
        expect(r.exitCode).toBe(0);
