...
describe("claim.ts — worktree field (081KRFA460008QG0R001SXP0C2)", () => {
    ...
    test("acquire combines --branch and --worktree in output", () => {
        const r = run("acquire", "--from", "otto-cli", "--item", "081KDWG5E1008QG0R00343E5J8", "--branch", "feat/b0603-test", "--worktree", "/tmp/zeta-test-worktree-081KDWG5E1008QG0R00343E5J8");
        expect(r.exitCode).toBe(0);
    ...
    test("two surfaces of the same identity in different worktrees — surface IDs still arbitrate, worktree is metadata only", () => {
        const r1 = run("acquire", "--from", "otto-cli", "--item", "081KDWG937008QG0R003AKCAGC", "--worktree", "/tmp/zeta-test-cli-worktree");
        expect(r1.exitCode).toBe(0);
        ...
        const r2 = run("acquire", "--from", "otto-desktop", "--item", "081KDWG937008QG0R003AKCAGC", "--worktree", "/tmp/zeta-test-desktop-worktree");
        expect(r2.exitCode).toBe(1);
        expect(r2.stderr).toContain("otto-cli");
    });
...
describe("claim.ts — multi-item isolation", () => {
    ...
    test("check returns only claims for the queried item", () => {
        ...
        const out = JSON.parse(r.stdout);
        expect(out.claims.every((c) => c.itemId === "081KPYCJH0008QG0R003MDS51N")).toBe(true);
    });
});
...
describe("claim.ts — unknown action protection (P2)", () => {
    ...
    test("unknown action on claim topic does not clear an existing claim", () => {
        ...
        spawnSync("bun", [BUS_SCRIPT, "publish", "--from", "vera", "--to", "*", "--topic", "claim",
            "--payload", JSON.stringify({ action: "relinquish", itemId: "081KR2E4K0008QG0R002MFK6AW" })], { encoding: "utf-8", env: { ...process.env, ZETA_BUS_DIR: TEST_DIR } });
