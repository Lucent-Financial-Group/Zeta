...
function lockFilePath(itemId) {
    return join(BUS_DIR, `acquire-${encodeURIComponent(itemId)}.lock`);
}
function withAcquireLock(itemId, fn) {
    ensureDir();
    ...
    if (fd === undefined)
        throw new Error(`${itemId}: acquire lock busy — retry`);
    try {
        return fn();
    }
    finally {
 ...
 */
export function activeClaims(itemId) {
    ...
    for (let i = 0; i < msgs.length; i++) {
        ...
        if (p.itemId !== itemId)
            continue;
        if (p.action !== "claim" && p.action !== "release")
            continue;
    ...
    for (const { envelope: m } of byKey.values()) {
        const p = m.payload;
        if (p.action !== "claim")
            continue; // release means no active claim
        records.push({
            id: m.id,
            from: m.from,
            itemId: p.itemId,
 ...
 */
export function allActiveClaims() {
    const msgs = list({ topic: "claim" });
    const byKey = new Map();
    for (let i = 0; i < msgs.length; i++) {
        const m = msgs[i];
