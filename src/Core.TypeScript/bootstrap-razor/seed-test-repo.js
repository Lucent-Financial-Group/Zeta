...
export function buildGhApiInvocation(request) {
...
export function buildGhRunnerEnv(baseEnv = process.env) {
...
export function runGhApiJson(request, runner = spawnGhApiRunner()) {
...
export function executeGhApiRequest(request, parseResponse, runner = spawnGhApiRunner()) {
    const result = runGhApiJson(request, runner);
    if (!result.ok)
        return result.error;
    return parseResponse(result.response);
}
export function parseSeedManifest(content) {
    const include = [];
    const exclude = [];
    let inYaml = false;
    let section = null;
...
function readManifest(path) {
    if (!existsSync(path))
        return `missing seed manifest: ${MANIFEST_DISPLAY_PATH}`;
    const manifest = parseSeedManifest(readFileSync(path, "utf8"));
    if (manifest.include.length === 0)
        return `seed manifest has no include entries: ${MANIFEST_DISPLAY_PATH}`;
    if (manifest.exclude.length === 0)
        return `seed manifest has no exclude entries: ${MANIFEST_DISPLAY_PATH}`;
    return manifest;
}
/**
 ...
 */
export function resolveSeedFiles(candidates, manifest) {
 ...
 */
export function computeSeedTree(resolved, root) {
    return resolved
        .map((path) => ({
        path,
 ...
 */
export function buildGetTreeRequest(owner, repo, treeSha) {
 ...
 * the diff needs.
 */
export function parseGitTreeResponse(response) {
 ...
 */
const AUTHORIZED_ORGS = ["Lucent-Financial-Group", "AceHack"];
/**
 ...
 */
export function parseCreateRepoResponse(response) {
    if (typeof response !== "object" || response === null || Array.isArray(response)) {
        return "create-repo response is not an object";
    }
    const { full_name, html_url, clone_url, default_branch } = response;
    if (typeof full_name !== "string")
        return "create-repo response missing string `full_name`";
    if (typeof html_url !== "string")
        return "create-repo response missing string `html_url`";
    if (typeof clone_url !== "string")
        return "create-repo response missing string `clone_url`";
    if (typeof default_branch !== "string")
        return "create-repo response missing string `default_branch`";
 ...
 */
export function buildSeedBlobRequest(content) {
 ...
 * Pure: no network, no gh, no filesystem; operates only on the already-parsed JSON value.
 */
export function parseSeedBlobResponse(response) {
    if (typeof response !== "object" || response === null || Array.isArray(response)) {
        return "blob-create response is not an object";
    }
    const { sha } = response;
    if (typeof sha !== "string")
        return "blob-create response missing string `sha`";
    return { sha };
}
/**
 ...
 */
export function buildSeedTreeRequest(diff) {
    return diff.entries
 ...
 */
export function buildSeedCommitRequest(treeSha, parentSha, fileCount) {
    return {
 ...
 * Pure: no network, no gh, no filesystem; operates only on the already-parsed JSON value.
 */
export function parseSeedCommitResponse(response) {
    if (typeof response !== "object" || response === null || Array.isArray(response)) {
        return "commit-create response is not an object";
    }
    const { sha } = response;
    if (typeof sha !== "string")
        return "commit-create response missing string `sha`";
    return { sha };
}
/**
 ...
 */
export function buildSeedRefUpdateRequest(owner, repo, branch, commitSha, refExists) {
 ...
 * Pure: no network, no gh, no filesystem; operates only on the already-parsed JSON value.
 */
export function parseSeedRefUpdateResponse(response) {
    if (typeof response !== "object" || response === null || Array.isArray(response)) {
        return "ref-update response is not an object";
    }
    const { ref, object } = response;
    if (typeof ref !== "string")
        return "ref-update response missing string `ref`";
    if (typeof object !== "object" || object === null || Array.isArray(object)) {
        return "ref-update response missing object field `object`";
    }
    const { sha } = object;
    if (typeof sha !== "string")
        return "ref-update response missing string `object.sha`";
    return { ref, sha };
}
/**
 ...
 * network, no gh.
 */
function collectSeedCandidates(root, manifest) {
    const found = new Set();
...
function emitDryRun(manifest, root) {
    ...
    if (typeof createReq === "string") {
        ...
        console.log(`  name=${createReq.body.name} private=${createReq.body.private} auto_init=${createReq.body.auto_init}`);
    }
    const refused = buildCreateRepoRequest("ServiceTitan", "zeta-recreation-experiment");
    console.log(`Authorization scope guard (ServiceTitan): ${typeof refused === "string" ? `REFUSED — ${refused}` : "ALLOWED (BUG — should be refused)"}`);
