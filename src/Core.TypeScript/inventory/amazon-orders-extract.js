...
async function ensureModule(name) {
    try {
        await import(__rewriteRelativeImportExtension(name));
        return true;
    }
    catch {
        return false;
...
function loadPartial() {
    ...
    try {
        const data = JSON.parse(readFileSync(PARTIAL_FILE, "utf-8"));
        if (data.year !== YEAR) {
...
async function main() {
    ...
    await pressEnterToContinue(`Amazon orders page is open in the browser.\n` +
    ...
    for (let p = 1; p <= totalPages; p++) {
        ...
        let items = [];
        try {
            items = await page.evaluate(({ pageNumber }) => {
    ...
    writeFileSync(FULL_OUT, JSON.stringify({ year: YEAR, totalPages, itemCount: state.items.length, items: state.items }, null, 2));
    try {
        chmodSync(FULL_OUT, 0o600);
    }
    catch { /* best-effort — personal financial data */ }
    console.log(`\nWrote ${FULL_OUT} — ${state.items.length} items`);
    const hardwareItems = state.items.filter((i) => HARDWARE_KEYWORDS.test(i.title));
    writeFileSync(HARDWARE_OUT, JSON.stringify({ year: YEAR, itemCount: hardwareItems.length, items: hardwareItems }, null, 2));
    try {
