...
import { classifyCascadeUrgency, DEFAULT_CONFIG, MAX_REPORTED_MISSING_COMMITS, parseArgs, parsePositiveMinutes, pollOnce, realCascadeDetector, } from "./missed-substrate-detector";
function adapters(opts) {
    ...
    return {
        ...
        publishCascade: opts.publishImpl ?? ((from, to, finding) => {
...
function cascadeAdapters(opts) {
    return {
        ...
        fetchPRRefs: (prNumber) => opts.prRefs?.[prNumber] ?? { status: "error", reason: "no fake configured" },
        compareBranchToMerged: (branchName) => opts.branchCompares?.[branchName] ?? { status: "error", reason: "no fake configured" },
    };
}
...
describe("missed-substrate-detector slice 5b (auto-recover wiring)", () => {
    ...
    function makeFinding(prNumber = 1234) {
        return {
            prNumber,
