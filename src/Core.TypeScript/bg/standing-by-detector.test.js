...
import { DEFAULT_CONFIG, parseArgs, parsePositiveMinutes, pollOnce, } from "./standing-by-detector";
function fakeAdapters(nowIso, lastCommitIso, lastPrActivityIso = null, capturedCalls = [], publishImpl) {
    return {
        now: () => new Date(nowIso),
        lastCommitIso: () => lastCommitIso,
        lastPrActivityIso: () => lastPrActivityIso,
        publishNudge: publishImpl ?? ((from, to, idleMinutes, rationale) => {
...
describe("standing-by-detector slice 3 (P0 fix: PR-activity poll)", () => {
    ...
    describe("P0 fix — idle uses MAX(commit, pr) so PR-only agents are NOT flagged", () => {
        test("recent COMMIT only (no PR activity) → NOT idle", () => {
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15, noPublish: true }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:58:00Z", null));
        ...
        test("recent PR activity only (no commit) → NOT idle (was Riven's P0 false-negative)", () => {
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15, noPublish: true }, fakeAdapters("2026-05-13T18:00:00Z", null, "2026-05-13T17:58:00Z"));
            expect(result.idleDetected).toBe(false);
            expect(result.idleMinutes).toBe(2);
        ...
        test("OLD commit + recent PR activity → NOT idle (PR work counts)", () => {
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15, noPublish: true }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:00:00Z", "2026-05-13T17:55:00Z"));
            expect(result.idleDetected).toBe(false);
            expect(result.idleMinutes).toBe(5);
        });
        test("recent commit + OLD PR activity → NOT idle (commit work counts)", () => {
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15, noPublish: true }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:55:00Z", "2026-05-13T17:00:00Z"));
            expect(result.idleDetected).toBe(false);
            expect(result.idleMinutes).toBe(5);
        });
        test("BOTH old → idle flagged", () => {
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15, noPublish: true }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:30:00Z", "2026-05-13T17:35:00Z"));
    ...
    describe("bus publish — slice 4 behavior preserved", () => {
        test("publishes nudge when idle detected", () => {
            const captured = [];
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15 }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:30:00Z", "2026-05-13T17:35:00Z", captured));
        ...
        test("does NOT publish when noPublish=true", () => {
            const captured = [];
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15, noPublish: true }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:30:00Z", null, captured));
            expect(result.idleDetected).toBe(true);
        ...
        test("P1 fix — publish failure surfaces in structured lastPublishError field", () => {
            const captured = [];
            const result = pollOnce({ ...DEFAULT_CONFIG, idleThresholdMin: 15 }, fakeAdapters("2026-05-13T18:00:00Z", "2026-05-13T17:30:00Z", null, captured, () => { throw new Error("bus IO failure: disk full"); }));
            expect(result.idleDetected).toBe(true);
            expect(result.publishedEnvelopeId).toBeNull();
