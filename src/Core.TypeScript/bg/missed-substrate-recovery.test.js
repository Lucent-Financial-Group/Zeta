...
import { buildRecoveryBranchName, buildRecoveryPRBody, openRecoveryPR, } from "./missed-substrate-recovery";
// ─────────────────────────────────────────────────────────────────────
// Helpers
// ─────────────────────────────────────────────────────────────────────
...
function stubAdapters(log, overrides = {}) {
    return {
        ...
        ghPrCreate: (title, body, head) => {
            ...
            if (overrides.ghPrCreate)
                return overrides.ghPrCreate(title, body, head);
...
describe("openRecoveryPR — opened", () => {
    ...
    it("PR title encodes prNumber + missing commits count", () => {
        const log = newCallLog();
        const r = openRecoveryPR(makeFinding({ prNumber: 7777, missingCommits: ["a", "b", "c"] }), false, stubAdapters(log));
        expect(r.status).toBe("opened");
...
describe("openRecoveryPR — cherry-pick-conflict", () => {
    it("returns cherry-pick-conflict on the conflicting sha; push NOT called", () => {
        ...
        if (r.status !== "cherry-pick-conflict")
            throw new Error("narrowing");
