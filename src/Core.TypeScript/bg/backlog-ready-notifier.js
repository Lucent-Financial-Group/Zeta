...
export function parseRow(content, filename) {
    ...
    if (!idMatch || !priorityMatch || !statusMatch)
        return null;
    return {
        id: idMatch[1] ?? "",
...
const REAL_ADAPTERS = {
    ...
    publishAssignment: (from, to, rowId, priority, rationale) => publish(from, to, {
        topic: "work-assignment",
        payload: { rowId, priority, rationale },
    }),
    agentPatterns: AGENT_MAP,
    execGitLog: (sinceMinutes) => {
        ...
        const result = spawnSync("git", ["log", "--all", `--since=${cutoff}`, "--format=%an %B"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
    ...
    execGhPrList: () => {
        ...
        const result = spawnSync("gh", ["pr", "list", "--state", "open", "--limit", "1000", "--json", "title,body,author"], { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] });
        if (result.status !== 0 || result.error)
            return null;
        return result.stdout ?? "";
    },
    readHistoryFile: (path) => {
        try {
            ...
            const entries = parsed.entries.filter((e) => typeof e === "object" && e !== null
 ...
 */
export function isAgentQueueEmpty(agentName, adapters = REAL_ADAPTERS) {
    const patterns = adapters.agentPatterns[agentName.toLowerCase()] ?? [];
    if (patterns.length === 0)
        return true; // unknown agent = queue empty
    const logOutput = adapters.execGitLog(30);
    if (logOutput === null)
        return false; // git unavailable — treat as busy
    ...
    if (prOutput === null)
        return false; // gh unavailable — treat as busy
 ...
 */
function isDepSatisfied(depStatus) {
...
export function pollOnce(config, adapters = REAL_ADAPTERS) {
    ...
    const skippedDueToCooldown = [];
    const publishedRowIds = [];
    ...
    if (!config.noPublish && readyRows.length > 0) {
        ...
        const activeEntries = new Set(history.entries
            .filter(e => pollAt.getTime() - new Date(e.publishedAt).getTime() < cooldownMs)
            .map(e => e.rowId));
        for (const row of readyRows) {
            if (publishedEnvelopeIds.length >= config.maxAssignments)
                break;
...
export function runOnce(config = DEFAULT_CONFIG, adapters = REAL_ADAPTERS) {
    const result = pollOnce(config, adapters);
    console.log(JSON.stringify(result));
    return result;
}
export async function runDaemon(config = DEFAULT_CONFIG, adapters = REAL_ADAPTERS) {
    while (true) {
...
export function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === "--once") {
            ...
            const next = argv[++i];
            if (next === undefined)
                throw new Error("--backlog-dir requires a value");
            ...
            const next = argv[++i];
            if (next === undefined)
                throw new Error("--target-agent requires a value");
