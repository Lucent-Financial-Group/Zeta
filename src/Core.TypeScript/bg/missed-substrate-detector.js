...
import { openRecoveryPR, } from "./missed-substrate-recovery";
 ...
 */
export function classifyCascadeUrgency(missingCommitCount, mergedAtIso, nowMs) {
    const mergedAtMs = new Date(mergedAtIso).getTime();
    const ageHours = (nowMs - mergedAtMs) / (1000 * 60 * 60);
    if (missingCommitCount >= 4)
        return "high"; // multiple-commit drift = serious
    if (ageHours < 1)
        return "high"; // fresh drift = likely about to be lost
    if (missingCommitCount >= 2)
        return "medium";
    if (ageHours < 24)
        return "medium"; // recent single-commit drift
    return "low";
}
/**
 ...
 */
export function realCascadeDetector(pr, adapters) {
    ...
    if (compare.status !== "ok")
        return null;
    if (compare.missingCommits.length === 0)
        return null;
    return {
        prNumber: pr.number,
...
const REAL_ADAPTERS = {
    now: () => new Date(),
    fetchRecentMergedPRs: (lookbackMin, fetchLimit) => {
        ...
        try {
            ...
            const prs = parsed.filter((p) => typeof p === "object" &&
                p !== null &&
    ...
    detectCascade: (pr) => realCascadeDetector(pr, REAL_CASCADE_SUB_ADAPTERS),
    publishCascade: (from, to, finding) => publish(from, to, {
    ...
    openRecoveryPR: (finding, dryRun) => {
        if (!dryRun && !isWorkingTreeClean()) {
            return {
                status: "error",
                reason: "refusing recovery: working tree is not clean. Run auto-recovery from a dedicated `git worktree add` scratch dir, or pass --recovery-dry-run to skip mutations.",
            };
        }
...
function isWorkingTreeClean() {
    ...
    if (r.status !== 0)
        return false; // conservative: assume dirty on git error
...
const REAL_RECOVERY_ADAPTERS = {
    checkRecoveryPRExists: (branchName) => {
        ...
        const r = spawnSync("gh", ["pr", "list", "--head", branchName, "--state", "open", "--json", "number"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        if (r.status !== 0)
            return false; // conservative: assume no existing PR on gh failure
        try {
    ...
    gitCreateBranch: (branch, base) => {
        ...
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- git invoked as explicit args array.
        const r = spawnSync("git", ["checkout", "-b", branch, base], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        return r.status === 0;
    },
    gitCherryPick: (sha) => {
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- git invoked as explicit args array.
    ...
    gitPush: (branch) => {
        ...
        // than --force).
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- git invoked as explicit args array.
        const r = spawnSync("git", ["push", "--force-with-lease", "origin", branch], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        return r.status === 0;
    },
    ghPrCreate: (title, body, head) => {
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- gh invoked as explicit args array.
        const r = spawnSync("gh", ["pr", "create", "--title", title, "--body", body, "--head", head, "--base", "main"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        if (r.status !== 0)
            return null;
 ...
 */
export const REAL_CASCADE_SUB_ADAPTERS = {
    now: () => new Date(),
    fetchPRRefs: (prNumber) => {
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- gh invoked as explicit args array; no shell, no injection risk.
        const result = spawnSync("gh", ["pr", "view", String(prNumber), "--json", "headRefOid,mergeCommit"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        if (result.status !== 0) {
            return {
                status: "error",
    ...
    compareBranchToMerged: (branchName, headRefOid) => {
        ...
        const fetchResult = spawnSync("git", ["fetch", "--quiet", "origin", branchName], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        ...
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- git invoked as explicit args array.
        const ancestorResult = spawnSync("git", ["merge-base", "--is-ancestor", headRefOid, `origin/${branchName}`], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
        ...
        // eslint-disable-next-line sonarjs/no-os-command-from-path -- git invoked as explicit args array.
        const logResult = spawnSync("git", ["log", `${headRefOid}..origin/${branchName}`, "--format=%H"], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
...
const KNOWN_FLAGS = ["--once", "--poll-min", "--lookback-min", "--fetch-limit", "--no-publish", "--agent", "--to", "--auto-recover", "--recovery-dry-run"];
