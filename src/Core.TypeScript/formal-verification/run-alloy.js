...
function checkToolchain(root) {
    ...
    if (javaPath === null || javacPath === null)
        return null;
    if (!fileExists(alloyJarPath))
        return null;
    if (!fileExists(alloyRunnerSource))
        return null;
...
function classIsFresh(toolchain) {
    if (!fileExists(toolchain.runnerClassFile))
        return false;
    try {
...
function compileRunnerIfNeeded(toolchain) {
    if (classIsFresh(toolchain))
        return true;
    try {
    ...
    const result = spawnSync(toolchain.javacPath, [
        "-cp",
...
function runAlloy(toolchain, specName) {
...
function runAll(toolchain) {
    ...
    process.stdout.write(`summary: ${String(passed.length)} passed, ${String(failed.length)} failed, ${String(missing.length)} missing-from-catalogue (out of ${String(CATALOGUE.length)} catalogued)\n`);
...
function main(argv) {
    ...
    if (argv[0] === "--check-toolchain") {
        ...
        if (tc === null) {
            process.stderr.write("ERROR: Alloy toolchain not ready (need java + javac on PATH + src/Core.Alloy/alloy.jar + src/Core.Alloy/AlloyRunner.java). Run tools/setup/install.sh\n");
            return 2;
        }
