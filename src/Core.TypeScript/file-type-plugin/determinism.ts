...
import { type Expr } from "../bonsai/bonsai";

export interface DeterminismFeedback {
  readonly ok: boolean;
...
function checkQueryExpression(expr: Expr, depth: number): void {
 ...
 */
export function validatePlugin(plugin: FileTypePlugin): DeterminismFeedback {
  ...
  for (const view of plugin.views) {
    try {
      ...
      return {
        ok: false,
        reason: `View '${view.name}' violates determinism contract: ${(e as Error).message}`
      };
    }
  }
