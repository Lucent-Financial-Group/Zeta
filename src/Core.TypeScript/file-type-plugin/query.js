...
function evaluateCall(fn, args, env) {
    switch (fn) {
        case "filter": {
            if (args.length !== 2)
                throw new Error("filter expects 2 arguments");
            const zsetVal = evaluateQuery(args[0], env);
            const lambdaExpr = args[1];
            if (lambdaExpr.kind !== "lambda")
                throw new Error("filter second argument must be a lambda");
        ...
        case "map": {
            if (args.length !== 2)
                throw new Error("map expects 2 arguments");
            const zsetVal = evaluateQuery(args[0], env);
            const lambdaExpr = args[1];
            if (lambdaExpr.kind !== "lambda")
        ...
        case "get_field": {
            if (args.length !== 2)
                throw new Error("get_field expects 2 arguments");
            const entryVal = evaluateQuery(args[0], env);
            const nameVal = evaluateQuery(args[1], env);
            if (nameVal.t !== "str")
                throw new Error("get_field field name must be a string");
            return getField(entryVal, nameVal.v);
        }
        case "get_zset_field": {
            if (args.length !== 2)
                throw new Error("get_zset_field expects 2 arguments");
            const zsetVal = evaluateQuery(args[0], env);
            const nameVal = evaluateQuery(args[1], env);
            if (nameVal.t !== "str")
                throw new Error("get_zset_field field name must be a string");
            const zset = taggedToZSet(zsetVal);
            for (const entry of zset) {
 ...
 */
export function evaluateQuery(expr, env) {
    switch (expr.kind) {
        case "const":
