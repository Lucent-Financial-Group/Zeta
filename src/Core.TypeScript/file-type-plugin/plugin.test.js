...
import { pluginToTagged, taggedToPlugin, zsetToTagged, taggedToZSet } from "./types";
            ...
            views: [
                {
                    ...
                    query: call("filter", [
                        param("zset"),
                        lambda(["entry"], binary("and", binary("eq", call("get_field", [param("entry"), cstr("type")]), cstr("task")), binary("eq", call("get_field", [param("entry"), cstr("priority")]), cstr("P1"))))
                    ])
                }
            ]
        };
...
This is a P1 task workitem.`;
        ...
        const isP1TaskExpr = binary("and", binary("eq", call("get_zset_field", [param("zset"), cstr("type")]), cstr("task")), binary("eq", call("get_zset_field", [param("zset"), cstr("priority")]), cstr("P1")));
        ...
        const filterExpr = call("filter", [
            param("zset"),
            lambda(["entry"], binary("eq", call("get_field", [param("entry"), cstr("type")]), cstr("task")))
        ]);
