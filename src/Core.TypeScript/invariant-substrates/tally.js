...
function extractTopScalar(source, key) {
...
function extractCount(source, key) {
...
async function collectRow(path) {
    ...
    const sum = h + o + v;
    const sumCheck = t === sum ? "ok" : "MISMATCH";
    return {
        skill,
        specVer,
        specRound,
...
async function main() {
    ...
    out.push("Aggregated from every `.claude/skills/<pack>/blueprints/<name>.skill.yaml`. " +
    ...
    out.push("| Skill | Spec ver | Round | Portable | Hypothesis | Observed | Verified | Total | Sum-check | Legacy key? |");
    out.push("|---|---|---|---|---:|---:|---:|---:|---|---|");
    let totH = 0;
    let totO = 0;
    let totV = 0;
    ...
    for (const row of rows) {
        out.push(`| \`${row.skill}\` | ${row.specVer} | ${row.specRound} | ${row.portableCell} | ` +
        ...
        totV += row.verified;
        totT += row.total;
        if (row.portableState === "yes")
            portableYes += 1;
        else if (row.portableState === "no")
            portableNo += 1;
        if (row.sumCheck === "MISMATCH")
            mismatches += 1;
        if (row.legacyKey)
            legacyKeyHits += 1;
    }
    ...
    out.push(`| **total (${rows.length} substrates)** | - | - | - | **${totH}** | **${totO}** | **${totV}** | **${totT}** | ${grandCheck} | - |`);
    out.push("");
    out.push(`Portable / Zeta-specific split: **${portableYes} portable**, **${portableNo} zeta-specific**, ` +
        `${rows.length - portableYes - portableNo} unspecified.`);
    if (legacyKeyHits > 0) {
        out.push("");
        out.push(`**${legacyKeyHits} substrate(s) still use the legacy \`guess:\` key.** ` +
            "Rename to `hypothesis:` per " +
            "`docs/DECISIONS/2026-04-20-tools-scripting-language.md` §Terminology.");
    }
    if (mismatches > 0) {
        out.push("");
        out.push(`**${mismatches} substrate(s) have a declared \`total\` that does not match the sum of tiers.** ` +
