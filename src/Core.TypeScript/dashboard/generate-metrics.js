...
function mergedWithin(dateNow, windowMs) {
    return (pr) => pr.merged_at !== null && dateNow - new Date(pr.merged_at).getTime() < windowMs;
}
...
async function fetchClosedPRsUntilWindow(windowMs, maxPages = 10) {
    const all = [];
    const cutoff = Date.now() - windowMs;
    for (let page = 1; page <= maxPages; page++) {
        const batch = await apiFetch(`${API}/pulls?state=closed&sort=updated&direction=desc&per_page=100&page=${page}`);
        if (batch.length === 0)
            break;
        all.push(...batch);
        if (batch.length < 100)
...
async function main() {
    ...
    function decomposeToActionRatio(prsM, rowsF) {
    ...
    const agents = Object.entries(agentActivity)
        ...
        return {
            ...
            status: now - new Date(info.lastCommit).getTime() < 30 * 60000
                ? "active"
    ...
    console.log(`Wrote ${outPath}: ${mergedToday.length} PRs merged, ${commits24h.length} commits, ${activeAgents} active agents`);
}
if (import.meta.main) {
    main().catch((e) => {
        console.error(e);
