...
import { FerryThrottler } from "../ferry-throttler/ferry-throttler.js";
...
export function parseRestoreArrowOutput(raw) {
...
export function log(event, logFile) {
...
async function runOsascript(scriptPath, terminalApp) {
    ...
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error(`osascript timed out after ${OSASCRIPT_TIMEOUT_MS}ms`)), OSASCRIPT_TIMEOUT_MS));
    try {
 ...
 */
export async function detectGreyTextMacOS(terminalApp, _runner, _platform) {
    ...
    if (result.exitCode !== 0) {
        const isPermDenied = result.stderr.includes("-1002") ||
...
export async function detectGreyText() {
 ...
 */
export async function detectViaCommand(cmd) {
 ...
 */
export async function pressRestoreArrow(_config, _runner, _platform) {
    const platform = _platform ?? process.platform;
    if (platform !== "darwin") {
...
export async function acceptGreyText(config) {
    if (config.dryRun) {
        return true;
    }
...
const frontmostHistory = new Map();
/** Test helper — clears the per-process frontmost history. */
export function _resetFrontmostHistory() {
...
export async function getFrontmostState(_runner, _platform) {
    const platform = _platform ?? process.platform;
    if (platform !== "darwin")
        return null;
    const runner = _runner ?? runOsascript;
    ...
    if (!Number.isFinite(pid) || pid <= 0 || name.length === 0)
        return null;
    return { pid, name };
}
...
export function checkWindowFresh(state, thresholdMs, nowMs, history = frontmostHistory) {
    if (thresholdMs <= 0)
        return true;
...
export async function runOneCycle(config, detectFn = detectGreyText, acceptFn = acceptGreyText, restoreArrowFn = () => pressRestoreArrow(config), getFrontmostStateFn = getFrontmostState, nowMs = Date.now) {
...
async function runInner(config, detectFn, acceptFn, eventSink) {
    const startEvent = {
        timestamp: new Date().toISOString(),
        type: "started",
    ...
    if (eventSink)
        eventSink.tryEnqueue(startEvent);
    if (config.once) {
...
export async function run(config, detectFn = config.detectCmd
    ? () => detectViaCommand(config.detectCmd)
    : detectGreyText, acceptFn = acceptGreyText) {
    ...
    const eventSink = new FerryThrottler({ maxDegreeOfParallelism: 1, maxBatchSize: 64, maxQueueSize: 256 }, async (_boat) => {
