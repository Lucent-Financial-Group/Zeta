 ...
 */
export function tryDetect(cmd, args) {
    try {
 ...
 */
export function requireAbsolute(name, value) {
    if (!isAbsolute(value)) {
...
export function parseArgs(argv) {
    ...
    try {
        ...
        if (code === "ENOENT") {
            ...
            console.error(`Could not stat --bun-path "${bunPath}" (${e.message})`);
        }
        process.exit(1);
    }
...
export function substitutePlaceholders(template, repoRoot, bunPath) {
...
export function plutilLint(content) {
...
export function main() {
    ...
    try {
        ...
        if (e.code !== "ENOENT") {
            ...
            console.error(`Could not read existing plist for backup (${e.message}); proceeding without backup`);
        }
    }
    ...
    if (oldContent !== undefined) {
        ...
        try {
            ...
            console.error(`Could not write backup at ${backup} (${e.message}); install OK without backup`);
        }
    }
    if (args.bootstrap) {
