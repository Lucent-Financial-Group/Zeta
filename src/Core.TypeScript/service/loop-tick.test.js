...
describe("loop-tick", () => {
    ...
    test("runs successfully for valid persona (kiro) — acquires lock and ticks", () => {
        ...
        try {
            ...
            writeExecutable(join(bin, "git"), ["#!/usr/bin/env bash", 'case "$1" in', "  fetch|branch|status) exit 0 ;;", "  *) exit 0 ;;", "esac", ""].join("\n"));
            writeExecutable(join(bin, "gh"), ["#!/usr/bin/env bash", 'if [ "$1" = "pr" ] && [ "$2" = "list" ]; then', "  echo 0", "fi", "exit 0", ""].join("\n"));
