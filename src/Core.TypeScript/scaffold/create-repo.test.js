...
function assertCommonInvariants(result, repoName) {
...
describe("forge dry-run", () => {
    ...
    test("step 02 branch protection has empty required_status_checks (avoids deadlock)", () => {
        const op = result.operations.find((o) => o.step === "02-branch-protection");
        expect(op).toBeDefined();
        const checks = op.data.required_status_checks;
    ...
    test("step 06 lists scaffold files to push", () => {
        ...
        // GHA injection protection must be present from day one (081KDW5XNR008QG0R00132M0Q6).
        expect(files.some((f) => f.includes(".semgrep.yml"))).toBe(true);
        // Toolchain pin for semgrep must be scaffolded alongside it (081KDW5XNR008QG0R00132M0Q6).
        expect(files.some((f) => f.includes(".mise.toml"))).toBe(true);
        // Dependabot config must be present from day one (081KDW5XNR008QG0R000GWPZG3).
    ...
    test("step 07 lists manual steps including budget-cap verification", () => {
        ...
        expect(op).toBeDefined();
        const manual = op.data.manualSteps;
...
describe("ace dry-run", () => {
    ...
    test("step 01 targets LFG/ace as public squash-merge repo (no homepage)", () => {
        ...
        expect(op.data["homepage"]).toBeUndefined();
    });
    test("step 05 forks to AceHack org", () => {
        const op = result.operations.find((o) => o.step === "05-fork-to-acehack");
        expect(op).toBeDefined();
...
// --- scaffold template content assertions (081KDW5XNR008QG0R002E476TC) ---
//
...
const REQUIRED_SEMGREP_RULES = [
    "gha-untrusted-in-run-line", // 081KDW5XNR008QG0R00132M0Q6 — GHA workflow-injection (present from day one)
    "invisible-unicode-in-text", // 081KDW5XNR008QG0R002E476TC — BP-10 invisible Unicode / prompt injection
    "gha-action-mutable-tag", // 081KDW5XNR008QG0R002E476TC — supply-chain SHA-pin (CVE-2025-30066)
];
describe("forge .semgrep.yml content (081KDW5XNR008QG0R002E476TC)", () => {
    let content;
    beforeAll(() => {
...
describe("ace .semgrep.yml content (081KDW5XNR008QG0R002E476TC)", () => {
    let content;
    beforeAll(() => {
