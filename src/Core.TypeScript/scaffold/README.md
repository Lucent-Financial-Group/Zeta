# src/Core.TypeScript/scaffold — 081KRFA460008QG0R001H98EXJ Stage 1 repo scaffolding
...
bun src/Core.TypeScript/scaffold/create-repo.ts --repo forge --dry-run
bun src/Core.TypeScript/scaffold/create-repo.ts --repo ace  --dry-run

# Execute for real (creates GitHub repos under LFG org):
bun src/Core.TypeScript/scaffold/create-repo.ts --repo forge --apply
bun src/Core.TypeScript/scaffold/create-repo.ts --repo ace  --apply
```
...
| 06 | Push day-one governance files from `src/Core.TypeScript/scaffold/<name>/` — **runs before branch protection** so files can be pushed directly to `main` |
