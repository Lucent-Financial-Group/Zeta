...
export interface ByteCost {
  readonly bytes: number;
}

export const Zero: ByteCost = { bytes: 0 };

export const ofBytes = (n: number): ByteCost => ({ bytes: n });

/** Monoid combine — addition of byte counts. */
export const add = (a: ByteCost, b: ByteCost): ByteCost => ({ bytes: a.bytes + b.bytes });

const encoder = new TextEncoder();

/** Measure a surface from its text: UTF-8 byte length (the canonical encoding). */
export const measureText = (text: string): ByteCost => ({ bytes: encoder.encode(text).length });

/** Order-independent total of a fileset's costs (monoid fold over `add`). */
export const sum = (costs: readonly ByteCost[]): ByteCost => costs.reduce(add, Zero);
