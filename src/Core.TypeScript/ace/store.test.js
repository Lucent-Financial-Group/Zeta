...
import { contentHash, installPackage, validatePackagePaths, loadTrustStore, addTrustedKey, listTrustedKeys, trustStorePath, bundledRegistryPath, registryPath, loadRegistry, listRegistry, addRegistryEntry } from "./store.js";
...
describe("installPackage", () => {
    ...
    function makePkg(files, name = "demo") {
        const filesJson = JSON.stringify(files);
        const content_hash = "blake3:" + ContentHash256.ofBytes(new TextEncoder().encode(filesJson)).toHex();
        return {
    ...
    test("installPackage ignores a manifest's dependencies field (leaf back-compat)", () => {
        ...
        const pkg = {
            manifest: {
                ...
                dependencies: [{ kind: "inline", name: "x", version: "1.0.0", url: "http://e/x.json", package_hash: "blake3:0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20" }],
            },
            files,
        };
        const result = installPackage(store, pkg);
...
describe("trust store", () => {
    ...
    test("addTrustedKey corrects a pre-existing permissive trust file to 0o600 on POSIX", () => {
        if (process.platform === "win32")
            return; // chmod advisory on Windows; skip
...
import { readRegistriesConfig, writeRegistryRemote, removeRegistryRemote, registriesPath, registryCacheDir } from "./store.js";
describe("remote-registry config", () => {
    let home, savedHome, savedUP;
    beforeEach(() => {
