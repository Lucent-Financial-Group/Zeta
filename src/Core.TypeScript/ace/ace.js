...
import { defaultStorePath, listInstalled, installPackage, contentHash, loadTrustStore, addTrustedKey, listTrustedKeys, validatePackagePaths, addRegistryEntry, listRegistry, writeRegistryRemote, removeRegistryRemote, readRegistriesConfig, } from "./store";
import { generateKeypair, signManifest, verifySignature, keyId, publicKeyInfoFromPrivatePem } from "./signing";
import { verifyIndexSignature, signIndex } from "./index-signature.js";
...
import { buildLockfile, serializeLockfile, parseLockfile, verifyRootMatchesLock, lockfilesEqual, buildLeafLockfile, } from "./lockfile.js";
import { solve } from "./solver.js";
function isValidDepEdge(e) {
    ...
    if (d.kind === "registry")
        return true;
    if (d.kind === "inline")
        return typeof d.url === "string" && typeof d.package_hash === "string";
    return false;
}
import { buildIndexDoc, nextSequence } from "./registry-publish.js";
import { loadRegistries, parseIndex } from "./registry-remote.js";
import { resolve as toAbsolutePath, join } from "node:path";
import { parseYaml, stringifyYaml, resolveGraph, generateFlux, generateArgoCD, getResolvedVersion, getMigrationPhase, checkRollbackSafety, generateMigrationRunbook, } from "./deps";
...
function preflightGraph(order) {
    const byStoreKey = new Map(); // content_hash -> package_hash
    for (const node of order) {
        const fh = contentHash(new TextEncoder().encode(JSON.stringify(node.files)));
        if (fh !== node.manifest.content_hash)
            return `bad-content-hash in ${node.manifest.name}`;
        const unsafe = validatePackagePaths(node);
        if (unsafe !== null)
            return `unsafe file path in ${node.manifest.name}: ${unsafe}`;
        const phr = safePackageHash(node);
        if (!phr.ok)
            return `invalid-package in ${node.manifest.name}: ${phr.reason}`;
        const ph = phr.hash;
...
export function parseArgs(argv) {
        ...
        for (let i = 1; i < argv.length; i++) {
            if (argv[i] === "--out" || argv[i] === "-o") {
                ...
                if (!next || next.startsWith("-"))
                    return { error: "--out requires a prefix argument" };
                outPrefix = next;
                i++;
            }
            else {
    ...
    if (command === "sign") {
        ...
        if (!pkgPath || pkgPath.startsWith("-"))
            return { error: "sign requires a <pkg-path> argument" };
        let keyPath;
        let outPath;
        for (let i = 2; i < argv.length; i++) {
            if (argv[i] === "--key") {
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--key requires a path argument" };
                keyPath = next;
                i++;
            }
        ...
        if (!keyPath)
            return { error: "sign requires --key <priv.key>" };
        const result = { command: "sign", pkgPath, keyPath };
    ...
    if (command === "trust") {
        const sub = argv[1];
        if (!sub)
            return { error: "trust requires a subcommand: add | list" };
        if (sub === "list")
            return { command: "trust", sub: "list" };
        if (sub === "add") {
            const arg = argv[2];
            if (!arg || arg.startsWith("-"))
                return { error: "trust add requires a <pubkey-file-or-b64> argument" };
            let label;
            for (let i = 3; i < argv.length; i++) {
                if (argv[i] === "--label") {
                    const next = argv[i + 1];
                    if (!next || next.startsWith("-"))
                        return { error: "--label requires a name argument" };
                    label = next;
                    i++;
                }
                else {
            ...
            const result = { command: "trust", sub: "add", arg };
            if (label !== undefined)
                return { ...result, label };
            return result;
        }
    ...
    if (command === "registry") {
        const sub = argv[1];
        if (sub === "list")
            return { command: "registry", sub: "list" };
        if (sub === "add") {
            ...
            for (let i = 5; i < argv.length; i++) {
                if (argv[i] === "--hash") {
                    ...
                    return { error: `Unknown option for registry add: ${argv[i]}` };
            }
            const result = { command: "registry", sub: "add", regName: name, regVersion: version, regUrl: url };
            if (hash !== undefined)
                return { ...result, regHash: hash };
            return result;
        }
        if (sub === "remote") {
            const action = argv[2];
            if (action === "list")
                return { command: "registry", sub: "remote-list" };
            if (action === "rm") {
                const url = argv[3];
                if (!url || url.startsWith("-"))
                    return { error: "registry remote rm requires <url>" };
                return { command: "registry", sub: "remote-rm", remoteUrl: url };
            }
            if (action === "add") {
                const url = argv[3];
                if (!url || url.startsWith("-"))
                    return { error: "registry remote add requires <url> --key <keyid>" };
                let key;
                let msd;
                for (let i = 4; i < argv.length; i++) {
                    if (argv[i] === "--key") {
                        ...
                        const v = argv[++i];
                        if (!v || v.startsWith("-"))
                            return { error: "--max-staleness-days requires a value" };
                        msd = Number(v);
                        if (!Number.isInteger(msd) || msd <= 0)
                            return { error: "--max-staleness-days must be a positive integer" };
                    }
                    else
                        return { error: `Unknown option for registry remote add: ${argv[i]}` };
                }
                if (!key)
                    return { error: "registry remote add requires --key <keyid>" };
                const r = { command: "registry", sub: "remote-add", remoteUrl: url, remoteKey: key };
                return msd !== undefined ? { ...r, remoteMaxStaleness: msd } : r;
            }
            return { error: "registry remote requires 'add', 'list', or 'rm'" };
        }
        if (sub === "publish") {
            let dir, base, key, out;
            let seq;
            for (let i = 2; i < argv.length; i++) {
                if (argv[i] === "--packages") {
                    dir = argv[++i];
                    if (!dir || dir.startsWith("-"))
                        return { error: "--packages requires a value" };
                }
                    ...
                    const sv = argv[++i];
                    const n = Number(sv);
                    if (!sv || !Number.isInteger(n) || n <= 0)
                        return { error: "--sequence requires a positive integer" };
                    seq = n;
                }
                else
                    return { error: `Unknown option for registry publish: ${argv[i]}` };
            }
            if (!dir)
                return { error: "registry publish requires --packages <dir>" };
            if (!base)
                return { error: "registry publish requires --base-url <url>" };
            if (!key)
                return { error: "registry publish requires --key <pem-path>" };
            let r = {
                command: "registry",
                sub: "publish",
                pubPackagesDir: dir,
        ...
        if (sub === "revoke" || sub === "quarantine" || sub === "unquarantine") {
            ...
            if (!spec || spec.startsWith("-"))
                return { error: `registry ${sub} requires <name>@<version>` };
            ...
            for (let i = 3; i < argv.length; i++) {
                if (argv[i] === "--key") {
                    ...
                    if (sub === "unquarantine")
                        return { error: "registry unquarantine does not take --reason" };
                    reason = argv[++i];
                    if (reason === undefined || reason.startsWith("-"))
                        return { error: "--reason requires a value" };
                }
                else
                    return { error: `Unknown option for registry ${sub}: ${argv[i]}` };
            }
            if (!key)
                return { error: `registry ${sub} requires --key <pem-path>` };
            let r = { command: "registry", sub, revName: name, revVersion: version, pubKeyPath: key };
            if (out !== undefined)
                r = { ...r, pubOut: out };
            if (reason !== undefined)
                r = { ...r, revReason: reason };
            return r;
        }
    ...
    if (command === "update") {
        ...
        if (!source || source.startsWith("-"))
            return { error: "update requires a <url-or-path> argument" };
        ...
        for (let i = 2; i < argv.length; i++) {
            if (argv[i] === "--lockfile") {
                const next = argv[++i];
                if (!next || next.startsWith("-"))
                    return { error: "--lockfile requires a path argument" };
                lockfilePath = next;
            }
        ...
        const updateResult = { command: "update", source, lockfile: lockfilePath, allowNoSignature };
        return offline ? { ...updateResult, offline: true } : updateResult;
    }
    if (command === "install") {
        const source = argv[1];
        if (!source || source.startsWith("-"))
            return { error: "install requires a <url-or-path> argument" };
        ...
        if (locked && frozen)
            return { error: "--locked and --frozen are mutually exclusive" };
        const baseResult = {
            command: "install",
            source,
            storePath,
        ...
        if (printResolution)
            return { ...baseResult, printResolution: true };
        return baseResult;
    }
    if (command === "verify") {
        const hash = argv[1];
        if (!hash || hash.startsWith("-"))
            return { error: "verify requires a <hash> argument" };
    ...
    if (command === "deps") {
        ...
        for (let i = 2; i < argv.length; i++) {
            ...
            if (arg === "--graph" || arg === "-g") {
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--graph requires a path argument" };
                graphPath = next;
                i++;
            }
                ...
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--output-engine requires flux | argocd | both" };
                ...
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--charts-dir requires a path argument" };
                chartsDir = next;
                i++;
            }
                ...
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--namespace requires a name argument" };
                namespace = next;
                i++;
            }
            else if (arg === "--as-of") {
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--as-of requires a date/time argument" };
                asOf = next;
                i++;
            }
                ...
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--during-migration requires a name argument" };
                ...
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--rollback-window requires a release/chart name" };
                rollbackWindow = next;
                i++;
            }
                ...
                const next = argv[i + 1];
                if (!next || next.startsWith("-"))
                    return { error: "--schedule requires a path argument" };
                schedulePath = next;
                i++;
            }
            else {
        ...
        if (!graphPath)
            return { error: "deps requires --graph <path>" };
        if (sub === "resolve" && !outDir)
            return { error: "deps resolve requires --out-dir <dir>" };
        if (sub === "evaluate-schedule" && !outDir)
            return { error: "deps evaluate-schedule requires --out-dir <dir>" };
...
function loadDependencyGraph(graphPath) {
...
function writeManifestDir(outDir, files) {
...
async function checkLockedMarks(pins, allowQuarantined, offline) {
    let revoked;
    let quarantined;
    try {
...
export async function main(argv) {
    ...
    if (parsed.command === "sign") {
        let pkg;
        try {
            pkg = JSON.parse(readFileSync(parsed.pkgPath, "utf8"));
        }
        catch {
    ...
    if (parsed.command === "trust") {
        if (parsed.sub === "list") {
            ...
            for (const r of rows)
                console.log(`  ${r.key_id}  [${r.source}]${r.label ? "  " + r.label : ""}`);
            return 0;
        }
        ...
        try {
            ...
            publicB64 = raw.startsWith("{") ? JSON.parse(raw).public_key : raw;
        ...
        try {
            ...
            canonicalB64 = pub.export({ type: "spki", format: "der" }).toString("base64");
        }
        catch {
        ...
        const entry = { key_id: kid, public_key: canonicalB64 };
    ...
    if (parsed.command === "registry") {
        ...
        if (parsed.sub === "list") {
            ...
            for (const r of rows)
                console.log(`  ${r.name}@${r.version}  ${r.url}  [${r.source}]`);
            return 0;
        }
        if (parsed.sub === "publish") {
            ...
            try {
                ...
                console.error(`ace: publish: cannot read key ${parsed.pubKeyPath}: ${e.message}`);
                return 1;
            }
            ...
            try {
                ...
                console.error(`ace: publish refused: invalid private key: ${e.message}`);
                return 1;
            }
            const dirs = parsed
            ...
            for (const d of dirs) {
                ...
                try {
                    ...
                    console.error(`ace: publish: cannot read dir ${d}: ${e.message}`);
                    return 1;
                }
                for (const f of entries) {
                    ...
                    if (typeof obj.manifest.content_hash !== "string") {
                        console.error(`ace: publish: skip ${f} — missing manifest.content_hash`);
                        continue;
                    }
                    const fh = contentHash(new TextEncoder().encode(JSON.stringify(obj.files)));
                    if (fh !== obj.manifest.content_hash) {
                    ...
                    if (urlOverride === undefined) {
                        const expectedFile = `${obj.manifest.name}-${obj.manifest.version}.json`;
                        if (f !== expectedFile) {
                            console.error(`ace: publish: skip ${f} — filename must be ${expectedFile} to match its derived consumer URL`);
                            continue;
                        }
                    }
                    const deps = obj.manifest.dependencies;
                    ...
                    const fileVals = Object.values(obj.files);
                    ...
                    const nm = obj.manifest.name;
                    const ver = obj.manifest.version;
                    ...
                    const depEdges = obj.manifest.dependencies;
                    ...
                    const unsafePath = validatePackagePaths(obj);
                    if (unsafePath !== null) {
                        console.error(`ace: publish: skip ${f} — unsafe file path: ${unsafePath}`);
                        continue;
                    }
                    packages.push(urlOverride !== undefined ? { pkg: obj, url: urlOverride } : { pkg: obj });
                }
            }
            ...
            if (existsSync(outPath)) {
                ...
                try {
                    ...
                    console.error(`ace: publish refused: cannot read existing ${outPath}: ${e.message}`);
                    return 1;
                }
                const p = parseIndex(prevRaw);
                if ("error" in p) {
                    console.error(`ace: publish refused: existing ${outPath} is not a valid index (${p.error}) — refusing to reset sequence (would look like a rollback to consumers); remove or fix it`);
                    return 1;
                }
                ...
                if (!prevVerify.ok) {
                    console.error(`ace: publish refused: existing ${outPath} signature does not verify under --key (${prevVerify.reason}) — refusing to auto-bump from an untrusted index; fix or remove it`);
                    return 1;
                }
                prev = p;
            }
            ...
            try {
                ...
                console.error(`ace: publish refused: signing failed (check --key is a valid Ed25519 PEM): ${e.message}`);
                return 1;
            }
            try {
                ...
                console.error(`ace: publish failed: cannot write ${outPath}: ${e.message}`);
                return 1;
            }
        ...
        if (parsed.sub === "revoke" || parsed.sub === "quarantine" || parsed.sub === "unquarantine") {
            ...
            try {
                ...
                console.error(`ace: ${verb}: cannot read key ${parsed.pubKeyPath}: ${e.message}`);
                return 1;
            }
            try {
                if (createPrivateKey(pem).asymmetricKeyType !== "ed25519") {
                ...
                console.error(`ace: ${verb} refused: invalid private key: ${e.message}`);
                return 1;
            }
            const outPath = parsed.pubOut ?? "index.json";
            if (!existsSync(outPath)) {
            ...
            try {
                ...
                console.error(`ace: ${verb} refused: cannot read ${outPath}: ${e.message}`);
                return 1;
            }
            const p = parseIndex(prevRaw);
            if ("error" in p) {
            ...
            const prevVerify = verifyIndexSignature(prevContent, prevSig, new Map([[prevInfo.keyId, { public_key: prevInfo.public_key }]]));
            if (!prevVerify.ok) {
                console.error(`ace: ${verb} refused: ${outPath} signature does not verify under --key (${prevVerify.reason}) — not your index`);
                return 1;
            }
            ...
            let next;
            if (verb === "revoke")
                next = applyRevoke(prevContent, name, version, parsed.revReason, at);
            else if (verb === "quarantine")
                next = applyQuarantine(prevContent, name, version, parsed.revReason, at);
            else
                next = applyUnquarantine(prevContent, name, version, at);
            if ("error" in next) {
                console.error(`ace: ${verb} refused: ${next.error}`);
                return 1;
            }
            const seq = p.sequence + 1;
            const content = { ...next, sequence: seq };
            let serialized;
            try {
                ...
                console.error(`ace: ${verb} refused: signing failed: ${e.message}`);
                return 1;
            }
            try {
                writeFileSync(outPath, serialized);
                ...
                console.error(`ace: ${verb} failed: cannot write ${outPath}: ${e.message}`);
                return 1;
            }
        ...
        if (pkgHash === undefined) {
            ...
            try {
                ...
                console.error(`ace: registry add: fetch/read failed: ${e.message}`);
                return 1;
            }
            let pkg;
            try {
    ...
    if (parsed.command === "update") {
        ...
        try {
            ...
            console.error(`ace: download/read failed: ${e.message}`);
            return 1;
        }
        let pkg;
        try {
        ...
        if (Array.isArray(pkg.manifest.dependencies) && pkg.manifest.dependencies.length > 0) {
            const fetchPackage = async (u) => u.startsWith("http://") || u.startsWith("https://") ? await (await fetch(u)).text() : readFileSync(u, "utf8");
            ...
            if (errors.length > 0) {
                for (const e of errors)
                    console.error(`ace: update refused: ${e}`);
                return 1;
            }
            const solveResult = await solve(pkg, fetchPackage, registry);
            if (!solveResult.ok) {
                console.error(`ace: update refused: ${solveResult.reason} — ${solveResult.detail} (path: ${solveResult.path.join(" → ")})`);
                return 1;
            }
            ...
            try {
                ...
                console.error(`ace: update failed: could not write lockfile ${parsed.lockfile}: ${e.message}`);
                return 1;
            }
    ...
    if (parsed.command === "install") {
        ...
        if (Array.isArray(pkg.manifest.dependencies) && pkg.manifest.dependencies.length > 0) {
            ...
            if (parsed.frozen) {
                ...
                const verified = []; // locked nodes in lock order; root installed separately
                for (const node of lf.nodes) {
                    ...
                    try {
                        ...
                        console.error(`ace: install refused: fetch failed for ${node.name}@${node.version} (${node.url}): ${e.message}`);
                        return 1;
                    }
                    let np;
                    try {
                        np = JSON.parse(nodeRaw);
                    }
                    catch {
                    ...
                    if (prior !== undefined && prior !== nph) {
                        console.error(`ace: install refused: store-collision — ${node.name}@${node.version} shares a content_hash store key with a different package`);
                        return 1;
                    }
            ...
            if (errors.length > 0) {
                for (const e of errors)
                    console.error(`ace: install refused: ${e}`);
                return 1;
            }
            const solveResult = await solve(pkg, fetchPackage, registry);
            if (!solveResult.ok) {
            ...
            if (parsed.locked) {
                ...
                try {
                    ...
                    console.error(`ace: install refused: --locked but no lockfile at ${parsed.lockfile} — run 'ace update' or install without --locked`);
                    return 1;
                }
            ...
            if ("error" in lf) {
                ...
                try {
                    ...
                    console.error(`ace: WARNING: could not write lockfile ${parsed.lockfile}: ${e.message}`);
                }
            }
        ...
        try {
            ...
            if (parsed.sub === "query") {
                ...
                if (parsed.duringMigration) {
                    ...
                    if (!found) {
                        if (parsed.schedulePath && existsSync(parsed.schedulePath)) {
                            const schedule = parseYaml(readFileSync(parsed.schedulePath, "utf8"));
                ...
                for (const name of resolved.order) {
                    ...
                    if (!node || name === graph.metadata.name)
                        continue;
            ...
            console.error(`ace: deps ${parsed.sub} failed: ${e.message}`);
            return 1;
        }
    }
    return 1;
...
if (import.meta.main) {
    ...
    main(process.argv.slice(2))
        ...
        .catch((e) => {
        console.error(`ace: fatal: ${e.message}`);
        process.exit(1);
    });
}
