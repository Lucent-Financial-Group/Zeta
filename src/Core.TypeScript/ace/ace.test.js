...
import { parseArgs, main } from "./ace.js";
import { listInstalled, contentHash, listTrustedKeys, loadRegistry } from "./store.js";
import { readRegistriesConfig } from "./store.js";
import { generateKeypair, signManifest } from "./signing.js";
import { generateKeypair as gkpA } from "./signing.js";
import { signIndex as sidxA } from "./index-signature.js";
...
function readIndexFile(path) {
    ...
    if ("error" in doc)
        throw new Error(`readIndexFile: ${doc.error}`);
    return doc;
}
...
function signedPkgFixture(files = { "a.txt": "hi" }) {
...
describe("parseArgs", () => {
    ...
    test("install --allow-no-signature parses", () => {
        ...
        expect("error" in result).toBe(false);
        if (!("error" in result) && result.command === "install") {
            expect(result.allowNoSignature).toBe(true);
        }
    });
    ...
    test("keygen --out custom-prefix parses", () => {
        ...
        if (!("error" in result) && result.command === "keygen") {
            expect(result.outPrefix).toBe("mykey");
        }
    });
...
describe("parseArgs — install lockfile flags", () => {
    ...
    test("--lockfile <path> overrides", () => {
        ...
        if ("command" in a && a.command === "install")
            expect(a.lockfile).toBe("custom.lock");
    });
...
describe("listInstalled", () => {
    ...
    test("skips manifests missing version field", () => {
        ...
        mkdirSync(pkgDir);
        writeFileSync(join(pkgDir, "manifest.json"), JSON.stringify({ format_version: 1, name: "x", content_hash: "blake3:abc" }));
        const result = listInstalled(dir);
        expect(result).toEqual([]);
    });
    test("sorts by name", () => {
...
describe("main", () => {
    ...
    test("trust add normalizes a padded SPKI to the canonical key_id (not the padded bytes)", async () => {
        ...
        const canon = publicKey.export({ type: "spki", format: "der" });
    ...
    test("e2e: install a small graph (root->A->B) installs all three", async () => {
        ...
        const h = (files) => contentHash(new TextEncoder().encode(JSON.stringify(files)));
        ...
        const A = { manifest: { format_version: 1, name: "A", version: "1.0.0", content_hash: h({ "a.txt": "a" }), dependencies: [{ kind: "inline", name: "B", version: "1.0.0", url: join(dir, "B.json"), package_hash: packageHash(B) }] }, files: { "a.txt": "a" } };
        writeFileSync(join(dir, "A.json"), JSON.stringify(A));
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "inline", name: "A", version: "1.0.0", url: join(dir, "A.json"), package_hash: packageHash(A) }] }, files: { "r.txt": "r" } };
    ...
    test("atomic: a graph with an unsafe-path node installs NOTHING (preflight)", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "inline", name: "BAD", version: "1.0.0", url: join(dir, "BAD.json"), package_hash: packageHash(bad) }] }, files: { "r.txt": "r" } };
        writeFileSync(join(dir, "root.json"), JSON.stringify(root));
        const code = await main(["install", join(dir, "root.json"), "--store", store, "--allow-no-signature"]);
        expect(code).toBe(1);
    ...
    test("atomic: a graph whose ROOT has a bad content_hash installs NOTHING", async () => {
        ...
        const B = { manifest: { format_version: 1, name: "B", version: "1.0.0", content_hash: h({ "b.txt": "b" }) }, files: { "b.txt": "b" } };
        writeFileSync(join(dir, "B.json"), JSON.stringify(B));
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: "blake3:deadbeef", dependencies: [{ kind: "inline", name: "B", version: "1.0.0", url: join(dir, "B.json"), package_hash: packageHash(B) }] }, files: { "r.txt": "r" } };
        writeFileSync(join(dir, "root.json"), JSON.stringify(root));
        const code = await main(["install", join(dir, "root.json"), "--store", store, "--allow-no-signature"]);
        expect(code).toBe(1);
        expect(listInstalled(store).length).toBe(0);
    ...
    test("store-collision: two distinct packages with identical files install NOTHING", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [
                    { kind: "inline", name: "X", version: "1.0.0", url: join(dir, "X.json"), package_hash: packageHash(X) },
                    { kind: "inline", name: "Y", version: "1.0.0", url: join(dir, "Y.json"), package_hash: packageHash(Y) },
                ] }, files: { "r.txt": "r" } };
        writeFileSync(join(dir, "root.json"), JSON.stringify(root));
        const code = await main(["install", join(dir, "root.json"), "--store", store, "--allow-no-signature"]);
        expect(code).toBe(1);
...
describe("registry commands", () => {
    test("ace registry add fetches + computes hash + stores; registry list shows it", async () => {
        ...
        expect(reg.get("D")?.get("1.0.0")?.package_hash).toBe(packageHash(D));
    ...
    test("e2e: graph install writes ./ace.lock pinning the installed deps", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "registry", name: "A", version: "1.0.0" }] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
...
describe("install --frozen (slice 5.3)", () => {
    ...
    function buildInlineGraph() {
        ...
        const aPath = join(dir, "A.json");
        writeFileSync(aPath, JSON.stringify(A));
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "inline", name: "A", version: "1.0.0", url: aPath, package_hash: packageHash(A) }] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
        const lockPath = join(dir, "ace.lock");
    ...
    test("--frozen with a drifted root (deps changed vs the lock) is refused", async () => {
        ...
        const drifted = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "inline", name: "A", version: "2.0.0", url: g.aPath, package_hash: packageHash(g.A) }] }, files: { "r.txt": "r" } };
    ...
    test("--frozen refuses an untrusted-signature locked node even WITH --allow-no-signature", async () => {
        ...
        const aHash = packageHash(signedA);
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "inline", name: "A", version: "1.0.0", url: aPath, package_hash: aHash }] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
    ...
    test("--frozen atomicity: a 2-node lock whose 2nd node fails verification installs NEITHER (verify-all-before-install-any)", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [
                    { kind: "inline", name: "A", version: "1.0.0", url: aPath, package_hash: packageHash(A) },
                    { kind: "inline", name: "B", version: "1.0.0", url: bPath, package_hash: packageHash(B) },
                ] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
        ...
        const lock = {
            ...
            root: { name: "root", version: "1.0.0", package_hash: packageHash(root) },
            nodes: [
                { name: "A", version: "1.0.0", url: aPath, package_hash: packageHash(A) },
                { name: "B", version: "1.0.0", url: bPath, package_hash: packageHash(B) },
            ],
        };
        const lockPath = join(dir, "ace.lock");
        writeFileSync(lockPath, JSON.stringify(lock));
    ...
    test("--frozen store-collision: two nodes sharing a content_hash store key with different package_hash install NOTHING", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [
                    { kind: "inline", name: "X", version: "1.0.0", url: xPath, package_hash: packageHash(X) },
                    { kind: "inline", name: "Y", version: "1.0.0", url: yPath, package_hash: packageHash(Y) },
                ] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
        const lock = {
            ...
            root: { name: "root", version: "1.0.0", package_hash: packageHash(root) },
            nodes: [
                { name: "X", version: "1.0.0", url: xPath, package_hash: packageHash(X) },
                { name: "Y", version: "1.0.0", url: yPath, package_hash: packageHash(Y) },
            ],
        };
        const lockPath = join(dir, "ace.lock");
        writeFileSync(lockPath, JSON.stringify(lock));
...
describe("install — semver ranges (slice 5.2)", () => {
    test("e2e: ranged registry dep resolves to newest satisfying version", async () => {
        ...
        const dir = mkdtempSync(join(tmpdir(), "ace-pkgs-"));
        const h = (files) => contentHash(new TextEncoder().encode(JSON.stringify(files)));
        const mkA = (v) => ({ manifest: { format_version: 1, name: "A", version: v, content_hash: h({ "a.txt": v }) }, files: { "a.txt": v } });
    ...
    test("e2e: unsatisfiable range → exit 1, store empty", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "registry", name: "A", version: ">=2.0.0" }] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
    ...
    test("e2e: inline-only graph still installs (empty registry, no registry-miss)", async () => {
        ...
        const ap = join(dir, "A.json");
        writeFileSync(ap, JSON.stringify(A));
        const aHash = packageHash(A);
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "inline", name: "A", version: "1.0.0", url: ap, package_hash: aHash }] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
        expect(await main(["install", rootPath, "--store", store, "--allow-no-signature"])).toBe(0);
...
describe("leaf-install lockfiles (slice 5.4)", () => {
    ...
    function leafFixture(files = { "leaf.txt": "v1" }) {
    ...
    test("leaf install writes an empty-nodes lock", async () => {
        ...
        if (!("error" in lf)) {
            ...
            expect(lf.root.package_hash).toBe(packageHash(pkg));
...
describe("ace update (slice 5.4)", () => {
    ...
    test("update rewrites ./ace.lock to the freshly-solved graph + installs NOTHING", async () => {
        ...
        if (!("error" in before))
            expect(before.nodes.map((n) => `${n.name}@${n.version}`)).toEqual(["A@1.0.0"]);
        ...
        if (!("error" in after)) {
            ...
            expect(after.nodes[0].package_hash).toBe(packageHash(A11));
        }
    ...
    test("update refuses (no lock written) when a freshly-solved node fails preflight", async () => {
        ...
        const root = { manifest: { format_version: 1, name: "root", version: "1.0.0", content_hash: h({ "r.txt": "r" }), dependencies: [{ kind: "registry", name: "BAD", version: "^1.0.0" }] }, files: { "r.txt": "r" } };
        const rootPath = join(dir, "root.json");
        writeFileSync(rootPath, JSON.stringify(root));
        const lockPath = join(dir, "ace.lock");
        const code = await main(["update", rootPath, "--lockfile", lockPath, "--allow-no-signature"]);
...
describe("ace install via remote registry (slice 6)", () => {
    ...
    test("resolves + installs a package from a signed remote index", async () => {
        ...
        const pkgHash = packageHash(pkg);
        const pkgUrl = "https://pkgs/leaf-1.0.0.json";
        const idxContent = { format_version: 1, sequence: 1, issued_at: new Date(now).toISOString(),
        ...
        globalThis.fetch = (async (u) => new Response(u === pkgUrl ? pkgJson : idxJson, { status: 200 }));
    ...
    test("update --offline uses cached registry-index (no index fetch) + writes lockfile", async () => {
        ...
        const { registryCacheDir } = await import("./store.js");
...
describe("ace registry publish (slice 6.1)", () => {
    function writeSignedPkg(dir, name, version) {
    ...
    test("publish a dir → signed index; sequence auto-bumps; non-package skipped", async () => {
        const { parseIndex } = await import("./registry-remote.js");
    ...
    test("tampered existing --out (signature flipped) → publish refused; index unchanged", async () => {
        ...
        const tampered = JSON.parse(readFileSync(outPath, "utf8"));
        tampered.sequence = 0;
    ...
    test("package with non-array dependencies is skipped; well-formed sibling is indexed", async () => {
        ...
        const badPkg = { manifest: { ...badM, dependencies: "not-an-array", signature: badSig }, files: badFiles };
        writeFileSync(join(pkgDir, "bad-1.0.0.json"), JSON.stringify(badPkg));
    ...
    test("malformed dep edge (missing version) is skipped; well-formed sibling is indexed", async () => {
        ...
        const badM = { format_version: 1, name: "bad", version: "1.0.0", content_hash: badCh };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const badPkg = { manifest: { ...badM, dependencies: [{ kind: "registry", name: "dep" }], signature: signManifest(badM, badKp.privatePem) }, files: badFiles };
        writeFileSync(join(pkgDir, "bad-1.0.0.json"), JSON.stringify(badPkg));
    ...
    function writeUrlPkg(dir, file, name, version, url) {
        const files = { [`${name}.txt`]: "hi" };
        const ch = contentHash(new TextEncoder().encode(JSON.stringify(files)));
        const kp = generateKeypair();
        const m = { format_version: 1, name, version, content_hash: ch };
...
describe("ace registry revoke/quarantine/unquarantine (slice 7)", () => {
    ...
    async function setupConsumer(opts) {
        ...
        const serve = (u) => new Response(u === PKG_URL ? pkgJson : readFileSync(idxPath, "utf8"), { status: 200 });
    ...
    async function freshFetchAfterCacheWipe(serve) {
        const { rmSync } = await import("node:fs");
        const { registryCacheDir } = await import("./store.js");
        rmSync(registryCacheDir(), { recursive: true, force: true });
        globalThis.fetch = (async (u) => serve(u));
