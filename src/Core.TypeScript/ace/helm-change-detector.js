...
function extractChartSection(yamlText, chartName) {
    const lines = yamlText.split(/\r?\n/);
    const chartHeader = `  ${chartName}:`;
    let collecting = false;
    const collectedVersions = [];
    let currentVersion = null;
    let listItemIndent = null;
...
export async function fetchLatestVersion(repoUrl, chartName) {
    ...
    const versions = chartEntries.map((e) => e.version).filter((v) => semver.valid(v));
