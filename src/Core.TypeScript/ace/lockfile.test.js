...
describe("buildLockfile", () => {
    test("records registry node url from the registry + excludes root", () => {
        ...
        const order = [A, root]; // resolve order: deps-first, root last
    ...
    test("records a kind-OMITTED inline edge's url (pre-DU back-compat parity with resolve())", () => {
        ...
        const kindlessEdge = { name: "C", version: "3.0.0", url: "http://e/C", package_hash: packageHash(C) };
...
import { serializeLockfile, parseLockfile } from "./lockfile.js";
...
import { verifyRootMatchesLock } from "./lockfile.js";
describe("verifyRootMatchesLock", () => {
    test("true when root packageHash matches, false on any root change", () => {
        const root = pkgAt("root", "1.0.0", [regEdge("A", "^1.0.0")]);
        const lf = { format_version: 1, root: { name: "root", version: "1.0.0", package_hash: packageHash(root) }, nodes: [] };
...
import { lockfilesEqual, buildLeafLockfile } from "./lockfile.js";
describe("lockfilesEqual", () => {
    ...
    test("false when a node version differs", () => {
        const diff = { ...base, nodes: [{ ...base.nodes[0], version: "1.3.0" }] };
        expect(lockfilesEqual(base, diff)).toBe(false);
    });
    test("false when root differs", () => {
        const diff = { ...base, root: { ...base.root, package_hash: "blake3:other" } };
        expect(lockfilesEqual(base, diff)).toBe(false);
    });
});
