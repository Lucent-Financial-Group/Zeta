...
import { parseMechanismManifest } from "./setup-manifest.js";
import { buildSetupMechanismPointers, serializeSetupMechanismPointers, } from "./setup-mechanism-pointers.js";
...
describe("setup mechanism pointers (Ace time-crystal deps)", () => {
    ...
    test("from-shim includes jammy cvc5 alias", () => {
        ...
        const entries = parseMechanismManifest(text);
        expect(entries.some((e) => e.tokens.join(" ") === "cvc5 cvc4" && e.attrs.when === "ubuntu-22.04")).toBe(true);
    });
