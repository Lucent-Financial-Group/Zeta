...
describe("Rust toolchain provisioning is DECLARED, not topped up by shell", () => {
  ...
  test("the pinned version survives the move from string to table form", () => {
    expect(rustDeclaration().version).toBe("1.99.0-beta.3");
  });
...
describe("Rust setup stays offline when the cache is complete", () => {
  ...
  test("installed Rust and wasm target do not mutate the rustup default", () => {
    withFakeCommands((bin, home, log) => {
      writeCommand(bin, "rustc", 'echo "rustc 1.99.0-beta.3"');
      writeCommand(
        bin,
        "rustup",
        'case "$*" in "--version") echo "rustup 1.28.2" ;; "toolchain list") echo "1.99.0-beta.3-x86_64-unknown-linux-gnu" ;; "target list --installed") echo "wasm32-unknown-unknown" ;; *) echo "$*" >> "$RUSTUP_CALL_LOG"; exit 99 ;; esac',
      );
      ...
      expect(result.stdout).toContain("Rust 1.99.0-beta.3 + wasm32-unknown-unknown ready");
