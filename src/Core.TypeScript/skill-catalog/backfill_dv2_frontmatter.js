...
function fieldPresent(content, field) {
    ...
    for (const line of lines) {
        ...
        if (count === 1 && line.startsWith(`${field}:`))
            return true;
    }
    return false;
}
function dashCount(content) {
    let count = 0;
...
function buildInjections(file, content, today) {
    const inject = [];
    ...
    if (!fieldPresent(content, "status"))
        inject.push("status: active");
    if (!fieldPresent(content, "bp_rules_cited")) {
        inject.push(`bp_rules_cited: ${computeBpRules(file)}`);
    }
    return inject;
}
function injectBeforeSecondFence(content, blob) {
    const lines = content.split("\n");
    const out = [];
    let count = 0;
    for (const line of lines) {
...
function processOne(file, dryRun, today) {
...
export function main(argv) {
    ...
    if (files.length === 0) {
        process.stderr.write("usage: bun tools/skill-catalog/backfill_dv2_frontmatter.ts [--dry-run] <SKILL.md path>... | --all\n");
        return 1;
    }
