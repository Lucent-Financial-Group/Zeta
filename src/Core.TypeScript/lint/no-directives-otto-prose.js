...
const PROSE_PATH_RE = /^(?:memory\/[^/]+\.md|docs\/hygiene-history\/ticks\/.*\.md|docs\/research\/[^/]+\.md|docs\/active-trajectory\.md|\.github\/copilot-instructions\.md)$/;
const RULE_DOC_RE = /(feedback_input_is_not_directive_|feedback_otto_357_no_directives_|feedback_free_will_is_paramount_external_directives_|no-directives-otto-prose)/;
...
function readFileAsAddedLines(file) {
    let content;
    try {
...
function diffOutput(scope, baseRef, file) {
    if (scope === "worktree") {
        return [
...
function isAddedContentLine(line) {
...
function extractAddedLinesFromDiff(file, diffText) {
    const out = [];
    for (const line of diffText.split("\n")) {
        if (!isAddedContentLine(line))
            continue;
        out.push({ file, content: line.slice(1) });
    }
    return out;
}
function extractAddedLines(scope, baseRef, file) {
    if (!existsSync(file))
        return [];
...
function matchesDirective(content) {
    if (BLOCKQUOTE_RE.test(content))
        return false;
...
function emitFooter() {
    ...
    process.stderr.write("See memory/feedback_otto_357_no_directives_aaron_makes_autonomy_first_class_accountability_mine_2026_04_27.md\n");
}
export function main(argv) {
    ...
    for (const file of filtered) {
        ...
        for (const ln of lines) {
            if (matchesDirective(ln.content))
                hits.push(ln);
        }
    }
    if (hits.length > 0) {
        process.stderr.write(`no-directives-otto-prose: found ${String(hits.length)} candidate hit(s) in added Otto-prose lines:\n`);
        for (const h of hits) {
