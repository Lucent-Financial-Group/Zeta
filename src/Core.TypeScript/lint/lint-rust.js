...
function main() {
    ...
    for (const tomlPath of cargoTomls) {
        ...
        const fmtOk = runCommand("cargo", ["fmt", "--check", "--manifest-path", tomlPath], `Formatting check for Rust crate: ${crateName}`);
        if (!fmtOk)
            return 1;
        const clippyOk = runCommand("cargo", ["clippy", "--manifest-path", tomlPath, "--all-targets", "--", "-D", "warnings"], `Clippy lint checks for Rust crate: ${crateName}`);
        if (!clippyOk)
            return 1;
    }
