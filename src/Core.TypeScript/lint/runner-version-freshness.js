...
const VERIFY_URL = "https://docs.github.com/en/actions/how-tos/write-workflows/choose-where-workflows-run/choose-the-runner-for-a-job#standard-github-hosted-runners-for-public-repositories";
...
function findMatchingLines(uncommented, prefilter, boundary) {
...
function scanStaleOrRolling(uncommented, pattern) {
...
function scanUnknownScalar(uncommented, rollingOrAllowed) {
    const lines = uncommented.split("\n");
    const out = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (exprRe.test(line))
            continue;
        if (allowedRe.test(line))
            continue;
        out.push(`${String(i + 1)}:${line}`);
    }
    return out;
}
function scanUnknownMatrix(uncommented, rollingOrAllowed, stalePattern) {
    const lines = uncommented.split("\n");
    const out = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (allowedRe.test(numbered))
            continue;
        if (staleRe.test(numbered))
            continue;
        out.push(numbered);
    }
    return out;
...
function emitFileFindings(file, r) {
    let any = false;
    if (r.stale.length > 0) {
        process.stdout.write(`STALE RUNNER LABEL(S) in ${file}:\n`);
        for (const h of r.stale)
    ...
    if (r.rolling.length > 0) {
        ...
        for (const h of r.rolling)
            process.stdout.write(`  ${h}\n`);
        any = true;
    }
    if (r.unknown.length > 0) {
        process.stdout.write(`NOT-ON-ALLOW-LIST RUNNER LABEL(S) in ${file} (label is neither stale nor allowed; allow-list may need refresh):\n`);
...
export function main(argv) {
    ...
    for (const file of files) {
        ...
        if (emitFileFindings(file, result))
            fail = true;
    }
