...
function parseFailOverArg(arg, next) {
    if (arg === "--fail-over") {
        if (next === undefined)
...
function classifyArg(arg, next) {
    if (arg === "--list-missing")
        return { kind: "set-mode", mode: "list-missing", skip: 0 };
    if (arg === "--verbose")
        return { kind: "set-mode", mode: "verbose", skip: 0 };
    if (arg === "-h" || arg === "--help")
        return { kind: "help", skip: 0 };
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (step.kind === "help")
            return { kind: "help" };
        if (step.kind === "error")
            return { kind: "usage-error", message: step.message };
        if (step.kind === "set-mode")
            mode = step.mode;
...
function classifySkill(content) {
    if (matchesAny(content, H1_PATTERNS))
        return "H1";
    if (matchesAny(content, H2_PATTERNS))
        return "H2";
    if (matchesAny(content, H3_PATTERNS))
        return "H3";
    return "MISSING";
}
function auditSkills(files) {
    const results = [];
...
function emitListMissing(c) {
    ...
    for (const name of c.missingList)
        process.stdout.write(`${name}\n`);
}
function emitVerboseRow(name, tier) {
    if (name.length > 0)
        process.stdout.write(`| \`${name}\` | ${tier} |\n`);
}
function emitVerbose(c) {
    process.stdout.write("# Safety-clause audit — per skill\n");
    process.stdout.write("\n");
    process.stdout.write("| Skill | Tier |\n");
    process.stdout.write("|---|---|\n");
    for (const n of c.h1List)
        emitVerboseRow(n, "H1 explicit");
    for (const n of c.h2List)
        emitVerboseRow(n, "H2 scope");
    for (const n of c.h3List)
        emitVerboseRow(n, "H3 authority");
    for (const n of c.missingList)
        emitVerboseRow(n, "MISSING");
    process.stdout.write("\n");
}
function emitSummary(c) {
