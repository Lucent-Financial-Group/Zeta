...
const SCAN_ROOTS = ["src", "tests", "bench", "tools"];
const SCAN_EXTS = [".fs", ".cs", ".sh", ".ts"];
...
function parseMode(argv) {
    const arg = argv[0];
    if (arg === undefined || arg === "check" || arg === "")
        return "check";
    if (arg === "--list")
        return "list";
    if (arg === "--fail-any")
        return "fail-any";
    if (arg === "--regenerate-baseline")
        return "regenerate-baseline";
    return null;
}
function isCommentLine(line) {
    if (COMMENT_TRIPLE_SLASH_RE.test(line))
        return true;
    if (COMMENT_DOUBLE_SLASH_RE.test(line))
        return true;
    if (COMMENT_SHEBANG_RE.test(line))
        return false;
    if (COMMENT_HASH_RE.test(line))
        return true;
    return false;
}
...
function processEntry(e, dir, root, stack, out) {
...
function walkRoot(root, top, out) {
    const stack = [join(root, top)];
...
function listScanFiles(root) {
    ...
    for (const top of SCAN_ROOTS)
        walkRoot(root, top, out);
    return out;
}
function collectViolationsForFile(rel, absPath) {
    let content;
    try {
...
function collectAllViolations(root) {
...
function emitListMode(violations) {
    for (const row of violations)
        process.stdout.write(`${row}\n`);
    return 0;
}
function emitFailAny(violations) {
...
function emitRegenerate(violations, baselinePath) {
...
function emitCheck(violations, baselinePath, scriptName) {
    ...
    if (newViolations.length === 0) {
        process.stdout.write(`doc-comment-history-audit: no new violations (${String(baseline.length)} entries in baseline)\n`);
        return 0;
    }
    process.stderr.write("doc-comment-history-audit: new violations not in baseline:\n");
    for (const row of newViolations)
        process.stderr.write(`${row}\n`);
...
export function main(argv) {
    ...
    if (mode === "list")
        return emitListMode(violations);
    if (mode === "fail-any")
        return emitFailAny(violations);
