...
function parseArgs(argv) {
    const args = { json: false, noGit: false };
    for (const a of argv) {
        switch (a) {
            case "--json":
                args.json = true;
...
for (const step of steps) {
    if (step.source.startsWith("("))
        continue; // synthetic/computed sources
...
for (const step of steps) {
    if (step.n === 8)
        continue; // Step 8 is rendered separately as the closing directive
