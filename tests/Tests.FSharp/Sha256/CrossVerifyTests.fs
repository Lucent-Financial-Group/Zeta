module Zeta.Tests.FSharp.Sha256.CrossVerifyTests
...
type private Sha256Vector = {
    Id: string
    InputBytes: byte[]
    ExpectedHex: string
}

let private toVector (idx: int) (item: YamlValue) : Sha256Vector =
...
let private loadVectors (yamlText: string) : Sha256Vector list =
    let root =
...
[<Fact>]
let ``cross-verify five sha256 vectors match TS+Rust hex`` () =
    let root = repoRoot ()
    let yamlPath = Path.Join(root, "tests", "cross-verification", "sha256", "vectors.yaml")
    let yamlText = File.ReadAllText(yamlPath)

    // Own-the-interface: parse through our YAML port (Zeta.Core.FSharp.Yaml.Dom.parse),
    // not YamlDotNet directly.
    ...
    for v in vectors do
        let hex = sha256Hex v.InputBytes
        results.[v.Id] <- hex
    ...
    let outputPath = Path.Join(root, "tests", "cross-verification", "sha256", "fsharp-output.json")
