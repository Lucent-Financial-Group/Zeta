...
let ``NEGATIVE CONTROL: perturbing the recipe by ONE instruction changes the bytes (the check can fail)``
    ...
    // ...and the byte comparison is the thing doing the work: re-running the UNPERTURBED recipe a
    // second time still agrees, so the difference above is the recipe's, not run-to-run noise (DST).
    Assert.Equal<byte[]>(original, bytes (run (recipeFor program)))
