...
let ``gates are unitary: norm preserved by Pauli and rotation gates`` () =
    ...
    Assert.Equal(QubitIso.normSq g, QubitIso.normSq (QubitIso.phaseS g), 12)
    Assert.Equal(QubitIso.normSq g, QubitIso.normSq (QubitIso.phaseT g), 12)
...
let ``Raw kernels match the algebraic gate surface`` () =
    ...
    assertSame (QubitIso.phaseS g) (QubitIso.Raw.phaseS raw)
    assertSame (QubitIso.phaseT g) (QubitIso.Raw.phaseT raw)
