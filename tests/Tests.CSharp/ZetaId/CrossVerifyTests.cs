...
public class CrossVerifyTests
{
    ...
    private static string RepoRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "Zeta.sln")))
            dir = dir.Parent;
    ...
    {
        var root = RepoRoot();
        var yamlPath = Path.Combine(root, "tests", "cross-verification", "zeta-id", "vectors.yaml");
        ...
        var outputPath = Path.Combine(root, "tests", "cross-verification", "zeta-id", "cs-output.json");
