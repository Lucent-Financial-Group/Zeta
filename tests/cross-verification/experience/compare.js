...
const mumpsExists = (() => {
    try {
        return JSON.parse(readFileSync("mumps-output.json", "utf8"));
    }
    catch {
        return null;
    }
...
function packProvisionalState(state) {
...
function unpackProvisionalState(buf) {
    let offset = 0;
...
for (const key of keys) {
    ...
    const matches = unpacked.languageCode === tsVal.uli.languageCode &&
