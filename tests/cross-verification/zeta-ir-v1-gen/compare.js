...
function assertSameVec(a, b, ctx) {
    const av = values(a);
    const bv = values(b);
...
const ts = load(join(DIR, "ts-output.json"));
const fs = load(join(DIR, "fsharp-output.json"));
const primitives = Object.keys(ts);
if (primitives.length === 0)
    throw new Error("ts-output.json has no primitives");
// (1) N-way agreement between the two v1 oracles.
for (const p of primitives) {
    const tsVec = ts[p];
    const fsVec = fs[p];
    if (!tsVec)
        throw new Error(`ts-output.json missing primitive "${p}"`);
    if (!fsVec)
        throw new Error(`fsharp-output.json missing primitive "${p}"`);
...
for (const p of primitives) {
    ...
    if (!goldenPath)
        throw new Error(`no legacy golden mapping for primitive "${p}"`);
