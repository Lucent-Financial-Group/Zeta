...
function field(obj, key) {
    if (obj.t !== "obj")
        throw new Error("v1 IR: expected object");
    const hit = obj.v.find(([k]) => k === key);
    if (!hit)
        throw new Error(`v1 IR: missing field "${key}"`);
    return hit[1];
}
function asInt(v) {
    if (v.t !== "int")
        throw new Error("v1 IR: expected int");
    return BigInt(v.v);
}
function asStr(v) {
    if (v.t !== "str")
        throw new Error("v1 IR: expected string");
    return v.v;
}
/** Decode a v1 envelope, REJECTING any shape deviation (mirrors ZetaIrV1.validate). */
function decodeV1(ir) {
    ...
    if (asStr(field(ir, "schema")) !== SCHEMA_TAG)
        throw new Error("v1 IR: wrong schema tag");
    const width = Number(asInt(field(ir, "width"))); // width is REQUIRED in v1
    const opsNode = field(ir, "ops");
    if (opsNode.t !== "arr")
        throw new Error("v1 IR: ops must be an array");
...
function fold(decoded, x) {
    const mask = (1n << BigInt(decoded.width)) - 1n;
    const uword = (z) => z & mask;
...
export function emit(spec) {
    ...
    if (!cjResult.ok)
        throw new Error(`v1 envelope for ${spec.generator} could not be canonicalized: ${cjResult.error}`);
    const reparsed = fromCanonicalJson(cjResult.value);
    if (!reparsed.ok)
        throw new Error(`v1 envelope for ${spec.generator} is not canonical: ${reparsed.error}`);
    const decoded = decodeV1(reparsed.value);
    const out = { _source: "generated-from-zeta-ir-v1" };
    for (const [id, x] of Object.entries(spec.inputs))
        out[id] = fold(decoded, x).toString();
    return out;
}
if (import.meta.main) {
    const result = {};
    for (const spec of [SPLITMIX64, FMIX32, FMIX64])
        result[spec.goldenPrimitive] = emit(spec);
...
export { SPLITMIX64, FMIX32, FMIX64 };
