...
import { emit, SPLITMIX64, FMIX32, type GenSpec } from "./_gen/gen.ts";
...
function golden(primitive: string): Vec {
  return values(JSON.parse(readFileSync(join(CV, primitive, "ts-output.json"), "utf8")) as Vec);
}
function sameAsGolden(spec: GenSpec): boolean {
  const got = values(emit(spec));
  const want = golden(spec.goldenPrimitive);
  const keys = new Set([...Object.keys(got), ...Object.keys(want)]);
  for (const k of keys) if (got[k] !== want[k]) return false;
  return true;
}
...
test("changing the declared width is load-bearing (splitmix64 at width 63 diverges)", () => {
  ...
  const narrowed: GenSpec = { ...SPLITMIX64, width: 63 };
  expect(sameAsGolden(narrowed)).toBe(false);
});

test("dropping the last op diverges from the golden", () => {
  const truncated: GenSpec = { ...FMIX32, ops: FMIX32.ops.slice(0, -1) };
...
test("reordering two ops diverges from the golden", () => {
  ...
  if (first === undefined || second === undefined) throw new Error("fmix32 needs >= 2 ops");
  const reordered: GenSpec = { ...FMIX32, ops: [second, first, ...rest] };
