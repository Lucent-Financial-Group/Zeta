...
function emitGo(ir: ZetaIrV1): string {
  const steps = ir.ops.map((op) => {
    if (op.op === "mul") {
      const unsigned = BigInt(op.k!) & ((1n << BigInt(ir.width)) - 1n);
      return `\tz = z * 0x${unsigned.toString(16).toUpperCase()}`;
    }
    return `\tz = z ^ (z >> ${op.s})`;
  });

  // For 32-bit, use uint32; for 64-bit, use uint64
  const uintType = ir.width <= 32 ? "uint32" : "uint64";
...
function emitQSharp(ir: ZetaIrV1): string {
  // Q# doesn't have native u64 wrapping — it uses Int (arbitrary precision BigInteger).
  // We emulate wrapping by AND-masking after each op, same as the Python approach.
  // Q# conformance tier: behavioral-equivalence (not byte-lock).
  const mask = ir.width === 64 ? "0x_FFFF_FFFF_FFFF_FFFF" : `0x${((1n << BigInt(ir.width)) - 1n).toString(16).toUpperCase()}`;
  const steps = ir.ops.map((op) => {
    if (op.op === "mul") {
      const unsigned = BigInt(op.k!) & ((1n << BigInt(ir.width)) - 1n);
      return `        set z = (z * ${unsigned}L) &&& MASK;`;
    }
...
namespace Zeta.CrossVerify {
    ...
    /// The wrapping mask for ${ir.width}-bit arithmetic.
...
${steps.join("\n")}
    ...
    operation VerifyGolden() : Unit {
        ...
        let result = Mix(1L);
        if result == ${(() => {
          // Compute mix(1) for the comment
          const MASK = (1n << BigInt(ir.width)) - 1n;
          let z = 1n;
          for (const op of ir.ops) {
            if (op.op === "mul") z = (z * (BigInt(op.k!) & MASK)) & MASK;
            else z = (z ^ (z >> BigInt(op.s!))) & MASK;
          }
          return z.toString();
        })()}L {
            Message("PASS: Mix(1) matches golden vector");
        } else {
            Message($"FAIL: Mix(1) = {result}, expected ${(() => {
              const MASK = (1n << BigInt(ir.width)) - 1n;
              let z = 1n;
              for (const op of ir.ops) {
                if (op.op === "mul") z = (z * (BigInt(op.k!) & MASK)) & MASK;
...
function main(): number {
  ...
  console.log(`[codegen] emitted 7 oracle scripts from ${basename(irPath)} → ${outDir}`);
  ...
  console.log(`  gen.go  — Go (uint64 wrapping)`);
...
export { emitTypeScript, emitFSharp, emitCSharp, emitRust, emitPython, emitGo, emitQSharp, parseIrJson, type ZetaIrV1 };
