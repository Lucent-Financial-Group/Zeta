...
function loadOracles(dir) {
    ...
    for (const { file, name } of ORACLES) {
        ...
        try {
            ...
            throw new Error(`oracle ${name} (${file}) is not valid JSON: ${e.message}`, {
                cause: e,
            });
        }
...
function canonicalExpected(vec) {
    for (const f of CANONICAL_SCALAR_FIELDS) {
        if (f in vec && vec[f] !== undefined)
            return vec[f];
    }
 ...
 */
// eslint-disable-next-line sonarjs/cognitive-complexity
export function runNWayDiff(opts) {
    ...
    if (oracles.length < minOracles) {
        console.error(`✗ ${dir}: only ${String(oracles.length)} oracle(s) present, need >= ${String(minOracles)} (assert-don't-skip)`);
        return 1;
    }
    if (oracles.length === 0) {
    ...
    if (divergences.length === 0) {
        if (verbose) {
            console.log(`✓ all ${String(oracles.length)} oracles agree with each other and with canonical on ${String(canonical.size)} vectors.`);
        }
        return 0;
    }
