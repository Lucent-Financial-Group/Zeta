...
${steps.join("\n")}
    ...
    operation VerifyGolden() : Unit {
        ...
        for (const op of ir.ops) {
            if (op.op === "mul")
                z = (z * (BigInt(op.k) & MASK)) & MASK;
            else
                z = (z ^ (z >> BigInt(op.s))) & MASK;
        }
        return z.toString();
    })()}L {
...
function main() {
    ...
    const irPath = args[0];
    const outDir = args[1];
    const ir = parseIrJson(readFileSync(irPath, "utf-8"));
