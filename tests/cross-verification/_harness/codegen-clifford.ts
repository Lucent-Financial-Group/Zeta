...
// ─── Cl(3,0) Multivector type ────────────────────────────────────────────

/**
 * A multivector in Cl(3,0): 8 components indexed by blade mask.
...
function scale(mv: Multivector, s: number): Multivector {
  return mv.map(x => x * s) as Multivector;
}
 ...
 */
export function interpretClifford(ir: ZetaIrV2, input: number): Multivector {
  ...
  for (const op of ir.ops) {
    if (op.op === "mul") {
      ...
      const reflector: Multivector = [0, 0, 0, 0, 0, 0, 0, 0];
...
export function emitCliffordTS(ir: ZetaIrV2): string {
