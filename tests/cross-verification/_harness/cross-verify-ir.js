...
function generateAndRunTS(ir, inputs, tmpDir) {
    const mask = `(1n << ${ir.width}n) - 1n`;
    const body = ir.ops.map(op => {
        if (op.op === "mul")
            return `  z = (z * ${getK(op, ir.width)}n) & MASK;`;
        if (op.op === "xorshr")
...
function generateAndRunPython(ir, inputs, tmpDir) {
    const mask = `(1 << ${ir.width}) - 1`;
    const body = ir.ops.map(op => {
        if (op.op === "mul")
            return `    z = (z * ${getK(op, ir.width)}) & MASK`;
        if (op.op === "xorshr")
...
function generateAndRunGo(ir, inputs, tmpDir) {
    const body = ir.ops.map(op => {
        if (op.op === "mul") {
...
function generateAndRunRust(ir, inputs, tmpDir) {
    const maskLine = ir.width < 64 ? `    z = z & ${(1n << BigInt(ir.width)) - 1n};\n` : "";
    const body = ir.ops.map(op => {
        if (op.op === "mul")
            return `    z = z.wrapping_mul(${getK(op, ir.width)});\n${maskLine}`.trimEnd();
        if (op.op === "xorshr")
            return `    z = z ^ (z >> ${op.s});\n${maskLine}`.trimEnd();
        return "";
    }).join("\n");
...
function generateAndRunFSharp(ir, inputs, tmpDir) {
    const maskLine = ir.width < 64 ? `    z <- z &&& ${(1n << BigInt(ir.width)) - 1n}UL\n` : "";
    const body = ir.ops.map(op => {
        if (op.op === "mul")
...
export function crossVerify(ir, inputs) {
