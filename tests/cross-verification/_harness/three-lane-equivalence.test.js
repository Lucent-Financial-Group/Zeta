...
import { parseIrJson } from "./codegen-from-ir";
// ─── Load the frozen IR ──────────────────────────────────────────────────────
...
function classicalMix(ir, x) {
    const MASK = (1n << BigInt(ir.width)) - 1n;
    return ir.ops.reduce((z, op) => {
        if (op.op === "mul")
            return (z * (BigInt(op.k) & MASK)) & MASK;
        if (op.op === "xorshr")
            return (z ^ (z >> BigInt(op.s))) & MASK;
        throw new Error(`unknown op: ${op.op}`);
    }, x & MASK);
}
// ─── Lane 2: Quantum basis-state (deterministic on |z⟩) ─────────────────────
function quantumBasisStateMix(ir, z) {
    const MASK = (1n << BigInt(ir.width)) - 1n;
    let state = z & MASK;
    for (const op of ir.ops) {
        if (op.op === "xorshr")
            ...
            state = (state * (BigInt(op.k) & MASK)) & MASK;
    }
    return state;
}
const EPS = 1e-12;
const magSq = (z) => z.re * z.re + z.im * z.im;
...
function pure1(state) {
    return [{ state, amp: { re: 1, im: 0 } }];
}
/** Apply one IR op to every frame in the ensemble, then merge. */
function softStep(ir, ensemble, op) {
    const MASK = (1n << BigInt(ir.width)) - 1n;
    const stepped = ensemble.map((frame) => {
...
function softMeasure(ensemble) {
    if (ensemble.length === 0)
        return null;
    let best = ensemble[0];
    for (const frame of ensemble) {
        if (magSq(frame.amp) > magSq(best.amp))
            best = frame;
    }
    return best.state;
}
/** Soft lane on a basis state: pure1 → fold → measure. */
function softBasisStateMix(ir, x) {
