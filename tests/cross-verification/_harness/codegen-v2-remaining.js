 ...
 */
// ─── F# emitter ─────────────────────────────────────────────────────────────
export function emitRingGenericFSharp(ir) {
...
${ir.ops.filter(op => op.op === "mul" || op.op === "xorshr").map(op => {
        if (op.op === "mul") {
            const u = BigInt(op.k) & MASK;
            return `    z <- (z * ${u}UL) &&& mask`;
        }
...
// ─── C# emitter ─────────────────────────────────────────────────────────────
export function emitRingGenericCSharp(ir) {
    const MASK = (1n << BigInt(ir.width)) - 1n;
...
${ir.ops.map((op, i) => {
        if (op.op === "mul") {
            const u = BigInt(op.k) & MASK;
            return `    state = (state * 0x${u.toString(16).toUpperCase()}UL) & Mask; // op ${i}: mul`;
        }
        if (op.op === "xorshr")
            return `    state = (state ^ (state >> ${op.s})) & Mask; // op ${i}: xorshr`;
        if (op.op === "join")
            return `    state = ((state >> ${op.control ?? 0}) & 1) == 1 ? (state ^ (1UL << ${op.target ?? 1})) & Mask : state; // op ${i}: join`;
...
${ir.ops.filter(op => op.op === "mul" || op.op === "xorshr").map(op => {
        if (op.op === "mul") {
            const u = BigInt(op.k) & MASK;
            return `        z = (z * 0x${u.toString(16).toUpperCase()}UL) & Mask;`;
        }
        return `        z = (z ^ (z >> ${op.s})) & Mask;`;
    }).join("\n")}
        return z;
...
// ─── Rust emitter ────────────────────────────────────────────────────────────
export function emitRingGenericRust(ir) {
    const MASK = ir.width >= 64 ? "u64::MAX" : `((1u64 << ${ir.width}) - 1)`;
    const unsigned = (k) => (BigInt(k) & ((1n << BigInt(ir.width)) - 1n));
...
${ir.ops.map(op => {
        if (op.op === "mul")
            return `    state = state.wrapping_mul(0x${unsigned(op.k).toString(16).toUpperCase()}) & MASK;`;
        if (op.op === "xorshr")
            return `    state = (state ^ (state >> ${op.s})) & MASK;`;
        if (op.op === "join")
            return `    state = if (state >> ${op.control ?? 0}) & 1 == 1 { (state ^ (1u64 << ${op.target ?? 1})) & MASK } else { state };`;
        if (op.op === "branch")
            return `    // branch: fork handled in ensemble mode only`;
...
${ir.ops.filter(op => op.op === "mul" || op.op === "xorshr").map(op => {
        if (op.op === "mul")
            return `    z = z.wrapping_mul(0x${unsigned(op.k).toString(16).toUpperCase()}) & MASK;`;
        return `    z = (z ^ (z >> ${op.s})) & MASK;`;
    }).join("\n")}
    z
}
...
// ─── Go emitter ──────────────────────────────────────────────────────────────
export function emitRingGenericGo(ir) {
...
${ir.ops.map(op => {
        if (op.op === "mul")
            return `\tstate = (state * 0x${unsigned(op.k).toString(16).toUpperCase()}) & mask`;
        if (op.op === "xorshr")
            return `\tstate = (state ^ (state >> ${op.s})) & mask`;
        if (op.op === "join")
            return `\tif (state>>${op.control ?? 0})&1 == 1 { state = (state ^ (1 << ${op.target ?? 1})) & mask }`;
        return `\t// ${op.op}: no-op in scalar mode`;
    }).join("\n")}
\treturn state
}
...
${ir.ops.filter(op => op.op === "mul" || op.op === "xorshr").map(op => {
        if (op.op === "mul")
            return `\tz = (z * 0x${unsigned(op.k).toString(16).toUpperCase()}) & mask`;
        return `\tz = (z ^ (z >> ${op.s})) & mask`;
    }).join("\n")}
\treturn z
}
...
// ─── Q# emitter ─────────────────────────────────────────────────────────────
export function emitRingGenericQSharp(ir) {
    const MASK = (1n << BigInt(ir.width)) - 1n;
    const unsigned = (k) => (BigInt(k) & MASK);
...
${ir.ops.map(op => {
        if (op.op === "mul")
            return `        set state = (state * ${unsigned(op.k)}L) &&& mask;`;
        if (op.op === "xorshr")
            return `        set state = (state ^^^ (state >>> ${op.s})) &&& mask;`;
        if (op.op === "join")
            return `        set state = if (state >>> ${op.control ?? 0}) &&& 1L == 1L { (state ^^^ (1L <<< ${op.target ?? 1})) &&& mask } else { state };`;
...
${ir.ops.filter(op => op.op === "mul" || op.op === "xorshr").map(op => {
        if (op.op === "mul")
            return `        set z = (z * ${unsigned(op.k)}L) &&& mask;`;
