...
import { complexRing, realRing } from "../../../src/Core.TypeScript/algebra/star-ring";
const WIDTH = 4;
const EPS = 1e-12;
const isZeroC = (w) => w.re * w.re + w.im * w.im < EPS;
const isZeroR = (w) => Math.abs(w) < EPS;
function makeIr(ops) {
    return { schema: "zeta-ir-v2", generator: "test", version: 1, width: WIDTH, ops };
}
function runComplex(ops, input) {
    return softMixGeneric(makeIr(ops), complexRing, isZeroC, [{ key: input, weight: complexRing.one }]);
}
function runReal(ops, input) {
