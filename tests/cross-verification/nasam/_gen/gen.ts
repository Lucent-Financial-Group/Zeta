...
/** A single total u64->u64 step in the mixer IR (v3 grammar: mul | xorshr | rotl | xrotxor | xshrxor). */
type MixOp =
  ...
  | { readonly op: "xrotxor"; readonly rs: readonly bigint[] } // z := z ^ rotl(z,r_i) ^ ...
  | { readonly op: "xshrxor"; readonly ss: readonly bigint[] }; // z := z ^ (z>>>s_i) ^ ...
...
if (schema !== "zeta-ir-v3") throw new Error(`IR: nasam expects schema zeta-ir-v3, got ${schema}`);
const width = asInt(field(decoded.value, "width"));
if (width !== 64n) throw new Error(`IR: nasam expects width 64, got ${width}`);
...
const NASAM_IR: readonly MixOp[] = parseOps(decoded.value);
...
console.log(
  `wrote ts-output.json (generated-from-ir, IR sourced from the relation row zetaId=${zetaId}, schema=zeta-ir-v3, width 64)`,
);
