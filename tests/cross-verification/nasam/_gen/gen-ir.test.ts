...
type MixOp =
  ...
  | { readonly op: "xrotxor"; readonly rs: bigint[] }
  | { readonly op: "xshrxor"; readonly ss: bigint[] };
...
function intOf(v: Tagged): bigint {
  if (v.t !== "int") throw new Error("expected int");
  return BigInt(v.v);
}
function intListOf(v: Tagged): bigint[] {
  if (v.t !== "arr") throw new Error("expected array");
  return v.v.map(intOf);
}
function parseIr(ir: Tagged): { width: bigint; ops: MixOp[] } {
  const width = intOf(field(ir, "width"));
  const opsNode = field(ir, "ops");
  if (opsNode.t !== "arr") throw new Error("ops must be array");
...
function runIr(width: bigint, ir: readonly MixOp[], x: bigint): bigint {
  const mask = (1n << width) - 1n;
  const m = (z: bigint) => z & mask;
  const rotl = (z: bigint, r: bigint) => {
...
test("a one-constant corruption of the IR diverges from the golden (fidelity bites)", () => {
  ...
  let flipped = false;
  const corrupt: MixOp[] = ops.map((op) => {
  ...
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
    if (runIr(width, corrupt, BigInt(input)).toString() !== expected) differedSomewhere = true;
  }
...
test("a ROTATE TERM is load-bearing: xrotxor [39,17] -> [40,17] diverges from the golden", () => {
  ...
  const { width, ops } = parseIr(decoded.value);
  const corrupt: MixOp[] = ops.map((op) =>
    op.op === "xrotxor" ? { op: "xrotxor", rs: op.rs.map((r, i) => (i === 0 ? r + 1n : r)) } : op,
  );
  let differedSomewhere = false;
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
...
test("a SHIFT TERM is load-bearing: xshrxor [23,51] -> [23,50] diverges from the golden", () => {
  ...
  const { width, ops } = parseIr(decoded.value);
  const corrupt: MixOp[] = ops.map((op) =>
    op.op === "xshrxor" ? { op: "xshrxor", ss: op.ss.map((s, i) => (i === op.ss.length - 1 ? s - 1n : s)) } : op,
  );
  let differedSomewhere = false;
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
...
test("DROPPING the 2nd term of a multi-term op diverges (xshrxor [23,51] -> [23] is not the same)", () => {
  ...
  const corrupt: MixOp[] = ops.map((op) =>
    op.op === "xshrxor" ? { op: "xshrxor", ss: op.ss.slice(0, 1) } : op,
  );
  let differedSomewhere = false;
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
...
test("dropping the xrotxor op diverges from the golden (xrotxor is not redundant)", () => {
  ...
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
    if (runIr(width, withoutXrotxor, BigInt(input)).toString() !== expected) differedSomewhere = true;
  }
  expect(differedSomewhere).toBe(true);
});
...
test("the WIDTH field is load-bearing: folding at width 32 diverges from the u64 golden", () => {
  ...
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
    if (runIr(32n, ops, BigInt(input)).toString() !== expected) differedSomewhere = true;
  }
  expect(differedSomewhere).toBe(true);
});
