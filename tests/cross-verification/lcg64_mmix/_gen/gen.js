...
if (schema !== "zeta-ir-v4")
    throw new Error(`IR: lcg64_mmix expects schema zeta-ir-v4, got ${schema}`);
const width = asInt(field(decoded.value, "width"));
if (width !== 64n)
    throw new Error(`IR: lcg64_mmix expects width 64, got ${width}`);
...
console.log(`wrote ts-output.json (generated-from-ir, IR sourced from the relation row zetaId=${zetaId}, schema=zeta-ir-v4, width 64)`);
