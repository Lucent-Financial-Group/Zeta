...
for (const v of vec.rfc8032) {
  const priv = createPrivateKey({
    key: { kty: "OKP", crv: "Ed25519", d: b64url(v.secret_hex), x: b64url(v.public_hex) },
    format: "jwk",
  });
...
console.log(
  `ed25519 cross-verify: rfc8032=${vec.rfc8032.length} key_id=${vec.key_id.length} envelope=${vec.envelope.length} invalid=${vec.invalid.length}, ${mismatches} mismatches.`,
);
