/** Standalone cross-verification dispatcher for the finite coded-Adinkra / half-spin census. */
...
} from "../../../src/Core.TypeScript/research/adinkra-ecc/nonquotient-adinkra-halfspin-intertwiner";

const REPOSITORY_ROOT = resolve(import.meta.dir, "../../..");
const F_SHARP_ORACLE = join(import.meta.dir, "adinkra-halfspin-intertwiner-oracle.fsx");
...
function isOracleReport(value: unknown): value is OracleReport {
  ...
  const numericFields = [
    "field", "repSeed", "sourceDimension", "targetDimension", "generatorCount",
    "sourceCliffordViolations", "targetCliffordViolations", "nullity",
    "consistentComponentCount", "inconsistentComponentCount", "maximalBasisRank", "unitCombinationRank",
  ];
...
function runFSharp(arguments_: readonly string[]): OracleReport {
  ...
  if (child.error !== undefined || child.status !== 0) {
    throw new Error(`intertwiner F# oracle failed: ${child.error?.message ?? (child.stderr || child.stdout)}`);
  }
  const line = child.stdout.trim().split(/\r?\n/).at(-1);
  if (line === undefined) throw new Error("intertwiner F# oracle emitted no census JSON");
  const decoded: unknown = JSON.parse(line);
  if (!isOracleReport(decoded)) throw new Error("intertwiner F# oracle emitted malformed census JSON");
  return decoded;
}

let failures = 0;
for (const oracleCase of CASES) {
  const fsharp = runFSharp(oracleCase.arguments);
  const typescript = summarize(measureFiniteAdinkraHalfSpinIntertwiner(oracleCase.options));
  if (JSON.stringify(fsharp) !== JSON.stringify(typescript)) {
    console.error(`adinkra-halfspin-intertwiner cross-verify: ${oracleCase.label} mismatch`);
    failures += 1;
  }
}

console.log(`adinkra-halfspin-intertwiner cross-verify: ${CASES.length} cases, ${failures} failure(s).`);
