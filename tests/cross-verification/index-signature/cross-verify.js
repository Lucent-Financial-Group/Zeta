...
import { signIndex, verifyIndexSignature, canonicalIndexBytes } from "../../../src/Core.TypeScript/ace/index-signature.js";
import {} from "../../../src/Core.TypeScript/ace/signing.js";
...
for (const v of vec.envelope) {
    ...
    const spkiB64 = createPublicKey(createPrivateKey(v.private_pem)).export({ type: "spki", format: "der" }).toString("base64");
    const trust = new Map([[sig.key_id, { public_key: spkiB64 }]]);
    const vr = verifyIndexSignature(v.content, sig, trust);
    envOut[v.id] = { key_id: sig.key_id, sig: sig.sig, verify_ok: vr.ok };
    if (sig.key_id !== v.expected_key_id)
        fail(`envelope ${v.id}: key_id MISMATCH got=${sig.key_id} exp=${v.expected_key_id}`);
    if (sig.sig !== v.expected_sig)
        fail(`envelope ${v.id}: sig MISMATCH`);
    if (dec.decode(canonicalIndexBytes(v.content)) !== v.expected_canonical_json)
        fail(`envelope ${v.id}: canonical MISMATCH`);
    if (!vr.ok)
        fail(`envelope ${v.id}: verifyIndexSignature rejected a freshly-signed index`);
}
out.envelope = envOut;
...
for (const v of vec.invalid) {
    ...
    const vr = verifyIndexSignature(v.content, v.signature, trust);
