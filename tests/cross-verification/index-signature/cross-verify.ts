...
import {
  ...
  canonicalIndexBytes,
  type IndexSignableContent,
} from "../../../tools/ace/index-signature.ts";
...
interface InvalidVec {
  id: string;
  content: IndexSignableContent;
  signature: { algo: string; key_id: string; sig: string };
...
for (const v of vec.envelope) {
  ...
  const spkiB64 = (
    createPublicKey(createPrivateKey(v.private_pem)).export({ type: "spki", format: "der" }) as Buffer
  ).toString("base64");
...
console.log(
  `index-signature cross-verify: canonical=${vec.canonical.length} envelope=${vec.envelope.length} invalid=${vec.invalid.length}, ${mismatches} mismatches.`,
);
