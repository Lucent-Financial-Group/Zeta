...
function parseIr(ir: Tagged): { width: bigint; ops: MixOp[] } {
  ...
  const ops = opsNode.v.map((node): MixOp => {
    ...
    if (opNode.v === "mul") return { op: "mul", k: intOf(field(node, "k")) & mask };
    if (opNode.v === "rotl") return { op: "rotl", r: intOf(field(node, "r")) };
...
test("the ROTATE AMOUNT is load-bearing: rotl 7 -> rotl 8 diverges from the golden", () => {
  ...
  if (!decoded.ok) throw new Error("row did not decode");
  const { width, ops } = parseIr(decoded.value);
  const corrupt: MixOp[] = ops.map((op) => (op.op === "rotl" ? { op: "rotl", r: 8n } : op));
  let differedSomewhere = false;
  for (const [input, expected] of Object.entries(GOLDEN)) {
    if (input === "0") continue;
    if (runIr(width, corrupt, BigInt(input)).toString() !== expected) differedSomewhere = true;
