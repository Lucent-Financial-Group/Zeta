...
/** A single total u64->u64 step in the scrambler IR (v2 grammar: mul | xorshr | rotl). */
type MixOp =
...
if (schema !== "zeta-ir-v2") throw new Error(`IR: xoshiro256ss expects schema zeta-ir-v2, got ${schema}`);

const width = asInt(field(decoded.value, "width"));
if (width !== 64n) throw new Error(`IR: xoshiro256ss expects width 64, got ${width}`);
...
const XOSHIRO_IR: readonly MixOp[] = parseOps(decoded.value);

/** rotate-left by r within `width` bits (constant rotate). */
const rotl = (x: bigint, r: bigint, w: bigint): bigint => u64(((x << r) | (x >> (w - r))) & MASK);
...
console.log(
  `wrote ts-output.json (generated-from-ir, IR sourced from the relation row zetaId=${zetaId}, schema=zeta-ir-v2, width 64)`,
);
