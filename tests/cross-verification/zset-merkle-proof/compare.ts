...
const present: Array<[string, OutputMap]> = [];
for (const [name, file] of implementations) {
  const out = loadOutput(file);
  if (out !== null) present.push([name, out]);
}
...
if (mismatches === 0) {
  console.log(
    `OK: ${present.length} implementation(s) agree on ${ids.length} inclusion proofs, all verify against their roots.`,
  );
  process.exit(0);
}
