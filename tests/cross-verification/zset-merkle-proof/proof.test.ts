...
import { ofBytes, toHex, type MerkleHash } from "../../../src/Core.TypeScript/merkle/merkle";

const dir = import.meta.dir;
const fsharp = JSON.parse(readFileSync(`${dir}/fsharp-output.json`, "utf8")) as Record<string, string>;
const ts = JSON.parse(readFileSync(`${dir}/ts-output.json`, "utf8")) as Record<string, string>;
...
function hashOfHex(hex: string): MerkleHash {
...
function verifyProofString(s: string): boolean {
  const parts = s.split("|");
  if (parts.length !== 3) return false;
  const [rootHex, leafPart, pathPart] = parts as [string, string, string];
  ...
  if (pathPart.length > 0) {
    for (const step of pathPart.split(",")) {
      ...
      if (dir === "R") acc = combine(acc, sib);
      else if (dir === "L") acc = combine(sib, acc);
      else return false;
    }
  }
...
test("the two oracles agree on every inclusion proof (byte-lock)", () => {
  ...
  for (const id of ids) expect(ts[id]).toBe(fsharp[id]);
});
