...
test("a one-constant corruption of the IR diverges from the golden (fidelity bites)", () => {
    ...
    const corrupt = ir.map((op) => op.op === "mul" && op.k === 0x94d049bb133111ebn ? { op: "mul", k: 0x94d049bb133111ean } : op);
    let differedSomewhere = false;
    for (const [input, expected] of Object.entries(GOLDEN)) {
        if (input === "0")
            continue; // 0 maps to 0 under any multiply
        if (runIr(corrupt, BigInt(input)).toString() !== expected)
