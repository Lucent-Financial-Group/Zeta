...
function extractBodyBLinks(
  path: string,
  headerEnd: number,
): Array<{ id: string; href: string; line: number; col: number }> {
  const content = readFileSync(path, "utf8");
  const lines = content.split("\n");
    ...
    if (refDir === fileP) {
      findings.push({
        ...
        message:
          ...
          "] link uses '../" +
          refDir +
          "/' prefix but target is in same directory (" +
          fileP +
          "); should be bare filename",
      });
    }
  }
  return findings;
...
function check2_composesCompleteness(
  path: string,
  fm: Frontmatter,
  refs: ReturnType<typeof extractBodyBLinks>,
): Finding[] {
  const findings: Finding[] = [];
...
function check4_redundantEdges(path: string, fm: Frontmatter): Finding[] {
  ...
  if (redundant.length > 0) {
    ...
    findings.push({
      ...
      check: 4,
      message:
        "Redundant composes_with entries (already in depends_on; depends_on is stronger): " +
        redundant.sort().join(", "),
    });
  }
  return findings;
...
function main(): void {
  ...
  for (const [file, findings] of byFile) {
    ...
    for (const f of findings) {
      process.stdout.write(
        "  " + f.line + ":" + f.col + " [" + f.priority + "] check " + f.check + ": " + f.message + "\n",
      );
    }
  }
  process.stdout.write(
