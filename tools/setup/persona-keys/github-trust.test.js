...
import { TEMP_TRUST_ROOT_BANNER, defaultAllowlistPath, formatStatus, isValidGithubUser, keyAlgo, parseAllowlistConfig, parseKeyLines, renderAuthorizedKeys, resolveIdentities, resolveTrustSet, } from "./github-trust.js";
...
function fixtureEffects(keys, opts) {
    ...
    return {
        fetchKeys: async (user) => {
            ...
            if (body === undefined)
                throw new Error(`GET https://github.com/${user}.keys -> HTTP 404`);
            return body;
        },
        fetchGpg: async (user) => {
            if (opts?.fetchCount)
