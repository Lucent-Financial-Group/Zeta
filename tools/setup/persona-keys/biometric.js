 ...
 */
export async function requireBiometric(auth, prompt) {
    if (auth === undefined) {
        return {
            ok: false,
            platform: "unsupported",
            reason: "no biometric approval door was provided — fail-closed (the sensitive action is " +
...
export function detectBiometricPlatform(plat = osPlatform()) {
    if (plat === "darwin")
...
function macTouchIdAuth(prompt) {
    ...
    if (!pamHasTouchId) {
        return {
            ...
            reason: "Touch ID for sudo is not configured (pam_tid.so absent from /etc/pam.d/sudo). " +
...
function windowsHelloAuth(_prompt) {
    return {
        ok: false,
        platform: "windows-hello",
        reason: "Windows Hello programmatic consent is not yet wired from this TS CLI " +
...
export function realBiometric() {
    return async (prompt) => {
        const plat = detectBiometricPlatform();
        if (plat === "macos-touchid")
            return macTouchIdAuth(prompt);
        if (plat === "windows-hello")
            return windowsHelloAuth(prompt);
        return {
            ok: false,
            platform: "unsupported",
