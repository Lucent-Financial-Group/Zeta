...
import { ensureCa, signMachineCert, realEffects, DEFAULT_CERT_VALIDITY } from "./ca.js";
...
async function main() {
    if (mode === "ca") {
        ...
        if (r.dryRun) {
            ...
            if (commitPub)
                console.log(`[dry-run] would write CA PUBLIC key to: ${r.caPublicPath}`);
            return 0;
        }
        if (r.action === "aborted-biometric") {
            console.error(`blocked: biometric ${r.biometric?.reason ?? "not approved"} — NO CA key generated (fail-closed).`);
            return 1;
        }
        console.log(`action=${r.action} ca=${ca}`);
        console.log(`CA private key (local only): ${r.caPrivatePath}`);
        if (r.committedPub)
            console.log(`wrote CA PUBLIC key -> ${r.caPublicPath} (NOT committed; you commit it)`);
        // Printing the CA PUBLIC key is safe (it is the git-distributed trust root).
        if (r.caPublicKey !== undefined)
            console.log(r.caPublicKey);
        return 0;
    }
    if (mode === "cert") {
        ...
        if (r.action === "aborted-biometric") {
            console.error(`blocked: biometric ${r.biometric?.reason ?? "not approved"} — NO cert signed (fail-closed).`);
            return 1;
        }
        ...
        if (r.certText !== undefined)
            console.log(r.certText);
        return 0;
    }
    console.error("usage:\n" +
