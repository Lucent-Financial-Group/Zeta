...
import { canonicalCbor, fromCanonicalCbor, toHex, fromHex } from "../../../src/Core.TypeScript/dynamic-value/cbor.js";
import { canonicalXml, fromCanonicalXml } from "../../../src/Core.TypeScript/dynamic-value/xml.js";
function ok(res) {
    if (!res.ok)
        throw new Error(`Encoding failed: ${res.error}`);
    return res.value;
}
...
export function keyringToTagged(v) {
    if (typeof v === "string")
        return { t: "str", v };
    if (Array.isArray(v))
        return { t: "arr", v: v.map(keyringToTagged) };
    if (v && typeof v === "object")
        return { t: "obj", v: Object.entries(v).map(([k, c]) => [k, keyringToTagged(c)]) };
...
export function serializeKeyring(pub) {
...
export function deserializeKeyring(json, cborHex, xml) {
...
export function keyring4x4(mnemonic, user = "zeta") {
