...
import { keyringInstruction, onboard, formatOnboard } from "./onboard.js";
...
function fixture(opts) {
    ...
    const biometricAuth = async (prompt) => {
...
test("formatOnboard: numbered readout + an operator-action list when pending exists", async () => {
    ...
    const { fx } = fixture({ existing: new Set() });
...
test("CA tie-in: CA door present but no CA private key -> cert-sign SKIPPED (no-ca), never signs", async () => {
    ...
    const fxWithCa = { ...fx, ca: fakeCa(new Set([machinePubPath]), { signCount }) };
