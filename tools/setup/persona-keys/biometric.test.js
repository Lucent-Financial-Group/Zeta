...
import { detectBiometricPlatform, requireBiometric, } from "./biometric.js";
...
test("requireBiometric: a declining door yields ok:false (the gate honors the operator)", async () => {
    const door = async () => ({ ok: false, platform: "macos-touchid", reason: "declined" });
...
test("a BiometricResult NEVER carries a secret — it is approval-only", async () => {
    const door = async () => ({ ok: true, platform: "macos-touchid" });
    const r = await requireBiometric(door, "Approve: x");
