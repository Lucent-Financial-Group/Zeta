...
const RATE_CARD: Record<string, { input: number; output: number; cacheRead: number; cacheCreate: number }> = {
  ...
  haiku: { input: 0.8 / 1_000_000, output: 4.0 / 1_000_000, cacheRead: 0.08 / 1_000_000, cacheCreate: 1.0 / 1_000_000 },
};
...
export function main(argv: string[] = process.argv) {
  ...
  for (const model of models) {
    ...
    const avg = durations.length > 0 ? (durations.reduce((a, b) => a + b, 0) / durations.length).toFixed(0) : "?";
    const med = durations.length > 0 ? median(durations).toFixed(0) : "?";
  ...
  if (includeCosts) {
    ...
    const costWidths = costHeader.map((h, i) => Math.max(h.length, ...costRows.map((r) => (r[i] ?? "").length)));
    ...
    const rateLines = Object.entries(RATE_CARD)
      .map(([tier, r]) => `${tier}=$${(r.input * 1_000_000).toFixed(2)}/$${(r.output * 1_000_000).toFixed(2)}`)
      .join(" ");
    console.log(`\nRate card: ${rateLines} per MTok (in/out)`);
    console.log(
      `Cache: read=${Object.values(RATE_CARD).map((r) => `${((r.cacheRead / r.input) * 100).toFixed(0)}%`)[0]} of input, create=${Object.values(RATE_CARD).map((r) => `${((r.cacheCreate / r.input) * 100).toFixed(0)}%`)[0]} of input`,
    );
  }
  ...
  if (includeReviews) {
    ...
    if (allFindings.length === 0) {
      ...
      const catWidths = catHeader.map((h, i) => Math.max(h.length, ...catRows.map((r) => (r[i] ?? "").length)));
      ...
      const sevWidths = sevHeader.map((h, i) => Math.max(h.length, ...sevRows.map((r) => (r[i] ?? "").length)));
  ...
  for (const r of ratings.slice(-10)) {
    ...
    console.log(
      `  ${r.started_at} ${(r.model ?? "unknown").padEnd(8)} ${r.mode.padEnd(7)} ${dur}s ${pr.padEnd(8)} ${err.padEnd(10)} ${cost}`,
    );
  }
