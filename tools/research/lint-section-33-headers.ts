...
const REQUIRED_LABELS: ReadonlyArray<readonly [string, RegExp]> = [
  ...
  [
    "Operational status",
    /^(?:\*\*Operational status:?\*\*|#{1,6}\s+Operational status:?\s*$|Operational status:\s*\S)/,
  ],
  [
    "Non-fusion disclaimer",
    /^(?:\*\*Non-fusion disclaimer:?\*\*|#{1,6}\s+Non-fusion disclaimer:?\s*$|Non-fusion disclaimer:\s*\S)/,
  ],
];
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--file" && next) {
      ...
      process.stdout.write(
        "Usage: bun tools/research/lint-section-33-headers.ts [--file PATH] [--strict] [--max-header N] [--base-dir D]\n",
      );
      process.exit(0);
    } else {
