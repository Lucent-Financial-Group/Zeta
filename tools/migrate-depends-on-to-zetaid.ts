...
const map: Record<string, string> = JSON.parse(readFileSync(MAP_PATH, "utf-8"));
...
function replaceRefs(value: string): { result: string; replaced: number; missed: string[] } {
  let replaced = 0;
  const missed: string[] = [];
...
function migrateFile(filePath: string): { changed: boolean; replaced: number; missed: string[] } {
  const content = readFileSync(filePath, "utf-8");
  const lines = content.split("\n");

  if (lines[0] !== "---") return { changed: false, replaced: 0, missed: [] };

  const endIdx = lines.indexOf("---", 1);
  if (endIdx === -1) return { changed: false, replaced: 0, missed: [] };
