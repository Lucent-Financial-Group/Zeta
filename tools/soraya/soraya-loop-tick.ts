...
function runTlcOnSpec(spec: string): { result: "pass" | "fail" | "skip" | "timeout"; durationMs: number } {
  ...
  else if (tlc.status === 2)
    result = "skip"; // toolchain not ready
  else result = "fail";
...
function maybeRunClaudeGate(): string {
  ...
  try {
    lastRun = JSON.parse(readFileSync(claudeStateFile, "utf8"));
  } catch {
    /* first run */
  }
  ...
  const gate = run(
    "claude",
    ["-p", prompt, "-w", "--model", claudeModel, "--permission-mode", "auto", "--output-format", "json"],
    claudeTimeoutMs,
  );
...
function heartbeat(): void {
  ...
  const summary = `heartbeat run_id=${runId} fetch=${fetchOk} sha=${gitSha} specs_total=${specs.length} ${
    specResult
      ? `last_spec=${specResult.spec} result=${specResult.result} duration_ms=${specResult.durationMs}`
      : "no_spec"
  } claude=${claudeGate}`;
  log(summary);
