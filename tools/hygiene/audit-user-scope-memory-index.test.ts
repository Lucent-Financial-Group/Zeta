...
describe("audit", () => {
  test("counts a tiny well-formed index correctly", () => {
    const content = ["# header", "", "- [Title A](a.md) — short hook.", "- [Title B](b.md) — short hook.", ""].join(
      "\n",
    );
  ...
  test("flags over-limit entries", () => {
    ...
    const content = ["- [Short Entry](a.md) — fine.", `- [Bloat Entry](b.md) — ${longHook}`].join("\n");
    withTempMemory(content, (path) => {
      const r = audit(path);
      expect(r.totalEntries).toBe(2);
    ...
    const md = renderReport(
      {
        ...
        avgEntryChars: 280,
        entriesOverLimit: 80,
        bloatEntries: [{ lineNumber: 5, chars: 620, preview: "- [Long entry](a.md) — ..." }],
        linesPastCutoff: 100,
        truncationRisk: true,
      },
      fixed,
