...
function runOrderCheck(content: string): ReturnType<typeof Bun.spawnSync> {
...
describe("check-tick-history-order", () => {
  ...
  test("flags out-of-order minute precision rows", () => {
    const result = runOrderCheck(
      ["| 2026-05-17T12:35Z | a | b | c | d | e |", "| 2026-05-17T12:34Z | a | b | c | d | e |"].join("\n"),
    );
  ...
  test("sorts minute precision as the start of that minute", () => {
    const result = runOrderCheck(
      ["| 2026-05-17T12:34:56Z | a | b | c | d | e |", "| 2026-05-17T12:34Z | a | b | c | d | e |"].join("\n"),
    );
