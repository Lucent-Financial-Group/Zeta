...
Completed tool/Bash/Agent invocations that linger in process state without explicit cleanup. Distinct from the other 15 classes because the "lost file" is not git-tracked content — it's harness-level live process state that accumulates until a forced-cleanup-by-crash event reclaims it. Pattern surfaced by the maintainer 2026-05-05 (_"if you leve them dirty they build up like lost files"_) with empirical correlation: Bun v1.3.14 segfault observed 2026-05-05 when 4+ background tasks were running concurrently (resource-pressure failure mode, not a content-loss failure mode — but the failure can take running content with it).
