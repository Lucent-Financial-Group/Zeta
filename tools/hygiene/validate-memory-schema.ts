...
function getDefaultMemoryDir(): string {
  try {
    ...
    throw new Error(
      "Not a git repository or git is unavailable. Pass --memory-dir to specify the memory directory explicitly.",
    );
  }
}
...
const SPECIAL_FILES = new Set(["MEMORY.md", "README.md", "MEMORY-AUTHOR-TEMPLATE.md"]);
...
function validate(memoryDir: string): ValidateResult {
  ...
  for (const file of regularFiles) {
    ...
    const fileType = (fm.fields["type"] as MemoryType | undefined) ?? (prefix ? PREFIX_TO_TYPE[prefix] : undefined);
    ...
    for (const link of checkLinkIntegrity(body, allFilesSet)) {
      violations.push({
        ...
        severity: "warning",
        message: `Link target not found in memory dir: ${link}`,
        fixable: false,
      });
    }
  }
  ...
  const inIndexNotMemory = [...indexedFiles].filter((f) => !allEntries.includes(f) && !isSpecialFile(f));
...
function main(argv: string[] = process.argv): number {
  ...
  if (fix) {
    const fixableFiles = new Set(result.violations.filter((v) => v.fixable).map((v) => v.file));
