...
| convention drift | `convention-drift-supersedes-no-reciprocal.md` (+ pair-mate `convention-drift-earlier-adr.md`) | verify-then-claim memo **convention sub-class** — the smallest v0.9 ADR sub-slice the checker currently implements is reciprocal supersession: a current ADR's `Supersedes \`<adr>\`` claim must be reciprocated by a top-of-file `**Superseded by**` marker on the earlier ADR; this fixture pins the no-marker failure path | supersession claim resolves to an existing earlier ADR that lacks the reciprocal marker — one finding |
