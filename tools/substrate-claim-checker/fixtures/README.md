...
| convention drift | `convention-drift-no-reciprocity.md` (+ co-located target stub `convention-drift-target-no-marker.md`) | verify-then-claim memo **instance #16 / PR #1256** — claim that BOTH router-coherence v1 AND v2 ADRs contained the `**Superseded by**` blockquote pattern; only v1 had it. The real ADRs have since been brought into compliance, so this fixture uses a synthetic exemplar to keep the regression test stable | the claim file's `Supersedes ADR` line resolves to the target stub; the stub lacks the canonical `> **Superseded by** [link]` blockquote → one non-reciprocity finding |
