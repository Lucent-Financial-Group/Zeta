...
test("NON-VACUITY: a witness pair exists, so LEMMA 2 is not a statement about an empty regime", () => {
  if (!z3Available) {
...
test("FALSIFIER: a witness family that never pays turns V1 unsat", () => {
  if (!z3Available) {
...
test("FALSIFIER: a non-saturating credit function turns V2 sat", () => {
  if (!z3Available) {
    console.warn("  [skip] z3 not on PATH — falsifier leg not run");
    return;
  }
