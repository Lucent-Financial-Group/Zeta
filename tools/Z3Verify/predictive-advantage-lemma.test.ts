...
test("NON-VACUITY: both hypotheses are load-bearing (V1 and V2 are sat)", () => {
  if (!z3Available) {
...
test("FALSIFIER: restoring the ablated window ordering turns this runner red", () => {
  if (!z3Available) {
...
test("FALSIFIER: reversing the theorem's inequality turns P1 sat", () => {
  if (!z3Available) {
    console.warn("  [skip] z3 not on PATH — falsifier leg not run");
    return;
  }
