...
export async function main(): Promise<number> {
  ...
  const existing = (await ghApi("GET", `repos/${OWNER}/${REPO}/rulesets?includes_parents=false`)) as Array<{
    id: number;
    name: string;
  }>;
  ...
  if (dryRun) {
    if (alreadyExists) {
      console.log(`[DRY RUN] "Review Policy" already exists (id: ${alreadyExists.id}), would skip step 1.`);
    } else {
      console.log("[DRY RUN] Would create Review Policy ruleset:");
      console.log(JSON.stringify(reviewPolicyPayload, null, 2));
    }
    console.log();
    console.log(`[DRY RUN] Would update Default ruleset (${DEFAULT_RULESET_ID}) to:`);
  ...
  if (alreadyExists) {
    const detail = (await ghApi("GET", `repos/${OWNER}/${REPO}/rulesets/${alreadyExists.id}`)) as {
      id: number;
      name: string;
      rules: Array<{ type: string; parameters?: unknown }>;
    };
    if (rulesMatch(detail.rules ?? [], reviewPolicyPayload.rules)) {
      console.log(`Step 1: "Review Policy" already exists (id: ${alreadyExists.id}) with matching rules — skipping`);
    } else {
      console.log(`Step 1: "Review Policy" exists (id: ${alreadyExists.id}) but rules differ — updating in-place`);
      await ghApi("PUT", `repos/${OWNER}/${REPO}/rulesets/${alreadyExists.id}`, reviewPolicyPayload);
      console.log("  Updated Review Policy ruleset to match expected payload");
    }
  } else {
    console.log("Step 1: Creating Review Policy ruleset...");
    const created = (await ghApi("POST", `repos/${OWNER}/${REPO}/rulesets`, reviewPolicyPayload)) as {
      id: number;
      name: string;
    };
