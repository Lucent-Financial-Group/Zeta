...
function writeBroadcast(summary: string): void {
  ...
  writeFileSync(
    join(broadcastDir, "kiro.md"),
    [`# Kiro broadcast — ${nowIso()}`, "", "## Background tick status", summary].join("\n"),
  );
}
...
function heartbeat(): void {
  ...
  const summary =
    `heartbeat complete run_id=${runId} fetch=${fetchOk} claims=${claimCount} open_prs=${prCount} dirty=${dirtyCount} kiro=${agentStatus} forward=${forwardStatus} ${dueIn}`.trim();
  log(summary);
