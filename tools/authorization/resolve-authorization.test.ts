...
function pi(source: string, timestamp: string | null, raw: string, file = "memory/feedback_test.md"): PaceInstruction {
...
describe("resolveAuthorization", () => {
  ...
  test("unknown source is filtered out", () => {
    const instructions: PaceInstruction[] = [pi("unknown", "2026-05-02", "go hard on the backlog")];
  ...
  test("single maintainer instruction → operative directly", () => {
    const instructions: PaceInstruction[] = [pi("aaron", "2026-05-07", "keep grinding through backlog")];
  ...
  test("rescind-only as only authorized instruction → operative: null", () => {
    const instructions: PaceInstruction[] = [pi("aaron", "2026-05-03", "forget what I said about pace")];
