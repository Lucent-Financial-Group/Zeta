...
import { isSafeFilename, parseAllowlist, splitRoms, main } from "./split-by-license.ts";

function makeTree(): { romDir: string; safeDir: string; unsafeDir: string; allowlist: string } {
...
describe("main", () => {
  ...
  test("returns 1 when the rom directory is missing", () => {
    ...
    expect(
      main(["--rom-dir", "/no/such/dir", "--safe-dir", safeDir, "--unsafe-dir", unsafeDir, "--allowlist", allowlist]),
    ).toBe(1);
  });
