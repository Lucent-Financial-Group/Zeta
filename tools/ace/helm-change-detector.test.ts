...
`;
  ...
  it("should throw an error for OCI repository URLs", async () => {
    await expect(fetchLatestVersion("oci://registry-1.docker.io/bitnamicharts", "redis")).rejects.toThrow(
      "OCI registries are not supported. Only standard HTTP/HTTPS Helm repositories are supported.",
    );
  });
...
`;
  ...
  it("should throw an error if the HTTP fetch fails", async () => {
    ...
    await expect(fetchLatestVersion("https://example.com/charts", "my-chart")).rejects.toThrow(
      "Failed to fetch Helm repository index from https://example.com/charts/index.yaml: Not Found",
    );
  });
...
`;
    ...
    await expect(fetchLatestVersion("https://example.com/charts", "my-chart")).rejects.toThrow(
      "Chart 'my-chart' not found in repository index",
    );
  });
...
`;
    ...
    await expect(fetchLatestVersion("https://example.com/charts", "my-chart")).rejects.toThrow(
      "No valid semver versions found for chart 'my-chart'",
    );
  });
});
