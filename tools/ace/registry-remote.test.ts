...
describe("verifyIndex (three gates)", () => {
  ...
  test("untrusted signer refused", () => {
    ...
    expect(verifyIndex(doc, remoteOf(kp.keyId), new Map(), meta0, NOW, {}).ok).toBe(false);
  });
...
describe("fetchRemoteIndex", () => {
  ...
  test("200 verifies + returns entries + caches", async () => {
    ...
    globalThis.fetch = (async () =>
      new Response(indexJson(kp, 1, now), { status: 200, headers: { ETag: '"e1"' } })) as unknown as typeof fetch;
  ...
  test("304 uses cached body", async () => {
    ...
    const now = Date.parse("2026-06-01T12:00:00Z");
    globalThis.fetch = (async () =>
      new Response(indexJson(kp, 2, now), { status: 200, headers: { ETag: '"e2"' } })) as unknown as typeof fetch;
    const trust = new Map([[kp.keyId, { public_key: kp.publicSpkiB64 }]]);
  ...
  test("network error + no cache → skipped", async () => {
    ...
    expect(
      "skipped" in
        (await fetchRemoteIndex({ url: "https://x/index.json", key_id: kp.keyId }, new Map(), { now: Date.now() })),
    ).toBe(true);
  });
...
describe("loadRegistries merge precedence", () => {
  ...
  test("two remotes: first-listed wins on conflict (remote[0] > remote[1])", async () => {
    ...
    const body0 = mkBody(kp0, "https://R0/l.json"),
      body1 = mkBody(kp1, "https://R1/l.json");
    globalThis.fetch = (async (u: string) =>
      new Response(u === "https://r0/index.json" ? body0 : body1, { status: 200 })) as unknown as typeof fetch;
    const { writeRegistryRemote } = await import("./store.ts");
    ...
    const trust = new Map([
      [kp0.keyId, { public_key: kp0.publicSpkiB64 }],
      [kp1.keyId, { public_key: kp1.publicSpkiB64 }],
    ]);
    const r = await loadRegistries({ trustStore: trust, now });
    expect(r.errors).toEqual([]);
...
function mkV2Index(
  kp: { privatePem: string; keyId: string },
  seq: number,
  issuedAtMs: number,
...
describe("loadRegistries — union-merge marks (Task B)", () => {
  ...
  afterEach(() => {
    ...
    if (savedHome2 !== undefined) process.env.HOME = savedHome2;
    else delete process.env.HOME;
    if (savedUP2 !== undefined) process.env.USERPROFILE = savedUP2;
    else delete process.env.USERPROFILE;
  });
