...
describe("resolve — basic", () => {
  ...
  test("linear chain root->A->B installs leaves-first [B, A, root]", async () => {
    ...
    const r = await resolve(root, fetchOf({ "http://e/A": A, "http://e/B": B }), NO_TRUST, new Map(), new Map(), {
...
describe("resolve — dedup", () => {
  test("diamond (root->A->D, root->B->D, same D) installs D once", async () => {
    ...
    const r = await resolve(
      root,
      fetchOf({ "http://e/A": A, "http://e/B": B, "http://e/D": D }),
      NO_TRUST,
      new Map(),
      new Map(),
      { allowNoSignature: true },
  ...
  test("distinct packages with identical files (different names) both resolve", async () => {
    ...
    const r = await resolve(root, fetchOf({ "http://e/X": X, "http://e/Y": Y }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(true);
...
describe("resolve — conflicts", () => {
  test("version-skew (A->D@1.0, B->D@2.0) refuses", async () => {
    ...
    const B: AcePackage = {
      manifest: {
        ...
        dependencies: [
          { kind: "inline" as const, name: "D", version: "2.0.0", url: "http://e/D2", package_hash: packageHash(D2) },
        ],
      },
      files: { "b.txt": "b" },
    };
  ...
  test("root-involving skew (root@1 -> A -> root@2) refuses (root seeded)", async () => {
        ...
        dependencies: [
          {
            ...
            version: "2.0.0",
            url: "http://e/root2",
            package_hash: packageHash(root2),
          },
        ],
      },
      files: { "a.txt": "a" },
    ...
    const r = await resolve(
      root,
      fetchOf({ "http://e/A": A, "http://e/root2": root2 }),
      NO_TRUST,
      new Map(),
      new Map(),
      { allowNoSignature: true },
  ...
  test("root cycle (root@1 -> A -> root@1) refuses as cycle", async () => {
        ...
        dependencies: [
          {
            ...
            version: "1.0.0",
            url: "http://e/root",
            package_hash: packageHash(rootPlaceholder),
          },
        ],
      },
      files: { "a.txt": "a" },
    ...
    const r = await resolve(root, fetchOf({ "http://e/A": A, "http://e/root": root }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(false);
  ...
  test("cycle A->B->A refuses with the loop in path", async () => {
        ...
        dependencies: [
          {
            ...
            version: "1.0.0",
            url: "http://e/A",
            package_hash: packageHash(aPlaceholder),
          },
        ],
      },
      files: bFiles,
    ...
    const A: AcePackage = {
      manifest: {
        ...
        dependencies: [
          { kind: "inline" as const, name: "B", version: "1.0.0", url: "http://e/B", package_hash: packageHash(B) },
        ],
      },
      files: aFiles,
    };
...
describe("resolve — verification", () => {
  test("bad-content-hash (files don't hash to manifest.content_hash) refuses", async () => {
    ...
    const r = await resolve(root, fetchOf({ "http://e/D": tampered }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(false);
  ...
  test("pin-mismatch (edge package_hash != fetched) refuses", async () => {
    ...
    (root.manifest.dependencies as any)[0] = {
      kind: "inline" as const,
      ...root.manifest.dependencies![0],
      package_hash: "sha256:wrongwrong",
    };
    const r = await resolve(root, fetchOf({ "http://e/D": D }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(false);
  ...
  test("unsigned node refuses without allowNoSignature, resolves with it", async () => {
    const D = pkgOf("D", { "d.txt": "d" });
    const root = pkgOf("root", { "r.txt": "r" }, [{ pkg: D, url: "http://e/D" }]);
    const strict = await resolve(root, fetchOf({ "http://e/D": D }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: false,
    });
    expect(strict.ok).toBe(false);
    if (!strict.ok) expect(strict.reason).toBe("no-signature");
    const lax = await resolve(root, fetchOf({ "http://e/D": D }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
  ...
  test("untrusted/bad signature refuses even with allowNoSignature:true", async () => {
    ...
    const signed: AcePackage = {
      manifest: { ...D.manifest, signature: { algo: "ed25519", key_id: "ed25519:unknownkey", sig: "AAAA" } },
      files: D.files,
    };
    const root = pkgOf("root", { "r.txt": "r" }, [{ pkg: signed, url: "http://e/D" }]);
    const r = await resolve(root, fetchOf({ "http://e/D": signed }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(false);
...
describe("resolve — invalid package", () => {
  ...
  test("dep with non-array dependencies refuses invalid-package (no throw)", async () => {
    ...
    (root.manifest.dependencies as any)[0] = {
      ...
      version: "1.0.0",
      url: "http://e/D",
      package_hash: packageHash(badDep),
    };
    const r = await resolve(root, fetchOf({ "http://e/D": badDep }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason).toBe("invalid-package");
  ...
  test("dep with a non-safe-integer manifest field refuses invalid-package (no throw)", async () => {
    ...
    const root = pkgOf("root", { "r.txt": "r" }, [{ pkg: D, url: "http://e/D" }]);
    const r = await resolve(root, fetchOf({ "http://e/D": floatDep }), NO_TRUST, new Map(), new Map(), {
      allowNoSignature: true,
    });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason).toBe("invalid-package");
...
describe("resolve — registry deps", () => {
  ...
  test("an unknown dependency kind refuses with invalid-package (not silently treated as inline)", async () => {
    const root: AcePackage = {
      manifest: {
        ...pkgOf("root", { "r.txt": "r" }).manifest,
        dependencies: [{ kind: "frobnicate", name: "X", version: "1.0.0" }] as unknown as AceDependency[],
      },
      files: { "r.txt": "r" },
    };
    const r = await resolve(root, fetchOf({}), NO_TRUST, new Map(), new Map(), { allowNoSignature: true });
  ...
  test("an inline edge missing url/package_hash refuses with invalid-package", async () => {
    const root: AcePackage = {
      manifest: {
        ...pkgOf("root", { "r.txt": "r" }).manifest,
        dependencies: [{ kind: "inline", name: "X", version: "1.0.0" }] as unknown as AceDependency[],
      },
      files: { "r.txt": "r" },
    };
    const r = await resolve(root, fetchOf({}), NO_TRUST, new Map(), new Map(), { allowNoSignature: true });
...
describe("resolve — solved-map registry edges", () => {
  ...
  test("solved version that violates the edge range → unsatisfiable (defense-in-depth)", async () => {
    ...
    const root: AcePackage = {
      manifest: {
        ...pkgOf("root", { "r.txt": "r" }).manifest,
        dependencies: [{ kind: "registry" as const, name: "D", version: "^2.0.0" }],
      },
      files: { "r.txt": "r" },
    };
    const reg = new Map([["D", new Map([["1.0.0", { url: "http://e/D", package_hash: packageHash(D) }]])]]);
...
describe("resolve — untrusted registry edge version (regression / bad-range)", () => {
  test("non-string registry version → bad-range (no satisfies crash)", async () => {
    ...
    const root: AcePackage = {
      manifest: {
        ...pkgOf("root", { "r.txt": "r" }).manifest,
        dependencies: [{ kind: "registry", name: "D", version: 123 } as unknown as AceDependency],
      },
      files: { "r.txt": "r" },
    };
    const reg = new Map([["D", new Map([["1.0.0", { url: "http://e/D", package_hash: packageHash(D) }]])]]);
  ...
  test("malformed registry range string → bad-range (defense-in-depth; resolve does not assume solve ran)", async () => {
    ...
    const root: AcePackage = {
      manifest: {
        ...pkgOf("root", { "r.txt": "r" }).manifest,
        dependencies: [{ kind: "registry" as const, name: "D", version: "@@@" }],
      },
      files: { "r.txt": "r" },
    };
    const reg = new Map([["D", new Map([["1.0.0", { url: "http://e/D", package_hash: packageHash(D) }]])]]);
