...
export async function resolve(
  ...
  const walk = async (node: AcePackage, path: string[]): Promise<ResolveResult | null> => {
    for (const edge of Array.isArray(node.manifest.dependencies) ? node.manifest.dependencies : []) {
      ...
      if (edge.kind === "registry") {
        ...
        if (typeof edge.version !== "string") {
          return {
            ok: false,
            reason: "bad-range",
            detail: `${edge.name}: registry edge version must be a string`,
            path: here,
          };
        }
        ...
        if (typeof edge.url !== "string" || typeof edge.package_hash !== "string" || typeof edge.version !== "string") {
          return {
            ok: false,
            reason: "invalid-package",
            detail: `${edge.name}: inline edge missing/invalid url/package_hash/version`,
            path: here,
          };
        }
        url = edge.url;
      ...
      if (seen) {
        if (seen.version !== effectiveVersion) {
          return {
            ok: false,
            reason: "version-skew",
            detail: `${edge.name} required at ${seen.version} (via ${seen.path.join(" → ")}) and ${effectiveVersion} (via ${here.join(" → ")})`,
            path: here,
          };
        }
        if (seen.pkgHash !== package_hash) {
          return {
            ok: false,
            reason: "tamper",
            detail: `${edge.name}@${effectiveVersion} has two different package hashes`,
            path: here,
          };
        }
      ...
      let dep: AcePackage;
      try {
        dep = JSON.parse(await fetchPackage(url)) as AcePackage;
      } catch (e) {
      ...
      if (got !== package_hash) {
        return {
          ok: false,
          reason: "pin-mismatch",
          detail: `${edge.name}: expected package_hash ${package_hash} but fetched ${got}`,
          path: here,
        };
      }
      ...
      if (dep.manifest.name !== edge.name || dep.manifest.version !== effectiveVersion) {
        return {
          ok: false,
          reason: "pin-mismatch",
          detail: `${edge.name}@${effectiveVersion}: edge identity != fetched ${dep.manifest.name}@${dep.manifest.version}`,
          path: here,
        };
      }
