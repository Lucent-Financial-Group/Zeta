...
export function renderDiffReport(report: FeatureDiffReport): string {
  ...
  lines.push(
    ...
    `_Generated by \`tools/playwright/github-ui/feature-diff.ts\` (B-0323). ` +
      `New feature candidates (★) warrant manual triage — check the GitHub changelog for context._`,
  );
...
export async function main(argv: readonly string[]): Promise<number> {
  ...
  if (!parsed.prior) {
    process.stderr.write(`error: --prior is required\n`);
    return 1;
  }
  if (!parsed.current) {
    process.stderr.write(`error: --current is required\n`);
    return 1;
  }
  ...
  try {
    ...
    process.stderr.write(`error loading snapshot sets: ${msg}\n`);
    return 1;
  }
  ...
  if (parsed.out) {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      process.stderr.write(`error writing report: ${msg}\n`);
      return 1;
    }
  } else {
