...
const SCAN_DIRS = ["memory", "docs", ".claude/skills", ".claude/rules", ".claude/agents"];
...
const CONCEPT_QUERIES: [string, RegExp][] = [
  ...
  [
    "named-pattern",
    /\b(Confucius[- ]unfold|consensus[- ]smoothness|razor[- ]discipline|glass[- ]halo|retraction[- ]native|shield[- ]synthesis|superfluid|bidirectional[- ]alignment)\b/gi,
  ],
  [
    "lineage-anchor",
    /\b(Pearl|Hoare|Milner|Hejlsberg|Syme|Budiu|Lamport|Bernstein|Hadfield[- ]Menell|Honda|Yoshida|Carbone)\b/g,
  ],
  [
    "shadow-class",
    /\b(confident[- ]fabrication|narration[- ]over[- ]action|consensus[- ]smoothness|effort[- ]avoidance|narrative[- ]laundering|escalation[- ]cascade|pattern[- ]blindness|asking[- ]over[- ]checking)\b/gi,
  ],
...
export async function buildIndex(): Promise<ConceptIndex> {
  ...
  entries.sort((a, b) => a.conceptClass.localeCompare(b.conceptClass) || a.term.localeCompare(b.term));
