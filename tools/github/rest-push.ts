...
`);
  ...
  if (!args.message) {
    process.stderr.write("--message required\n");
    process.exit(2);
  }
  return args;
}
...
function ghApi(method: string, path: string, body?: object): unknown {
  ...
  if (result.status !== 0) {
    throw new Error(
      `gh ${method} ${path} failed (exit ${result.status}): ${result.stderr.trim() || result.stdout.trim()}`,
    );
  }
  try {
    return JSON.parse(result.stdout);
  } catch (e) {
    throw new Error(`gh ${method} ${path} returned non-JSON: ${result.stdout.slice(0, 200)}`);
  }
}
