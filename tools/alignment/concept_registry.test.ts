...
import { buildRegistry, extractByRegex, extractGlassHaloDoctrines } from "./concept_registry.ts";
...
describe("buildRegistry", () => {
  test("builds a schema-v1.1 registry with all expected concept classes", () => {
    ...
    expect(registry.anchoredCount).toBe(
      registry.concepts.filter((concept) => concept.anchorState && concept.anchorState !== "factory-native").length,
    );
    expect(
      registry.concepts.every((concept) => concept.id && concept.conceptClass && concept.source && concept.label),
    ).toBe(true);
