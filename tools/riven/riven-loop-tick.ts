...
const agentIntervalMs = Number(process.env.ZETA_RIVEN_LOOP_AGENT_INTERVAL_SECONDS ?? "60") * 1000;
...
function writeBroadcast(summary: string): void {
  ...
  writeFileSync(
    join(broadcastDir, "riven.md"),
    [`# Riven broadcast — ${nowIso()}`, "", "## Background tick status", summary].join("\n"),
  );
}
...
function forwardTick(): void {
  ...
  const prNumbers = prsResult.stdout
    .trim()
    .split("\n")
    .filter((n) => n.trim())
    .map(Number);
  for (const pr of prNumbers) {
    const gateResult = gh(
      ...
      "Lucent-Financial-Group/Zeta",
      "--json",
      "mergeStateStatus,autoMergeRequest,reviewThreads",
      "--jq",
      "{mergeState: .mergeStateStatus, autoMerge: (.autoMergeRequest != null), unresolvedThreads: ([.reviewThreads[]? | select(.isResolved == false)] | length)}",
    );
...
function heartbeat(): void {
        ...
        const gate = run(
          ...
          [
            ...
            [
              ...
              "Learn from Otto/Vera patterns. Critique failure modes as data.",
  ...
  if (forwardActions && fetchOk === "ok") {
    ...
    try {
      lastForward = JSON.parse(readFileSync(forwardStateFile, "utf8"));
    } catch {
      /* first run */
    }
  ...
  const summary =
    `heartbeat complete run_id=${runId} fetch=${fetchOk} claims=${claimCount} open_prs=${prCount} dirty=${dirtyCount} riven=${agentStatus} forward=${forwardStatus} ${dueIn}`.trim();
  log(summary);
