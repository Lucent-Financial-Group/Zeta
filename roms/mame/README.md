# `roms/mame/` — MAME arcade emulator (1997)

Multiple Arcade Machine Emulator. This folder IS the canonical MAME `roms/` load path — drop ROM-set ZIPs directly here.

## What to drop here

Arcade ROM-set ZIP files for the MAME arcade emulator. Each ZIP is
...
## Why `roms/mame/` is top-level (not under `arcade/`)
...
each gets its own top-level folder. `roms/mame/` IS the
