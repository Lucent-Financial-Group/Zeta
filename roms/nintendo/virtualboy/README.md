# `roms/nintendo/virtualboy/` — Virtual Boy (1995)

stereoscopic tabletop handheld; short-lived

## What to drop here

ROM / disc image / cassette dump files for **Virtual Boy**.
Common extensions: see the emulator core's documentation
for your chosen runtime. The directory slug `virtualboy` matches
the EmulationStation / libretro convention (under the
`nintendo/` manufacturer branch, so the fully-qualified path
